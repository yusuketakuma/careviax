...
## 運用体制・改善ループ（SSOT ポインタ）

**現行の運用体制・進行中タスク・次の一手は `ops/refactor/STATE.md` が唯一の正（SSOT）。**
台帳運用は `ops/refactor/README.md`（STATE/BACKLOG/LOG の3ファイル体制）。本ファイルや
`.agent-loop/` 配下に残る旧体制記述（agmsg 双方向ループ／Codex-only 等）は歴史的記録であり、
矛盾時は STATE.md に従う。

careviax 固有の不変ルール（体制が変わっても維持）:

- **maker/checker 分離**: 実装した側は自己承認しない。独立レビュー必須。最終判定は
  objective gate（`pnpm lint` / `typecheck` / `typecheck:no-unused` / `format:check` /
  `test` / `build` / 必要時 `test:e2e`）。
- **Long gate discipline**: `pnpm build` と `pnpm typecheck` 系は並列実行しない
  （`.next/types` race）。`typecheck:no-unused` は OOM 時 `NODE_OPTIONS=--max-old-space-size=8192`。
- **hard stop**: auth/security/破壊的 migration/本番 deploy は human 承認なしに触らない
  （`.agent-loop/BLOCKED.md` へ退避）。billing は解除済みだが算定値変更は要レビュー。
- **gbrain**（ローカル長期記憶、MCP 登録済・詳細 `.agent-loop/GBRAIN_SCHEMA.md`）: 意味検索
  `gbrain search/query`、シンボル `code-def/refs/callers`。既知の正確な文字列は Grep。
  gbrain 記憶は現在の repo/テスト/型より**優先しない**。
