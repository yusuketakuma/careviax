...
- **埋め込み未生成**: 埋め込みプロバイダ（`OPENAI_API_KEY` / `VOYAGE_API_KEY`）が未設定のため、careviax は**キーワード/語彙検索のみ可、自然言語の semantic 検索は空**。`gbrain embed` には API キーが必要。
  - ⚠️ キー設定で埋め込みを作ると **careviax の docs テキストが OpenAI/Voyage API へ送信**される（データ持ち出し）。医療コンプライアンス上、ユーザー承認が前提（`.agent-loop/BLOCKED.md` 参照）。
- MCP 登録済み（user scope）。`mcp__gbrain__*` ツールは**セッション開始時ロード** → 既存セッションは要 restart
- 既存 federated sources: default(/Users/yusuke/brain 1175p) ほか
