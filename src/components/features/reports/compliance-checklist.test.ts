...
import type { PhysicianReportContent, AudienceReportContent } from '@/types/care-report-content';
...
describe('deriveReportComplianceChecks', () => {
  ...
  it('checks the five audience report sections for nurse and facility shares', () => {
    const content: AudienceReportContent = {
    ...
    const checks = deriveReportComplianceChecks('nurse_share', content);

    expect(checks).toHaveLength(5);
    expect(checks.some((item) => item.key === 'medication' && !item.passed)).toBe(true);
    expect(checks.some((item) => item.key === 'summary' && item.passed)).toBe(true);
  });
});
