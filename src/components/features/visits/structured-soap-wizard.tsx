...
const STEP_LABELS = ['S（主観）', 'O-基本', 'O-機能', 'A（評価）', 'P（計画）'] as const;
    ...
    <div className="flex flex-col min-h-dvh bg-background">
      ...
      <div className="sticky top-0 z-10 bg-background border-b border-border px-4 pt-4 pb-3">
        <div className="flex items-center gap-3 mb-3">
          ...
          <h2 className="text-base font-semibold text-foreground flex-1">{STEP_LABELS[step]}</h2>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-5 pb-28">
        {step === 0 && <SubjectiveStep data={subjective} onChange={setSubjective} />}
        {step === 1 && <ObjectiveBasicStep data={objective} onChange={setObjective} />}
        {step === 2 && (
        ...
        {step === 3 && <AssessmentStep data={assessment} onChange={setAssessment} />}
        {step === 4 && <PlanStep data={plan} onChange={setPlan} />}
      </div>
