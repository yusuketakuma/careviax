...
describe('WorkflowPageIntro', () => {
  it('renders the back link, shortcuts, and optional actions in one compact block', () => {
    ...
    expect(screen.getByRole('link', { name: '患者一覧へ戻る' }).getAttribute('href')).toBe('/patients');
    expect(screen.getByRole('heading', { name: '患者詳細' })).toBeTruthy();
    expect(screen.getByRole('link', { name: '処方履歴' }).getAttribute('href')).toBe('/patients/p1/prescriptions');
  ...
  it('renders grouped shortcut headings when shortcut metadata includes groups', () => {
    ...
    expect(screen.getByRole('link', { name: '処方履歴' }).getAttribute('href')).toBe('/patients/p1/prescriptions');
    expect(screen.getByRole('link', { name: '外部共有' }).getAttribute('href')).toBe('/patients/p1/share');
  });
});
