...
export function validateEvidenceUploadRequest(input: EvidenceUploadRequest): void {
  ...
  assertSafeSegment(input.card_id, 'card_id');
  if (!input.evidence_type.trim()) throw new TenantStorageKeyError('evidence_type is required');
  if (!/^[a-z0-9][a-z0-9.+-]*\/[a-z0-9][a-z0-9.+-]*$/i.test(input.mime_type.trim())) {
  ...
  normalizeExtension(input.file_name);
}
