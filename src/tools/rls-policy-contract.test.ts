...
describe('RLS policy contract for billing and PCA rental domains', () => {
  it.each(['PatientInsurance', 'PcaPump', 'PcaPumpRental', 'PcaPumpMaintenanceEvent'])(
    'enforces app RLS context for %s in the hardening migration',
    (tableName) => {
      const source =
        tableName === 'PcaPumpMaintenanceEvent' ? maintenanceEventMigration : hardenedRlsMigration;
