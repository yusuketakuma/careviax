...
import type { PatientWorkspacePanelProvider } from '@/core/patient/workspace-panel';
...
export type BuildPharmacyPatientWorkspaceArgs = {
  orgId: string;
  patientId: string;
  caseIds: string[];
  allergyInfo: unknown;
 ...
 */
export async function buildPharmacyPatientWorkspace(
  db: DbClient,
  args: BuildPharmacyPatientWorkspaceArgs,
) {
...
export type PharmacyPatientWorkspaceReadModel = NonNullable<
  Awaited<ReturnType<typeof buildPharmacyPatientWorkspace>>
>;

export type PharmacyPatientWorkspaceProviderInput = Readonly<{
  db: DbClient;
  args: BuildPharmacyPatientWorkspaceArgs;
}>;

const currentMedicationCyclePanelProvider: PatientWorkspacePanelProvider<
  PharmacyPatientWorkspaceProviderInput,
  PharmacyPatientWorkspaceReadModel
> = {
  module: 'pharmacy',
  panelId: 'pharmacy.current_medication_cycle',
  label: 'Current pharmacy medication cycle',
  build({ db, args }) {
    return buildPharmacyPatientWorkspace(db, args);
  },
};

export function createPharmacyPatientWorkspacePanelProviders() {
  return [currentMedicationCyclePanelProvider] as const;
}
