...
import PatientVisitRecordsPage from './page';
...
describe('PatientVisitRecordsPage', () => {
  it('routes the legacy visit-records redirect through the shared patient href helper', async () => {
    vi.mocked(buildPatientHref).mockReturnValueOnce('/patients/__helper_patient_1__');

    await PatientVisitRecordsPage({ params: Promise.resolve({ id: 'patient_1' }) });
