...
export function ReportDeliveryDashboard() {
  ...
  const reminderMutation = useMutation({
    mutationFn: async () => {
      ...
      if (!response.ok) {
        throw new Error((payload as { message?: string }).message ?? 'リマインド起票に失敗しました');
      }
    ...
    onError: (error) => {
      toast.error(
        error instanceof Error ? error.message : 'リマインドタスクの起票に失敗しました'
      );
    },
  });
      ...
      <div className="grid gap-4 xl:grid-cols-[1.5fr_1fr]">
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <SummaryCard
            label="今月の送達成功率"
            value={
              analytics ? `${analytics.summary.current_month_success_rate}%` : '—'
            }
            detail={
              analytics
          ...
          <SummaryCard
            label="今月の確認率"
            value={
              analytics ? `${analytics.summary.current_month_confirmed_rate}%` : '—'
            }
            detail={
              analytics
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            ...
            <p className="text-sm text-muted-foreground">
              `response_waiting` が閾値を超えた送達を抽出し、担当者に follow-up タスクを作成します。
            </p>
          </CardContent>
        </Card>
      </div>
        ...
        {rows.length ? (
          <div className="overflow-x-auto">
            <table className="min-w-full text-sm">
              ...
              <tbody>
                {rows.map((row, index) => (
                  <tr key={`${title}-${index}`} className="border-b border-border/40 last:border-b-0">
