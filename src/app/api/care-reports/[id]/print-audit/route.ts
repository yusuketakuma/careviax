...
async function careReportPrintAuditPOST(
  req: NextRequest,
  ctx: AuthContext,
  ...
  if (!parsedIntent.success) {
    return validationError('入力値が不正です', parsedIntent.error.flatten().fieldErrors);
  }
  ...
  if (report.status !== 'confirmed') {
    return conflict('薬剤師確認済みの報告書のみ印刷できます');
  }
  if (
    ...
    return forbiddenResponse('この報告書を印刷する権限がありません');
  }
  ...
  if (
    ...
    return conflict('報告書が更新されています。再読み込みしてから印刷してください');
  }
  if (
    !(await canAccessCareReportSource(prisma, ctx.orgId, ctx, {
  ...
  try {
    ...
    return registeredError('PRINT_AUDIT_FAILED', '報告書の印刷監査を記録できませんでした');
  }
...
export const POST = withAuthContext(careReportPrintAuditPOST, {
