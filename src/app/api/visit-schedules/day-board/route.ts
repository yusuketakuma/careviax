...
import { resolveSequentially } from '@/lib/utils/resolve-sequentially';
...
import { loadDayBoardProposals } from '@/server/services/day-board-proposal-loader';
import { loadDayBoardSchedules } from '@/server/services/day-board-schedule-loader';
    ...
    const responseData = await withOrgContext(
      ...
      async (db) => {
        const [
          ...
          () =>
            loadDayBoardSchedules(db, {
              orgId: ctx.orgId,
              dayStart,
              dayEnd,
              pageSize: DAY_BOARD_SCAN_PAGE_SIZE,
              maxPages: DAY_BOARD_SCAN_MAX_PAGES,
            }),
          () =>
            db.task.groupBy({
              by: ['assigned_to'],
