...
const authenticatedPOST = withAuthContext<{ id: string }>(
  async (req, ctx, { params }) => {
    ...
    const parsed = submitPartnerVisitRecordSchema.safeParse(payload);
    if (!parsed.success) {
        ...
        async (tx) => {
          ...
          const findActiveRecord = () =>
            tx.partnerVisitRecord.findFirst({
              where: { id, org_id: ctx.orgId, share_case: { is: activeShareCaseWhere } },
              select: {
                id: true,
                status: true,
                updated_at: true,
          ...
          if (!initialRecord) return { response: notFound('協力訪問記録が見つかりません') };
          ...
          if (!record) return { response: conflict('患者共有の同意状態が更新されています') };
          if (
          ...
          if (
            ...
            return {
              response: conflict('有効な薬局間連携と協力薬局に紐づく訪問記録のみ提出できます'),
            };
          }
