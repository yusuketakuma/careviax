...
async function authenticatedDELETE(
  ...
  return runWithRequestAuthContext(ctx, async () => {
    ...
    const result = await withOrgContext(
      ctx.orgId,
      async (tx): Promise<DeleteSetBatchResult | null | { invalidStatus: string }> => {
        const existing = await tx.setBatch.findFirst({
          where: {
            id,
            org_id: ctx.orgId,
        ...
        if (!existing) return null;
        if (existing.plan.cycle.overall_status !== MUTABLE_SET_BATCH_CYCLE_STATUS) {
          return { invalidStatus: existing.plan.cycle.overall_status };
        }
        ...
        return { success: true as const, planId: existing.plan_id, batchId: existing.id };
      },
      { requestContext: ctx },
    );
    ...
    if ('invalidStatus' in result) {
      return immutableSetBatchConflict(result.invalidStatus);
    }
    ...
    await notifyWorkflowMutation({
      orgId: ctx.orgId,
      payload: { source: 'set_batches_delete', plan_id: result.planId, batch_id: result.batchId },
    });
