...
import {
  buildConferenceCareCase,
  buildConferenceFacility,
  buildConferenceNote,
  buildLatestConferenceVisitSchedule,
  buildUpdatedConferenceCareCase,
  buildUpdatedConferenceNote,
  createConferenceNoteGetRequest as createGetRequest,
  createConferenceNotePatchRequest as createRequest,
  findConferenceCareReports,
  unavailableCaseResponse,
...
describe('/api/conference-notes/[id] PATCH', () => {
  beforeEach(() => {
    ...
    conferenceNoteFindFirstMock.mockResolvedValue(buildConferenceNote('care_manager'));
    conferenceNoteUpdateMock.mockImplementation(async (args?: { data?: { metadata?: unknown } }) =>
      buildUpdatedConferenceNote('care_manager', args?.data?.metadata),
    );
    careCaseFindFirstMock.mockResolvedValue(buildConferenceCareCase());
    requireWritablePatientMock.mockResolvedValue({
      patient: { id: 'patient_1', archived_at: null },
    });
    careCaseUpdateMock.mockResolvedValue(buildUpdatedConferenceCareCase());
    taskFindManyMock.mockResolvedValue([]);
    taskCreateManyMock.mockResolvedValue({ count: 1 });
    billingCandidateUpsertMock.mockResolvedValue({ id: 'billing_1' });
    visitScheduleFindFirstMock.mockResolvedValue(buildLatestConferenceVisitSchedule());
    ...
    careReportFindManyMock.mockImplementation(findConferenceCareReports);
    careReportCreateManyMock.mockResolvedValue({ count: 1 });
    residenceFindFirstMock.mockResolvedValue({ facility_id: 'facility_1' });
    facilityFindFirstMock.mockResolvedValue(buildConferenceFacility());
