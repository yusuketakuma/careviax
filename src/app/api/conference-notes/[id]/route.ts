...
type PatchConferenceNoteTransactionResult =
  | Awaited<ReturnType<typeof ConferenceDataSyncService.syncSavedNote>>
  | { error: NextResponse };
...
const authenticatedPATCH = withAuthContext<{ id: string }>(
  async (req, ctx, routeContext) => {
    ...
    const result = await withOrgContext<PatchConferenceNoteTransactionResult>(
      ctx.orgId,
      async (tx) => {
        await tx.$queryRaw(
        ...
        const existing = await tx.conferenceNote.findFirst({
          where: {
            id,
            org_id: ctx.orgId,
          },
        ...
        const mergedPayload = {
          ...
          structured_content:
            parsed.data.structured_content ?? existingStructuredContent ?? undefined,
        ...
        if (!mergedValidation.success) {
          return { error: validationError('入力値が不正です', mergedValidation.error.issues) };
        }

        const resolvedNoteType = resolveConferenceNoteType(mergedValidation.data);
        if (!resolvedNoteType) {
          return { error: validationError('conference_type と note_type が一致していません') };
        }
