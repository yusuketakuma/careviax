...
import { fetchWorkflowCoreData, fetchWorkflowDependentData } from '@/server/services/workflow-dashboard-queries';
...
export const GET = withAuth(async (req: AuthenticatedRequest) => {
  ...
  const cacheKey = buildWorkflowCacheKey(req.orgId, req.role, req.userId, today);
  ...
  const dependent = await fetchWorkflowDependentData(prisma, req.orgId, today, core);
