...
export const GET = withAuth(async (req: AuthenticatedRequest) => {
  ...
  const averageMonthlyVisits =
    memberships.length > 0
      ? visitRecords.length / memberships.length
      : 0;
  ...
  const items = memberships.map((membership) => {
    ...
    const reportSubmissionRate =
      current.monthlyVisitCount > 0
        ? round(
            (current.reportVisitRecordIds.size / current.monthlyVisitCount) * 100,
            0
          )
        : 0;
    const workloadBalanceDeltaPercent =
      averageMonthlyVisits > 0
        ? round(
            ((current.monthlyVisitCount - averageMonthlyVisits) / averageMonthlyVisits) *
              100,
            0
          )
        : 0;
    ...
    const monthlyCapacity =
      weeklyCapacity != null ? weeklyCapacity * 4.3 : null;
    ...
    return {
      ...
      shift_hours:
        current.totalShiftMinutes > 0
          ? round(current.totalShiftMinutes / 60, 1)
          : 0,
  ...
  const avgReportSubmissionRate =
    items.length > 0
      ? round(
          items.reduce((sum, item) => sum + item.report_submission_rate, 0) /
            items.length,
          0
        )
      : 0;
