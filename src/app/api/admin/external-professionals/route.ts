...
export const GET = withAuth(async (req: AuthenticatedRequest) => {
  ...
  const professionType = professionTypeSchema
    .safeParse(req.nextUrl.searchParams.get('profession_type')?.trim())
    .data;
  const preferredContactMethod = contactMethodSchema
    .safeParse(req.nextUrl.searchParams.get('preferred_contact_method')?.trim())
    .data;
...
export const POST = withAuth(async (req: AuthenticatedRequest) => {
  ...
  const parsed = createExternalProfessionalSchema.safeParse(body);
  if (!parsed.success) {
