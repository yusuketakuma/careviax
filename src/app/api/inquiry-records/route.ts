...
async function authenticatedGET(req: NextRequest, ctx: AuthContext) {
  ...
  if (cycleIdParam.error) {
    return validationError('検索条件が不正です', { cycle_id: [cycleIdParam.error] });
  }
  const patientIdParam = readOptionalSearchParam(searchParams, 'patient_id');
  if (patientIdParam.error) {
    return validationError('検索条件が不正です', { patient_id: [patientIdParam.error] });
  }
  const statusParam = readOptionalSearchParam(searchParams, 'status');
  if (statusParam.error) {
    return validationError('検索条件が不正です', { status: [statusParam.error] });
  }
