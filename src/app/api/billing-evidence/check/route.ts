...
async function billingEvidenceCheckGET(req: NextRequest, ctx: AuthContext) {
  ...
  const data = (() => {
    const patientNameById = new Map(details.patients.map((patient) => [patient.id, patient.name]));
    ...
    const reviewRows: BillingCheckReviewRow[] = base.reviewCandidates.map((candidate) => {
      ...
      const preset = CHECK_ACTION_PRESETS.find((item) => item.pattern.test(candidate.billing_name));
...
export const GET = withAuthContext(billingEvidenceCheckGET, {
