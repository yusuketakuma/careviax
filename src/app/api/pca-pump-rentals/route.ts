...
function findDuplicateRentalQueryParams(searchParams: URLSearchParams) {
  const fieldErrors: Record<string, string[]> = {};
  for (const name of ['status', 'inspection_status', 'institution_id'] as const) {
...
async function authenticatedGET(req: NextRequest, ctx: AuthContext) {
  const duplicateQueryParams = findDuplicateRentalQueryParams(req.nextUrl.searchParams);
