...
export type ImportSourceUrlPolicy<TSource extends string = string> = {
  source: TSource;
  allowedHosts: readonly string[];
  maxBytes: number;
  timeoutMs?: number;
  maxRedirects?: number;
  extraAllowedHostsEnv?: string;
};
...
type FetchImportOptions<TSource extends string = string> = {
  fetchImpl?: FetchLike;
  policy: ImportSourceUrlPolicy<TSource>;
  accept?: string;
  maxBytes?: number;
...
function normalizeImportSourceForLog(source: string) {
...
function getAllowedHosts(policy: ImportSourceUrlPolicy) {
  const extraHosts =
    process.env[policy.extraAllowedHostsEnv ?? EXTRA_ALLOWED_IMPORT_HOSTS_ENV]
      ?.split(',')
...
function isAllowedHostname(hostname: string, policy: ImportSourceUrlPolicy) {
  const normalized = normalizeHostname(hostname);
...
export function isAllowedImportSourceUrl(url: string, policy: ImportSourceUrlPolicy) {
...
export function normalizeImportSourceUrl(url: string, policy: ImportSourceUrlPolicy) {
