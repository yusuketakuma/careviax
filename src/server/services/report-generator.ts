...
export async function generateReportsFromVisit(
  ...
  const [patient, medicationCycle, residualMedications, careTeamLinks, pharmacistUser, billingEvidence] =
    await Promise.all([
      ...
      prisma.residualMedication.findMany({
        ...
        select: { drug_name: true, remaining_quantity: true, excess_days: true, is_reduction_target: true },
      }),
  ...
  const prescriptionLines =
    medicationCycle != null
      ? await prisma.prescriptionLine.findMany({
          ...
          select: { drug_name: true, dose: true, frequency: true, days: true, route: true, dispensing_method: true },
  ...
  for (const type of typesToGenerate) {
    if (type === 'physician_report') {
      contentByType.set(type, buildPhysicianReport({
    ...
    : await withOrgContext(orgId, async (tx) => {
        ...
        tx.careReport.create({
          data: {
            ...
            report_type: type,
            status: 'draft',
            content: contentByType.get(type) as import('@prisma/client').Prisma.InputJsonValue,
            created_by: userId,
          },
          select: { id: true, report_type: true },
        })
