...
const taskAssigneeIdSchema = z
  .string()
  .trim()
  .min(1, 'assigned_to は空にできません')
  .max(191, 'assigned_to は191文字以内で指定してください');
...
export const createTaskSchema = z.object({
  ...
  assigned_to: taskAssigneeIdSchema.nullable().optional(),
  due_date: z.string().datetime('due_date の日時形式が不正です').nullable().optional(),
  sla_due_at: z.string().datetime('sla_due_at の日時形式が不正です').nullable().optional(),
