import { describe, expect, it } from 'vitest';
import { Pool, type PoolClient } from 'pg';

const databaseUrl = process.env.PAGINATION_ORDER_DATABASE_URL;
const isSafeLocalE2eDatabase =
  !databaseUrl ||
  /^postgresql:\/\/[^@/]+@(?:localhost|127\.0\.0\.1|\[::1\]):5433\/ph_os_e2e(?:\?|$)/.test(
    databaseUrl,
  );

if (!isSafeLocalE2eDatabase) {
  throw new Error(
    'PAGINATION_ORDER_DATABASE_URL must point to the local ph_os_e2e database on port 5433',
  );
}

const describeDatabase = databaseUrl ? describe : describe.skip;
const PAGE_LIMIT = 2;

type OrderedRow = {
  id: string;
  sort_key: Date;
};

type CursorPage = {
  ids: string[];
  hasMore: boolean;
  nextCursor: string | null;
};

type BillingOrderedRow = {
  id: string;
  total_count: string;
};

async function fetchPage(client: PoolClient, cursor: string | null): Promise<CursorPage> {
  const result = await client.query<OrderedRow>(
    `WITH rows(id, sort_key) AS (
       VALUES
         ('row_a', TIMESTAMPTZ '2026-07-15 00:00:00+00'),
         ('row_b', TIMESTAMPTZ '2026-07-15 00:00:00+00'),
         ('row_c', TIMESTAMPTZ '2026-07-15 00:00:00+00'),
         ('row_d', TIMESTAMPTZ '2026-07-15 00:00:00+00')
     ),
     cursor_row AS (
       SELECT sort_key, id
       FROM rows
       WHERE id = $1::text
     )
     SELECT rows.id, rows.sort_key
     FROM rows
     LEFT JOIN cursor_row ON true
     WHERE $1::text IS NULL
        OR (rows.sort_key, rows.id) < (cursor_row.sort_key, cursor_row.id)
     ORDER BY rows.sort_key DESC, rows.id DESC
     LIMIT $2`,
    [cursor, PAGE_LIMIT + 1],
  );
  const hasMore = result.rows.length > PAGE_LIMIT;
  const visibleRows = hasMore ? result.rows.slice(0, PAGE_LIMIT) : result.rows;

  return {
    ids: visibleRows.map((row) => row.id),
    hasMore,
    nextCursor: hasMore ? (visibleRows[visibleRows.length - 1]?.id ?? null) : null,
  };
}

async function fetchPartnerBillingPage(
  client: PoolClient,
  cursor: string | null,
): Promise<CursorPage & { totalCount: number }> {
  const result = await client.query<BillingOrderedRow>(
    `WITH rows AS (
       SELECT
         'candidate_' || lpad(series::text, 2, '0') AS id,
         DATE '2026-06-01' AS billing_month,
         TIMESTAMPTZ '2026-06-20 00:00:00+00' AS created_at,
         'partner_pharmacy_1'::text AS partner_pharmacy_id,
         'candidate'::text AS status
       FROM generate_series(1, 21) AS series
       UNION ALL
       SELECT
         'candidate_other_provider',
         DATE '2026-06-01',
         TIMESTAMPTZ '2026-06-20 00:00:00+00',
         'partner_pharmacy_2',
         'candidate'
     ),
     filtered AS (
       SELECT *
       FROM rows
       WHERE partner_pharmacy_id = $1::text
         AND status = $2::text
     ),
     cursor_row AS (
       SELECT billing_month, created_at, id
       FROM filtered
       WHERE id = $3::text
     )
     SELECT
       filtered.id,
       (SELECT count(*)::text FROM filtered) AS total_count
     FROM filtered
     LEFT JOIN cursor_row ON true
     WHERE $3::text IS NULL
        OR (filtered.billing_month, filtered.created_at, filtered.id)
           < (cursor_row.billing_month, cursor_row.created_at, cursor_row.id)
     ORDER BY filtered.billing_month DESC, filtered.created_at DESC, filtered.id DESC
     LIMIT $4`,
    ['partner_pharmacy_1', 'candidate', cursor, 21],
  );
  const hasMore = result.rows.length > 20;
  const visibleRows = hasMore ? result.rows.slice(0, 20) : result.rows;

  return {
    ids: visibleRows.map((row) => row.id),
    hasMore,
    nextCursor: hasMore ? (visibleRows[visibleRows.length - 1]?.id ?? null) : null,
    totalCount: Number(result.rows[0]?.total_count ?? 0),
  };
}

async function fetchShareCaseWorkflowPage(
  client: PoolClient,
  cursor: string | null,
): Promise<CursorPage & { totalCount: number }> {
  const result = await client.query<BillingOrderedRow>(
    `WITH rows AS (
       SELECT
         'share_case_' || lpad(series::text, 2, '0') AS id,
         TIMESTAMPTZ '2026-07-16 00:00:00+00' AS updated_at,
         'active'::text AS status
       FROM generate_series(1, 9) AS series
       UNION ALL
       SELECT
         'share_case_draft',
         TIMESTAMPTZ '2026-07-16 00:00:00+00',
         'draft'
     ),
     filtered AS (
       SELECT *
       FROM rows
       WHERE status = $1::text
     ),
     cursor_row AS (
       SELECT updated_at, id
       FROM filtered
       WHERE id = $2::text
     )
     SELECT
       filtered.id,
       (SELECT count(*)::text FROM filtered) AS total_count
     FROM filtered
     LEFT JOIN cursor_row ON true
     WHERE $2::text IS NULL
        OR (filtered.updated_at, filtered.id) < (cursor_row.updated_at, cursor_row.id)
     ORDER BY filtered.updated_at DESC, filtered.id DESC
     LIMIT $3`,
    ['active', cursor, 9],
  );
  const hasMore = result.rows.length > 8;
  const visibleRows = hasMore ? result.rows.slice(0, 8) : result.rows;

  return {
    ids: visibleRows.map((row) => row.id),
    hasMore,
    nextCursor: hasMore ? (visibleRows[visibleRows.length - 1]?.id ?? null) : null,
    totalCount: Number(result.rows[0]?.total_count ?? 0),
  };
}

async function fetchVisitRequestWorkflowPage(
  client: PoolClient,
  cursor: string | null,
): Promise<CursorPage & { totalCount: number }> {
  const result = await client.query<BillingOrderedRow>(
    `WITH rows AS (
       SELECT
         'visit_request_' || lpad(series::text, 2, '0') AS id,
         TIMESTAMPTZ '2026-07-16 00:00:00+00' AS updated_at,
         'requested'::text AS status
       FROM generate_series(1, 9) AS series
       UNION ALL
       SELECT
         'visit_request_accepted',
         TIMESTAMPTZ '2026-07-16 00:00:00+00',
         'accepted'
     ),
     filtered AS (
       SELECT *
       FROM rows
       WHERE status = $1::text
     ),
     cursor_row AS (
       SELECT updated_at, id
       FROM filtered
       WHERE id = $2::text
     )
     SELECT
       filtered.id,
       (SELECT count(*)::text FROM filtered) AS total_count
     FROM filtered
     LEFT JOIN cursor_row ON true
     WHERE $2::text IS NULL
        OR (filtered.updated_at, filtered.id) < (cursor_row.updated_at, cursor_row.id)
     ORDER BY filtered.updated_at DESC, filtered.id DESC
     LIMIT $3`,
    ['requested', cursor, 9],
  );
  const hasMore = result.rows.length > 8;
  const visibleRows = hasMore ? result.rows.slice(0, 8) : result.rows;

  return {
    ids: visibleRows.map((row) => row.id),
    hasMore,
    nextCursor: hasMore ? (visibleRows[visibleRows.length - 1]?.id ?? null) : null,
    totalCount: Number(result.rows[0]?.total_count ?? 0),
  };
}

async function fetchPartnerVisitRecordWorkflowPage(
  client: PoolClient,
  cursor: string | null,
): Promise<CursorPage & { totalCount: number }> {
  const result = await client.query<BillingOrderedRow>(
    `WITH rows AS (
       SELECT
         'partner_record_' || lpad(series::text, 2, '0') AS id,
         TIMESTAMPTZ '2026-07-16 00:00:00+00' AS updated_at,
         'submitted'::text AS status
       FROM generate_series(1, 9) AS series
       UNION ALL
       SELECT
         'partner_record_confirmed',
         TIMESTAMPTZ '2026-07-16 00:00:00+00',
         'confirmed'
     ),
     filtered AS (
       SELECT *
       FROM rows
       WHERE status = $1::text
     ),
     cursor_row AS (
       SELECT updated_at, id
       FROM filtered
       WHERE id = $2::text
     )
     SELECT
       filtered.id,
       (SELECT count(*)::text FROM filtered) AS total_count
     FROM filtered
     LEFT JOIN cursor_row ON true
     WHERE $2::text IS NULL
        OR (filtered.updated_at, filtered.id) < (cursor_row.updated_at, cursor_row.id)
     ORDER BY filtered.updated_at DESC, filtered.id DESC
     LIMIT $3`,
    ['submitted', cursor, 9],
  );
  const hasMore = result.rows.length > 8;
  const visibleRows = hasMore ? result.rows.slice(0, 8) : result.rows;

  return {
    ids: visibleRows.map((row) => row.id),
    hasMore,
    nextCursor: hasMore ? (visibleRows[visibleRows.length - 1]?.id ?? null) : null,
    totalCount: Number(result.rows[0]?.total_count ?? 0),
  };
}

async function fetchCorrectionRequestWorkflowPage(
  client: PoolClient,
  cursor: string | null,
): Promise<CursorPage & { totalCount: number }> {
  const result = await client.query<BillingOrderedRow>(
    `WITH rows AS (
       SELECT
         'correction_request_' || lpad(series::text, 2, '0') AS id,
         TIMESTAMPTZ '2026-07-16 00:00:00+00' AS created_at,
         'share_case_1'::text AS share_case_id,
         'open'::text AS status
       FROM generate_series(1, 9) AS series
       UNION ALL
       SELECT
         'correction_request_other_case',
         TIMESTAMPTZ '2026-07-16 00:00:00+00',
         'share_case_2',
         'open'
       UNION ALL
       SELECT
         'correction_request_resolved',
         TIMESTAMPTZ '2026-07-16 00:00:00+00',
         'share_case_1',
         'resolved'
     ),
     filtered AS (
       SELECT *
       FROM rows
       WHERE share_case_id = $1::text
         AND status = $2::text
     ),
     cursor_row AS (
       SELECT created_at, id
       FROM filtered
       WHERE id = $3::text
     )
     SELECT
       filtered.id,
       (SELECT count(*)::text FROM filtered) AS total_count
     FROM filtered
     LEFT JOIN cursor_row ON true
     WHERE $3::text IS NULL
        OR (filtered.created_at, filtered.id) < (cursor_row.created_at, cursor_row.id)
     ORDER BY filtered.created_at DESC, filtered.id DESC
     LIMIT $4`,
    ['share_case_1', 'open', cursor, 9],
  );
  const hasMore = result.rows.length > 8;
  const visibleRows = hasMore ? result.rows.slice(0, 8) : result.rows;

  return {
    ids: visibleRows.map((row) => row.id),
    hasMore,
    nextCursor: hasMore ? (visibleRows[visibleRows.length - 1]?.id ?? null) : null,
    totalCount: Number(result.rows[0]?.total_count ?? 0),
  };
}

async function fetchPatientShareConsentWorkflowPage(
  client: PoolClient,
  cursor: string | null,
): Promise<CursorPage & { totalCount: number }> {
  const result = await client.query<BillingOrderedRow>(
    `WITH rows AS (
       SELECT
         'share_consent_' || lpad(series::text, 2, '0') AS id,
         TIMESTAMPTZ '2026-07-16 00:00:00+00' AS created_at,
         'share_case_1'::text AS share_case_id,
         NULL::timestamptz AS revoked_at
       FROM generate_series(1, 9) AS series
       UNION ALL
       SELECT
         'share_consent_other_case',
         TIMESTAMPTZ '2026-07-16 00:00:00+00',
         'share_case_2',
         NULL::timestamptz
       UNION ALL
       SELECT
         'share_consent_revoked',
         TIMESTAMPTZ '2026-07-16 00:00:00+00',
         'share_case_1',
         TIMESTAMPTZ '2026-07-16 01:00:00+00'
     ),
     filtered AS (
       SELECT *
       FROM rows
       WHERE share_case_id = $1::text
         AND revoked_at IS NULL
     ),
     cursor_row AS (
       SELECT created_at, id
       FROM filtered
       WHERE id = $2::text
     )
     SELECT
       filtered.id,
       (SELECT count(*)::text FROM filtered) AS total_count
     FROM filtered
     LEFT JOIN cursor_row ON true
     WHERE $2::text IS NULL
        OR (filtered.created_at, filtered.id) < (cursor_row.created_at, cursor_row.id)
     ORDER BY filtered.created_at DESC, filtered.id DESC
     LIMIT $3`,
    ['share_case_1', cursor, 9],
  );
  const hasMore = result.rows.length > 8;
  const visibleRows = hasMore ? result.rows.slice(0, 8) : result.rows;

  return {
    ids: visibleRows.map((row) => row.id),
    hasMore,
    nextCursor: hasMore ? (visibleRows[visibleRows.length - 1]?.id ?? null) : null,
    totalCount: Number(result.rows[0]?.total_count ?? 0),
  };
}

describeDatabase('stable cursor ordering (PAGINATION_ORDER_DATABASE_URL)', () => {
  it('returns equal-key rows exactly once across two limit-plus-one pages', async () => {
    expect(databaseUrl).toBeTruthy();
    const pool = new Pool({
      connectionString: databaseUrl,
      max: 1,
      connectionTimeoutMillis: 3_000,
    });

    try {
      const client = await pool.connect();
      try {
        await client.query('BEGIN READ ONLY');
        const readOnly = await client.query<{ transaction_read_only: string }>(
          'SHOW transaction_read_only',
        );
        expect(readOnly.rows[0]?.transaction_read_only).toBe('on');

        const firstPage = await fetchPage(client, null);
        const secondPage = await fetchPage(client, firstPage.nextCursor);
        const allIds = [...firstPage.ids, ...secondPage.ids];

        expect(firstPage).toEqual({
          ids: ['row_d', 'row_c'],
          hasMore: true,
          nextCursor: 'row_c',
        });
        expect(secondPage).toEqual({
          ids: ['row_b', 'row_a'],
          hasMore: false,
          nextCursor: null,
        });
        expect(new Set(allIds).size).toBe(allIds.length);
        expect(allIds).toEqual(['row_d', 'row_c', 'row_b', 'row_a']);
      } finally {
        try {
          await client.query('ROLLBACK');
        } finally {
          client.release();
        }
      }
    } finally {
      await pool.end();
    }
  });

  it('keeps a filtered partner-billing 20/21 cursor chain exact across equal keys', async () => {
    expect(databaseUrl).toBeTruthy();
    const pool = new Pool({
      connectionString: databaseUrl,
      max: 1,
      connectionTimeoutMillis: 3_000,
    });

    try {
      const client = await pool.connect();
      try {
        await client.query('BEGIN READ ONLY');
        const firstPage = await fetchPartnerBillingPage(client, null);
        const secondPage = await fetchPartnerBillingPage(client, firstPage.nextCursor);
        const allIds = [...firstPage.ids, ...secondPage.ids];

        expect(firstPage).toMatchObject({
          hasMore: true,
          nextCursor: 'candidate_02',
          totalCount: 21,
        });
        expect(firstPage.ids).toHaveLength(20);
        expect(secondPage).toEqual({
          ids: ['candidate_01'],
          hasMore: false,
          nextCursor: null,
          totalCount: 21,
        });
        expect(new Set(allIds).size).toBe(21);
        expect(allIds).not.toContain('candidate_other_provider');
      } finally {
        try {
          await client.query('ROLLBACK');
        } finally {
          client.release();
        }
      }
    } finally {
      await pool.end();
    }
  });

  it('keeps a filtered share-case workflow 8/9 cursor chain exact across equal keys', async () => {
    expect(databaseUrl).toBeTruthy();
    const pool = new Pool({
      connectionString: databaseUrl,
      max: 1,
      connectionTimeoutMillis: 3_000,
    });

    try {
      const client = await pool.connect();
      try {
        await client.query('BEGIN READ ONLY');
        const firstPage = await fetchShareCaseWorkflowPage(client, null);
        const secondPage = await fetchShareCaseWorkflowPage(client, firstPage.nextCursor);
        const allIds = [...firstPage.ids, ...secondPage.ids];

        expect(firstPage).toMatchObject({
          hasMore: true,
          nextCursor: 'share_case_02',
          totalCount: 9,
        });
        expect(firstPage.ids).toHaveLength(8);
        expect(secondPage).toEqual({
          ids: ['share_case_01'],
          hasMore: false,
          nextCursor: null,
          totalCount: 9,
        });
        expect(new Set(allIds).size).toBe(9);
        expect(allIds).not.toContain('share_case_draft');
      } finally {
        try {
          await client.query('ROLLBACK');
        } finally {
          client.release();
        }
      }
    } finally {
      await pool.end();
    }
  });

  it('keeps a filtered visit-request workflow 8/9 cursor chain exact across equal keys', async () => {
    expect(databaseUrl).toBeTruthy();
    const pool = new Pool({
      connectionString: databaseUrl,
      max: 1,
      connectionTimeoutMillis: 3_000,
    });

    try {
      const client = await pool.connect();
      try {
        await client.query('BEGIN READ ONLY');
        const firstPage = await fetchVisitRequestWorkflowPage(client, null);
        const secondPage = await fetchVisitRequestWorkflowPage(client, firstPage.nextCursor);
        const allIds = [...firstPage.ids, ...secondPage.ids];

        expect(firstPage).toMatchObject({
          hasMore: true,
          nextCursor: 'visit_request_02',
          totalCount: 9,
        });
        expect(firstPage.ids).toHaveLength(8);
        expect(secondPage).toEqual({
          ids: ['visit_request_01'],
          hasMore: false,
          nextCursor: null,
          totalCount: 9,
        });
        expect(new Set(allIds).size).toBe(9);
        expect(allIds).not.toContain('visit_request_accepted');
      } finally {
        try {
          await client.query('ROLLBACK');
        } finally {
          client.release();
        }
      }
    } finally {
      await pool.end();
    }
  });

  it('keeps a filtered partner-record workflow 8/9 cursor chain exact across equal keys', async () => {
    expect(databaseUrl).toBeTruthy();
    const pool = new Pool({
      connectionString: databaseUrl,
      max: 1,
      connectionTimeoutMillis: 3_000,
    });

    try {
      const client = await pool.connect();
      try {
        await client.query('BEGIN READ ONLY');
        const firstPage = await fetchPartnerVisitRecordWorkflowPage(client, null);
        const secondPage = await fetchPartnerVisitRecordWorkflowPage(client, firstPage.nextCursor);
        const allIds = [...firstPage.ids, ...secondPage.ids];

        expect(firstPage).toMatchObject({
          hasMore: true,
          nextCursor: 'partner_record_02',
          totalCount: 9,
        });
        expect(firstPage.ids).toHaveLength(8);
        expect(secondPage).toEqual({
          ids: ['partner_record_01'],
          hasMore: false,
          nextCursor: null,
          totalCount: 9,
        });
        expect(new Set(allIds).size).toBe(9);
        expect(allIds).not.toContain('partner_record_confirmed');
      } finally {
        try {
          await client.query('ROLLBACK');
        } finally {
          client.release();
        }
      }
    } finally {
      await pool.end();
    }
  });

  it('keeps a share-case correction-request 8/9 cursor chain exact across equal keys', async () => {
    expect(databaseUrl).toBeTruthy();
    const pool = new Pool({
      connectionString: databaseUrl,
      max: 1,
      connectionTimeoutMillis: 3_000,
    });

    try {
      const client = await pool.connect();
      try {
        await client.query('BEGIN READ ONLY');
        const firstPage = await fetchCorrectionRequestWorkflowPage(client, null);
        const secondPage = await fetchCorrectionRequestWorkflowPage(client, firstPage.nextCursor);
        const allIds = [...firstPage.ids, ...secondPage.ids];

        expect(firstPage).toMatchObject({
          hasMore: true,
          nextCursor: 'correction_request_02',
          totalCount: 9,
        });
        expect(firstPage.ids).toHaveLength(8);
        expect(secondPage).toEqual({
          ids: ['correction_request_01'],
          hasMore: false,
          nextCursor: null,
          totalCount: 9,
        });
        expect(new Set(allIds).size).toBe(9);
        expect(allIds).not.toContain('correction_request_other_case');
        expect(allIds).not.toContain('correction_request_resolved');
      } finally {
        try {
          await client.query('ROLLBACK');
        } finally {
          client.release();
        }
      }
    } finally {
      await pool.end();
    }
  });

  it('keeps an active patient-share-consent 8/9 cursor chain exact across equal keys', async () => {
    expect(databaseUrl).toBeTruthy();
    const pool = new Pool({
      connectionString: databaseUrl,
      max: 1,
      connectionTimeoutMillis: 3_000,
    });

    try {
      const client = await pool.connect();
      try {
        await client.query('BEGIN READ ONLY');
        const firstPage = await fetchPatientShareConsentWorkflowPage(client, null);
        const secondPage = await fetchPatientShareConsentWorkflowPage(client, firstPage.nextCursor);
        const allIds = [...firstPage.ids, ...secondPage.ids];

        expect(firstPage).toMatchObject({
          hasMore: true,
          nextCursor: 'share_consent_02',
          totalCount: 9,
        });
        expect(firstPage.ids).toHaveLength(8);
        expect(secondPage).toEqual({
          ids: ['share_consent_01'],
          hasMore: false,
          nextCursor: null,
          totalCount: 9,
        });
        expect(new Set(allIds).size).toBe(9);
        expect(allIds).not.toContain('share_consent_other_case');
        expect(allIds).not.toContain('share_consent_revoked');
      } finally {
        try {
          await client.query('ROLLBACK');
        } finally {
          client.release();
        }
      }
    } finally {
      await pool.end();
    }
  });
});
