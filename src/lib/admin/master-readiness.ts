...
export const ADMIN_MASTER_READINESS_GROUPS: readonly AdminMasterReadinessGroup[] = [
  {
    ...
    items: [
      ...
      {
        label: '休日カレンダー',
        href: '/admin/business-holidays',
        purpose: '訪問候補生成の休業日判定',
      },
      {
        label: '通知設定',
        href: '/admin/notification-settings',
        purpose: '滞留・訪問・報告の通知制御',
      },
    ],
  },
  {
    ...
    items: [
      ...
      {
        label: 'PCAポンプ',
        href: '/admin/pca-pumps',
        purpose: '医療機関への貸出資産と返却検品の管理',
      },
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      ...
      {
        label: '薬剤師資格',
        href: '/admin/pharmacist-credentials',
        purpose: '在宅算定に関わる資格管理',
      },
...
export function listAdminMasterReadinessHrefs() {
  return ADMIN_MASTER_READINESS_GROUPS.flatMap((group) => group.items.map((item) => item.href));
}
