...
Improve PH-OS UI/UX to a level appropriate for a medical system by using current best-practice research, understanding existing code and workflows, planning, implementing, validating, reviewing, and revising autonomously.
...
| E2E gate readiness | Added `pnpm medical-ui:e2e:preflight` to check local `ph_os_e2e`, app/DB ports, required Playwright specs, and CareReport duplicate precheck script before final browser/a11y validation. Added `pnpm medical-ui:e2e:gate` to chain preflight, duplicate precheck, and targeted Playwright. | Covered as readiness gate; currently failing as expected |
...
- Playwright local auth requires `DATABASE_URL` pointing to local `ph_os_e2e`.
...
- `pnpm --config.verify-deps-before-run=false medical-ui:e2e:preflight` now passes both `DATABASE_URL` and `DIRECT_URL` target checks for local `ph_os_e2e`, but fails because `localhost:3012` and `localhost:5433` are closed.
...
- Validation for the preflight script:
  ...
  - Result: failed as expected; passed the `DATABASE_URL` local `ph_os_e2e` target check, then reported closed app port `3012` and closed DB port `5433`.
...
- Gate short-circuit validation:
  ...
  - Result: failed as expected at `medical-ui:e2e:preflight`; duplicate precheck and Playwright execution were not reached. The gate now pins `DATABASE_URL` and `DIRECT_URL` to local `ph_os_e2e` and fails only on closed app/DB ports.
...
- E2E local script alignment:
  - `dev:e2e:local`, `build:e2e:local`, `start:e2e:local`, `test:e2e:local`, and `test:e2e:local:list` now pin `DATABASE_URL` and `DIRECT_URL` to local `ph_os_e2e` so the app server and Playwright auth helper use the same database.
  - `pnpm --config.verify-deps-before-run=false test:e2e:local:list`
  - Result: passed; listed 430 tests in 16 files with the local `ph_os_e2e` target.
  ...
  - Result: failed as expected with Prisma `P1001`, confirming the command targets `ph_os_e2e` at `localhost:5433` and the DB server is currently unreachable.
  ...
  - Updated `medical-ui:e2e:preflight` to verify required package scripts and target Playwright spec files before the final browser gate. `db:e2e:prepare` now also pins the local `ph_os_e2e` URLs directly.
  - `pnpm --config.verify-deps-before-run=false medical-ui:e2e:preflight`
...
- Research medical UI/UX, accessibility, EHR safety, human factors, and medication-safety best practices and translate them into concrete PH-OS changes.
...
| Code/function understanding | `.codex/ralph-state.md` records inspected UI routes, APIs, Prisma models, auth/access services, tests, local Next docs, and PH-OS UI/UX SSOT across each implementation slice. | Met |
...
| Operation/browser validation | `medical-ui:e2e:gate` passed against production `next start` with local `ph_os_e2e`: 122 passed / 62 skipped. Skips are project-intentional desktop/mobile split tests. | Met |
...
- E2E DB setup:
  ...
  - Created local role/database `ph_os` / `ph_os_e2e`: passed.
...
- Security review: No new PHI logging was introduced; duplicate check prints IDs/counts only; E2E DB is constrained to local `ph_os_e2e`; existing unrelated lint warning remains non-security.
...
- The CareReport duplicate precheck has been proven on local `ph_os_e2e`; it must still be run against any target environment before applying the unique-index migration there.
...
- Local E2E database: initialized an isolated PostgreSQL 17 cluster under `.codex/pg-e2e`, created `ph_os_e2e`, ran `pnpm --config.verify-deps-before-run=false db:e2e:prepare`, then stopped and removed the temporary cluster after validation.
...
- Added `db:e2e:check-care-report-duplicates`, a local E2E-pinned duplicate precheck for `ph_os_e2e`.
...
- `pnpm --config.verify-deps-before-run=false db:e2e:prepare`: passed against a temporary local PostgreSQL 17 `ph_os_e2e`.
...
- Re-read the PH-OS UI/UX SSOT in `docs/ui-ux-design-guidelines.md`.
...
- Production-style medical UI browser/a11y gate:
  - Created a temporary local PostgreSQL 17 `ph_os_e2e` cluster on `localhost:5433`, ran `db:e2e:prepare`, and stopped/removed the cluster after validation.
...
Complete for the local verified PH-OS environment.
...
The follow-up recommendation set was completed for the local verified PH-OS
...
- Added `.github/workflows/ci.yml` job `medical-ui-e2e-gate` so CI runs the
  local `ph_os_e2e` prepare step and `medical-ui:e2e:gate:prod` after the
  main CI job.
...
- Full validation:
  ...
  - `git diff --check`: passed.
- Production-style medical UI browser/a11y gate:
  - Created a temporary local PostgreSQL 17 `ph_os_e2e` cluster on
    `localhost:5433`.
  - `pnpm --config.verify-deps-before-run=false db:e2e:prepare`: passed.
  - `pnpm --config.verify-deps-before-run=false medical-ui:e2e:gate:prod`: passed.
