...
- `API-CONTRACT-001CO`: `patients/:id/labs` の POST create success を `data` envelope へ移行済み。GET list は既に `data` envelope のまま維持し、患者検査値カードの作成 mutation は成功 body に依存せず再取得するため、route test の create success fixture だけ current envelope に更新した。旧 raw lab observation root fields は残さず、visit permission、patient id validation、患者書込guard、source visit provenance validation、display id allocation、RLS write context、GET no-store/error response は変更しない。allowlist debt は 89 → 88。
- `API-CONTRACT-001CP`: `patients/:id/overview` の GET patient overview success を `data` envelope へ移行済み。患者詳細 workspace、比較ボード、連携タブ、患者編集画面の overview reader は `payload.data` 契約へ更新した。旧 raw patient overview root fields は残さず、visit permission、patient id validation、org/RLS scoped overview read、PHI read audit、no-store wrapper、not-found/internal-error response は変更しない。allowlist debt は 88 → 87。
- `API-CONTRACT-001CQ`: `patients/:id/prescriptions` の GET patient prescriptions list success を `data` envelope へ移行済み。処方履歴画面、印刷ハブ、直近過去歴サマリーの prescriptions reader は outer `payload.data` を unwrap し、内側の `data` 配列契約は維持した。旧 root `patient` / `data` / `hasMore` / `nextCursor` / `diff_review` / `diff_meta` は残さず、visit permission、patient id validation、case_id validation、assignment scope、RLS request context、no-store wrapper、not-found/internal-error response は変更しない。allowlist debt は 87 → 86。
- `API-CONTRACT-001CR`: `patients/:id/readiness` の GET patient readiness success を `data` envelope へ移行済み。患者 readiness card reader は `payload.data` 契約へ更新した。旧 root readiness fields は残さず、visit permission、patient id validation、service auth context、no-store wrapper、not-found/internal-error response は変更しない。allowlist debt は 86 → 85。
- `API-CONTRACT-001CS`: `patients/:id/restore` の PATCH patient restore success を `data` envelope へ移行済み。consumer 依存はなく、route test で restored patient payload の envelope を固定した。旧 root restored patient fields は残さず、admin permission、patient id validation、org-scoped archived patient lookup、RLS write context、not-found/conflict response は変更しない。allowlist debt は 85 → 84。
- `API-CONTRACT-001CT`: `patients/:id` の GET patient detail success を `data` envelope へ移行済み。外部共有、訪問 capture fallback、処方 intake、患者安全確認、服薬画面、各種印刷画面の base patient detail reader は `payload.data` 契約へ更新した。PATCH success は今回の slice では変更せず、旧 root patient detail fields は GET から残さない。visit permission、patient id validation、assignment scope、PHI read audit、masking、no-store wrapper、not-found/internal-error response は変更しない。allowlist debt は 84 → 83。
- `API-CONTRACT-001CU`: `patients/:id/visits` の GET patient visits snapshot success を `data` envelope へ移行済み。直接の frontend reader 依存はなく、route slice test で visits snapshot payload の envelope を固定した。旧 root `monthly_visit_count` / `visit_schedules` / `visit_records` / `home_care_feature_summary` は残さず、visit permission、patient id validation、assignment scope、no-store wrapper、not-found/internal-error response は変更しない。allowlist debt は 83 → 82。
- `API-CONTRACT-001CV`: `patients/:id/workflow-preview` の GET patient workflow preview success を `data` envelope へ移行済み。患者 workflow preview card reader は `payload.data` 契約へ更新した。旧 root `visit_preparation` / `report_targets` / `communication_priority` は残さず、visit permission、patient id validation、assignment scope、no-store wrapper、not-found/internal-error response は変更しない。allowlist debt は 82 → 81。
- `API-CONTRACT-001CW`: `patients/check-duplicate` の GET duplicate candidate success を `data` envelope へ移行済み。患者フォームの重複候補 reader は `payload.data` 契約へ更新した。旧 root `duplicates` は残さず、visit permission、query validation、org-scoped duplicate detection、no-store wrapper、internal-error response は変更しない。allowlist debt は 81 → 80。
- `API-CONTRACT-001CX`: `patients` の GET list/search/palette/match success を outer `data` envelope へ移行済み。内側の list payload（`data` / `summary` / `hasMore` / `privacy`）は保持し、処方 intake、QR scan、検索ページ、global search palette の reader/schema は outer envelope を unwrap する契約へ更新した。POST patient create success と 409 duplicate conflict は今回の slice では変更せず、visit permission、rate limit、query validation、org/assignment scoped reads、sensitive no-store、internal-error response は変更しない。allowlist debt は 80 → 76。
- `API-CONTRACT-001CY`: `pharmacist-shifts` の POST upsert success を `data` envelope へ移行済み。管理シフト画面は POST success body に依存せず保存件数と query invalidation で進むため reader 変更は不要だった。GET list/data+meta 契約、visit permission、route performance wrapper、RLS request context、org reference validation、time normalization、no-store wrapper、sanitized internal-error response は変更しない。allowlist debt は 76 → 75。
- `API-CONTRACT-001CZ`: `pharmacists` の GET list success と POST create success を `data + meta` / `data` envelope へ移行済み。管理ユーザー画面は user list metadata を `payload.meta` から読む契約へ更新し、POST invite mutation と各スタッフ選択 reader は成功 body の root field に依存しない。auth context、collaborator permission guard、site_id validation、RLS request context、monthly visit count aggregation、Cognito rollback、no-store wrapper、sanitized internal-error response は変更しない。allowlist debt は 75 → 74。
- `API-CONTRACT-001DA`: `pharmacy-contracts/:id/documents` の POST preview/save success を `data` envelope へ移行済み。薬局間連携設定画面は preview/save schema で `payload.data` を unwrap し、旧 root preview/document fields を malformed success として拒否する契約を維持した。GET list、contract/org scope validation、template/article validation、signed PDF file validation、generated PDF cleanup、audit metadata minimization、no-store/error response は変更しない。allowlist debt は 74 → 72。
- `API-CONTRACT-001DB`: `pharmacy-contracts/:id/versions` の POST create success を `data` envelope へ移行済み。直接の frontend reader はなく、route test で safe version DTO の envelope を固定した。contract/org scope validation、有効化 eligibility、active period overlap guard、fee rule creation、audit metadata minimization、validation/conflict response は変更しない。allowlist debt は 72 → 71。
- `API-CONTRACT-001DC`: `pharmacy-contracts` の POST create success を `data` envelope へ移行済み。薬局間連携設定画面の create contract mutation は `payload.data` を unwrap し、旧 root row success を malformed success として拒否する。GET cursor page、partnership/org scope validation、有効化 eligibility、active period overlap guard、initial version/fee-rule creation、audit metadata minimization、validation/conflict response は変更しない。allowlist debt は 71 → 70。
- `API-CONTRACT-001DD`: `pharmacy-cooperation-message-threads` の GET thread list と POST message create success を `data + meta` / `data` envelope へ移行済み。薬局間連携 workflow の message thread page schema は `meta.has_more` / `meta.next_cursor` を current page shape へ変換し、create message schema は `payload.data` を unwrap する。active share-case access、visit request context validation、read/create audit minimization、notification PHI minimization、sensitive no-store/error response は変更しない。allowlist debt は 70 → 68。
- `API-CONTRACT-001DE`: `pharmacy-drug-stock-requests/:id` の PATCH decision success を `data` envelope へ移行済み。drug master formulary 画面の decision mutation は `payload.data` を unwrap し、承認/却下 toast と query invalidation を既存のまま維持する。admin permission、org-scoped request lookup、pending-only optimistic claim、approval payload validation、preferred generic validation、stock upsert、audit minimization、conflict/error response は変更しない。allowlist debt は 68 → 67。
- `API-CONTRACT-001DF`: `pharmacy-drug-stock-templates/:id/apply` の POST dry-run/apply success を `data` envelope へ移行済み。drug master formulary 画面の template apply mutation は `payload.data` を unwrap し、stale preview guard 用の request context stamp、preview toast、apply toast、query invalidation を既存のまま維持する。admin permission、same-org template/site lookup、persisted item/reference validation、existing-stock skip/overwrite calculation、stock upsert、audit summary、conflict/error response は変更しない。allowlist debt は 67 → 65。
- `API-CONTRACT-001DG`: `pharmacy-drug-stocks/bulk` の POST dry-run/apply success を `data` envelope へ移行済み。drug master formulary 画面の CSV 一括 preview/apply mutation は `payload.data` を unwrap し、stale preview guard 用の request context stamp、preview toast、apply toast、query invalidation を既存のまま維持する。admin permission、same-org site lookup、CSV/row validation、YJコード照合、重複検出、stock upsert、audit summary、conflict/error response は変更しない。allowlist debt は 65 → 63。
- `API-CONTRACT-001DH`: `pharmacy-drug-stocks/copy` の POST dry-run/apply success を `data` envelope へ移行済み。drug master formulary 画面の拠点間コピー preview/apply mutation は `payload.data` を unwrap し、stale preview guard 用の request context stamp、preview toast、apply toast、query invalidation を既存のまま維持する。admin permission、same-org source/target site lookup、source formulary lookup、skip/overwrite preview、stock upsert、audit summary、conflict/error response は変更しない。allowlist debt は 63 → 61。
- `API-CONTRACT-001DI`: `pharmacy-drug-stocks/impact` の GET success を `data` envelope へ移行済み。drug master formulary 画面の採用薬影響レビュー query は `payload.data` を unwrap し、impact queue 表示、master change report、price impact、follow-up summary を既存のまま維持する。admin permission、same-org site lookup、count aggregation、bounded queue samples、price-impact draft scan、sensitive no-store/error response は変更しない。allowlist debt は 61 → 60。
- `API-CONTRACT-001DJ`: `pharmacy-drug-stocks/review` の POST review/no-op success を `data` envelope へ移行済み。drug master formulary 画面のレビュー記録 mutation は `payload.data` を unwrap し、review count toast と query invalidation を既存のまま維持する。admin permission、same-org site lookup、stock scope、1000件上限、last_reviewed_at update、audit summary、validation/not-found response は変更しない。allowlist debt は 60 → 58。
- `API-CONTRACT-001DK`: `pharmacy-drug-stocks/safety-follow-up` の POST dry-run/apply success を `data` envelope へ移行済み。drug master formulary 画面の安全性フォローアップ mutation は `payload.data` を unwrap し、updated count toast と query invalidation を既存のまま維持する。admin permission、same-org site lookup、安全属性 queue filter、未解決 follow-up skip、1000件上限、follow_up update、audit summary、validation/not-found response は変更しない。allowlist debt は 58 → 56。
- `API-CONTRACT-001DL`: `pharmacy-drug-stocks/usage-mismatch` の GET success を `data` envelope へ移行済み。drug master formulary 画面の処方・採用品不一致 query は `body.data` を unwrap し、頻出未採用薬/未使用採用品/曖昧照合/未照合処方の既存表示データを維持する。admin permission、same-org site lookup、QR draft scan、YJ/receipt/HOT resolution、ambiguous candidate metadata、list count metadata、sanitized internal error behavior は変更しない。allowlist debt は 56 → 55。
- `API-CONTRACT-001DM`: `pharmacy-invoices/[id]` の PATCH success を `data` envelope へ移行済み。partner cooperation billing 画面の invoice lifecycle mutation は `apiDataSchema(pharmacyInvoiceTransitionResultSchema)` で envelope と inner transition result を検証してから `data` を unwrap する。Oracle/GPT-5.5 Pro は billing safety gate として Option A（response envelope only）を Go とし、consumer grep、inner malformed negative test、error response 非変更、focused gates を条件にした。canManageBilling permission、RLS org context、Serializable transaction、status transition、payment date、audit behavior、no-store/error response は変更しない。allowlist debt は 55 → 54。
- `API-CONTRACT-001DN`: `pharmacy-invoices` の POST draft success を `data` envelope へ移行済み。GET list は既に cursor/list `data` envelope のため変更しない。partner cooperation billing 画面の invoice draft mutation は `apiDataSchema(invoiceDraftResultSchema)` で envelope と inner draft result を検証してから `data` を unwrap し、route tests、component mocks、ui-major direct API checks、ui-route-mocked smoke mock を追随した。Oracle/GPT-5.5 Pro は billing safety gate として実行したが、大きい添付は upload timeout、縮小 inline 版は回答が 1 文で途切れ、follow-up は browser textarea timeout だったため十分な advisory 判定を回収できなかった。canManageBilling permission、RLS org context、Serializable transaction、draft reuse 200 / new draft 201、service/audit behavior、no-store/error response は変更しない。allowlist debt は 54 → 53。
- `API-CONTRACT-001DO`: `pharmacy-partnerships/:id/activate` の POST activation/no-op/concurrent-win success を `data` envelope へ移行済み。薬局間連携設定画面の activation mutation は `apiDataSchema(pharmacyPartnershipRowSchema)` で envelope を検証してから `data` を unwrap し、route tests と UI malformed success guard を追随した。canManagePatientSharing permission、RLS org context、optimistic activation updateMany、date-window conflict、audit/no-op/concurrent-win behavior、validation/conflict/not-found response は変更しない。allowlist debt は 53 → 52。
- `API-CONTRACT-001DP`: `pharmacy-partnerships` の GET list success と POST create success を `data + meta` / `data` envelope へ移行済み。薬局間連携設定画面の list schema は `meta.has_more` / `meta.next_cursor` を current page shape へ変換し、create mutation は `payload.data` を unwrap する。GET/POST route tests は strict envelope、legacy root field omission、no-store、sanitized internal-error fallback を固定し、base POST を protected POST auth/no-store matrix に追加した。canVisit / canManagePatientSharing permission、RLS org context、base site / partner pharmacy org-scope validation、archived partner rejection、audit metadata は変更しない。allowlist debt は 52 → 50。
- `API-CONTRACT-001DQ`: `pharmacy-sites/:id/insurance-configs/:configId` の DELETE success を legacy `{ ok: true }` root から `{ data: { id } }` envelope へ移行済み。PATCH は既存 `data` envelope のまま明示 DTO に寄せ、`org_id` / `display_id` / timestamps / backend-only fields を response から落とす。PATCH/DELETE 全 outcome は sensitive no-store + sanitized internal-error fallback で包み、route-local auth failure も no-store を固定した。更新 audit は free-form `config` 全量ではなく `config_changed_keys` と日付/label metadata へ最小化し、admin UI の DELETE reader は rollout 互換のため success body を厳格化しない。canAdmin permission、org/site/config scoped pre-check、RLS org context、overlap validation、delete/update/audit action は維持。allowlist debt は 50 → 49。
- `API-CONTRACT-001DR`: `push-subscription` の POST/DELETE success を legacy `{ ok: true }` root から exact `{ data: { ok: true } }` envelope へ移行。endpoint / `p256dh` / auth key / org・user scope / mutation result は response に返さず、POST upsert の org/user create+update、DELETE の org/user/endpoint scope、auth short-circuit、count=0 の idempotent success を route tests で固定した。nested ApiError、request_id、no-store、global endpoint unique と FORCE RLS の実DB証明は別 acceptance とし、既存 canVisit / RLS transaction / validation / status / mutation semantics は変更しない。allowlist debt は 49 → 47。
- `API-CONTRACT-001DS`: `presence` の POST success を legacy `{ ok: true }` root から exact `{ data: { ok: true } }` へ、GET success を raw array から exact `{ data: entries }` へ移行。GET route と既存 frontend reader の不一致で、認可済み presence 取得も false-empty になる契約バグを解消し、legacy raw array は client で fail-closed のままとした。GET/POST の auth short-circuit、canViewDashboard、entity/org/assignment scope、presence store/TTL、realtime channel、best-effort broadcast、safe warning metadata は変更しない。request_id、no-store、auth wrapper、helper error envelope は別 acceptance とする。allowlist debt は 47 → 45。
- `API-CONTRACT-001DT`: `visit-brief-feedback` の POST success を legacy `{ ok: true }` root から exact `{ data: { ok: true } }` envelope へ移行。既存の2つの frontend readerはどちらも success body を状態に使わないため production client 変更や dual-shape fallback は追加しない。success response は patient/generation ID、comment/corrected summary、provider/model、audit/mutation result を返さず、201 と sensitive no-store を維持する。canVisit、patient access、RLS request context、helpful/needs-review audit と fail-closed write、scope-miss 404、sanitized 500 は変更しない。auth wrapper、request_id、helper error envelope は別 acceptance とする。allowlist debt は 45 → 44。
- `API-CONTRACT-001DU`: `tasks/bulk` は runtime ですでに exact `{ data: { total, completed, failed, failures } }` を返していたが、`success(responseBody)` の変数経由を静的 response-shape checker が解決できず legacy として許容していた。call site に typed `data` object を直接明示し、response body、reader、status、mutation semantics を変えずに allowlist を ratchet した。canVisit、org/assignment scope、100件上限、ID dedupe、dedicated-flow 除外、patient write guard、RLS updateMany、stale partial failure は変更しない。allowlist debt は 44 → 43。
- `API-CONTRACT-001DV`: `POST /api/visit-routes` の route plan success を legacy top-level plan から exact `{ data: VisitRoutePlan }` envelope へ移行した。3つの production consumer のうち route-compare / emergency-route は既に envelope を期待していた一方、weekly optimizer だけが旧rootを読んでいたため、weekly readerを `data` unwrap + legacy success fail-closedへ統一した。route/locked/weekly/compare/emergency focused testsでsuccess、no-store、locked order、vehicle constraint、failure messageを固定し、canVisit、org/assignment scope、active schedule/open proposal filter、RLS transaction、route engine、status/error responseは変更しない。allowlist debt は 43 → 42。
- `API-CONTRACT-001DW`: `PATCH /api/visit-routes/reorder` の mixed schedule/proposal reorder success を legacy root fields から exact `{ data: { schedule_ids, proposal_ids } }` envelope へ移行した。production callers は mutation completion と query invalidation だけに依存し、success bodyを読まないことを全参照で確認した。route/weekly/compare/emergency focused testsで nested success と既存consumer回帰を固定し、`canVisit`、org/assignment scope、Serializable retry、optimistic `updateMany`、confirmed/locked/stale/duplicate/vehicle-duration guards、audit/workflow notification、status/error responseは変更しない。allowlist debt は 42 → 41。
- `API-CONTRACT-001DX`: `PATCH /api/visit-schedule-proposals/reorder` の success を legacy root `{ case_ids, ordered_proposal_ids }` から exact `{ data: { case_ids, ordered_proposal_ids } }` envelope へ移行した。唯一のproduction consumerは success bodyを状態に使わず、完了toastとquery invalidationだけを行うためclient変更やdual-shape fallbackは追加しない。route/proposals-screen focused testsで nested success、root field不在、mutation flowを固定し、`canVisit`、org/assignment scope、Serializable retry、optimistic `updateMany`、locked/stale/duplicate route-order guards、audit/workflow notification、status/error responseは変更しない。allowlist debt は 41 → 40。
- `API-CONTRACT-001DY`: `PATCH /api/visit-schedules/reorder` の success を legacy root `{ case_ids, schedule_ids, vehicle_assignment }` から exact `{ data: { case_ids, schedule_ids, vehicle_assignment } }` envelope へ移行した。route compare、conflict resolution、emergency route、team board、weekly optimizer、day-route helperのproduction callersは返却値を業務状態に使わず、mutation完了後のtoast/invalidation/画面状態だけを更新する。routeと全6 caller familyのfocused testsで nested successと既存flowを固定し、lifecycle permission、org/assignment scope、Serializable retry、guarded `updateMany`、confirmed/status/stale/vehicle capacity・duration・site・assignment guards、audit/workflow notification、status/error responseは変更しない。allowlist debt は 40 → 39。
- `API-CONTRACT-001DZ`: `POST /api/visit-schedules/:id/reopen` の success を raw updated schedule から exact `{ data: schedule }` envelope へ移行した。repo内production consumerはなく、route testでnested scheduleとroot field不在を固定した。cancelled→plannedのstatus遷移、version increment/optimistic `updateMany`、completed override/replacement lineage guard、lifecycle/assignment permission、reason audit、workflow notification、sensitive no-store、status/error responseは変更しない。allowlist debt は 39 → 38。
- `API-CONTRACT-001EA`: `GET /api/visit-schedules/:id` の success を legacy root schedule detail から exact `{ data: ScheduleDetail }` envelope へ移行した。visit record form、evidence capture、voice memo、visit brief review、offline evidence syncの5 consumerを同時に`data` unwrapへ揃え、legacy rootはschedule-specific resolverでfail-closedにした。provider/reader/shared-helperの7 focused test filesで177 testsを通し、patient summary、visit record link、cycle/patient ID、fallback順、assignment permission、org scope、sensitive no-store、PHI表示項目は変更しない。PATCH/DELETEの3 legacy successは後続sliceに残し、allowlist debt は 38 → 37。
- `API-CONTRACT-001EB`: `PATCH /api/visit-schedules/:id` の no-op と実更新 success を raw schedule から exact `{ data: schedule }` envelope へ同時移行した。唯一のproduction mutation callerは response bodyを使用せず、完了後のboard invalidationだけに依存する。route/team-board focused testsで両successとconsumer flowを固定し、lifecycle/ready gate、assignment permission、org scope、Serializable retry、version/confirmed/route-order/vehicle/time/shift guards、audit/workflow notification、sensitive no-store、status/error responseは変更しない。DELETEの1 legacy successだけを残し、allowlist debt は 37 → 35。
- `API-CONTRACT-001EC`: `DELETE /api/visit-schedules/:id` の cancel success を raw updated schedule から exact `{ data: schedule }` envelope へ移行した。repo内production consumerはなく、route testでnested dataとroot field不在を固定した。lifecycle/assignment permission、terminal status拒否、version/status optimistic claim、pending override取消、approval task解決、reschedule proposal supersede、structured reason audit、workflow notification、sensitive no-store、status/error responseは変更しない。同routeのallowlist entryを除去し、allowlist debt は 35 → 34。
- `API-CONTRACT-001ED`: `GET /api/visit-schedules` の cursor metadata を legacy root `hasMore` / `nextCursor` から standard `meta { limit, has_more, next_cursor }` へ移行した。共有window readerを `fetchAllMetaCursorPages` へ切り替え、calendar/weekly/conflicts/emergency/route-compareの全page収集とterminationを維持した。provider/helper/consumer focused testsで73 testsを通し、list data、query filters、assignment/org scope、bounded limit、sort、patient summary、sensitive no-storeは変更しない。POSTは後続sliceへ残し、allowlist debt は 34 → 33。
- `API-CONTRACT-001EE`: `POST /api/visit-schedules` の create success を raw schedule から exact `{ data: schedule }` envelope へ移行した。訪問記録詳細の次回訪問作成 mutation は `readApiJson` で `payload.data.id` を unwrap し、legacy root やIDのない malformed successを導線遷移前にfail-closedにする。provider/consumer focused testsで59 testsを通し、visit permission、request validation、org reference、workflow gate、create/audit/notification、201 status、sensitive no-store、error responseは変更しない。allowlist debt は 33 → 32。
- `API-CONTRACT-001EF`: `POST /api/qr-scan-drafts` の create success を root `draft` / `parse_result` / `session_id` から exact `{ data: { draft, parse_result, session_id } }` envelope へ移行した。QRスキャン画面は `readApiJson` と response boundary helper で `data.session_id` を検証し、legacy root、data欠落、空session IDを完了表示前にfail-closedにする。provider/reader/contract focused testsで48 testsを通し、raw QR text / payload hash / raw parser field非露出、patient/site/org照合、QR解析、draft/supplemental-record作成、realtime event、201/no-store、error responseは変更しない。GET一覧契約はこのsliceで変更せず、allowlist debt は 32 → 31。
- `API-CONTRACT-001EG`: `GET /api/qr-scan-drafts/:id` の detail success を sanitized draft rootから exact `{ data: draft }` envelope へ移行した。QR下書きreview画面は `payload.data` をunwrapし、legacy rootやIDのない malformed successをfalse-data表示前にfail-closedにする。provider/reader focused testsで24 testsを通し、assignment/org scope、supplemental-record safe select、raw QR text / payload hash / raw parser field非露出、sensitive no-store/error responseは変更しない。同routeのDELETE破棄契約は後続sliceへ残し、allowlist debt は 31 → 30。
- `API-CONTRACT-001EH`: `DELETE /api/qr-scan-drafts/:id` の discard success を sanitized draft rootから exact `{ data: draft }` envelope へ移行した。Production consumerは成功bodyを業務状態に使わずHTTP statusと完了後導線だけに依存するためreader変更は不要。routeの12 testsを通し、assignment/org scope、pending-only optimistic `updateMany` claim、claim-loss 409、raw QR/hash/parser data消去、supplemental-record削除、sanitized no-store/error responseは変更しない。同routeのallowlist entryを除去し、allowlist debt は 30 → 29。
- `API-CONTRACT-001EI`: `POST /api/qr-scan-drafts/:id/confirm` の201 success を root `intake` / `cycle` / `medicationChanges` / `profileSyncResult` から exact `{ data: { ... } }` envelope へ移行した。QR review画面は既存の詳細error parserを維持しつつ、成功時は `data.intake.id` と `data.cycle.id` を検証し、legacy/malformed successをtoast/navigation前にfail-closedにする。provider/consumer focused testsで42 testsを通し、patient/case/draft/org照合、drug master/packaging validation、pending claim、intake/cycle transaction、post-create medication sync、supplemental records、webhook/audit/realtime、status/error responseは変更しない。allowlist debt は 29 → 28。
- `API-CONTRACT-001EJ`: `GET /api/workflow-exceptions/:id` の success を raw workflow exceptionから exact `{ data: exception }` envelope へ移行した。Repo内production consumerはなく、routeの6 testsでnested DTOとGET/PATCHのvalidation/resolution回帰を通した。`canDispense`、org-scoped lookup、ID validation、404/error response、PATCHのstatus更新/cycle exception clear/transition logは変更しない。PATCH successと同routeのsensitive no-store欠落は後続の独立sliceとし、allowlist debt は 28 → 27。
- `API-CONTRACT-001EK`: `PATCH /api/workflow-exceptions/:id` の resolution success を raw updated exceptionから exact `{ data: exception }` envelope へ移行した。Repo内production consumerはなく、routeの6 testsでnested DTOとGET/PATCH全回帰を通した。`canDispense`、org/id/body validation、open-only guard、dispense-audit rerun guard、transactional exception update、remaining-open count、cycle `exception_status` clear、transition log、error responseは変更しない。同routeのallowlist entryを除去し、allowlist debt は 27 → 26。Sensitive no-storeは後続security sliceとする。
- `API-WORKFLOW-EXCEPTION-NOSTORE-001`: authenticated clinical workflow exception GET/PATCH をouter `withSensitiveNoStore` で保護し、successとroute/body validation errorに `Cache-Control: private, no-store, max-age=0` / `Pragma: no-cache` を固定した。Routeの6 testsでGET/PATCHの成功/拒否headerと既存body/domain回帰を通し、permission、org filter、exception/cycle transaction、transition logは変更しない。
- `API-CONTRACT-001EL`: `POST /api/residual-medications` の201 success を raw created arrayから exact `{ data: created[] }` envelope へ移行した。唯一のruntime callerであるoffline sync engineは2xx bodyを読まずqueue item削除だけに依存するためconsumer変更は不要。Routeの23 testsを通し、visit-record assignment/org scope、body/DrugMaster validation、stable display-ID batch allocation、excess-days/reduction-target導出、transactional create、201/no-store/sanitized error loggingは変更しない。GET listは既存`{ data }`のままで、allowlist debt は 26 → 25。
- `API-CONTRACT-001EM`: `POST /api/set-audits` の idempotent replay 200 success を raw persisted-audit objectから exact `{ data: replayResult }` envelope へ移行した。新規監査201とworkbench adapterは既に`{ data }`契約だったため、replayだけのFE/BE形状分岐を解消した。Routeの40 testsでnested replay、200 status、batch/file/cycle/schedule/audit/notificationの重複副作用0、通常監査・validation・conflict・no-store・sanitized error回帰を通した。監査データや承認条件は変更せず、allowlist debt は 25 → 24。
- `API-CONTRACT-001EN`: `GET /api/prescription-intakes/:id` の authorized detail success をraw intake rootから exact `{ data: intake }` envelope へ移行した。Inline detail paneとfull detail pageの2 readerは `payload.data` をunwrapし、legacy rootやID欠落をfalse-data表示前にfail-closedにする。Provider/consumer focused testsで47 testsを通し、`canVisit`、assignment/org scope、line/cycle/patient/inquiry/prescriber/supplemental-record detail、authorized raw JAHIS/payload表示、sensitive no-store、404/sanitized 500は変更しない。PATCHは後続sliceへ残し、allowlist debt は 24 → 23。
- `API-CONTRACT-001EO`: `PATCH /api/prescription-intakes/:id` の update success をraw updated intakeから exact `{ data: intake }` envelope へ移行した。Patient card workspaceのFAX原本到着/文書URL/原本管理の3 mutationと処方歴の原本回収mutationは、success bodyを業務分岐に使わずHTTP statusとinvalidate/toastだけに依存するためconsumer変更は不要。Routeの25 testsを通し、writable-patient/assignment/org guard、body/institution/original-management validation、transactional intake/task/audit update、sensitive no-store/error responseは変更しない。同routeのallowlist entryを除去し、allowlist debt は 23 → 22。
- `API-CONTRACT-001EP`: `POST /api/prescription-intakes` の通常作成とQR下書き確定の2つの201 successを、raw intake rootから共通のexact `{ data: intake }` envelopeへ移行した。唯一の作成フォームは既にenvelope fixtureを使い、success bodyを業務分岐に使わず完了後のdraft clearと一覧遷移だけに依存するためproduction consumer変更は不要。Provider/consumer focused testsで96 testsを通し、`canVisit`、org/assignment/patient/case validation、QR pending claim、intake/cycle/line/task/audit/supplemental-record transaction、webhook/realtime/workflow notification、status/error responseは変更しない。同routeのallowlist entryを除去し、allowlist debt は 22 → 20。
- `API-CONTRACT-001EQ`: `POST /api/prescription-intakes/facility-batch` の201 successをraw `facility_label` / `patient_count` / `entries` rootから exact `{ data: { facility_label, patient_count, entries } }` envelopeへ移行した。唯一の作成フォームcallerはsuccess bodyを業務分岐に使わずJSON parse後にdraft clearと一覧遷移だけを行うためproduction consumer変更は不要。Provider/consumer focused testsで38 testsを通し、`canVisit`、org/patient/case/施設/identity snapshot validation、all-or-nothing batch transaction、cycle/intake/task/profile write、post-create hooks、sensitive no-store/error responseは変更しない。同routeのallowlist entryを除去し、allowlist debt は 20 → 19。
- `API-CONTRACT-001ER`: `PUT /api/visit-records/:id/handoff` のhandoff confirmation successをraw handoff rootから exact `{ data: handoff }` envelopeへ移行した。`HandoffConfirmPanel` と `HandoffWorkspace` は既にenvelope fixture/mockを使い、success bodyを業務分岐に使わずmutation完了後のinvalidate/toastだけに依存するためproduction consumer変更は不要。Provider/consumer focused testsで3 files / 81 testsを通し、direct/owner override、version/assignment、typed conflict、sensitive no-store、GET contractは変更しない。同routeのallowlist entryを除去し、allowlist debt は 19 → 18。
- `API-CONTRACT-001ES`: `POST /api/visit-records/:id/handoff/extract` の201 successをraw handoff rootから exact `{ data: handoff }` envelopeへ移行した。Repository全体のcaller scanではproduction HTTP consumerはなく、rate-limit registryとroute testだけが同pathを参照するためprovider-only contract changeと判定した。Routeのbaseline/final 7 tests、protected POST 151 tests、GET 384 testsを通し、`canVisit`、org/assignment guard、patient lookup、structured SOAP validation、AI extraction、optimistic version、confirmation task、PHI-safe failure、sensitive no-storeは変更しない。同routeのallowlist entryを除去し、allowlist debt は 18 → 17。
- `API-CONTRACT-001ET`: `POST /api/visit-records/:id/handoff/supervision-request` のsuccessをraw request rootから exact `{ data: request }` envelopeへ移行した。`HandoffConfirmPanel`は既にenvelope fixtureを使い、success bodyを業務分岐に使わずinvalidate/toastだけに依存するためproduction consumer変更は不要。Provider/consumer baseline/final 3 files / 50 tests、protected POST 151 tests、GET 384 testsを通し、trainee assignment、supervisor selection/active membership、org scope、optimistic version、task/audit、request-note minimization、typed error、sensitive no-storeは変更しない。同routeのallowlist entryを除去し、allowlist debt は 17 → 16。
- `API-CONTRACT-001EU`: `POST /api/visit-records/:id/handoff/supervision-confirm` のsuccessをraw handoff rootから exact `{ data: handoff }` envelopeへ移行した。`HandoffConfirmPanel`は既にenvelope fixtureを使い、success bodyを業務分岐に使わずinvalidate/toastだけに依存するためproduction consumer変更は不要。Provider/consumer baseline/final 3 files / 53 tests、protected POST 151 tests、GET 384 testsを通し、assigned supervisor、active same-org membership、task status/assignee/metadata binding、optimistic version、final confirmation/task completion/audit、typed PHI-safe errors、sensitive no-storeは変更しない。同routeのallowlist entryを除去し、allowlist debt は 16 → 15。
- `API-CONTRACT-001EV`: `POST /api/visit-records` の201 create / 200 conflict overwrite successをraw `{ record, suggestedSchedule, conflictResolved }` rootから exact `{ data: { record, suggestedSchedule, conflictResolved } }` envelopeへ移行した。Visit record formは`payload.data.record`をunwrapし、2 success fixtureを同期した。Offline syncは成功bodyを読まず既にenvelope fixtureを使うためproduction変更不要。Provider/form/offline baseline/final 3 files / 166 tests、protected POST 151 tests、GET 384 testsを通し、clinical validation、schedule/patient/assignment/org scope、optimistic overwrite、transaction、audit/derived rows、async handoff extraction、sensitive no-storeは変更しない。同routeのallowlist entryを除去し、allowlist debt は 15 → 14。
- `API-CONTRACT-001EW`: `GET /api/visit-records/:id` のsuccessをraw visit record rootから exact `{ data: VisitRecord }` envelopeへ移行した。Detail、brief/captureのpatient fallback、voice memo、offline evidence uploadの全body readerを`data` unwrapへ揃え、存在確認だけのGET callerはbody非依存を確認した。Provider/consumer/offline final 6 files / 97 tests、protected GET 384 testsを通し、auth、assignment、tenant scope、internal snapshot/geo redaction、attachment normalization、sensitive no-storeは変更しない。同routeのallowlist expectedCountを2→1へ減らし、PATCHは次sliceへ分離、allowlist debt は 14 → 13。
- `API-CONTRACT-001EX`: `PATCH /api/visit-records/:id` のsuccessをraw visit record rootから exact `{ data: VisitRecord }` envelopeへ移行した。Formの添付紐づけ後recordとcaptureの訪問終了日時readerを`data` unwrapへ揃え、voice memo/offline evidenceはsuccess body非依存を確認してfixtureだけを新契約へ同期した。Provider/consumer/offline final 5 files / 118 tests、protected PATCH/DELETE 80 testsを通し、auth、assignment、tenant scope、optimistic version、clinical readiness/previous-source/attachment validation、derived rows、sensitive no-storeは変更しない。同routeのallowlist entryを除去し、allowlist debt は 13 → 12。
- `API-CONTRACT-001EY`: `GET /api/visit-schedule-proposals/billing-preview` のsuccessをraw preview rootから exact `{ data: VisitScheduleBillingPreview }` envelopeへ移行した。Day plannerとweekly optimizerの2 readerを`data` unwrapへ揃え、provider/consumer/billing service 4 files / 39 tests、protected GET 384 testsを通した。`canVisit`、org pharmacist/site reference、case access、tenant RLS、cadence/alerts/recommended visit type/priority/slot count、sensitive no-storeは変更しない。同routeのallowlist entryを除去し、allowlist debt は 12 → 11。
- `API-CONTRACT-001EZ`: `GET /api/visit-billing-candidates/summary` のsuccessをraw PHI-free monthly summary rootから exact `{ data: PartnerCooperationSummary }` envelopeへ移行した。Partner cooperation billing readerはenvelope全体をZod検証して`data`を返す。Provider/consumer 2 files / 20 tests、protected GET 384 testsを通し、`canManageBilling`、tenant RLS、strict billing month/filter、record/candidate counts、planned amount、PHI-safe log、sensitive no-storeは変更しない。同routeのallowlist entryを除去し、allowlist debt は 11 → 10。
- `API-CONTRACT-001FA`: `GET /api/visit-billing-candidates` のlegacy `{ data, hasMore, nextCursor }` paginationを exact `{ data, meta: { limit, has_more, next_cursor } }`へ移行した。Partner cooperation billing readerはstandard metaをZod検証し、`has_more=true`時のcursor欠落も拒否する。Provider/consumer 2 files / 32 tests、protected GET 384 testsを通し、`canManageBilling`、tenant/filter/cursor、amount snapshot minimization、POST generation、sensitive no-storeは変更しない。Static gateにより同route allowlist 1件はPOST raw success由来と確定したためentryを保持し、debtは10据え置き。
- `API-CONTRACT-001FB`: `POST /api/visit-billing-candidates` のgeneration successをraw result rootから exact `{ data: CandidateGenerationResult }` envelopeへ移行した。Partner cooperation mutationはenvelope全体をZod検証して`data`を返す。GET paginationと合わせたroute/consumer 2 files / 32 tests、protected POST 151 testsを通し、`canManageBilling`、tenant RLS、consent/contract/fee evaluation、candidate create/update/lock race、visit request status、audit、sensitive no-storeは変更しない。同routeのallowlist entryを除去し、debtは10→9。
- `API-CONTRACT-001FC`: `DELETE /api/platform/break-glass/:id` のrevoke successをraw `{ session }`から exact `{ data: { session } }` envelopeへ移行した。専用route testを新設してplatform guard拒否時のrevoke 0、missing 404、operator/id binding、serialized revoked session、sensitive no-storeを固定した。Route/operator/MFA/service/panel 5 files / 45 testsを通し、ownership/owner override、expiry、revoke audit、tenant boundaryは変更しない。同routeのallowlist entryを除去し、debtは9→8。
- `API-CONTRACT-001FD`: `GET /api/platform/break-glass` のactive-session listをraw `{ sessions }`から exact `{ data: { sessions } }` envelopeへ移行し、shared hook内でunwrapしてpanel contractを維持した。専用route testでguard deny時list 0、operatorId binding、serialized list、sensitive no-storeを固定し、直接serializerへmap index/arrayを渡していた潜在不安定性も1引数lambdaへ修正した。Route/operator/service/panels 5 files / 39 testsを通し、session ownership/lifecycleは変更しない。同route allowlist expectedCountを2→1へ減らし、debtは8→7。
- `API-CONTRACT-001FE`: `POST /api/platform/break-glass` のactivation successをraw `{ session }`から exact `{ data: { session } }` envelopeへ移行し、panel mutation typeを同期した。専用route testでguard deny時credential verify/create 0、step-up失敗時create 0とsafe warning、trimmed target/reason/ticket/MFA、scope、201 serialized session、sensitive no-storeを固定した。Route/operator/MFA/service/panels 6 files / 48 testsを通し、password/MFA handling、TTL、tenant scope、auditは変更しない。同route allowlist entryを除去し、debtは7→6。
- `API-CONTRACT-001FF`: `GET /api/platform/tenants` のtenant directory successをraw `{ tenants }`から exact `{ data: { tenants } }` envelopeへ移行し、directory/detailの2 readerを共有response typeへ同期した。専用route testでplatform guard拒否時のtenant/session query 0、metadata-only bounded select/order/take、operator-owned active break-glass合成、exact body、sensitive no-storeを固定した。Route/consumer/operator/service 4 files / 39 testsを通し、platform operator認可、tenant metadata field、session ownershipは変更しない。同route allowlist entryを除去し、debtは6→5。
- `API-CONTRACT-001FG`: `GET /api/platform/tenants/:orgId/audit` のsuccessをroot `{ entries, truncated }`から standard `{ data: entries, meta: { limit, has_more } }`へ移行し、AuditLogPanel reader/noticeを同期した。専用route testでplatform guard拒否時session/query 0、active sessionなし403、operatorId/orgId binding、RLS-scoped break-glass context、bounded action-filtered read、read-audit metadata、exact body、sensitive no-storeを固定した。Route/panel/operator/break-glass/audit convention 5 files / 42 testsを通し、認可、tenant RLS、fail-closed audit、PHI fieldは変更しない。同route allowlist entryを除去し、debtは5→4。
- `API-CONTRACT-001FH`: `GET /api/platform/tenants/:orgId/data` のmodel未指定分岐をraw `{ models }`から exact `{ data: models }`へ移行し、DataExplorerPanel model selectorを同期した。専用route testでplatform guard拒否時session/read/service 0、active sessionなし403、operatorId/orgId binding、read-before-query audit metadata、exact body、sensitive no-storeを固定した。Route/panel/operator/break-glass 4 files / 39 testsを通し、model allowlist/count、tenant RLS、audit、rows分岐は変更しない。同route allowlist expectedCountを2→1へ減らし、debtは4→3。
- `DATA-EXPLORER-DRUG-PRICE-SCOPE-001`: `DrugPriceVersion`がschema上のglobal master、display-id registryのglobal scope、coverage catalog対象である一方、data explorer global registryから漏れてmodule import時に全explorerをfail-closed停止させていた不整合を修正した。Global read-only registryへ1件追加し、catalog/count/rowsのorg predicateなし、`source_file_hash`非公開、全field non-editable、PATCH/DELETE SQL前拒否をservice testで固定した。Data explorer/platform caller 5 files / 72 tests、catalog/display-idを含む3 files / 57 pass・6 skip、Prisma validate、raw-read org guardを通し、DB/migration/薬価値/請求計算は変更していない。
- `API-CONTRACT-001FI`: `GET /api/platform/tenants/:orgId/data?model=...` のrows successをraw `DataExplorerTableRows` rootから admin explorerと同じ exact `{ data: DataExplorerTableRows }`へ移行し、ModelRows query readerをunwrapした。専用route testでoperator/session binding、audit-first metadata、trimmed search/service args、exact no-store body、unknown model sanitized 400を固定した。Route/panel/service/break-glass/operator 5 files / 74 testsを通し、model allowlist、RLS、redaction、pagination/search意味論は変更しない。同route allowlist entryを除去し、debtは3→2。
- `PLATFORM-DATA-ROUTE-HARDENING-001`: Platform data explorerの独自silent parserをshared bounded integer validationとsingle-value query checkへ置換し、model/search 100文字、limit 1..100、offset 0..999900を明示400にした。Trim/default後のcanonical model/limit/offset/searchをaudit metadataとservice argsへ同値伝播し、unknown tableだけを400、audit/service/guard等の想定外例外はraw message非露出のsensitive no-store 500へ変換する。Route 18 tests、route/panel/service/break-glass/operator 5 files / 87 testsと全static gateを通し、DB/schema/tenant RLS/PHI projectionは変更していない。
- `API-CONTRACT-001FJ`: `POST /api/pharmacy-visit-requests/:id/decision` のsafe visit request successをraw rootから exact `{ data: safeVisitRequest }`へ移行し、workflow mutationは`apiDataSchema`でid/status envelopeをruntime検証する。Route/workflow/access/transition 4 files / 63 tests、protected POST 151 testsを通し、caller-supplied actor拒否、認証actor attribution、accept consent advisory lock/race、decline deferred policy、optimistic update、PHI-minimized audit、sensitive no-storeは変更しない。同route allowlist entryを除去し、debtは2→1。
- `API-CONTRACT-001FK`: `POST /api/pharmacy-visit-requests` の201 create successをraw safe visit request rootから exact `{ data: safeVisitRequest }`へ移行し、workflow create mutationは`apiDataSchema`でfull safe DTO envelopeをruntime検証する。Route/workflow/access 3 files / 52 tests、protected GET 384 / POST 151 testsを通し、`canManagePatientSharing`、active share-case/consent advisory lock、Serializable transaction、pharmacist attribution、PHI-minimized safe flags/audit、sensitive no-storeは変更しない。同route allowlist entryを除去し、debtは1→0。親タスクはhelper error/request_id/no-store/frontend reader acceptanceが残るためPartialを維持する。
- `API-CONTRACT-001FL`: Shared `readApiJson`がstandard nested `{ error: { code, message } }`のmessageをlegacy root `message` / string `error`より優先して読み、blank/malformed nested messageはcaller固定fallbackへ閉じる契約を追加した。Legacy string envelopeはhelper/provider移行中のみ維持する。Client/schema focused 2 files / 16 tests、frontend/API/PHI-log gates、full typecheckを通し、provider response、HTTP status、logging、UIは変更していない。
- `FE-REPORT-EDIT-LEAVE-GUARD-001`: Plans上はNot startedだったが、live codeとcommit `060390547`で実装済みと再照合した。ReportEditFormはbaseline比較のdirty state、`useUnsavedChangesGuard({ enabled: isDirty || isSaving })`、save success後clear、conflict/error後のdraft/guard保持を実装し、component/hook 2 files / 11 testsとfull typecheckを通した。Focused suiteで見つかった別testのReact act warningは直後の`TEST-REPORT-EDIT-ACT-001`で解消した。
- `TEST-REPORT-EDIT-ACT-001`: 上記focused suiteのnon-Error fallback testがmutation `onError`を直接呼んでstate updateを`act`外で発生させていた。Callback 1件だけを`act`で包み、production code/behaviorを変えず2 files / 11 testsをwarningなし、exact lint/formatとfull typecheckをpassさせた。
- `API-CONTRACT-001FM`（PARTIAL probe）: `success` / `successWithMeasuredJsonPayload`をstrict `ApiSuccess{data,meta?}`へ一時的に絞ったtype probeで、現行checkerが見逃す契約負債を再現した。Patients create/PATCH、referrals POST、movement timeline measured responseの4 public経路とflush-metrics helperは`data`なし。加えて60+ callsiteにroot `warnings` / `metadata` / `hasMore` / `summary`等がある。Strict差分は全体typecheckをredにするため完全逆適用し、baseline greenを復旧した。次はchecker拡張とroute-family単位のprovider/consumer移行後にstrict型を再導入する。
- `API-CONTRACT-001FN`: Response-shape checkerをdirect `success`だけでなく`successWithMeasuredJsonPayload`へ広げ、object literalのexact top-level keysを`data` / optional `meta`だけに限定した。Variable payload、top-level spread、data欠落、legacy root keysを拒否する11 fixture testsを追加し、live debtを53 paths / 78 violationsとしてpath別expectedCount allowlistへ登録した。Production runtimeは不変で、checker/lint/formatがpass。以後allowlist増加はCIで失敗する。
- `API-CONTRACT-001FO`: `POST /api/referrals`の201 successをlegacy root `patient/case/warnings/metadata`からexact `{ data: { patient, case, warnings }, meta }`へ移行した。Single referral form consumerはshared `apiDataSchema`で`data.patient.id`をruntime検証してから遷移する。Route/form focused 2 files / 35 tests、protected POST 151 tests、frontend/auth/PHI gates、full typecheckを通し、atomic patient+case transaction、duplicate acknowledgement/409、PHI-minimized DTO、sensitive no-storeは変更していない。Exact-root debtは78→77。
- `API-CONTRACT-001FP`: `PATCH /api/cases/:id/transition`のsuccessを`{ data: careCase, warnings }`からexact `{ data: careCase, meta: { warnings } }`へ移行した。Repository scanでproduction HTTP consumerはなく、provider testだけを同期。Focused 8 tests、protected PATCH/DELETE 80 tests、route-auth/response-shape、full typecheckを通し、assignment guard、status preflight、optimistic `updateMany` conflict、first-visit document warning/task upsert、RLS transactionは変更していない。Exact-root debtは77→76。
- `API-CONTRACT-001FQ`: Pharmacist shifts GETの`{ data, ...(limit ? { meta } : {}) }`を明示的なdata-only/data-meta branchへ置換した。Response bytes/shapeは不変だがtop-level spreadが消え、強化checkerでexact envelopeを静的証明できる。Focused 18 tests、protected GET 384 tests、query-shape、lint/format、full typecheckを通し、auth/RLS/query bounds/no-storeを変更せずdebtを76→75。
- `API-CONTRACT-001FR`: Inquiry records GETの`{ data, ...(limit ? { meta } : {}) }`を明示的なdata-only/data-meta branchへ置換した。Response bytes/shapeは不変だがtop-level spreadが消え、強化checkerでexact envelopeを静的証明できる。Focused 19 tests、protected GET 384 tests、query-shape、lint/format、full typecheckを通し、auth/RLS/filter/pagination/no-storeを変更せずdebtを75→74。
- `API-CONTRACT-001FS`: Management plans org-wide GETのroot `hasMore`をstandard `meta.has_more`へ移した。唯一のproduction list consumerは常に`case_id`を指定して`data`だけを読むため不変。Focused route 18 tests、consumer 1 targeted test、protected GET 384 tests、shape/query/frontend/auth/typecheckを通し、auth/RLS/query bound/no-storeを変更せずdebtを74→73。
- `API-CONTRACT-001FT`: Pharmacy visit requests / partner visit records GETのroot cursor fieldsをstandard `meta.has_more/next_cursor`へ移し、workflow consumerをshared strict `apiCursorPageSchema`へ同期した。Internal page shape/UIは不変でlegacy rootはfail-closed。Provider/consumer/schema 4 files / 69 tests、protected GET 384 tests、shape/query/frontend/auth/privacy/typecheckを通し、auth/RLS/active-consent/PHI-safe DTO/no-storeを変更せずdebtを73→71。
- `API-CONTRACT-001FU`: Pharmacy contracts GETのroot cursor fieldsをstandard `meta.has_more/next_cursor`へ移し、admin setup consumerをshared strict `apiCursorPageSchema`へ同期した。Contract/fee/approval DTOとinternal page stateは不変。Route/setup/schema 3 files / 35 tests、protected GET 384 tests、shape/query/frontend/auth/privacy/typecheckを通し、auth/RLS/write/audit/billing/no-storeを変更せずdebtを71→70。
- `API-CONTRACT-001FV`: Pharmacy resource-map GETのroot `summary`をstandard `meta.summary`へ移し、analytics consumerをstrict runtime schemaへ同期した。通常のpharmacy-sites data-only branchと多数consumerは不変。Route/analytics 2 files / 15 tests、protected GET 384 tests、shape/query/frontend/auth/privacy/typecheckを通し、Japan date boundary/query/aggregate semantics/no-storeを変更せずdebtを70→69。
- `API-CONTRACT-001FW`: Contact profiles bounded searchのroot `hasMore`をstandard `meta.limit/has_more`へ移し、global search contact schemaをstrict envelopeへ同期した。Full listとdirect searchのdata readerは不変。Route/category/hook 3 files / 33 tests、protected GET 384 tests、shape/frontend/auth/privacy/typecheckを通し、bounded minimal projection/PII omission/no-storeを変更せずdebtを69→68。
- `TEST-GLOBAL-SEARCH-ACT-001`: Global search hook testがstale-query用の未発火debounce timerを`afterEach`で境界外実行していたため、cleanupを`clearAllTimers`へ変更し、rerender/microtaskをasync `act`へ含めた。Production code/assertionは不変で14 testsをwarningなし、lint/format/typecheck greenで固定。
- `API-CONTRACT-001FX`: Tracing reports GETのroot `hasMore/nextCursor`をstandard `meta.has_more/next_cursor`へ移した。Production list consumerは見つからず、route contractだけを更新。Focused 17 tests、protected GET 384 tests、shape/query/auth/privacy/typecheckを通し、assignment/tenant filter、patient-name enrichment、PHI-safe logging/no-storeを変更せずdebtを68→67。
- `API-CONTRACT-001FY`: Set plans POSTのroot `replayed`をstandard `meta.replayed`へ移した。Production POST consumerは見つからず、GET/print consumerはdata-onlyのため不変。Focused 23 tests、protected POST 151 tests、shape/query/auth/privacy/typecheckを通し、新規201/既存・race replay 200、cycle transition、notification suppression、RLS/no-storeを変更せずdebtを67→66。
- `API-CONTRACT-001FZ`: Admin webhooks GETのroot aggregate fieldsをstandard `meta`へ集約した。Production consumerは見つからず、existing workspace diffをfocused 15 tests、protected GET 384 tests、shape/auth/lint/format/typecheckで再検証。Webhook secret非公開、tenant/auth、bounded count semanticsを変えずdebtを66→65。
- `API-CONTRACT-001FZQ`: Qualification-check POSTのroot `capabilities`をstandard `meta.capabilities`へ移した。Patient form consumerはdata-onlyで不変。Route 13 tests、consumer 3 targeted tests、protected POST 151 tests、shape/auth/privacy/typecheckを通し、identity/payer result、webhook、adapter behavior、PII omission/no-storeを変えずdebtを65→64。
- `API-CONTRACT-001FZH`: Visit handoff GETのroot extraction/version/confirmation metadataをstandard `meta`へ移した。Provider route 36 tests、protected GET 384 tests、shape/query/auth/privacy/lint/format/typecheckを通し、debtを64→63。初回consumer searchの見落としは`API-CONTRACT-001FZH-VERIFY`で訂正済み。
- `API-CONTRACT-001FZI`: Facility patient selector GETのroot `metadata`をstandard `meta`へ移した。Production consumerは見つからず、focused 5 tests、protected GET 384 tests、shape/query/auth/privacy/lint/format/typecheckを通した。Tenant/facility filter、case assignment scope、archive filter、patient DTO、count semantics、no-storeを変えずdebtを63→62。
- `API-CONTRACT-001FZJ`: Tasks GETのroot `hasMore/nextCursor`をstandard `meta.has_more/next_cursor`へ移し、Tasks/My Day consumerを`fetchAllMetaCursorPages`へ同期した。Legacy root cursorのfail-closed testとbrowser fixturesもcurrent providerへ揃え、関連5 files / 135 tests、protected GET 384 tests、shape/query/frontend/auth/privacy/lint/format/typecheckを通した。Task DTO、tenant filter、assignment/inline completion、POST mutations、rendered UIを変えずdebtを62→61。
- `API-CONTRACT-001FZKV`: Visit records GETのroot `hasMore/nextCursor`をstandard `meta.has_more/next_cursor`へ移し、patient visit-record windowをstrict shared meta cursor readerへ同期。Evidence-gallery fixtureもproviderに揃え、route/helper/evidence/patient-history/print 6 files / 119 tests、protected GET 384 tests、shape/query/frontend/auth/privacy/lint/format/typecheckを通した。Assignment、keyset order、history SQL、attachments、POST、rendered UIを変えずdebtを61→60。
- `API-CONTRACT-001FZK`: Drug masters GETのroot `hasMore/nextCursor/totalCount`をstandard `meta.has_more/next_cursor/total_count`へ移し、admin infinite reader、lightweight suggestion schema、palette schema、browser fixtureをcurrent contractへ同期した。Legacy rootをfail-closedに固定し、関連7 files / 181 tests、protected GET 384 tests、shape/query/frontend/auth/privacy/lint/format/typecheckを通した。Drug/stock DTO、global master cache、site/org filter、import/write、rendered UIを変えずdebtを60→59。
- `TEST-TEMPLATE-AUTH-TX-001`: Formulary template forbidden POST testのstale `$transaction` zero assertionを、domain site/stock/template/audit副作用0のassertionへ置換した。Auth contextはpermission判定前のactor-site解決でtransactionを利用できるため、403/no-storeとdomain handler非実行を正しい境界で固定。Focused/full 13 tests、lint/formatがpassしproduction codeは不変。
- `API-CONTRACT-001FZL`: Formulary template POST/DELETEのroot `site` / `deleted`をstandard `meta.site` / `meta.deleted`へ移し、admin create/delete mutation readerをstrict exact schemaへ同期した。Legacy delete rootをfail-closedに固定し、provider/admin 3 files / 113 tests、protected POST 151、PATCH/DELETE 80、shape/query/frontend/auth/privacy/lint/format/typecheckを通した。Admin permission、tenant filter、template transaction/audit、apply/GET、rendered UIを変えずdebtを59→57。
- `API-CONTRACT-001FZMC`: Communication request POSTのreused-draft root `reused_existing_draft`をstandard `meta.reused_existing_draft`へ移した。共有strict Zod schemaでexact root、required `data.id/status`、optional exact `meta.reused_existing_draft`を固定し、reports/patient-share/workflowの3 consumersはempty/legacy 2xxを成功扱いしない。Route 44 testsを含る4 suites / 95 tests、protected POST 151、shape/query/frontend/auth/privacy/lint/format/typecheck/no-unusedを通した。Auth、tenant、recipient、content、audit、新規draft分岐を変えずdebtを57→56。
- `API-CONTRACT-001FZN`: Conference note POST/PATCHのroot `sync`をstandard `meta.sync`へ移し、作成画面をstrict exact-root Zod schemaとnested readerへ同期した。Legacy root 2xxをfail-closedに固定し、provider/consumer 3 files / 77 tests、protected POST 151、PATCH/DELETE 80、shape/query/frontend/auth/privacy/lint/format/typecheckを通した。Conference sync service、請求候補、報告書draft、訪問提案、task/medication issue、audit、DB/schema、rendered UIを変えずdebtを56→54。初回focused実行で残った旧root assertion 2件を検出して更新後に同suiteを再実行した。
- `API-CONTRACT-001FZRC`: Communication request responses GET、stale-version retry replay、normal POSTのroot `request_updated_at`をstandard `meta.request_updated_at`へ移し、代表分岐でexact root keysを固定した。Repo-wide path/helper/reference scanでproduction HTTP consumerはなく、focused 24 tests、protected GET 384 / POST 151、shape/query/frontend/auth/privacy/lint/format/typecheckを通した。Auth、org/patient/case access、care-report role、writable-patient guard、optimistic version、intent idempotency、race recovery、PHI-minimized audit、no-storeを変えずdebtを54→51へ削減（FZNの56→54と独立）。
- `API-CONTRACT-001FZP`: Staff workload GETの空/通常分岐にあるroot `date`をstandard `meta.date`へ移し、Tasks consumerをstrict exact-root Zod schemaへ同期した。Legacy root 2xxをfail-closedに固定し、provider/consumer 2 files / 30 tests、protected GET 384、shape/query/frontend/auth/privacy/lint/format/typecheckを通した。Membership/task/visit/dispense query、JST date、workload score/order、staff/patient DTO、auth/no-store、rendered UIを変えずdebtを51→49。
- `API-CONTRACT-001FZQRL`: QR draft GET listのroot `hasMore/nextCursor/unmatchedCount`をstandard `meta.has_more/next_cursor/unmatched_count`へ移し、all/unmatched readersをstrict exact-root Zod schemaへ同期した。Legacy root 2xxをfail-closedに固定し、provider/consumer 2 files / 27 tests、protected GET 384、shape/query/frontend/auth/privacy/lint/format/typecheckを通した。QR raw text/hash/parsed raw sanitizer、assignment/patient access、SSE、POST/confirm/detail、PHI DTO、rendered UIを変えずdebtを49→48。
- `API-CONTRACT-001FZSR`: Patient self-report GETのaccessible/empty両分岐にあるroot `hasMore/nextCursor`をstandard `meta.has_more/next_cursor`へ移し、External Viewer readerをshared strict cursor page schemaへ同期した。Legacy root 2xxをfail-closedに固定し、provider/consumer 2 files / 28 tests、protected GET 384、shape/query/frontend/auth/privacy/lint/format/typecheckを通した。Assignment-scoped patient ids、bounded query/order、self-report DTO、no-store、POST/status mutation、rendered UIを変えずdebtを48→46。
- `API-CONTRACT-001FZCOND`: Patient conditions GET/PUTのroot `metadata`をstandard `meta`へ移行したworkspace差分を、route 16 tests、shape/query/frontend/auth/privacy/lint/format/typecheckで再検証した。Repo-wide route/path scanでproduction HTTP consumerはなく、patient cardは別の患者集約DTOを利用する。Expected patient version、condition DTO、replacement transaction、audit、auth/org/archive/conflict/no-store、DB/schemaを変えずdebtを46→44。
- `API-CONTRACT-001FZPACK`: Patient packaging PUTのroot `effective_summary`を`data`内へ集約し、GET/PUTを同じ exact `{ data: { packaging_profile, effective_summary } }` contractへ統一した。PatientPackagingCardはstrict Zod schemaでread/saveを検証し、legacy root 2xxをfail-closedにする。Provider/consumer 2 files / 17 tests、shape/query/frontend/auth/privacy/lint/format/typecheckを通し、profile field、write guard、upsert、GET表示、DB/schema、medical semanticsを変えずdebtを44→43。
- `API-CONTRACT-001FZAUD`: Audit log GETのroot `summary/pagination`をstandard `meta.summary/meta.pagination`へ移し、Admin Audit LogsとMy Day status-change readerをshared exact-envelope schema factoryへ同期した。Legacy root summary 2xxをfail-closedに固定し、provider/admin/My Day 3 suites / 70 tests、protected GET 384、shape/query/frontend/auth/privacy/lint/format/typecheckを通した。Redaction、risk/review集計、filter、view audit、export/review mutation、auth/no-store、DBを変えずdebtを43→42。
- `API-CONTRACT-001FZCONT`: Patient contacts GET/PUTのroot `warnings/metadata`をstandard `meta`へ集約したworkspace差分を検証した。Save readerはstrict schemaでwarnings/readiness/duplicates/versionを読み、legacy rootを拒否する。Provider/panel/report-share/patient-share 4 suites / 70 tests、protected GET 384、shape/query/frontend/auth/privacy/lint/format/typecheckを通し、contact masking、PHI omission、duplicate warning、version claim、replace transaction/audit、auth/archive/conflictを変えずdebtを42→40。
- `API-CONTRACT-001FZPB`: Patient boardの既存exact `PatientBoardPageResponse`をmeasured JSON helperへvariable渡しせずliteral `{ data, meta }`で渡し、wire payloadを変えずstatic exact-root証明を可能にした。代表responseでroot keysを固定し、provider/consumer 2 files / 58 tests、protected GET 384、shape/query/frontend/auth/privacy/lint/format/typecheckを通した。Cursor/count/filter/facet、PHI、query、payload measurement、no-store、rendered UIを変えずdebtを40→39。
- `API-CONTRACT-001FZEP`: Admin external-professional master list 2分岐とlinked-patient listのroot count/filter/cursor metadataをstandard `meta`へ移し、ExternalProfessionalsContentをstrict exact-root schemasへ同期した。Legacy root 2xxとunknown filter enumをfail-closedに固定し、provider/consumer 3 files / 40 tests、protected GET 384、shape/query/frontend/auth/privacy/lint/format/typecheckを通した。Auth、tenant/assignment、archive、contact/patient DTO、count/filter、no-store、DB/schema、rendered UIを変えずdebtを39→36。
- `API-CONTRACT-001FZMS`: Patient medication-stockの既存exact typed service resultをmeasured JSON helperへliteral `{ data, meta }`で渡し、wire payloadを変えずstatic exact-root証明を可能にした。代表responseでroot keysを固定し、route/service/visit/inbound 4 files / 33 tests、protected GET 384、shape/query/frontend/auth/privacy/lint/format/typecheckを通した。Stock/medication DTO、risk/equivalence、RLS/assignment、PHI read audit、query、content length、no-store、rendered UIを変えずdebtを36→35。
- `API-CONTRACT-001FZFAC`: Admin facility list 2分岐とadmin/public facility contacts GET/PUTのroot count/filter/version metadataをstandard `meta`へ移し、FacilitiesContentをstrict exact-root schemaへ同期した。Legacy root 2xxをfail-closedに固定し、provider/consumer 4 files / 44 tests、protected GET 384、shape/query/frontend/auth/privacy/lint/format/typecheckを通した。Auth、tenant、contact PHI、version conflict、replacement transaction、count/filter、no-store、DB/schema、rendered UIを変えずdebtを35→29。
- `API-CONTRACT-001FZINV`: Pharmacy invoices GETのroot `hasMore/nextCursor`をstandard `meta.has_more/next_cursor`へ移し、partner billing readerをstrict shared cursor schemaへ同期した。Legacy root 2xxをfail-closedに固定し、provider/consumer 2 files / 27 tests、protected GET 384、shape/query/frontend/auth/privacy/lint/format/typecheckを通した。金額、税額、請求状態、PDF、生成/遷移mutation、auth、tenant query、DB/schema、rendered UIを変えずdebtを29→28。
- `API-CONTRACT-001FZPAT`: Patient create POSTとpatient update PATCHのroot patient/warnings/duplicate metadataをexact `{ data, meta }`へ移し、PatientForm create/update readerをstrict schemasへ同期した。Legacy root 2xxをfail-closedに固定し、provider/consumer 3 files / 115 tests、protected POST 151、PATCH/DELETE 80、shape/query/frontend/auth/privacy/lint/format/typecheckを通した。患者入力、重複判定、PHI DTO、transaction/audit、auth/tenant、DB/schema、navigation/UIを変えずdebtを28→26。
- `API-CONTRACT-001FZTL`: Patient Movement GETのroot `movement_events/partial_failures`を`data`へ移し、CardWorkspace readerをstrict exact-root runtime schemaへ同期して既存internal snapshotへ変換した。Legacy root 2xxをfail-closedに固定し、provider/consumer 2 files / 106 tests、protected GET 384、shape/query/frontend/auth/privacy/lint/format/typecheckを通した。Cursor/filter/event DTO、PHI read audit、bounded query、content length、no-store、rendered UIを変えずdebtを26→25。
- `API-CONTRACT-001FZCT`: Patient care-team GET/PUTのroot case/warning/reliability metadataをstandard `meta`へ移し、PatientCareTeamPanel save readerをstrict exact-root schemaへ同期した。External-professional option consumerの残ったroot `visible_count`参照も`meta.visible_count`へ修正し、legacy root 2xxをfail-closedに固定した。Provider/panel/report-share/patient-share 4 files / 77 tests、protected GET 384、PATCH/DELETE 80、shape/query/frontend/auth/privacy/lint/format/typecheckを通した。Care-team contact PHI、assignment/case scope、reliability semantics、replace transaction/audit、DB/schema、rendered UIを変えずdebtを25→23。
- `API-CONTRACT-001FZMI`: Medication issues GETのassignment-denied/normal 2分岐にあるroot `hasMore/nextCursor`をstandard `meta.has_more/next_cursor`へ移し、SafetyCheck/Medications readersをshared strict cursor/item schemaへ同期した。両consumerでlegacy root 2xxをfail-closedに固定し、provider/consumers 3 files / 83 tests、protected GET 384、shape/query/frontend/auth/privacy/lint/format/typecheckを通した。Issue/status/priority/medical DTO、assignment/patient/case access、query、create/update、DB/schema、rendered UIを変えずdebtを23→21。
- `API-CONTRACT-001FZSH`: Pharmacy drug-stock history GETのstock不存在/通常2分岐にあるroot `site/stock`をstandard `meta`へ移し、DrugMaster history readerをstrict exact-root schemaへ同期した。Legacy root 2xxをfail-closedに固定し、provider/consumer 2 files / 106 tests、protected GET 384、shape/query/frontend/auth/privacy/lint/format/typecheckを通した。Audit events/changes、stock/site identity、org filter、limit/query、DB/schema、rendered UIを変えずdebtを21→19。
- `API-CONTRACT-001FZREQ`: Pharmacy drug-stock requests GETのroot `summary`とPOSTのroot `site/drug`をstandard `meta`へ移し、DrugMaster request list/create readersをstrict exact-root schemasへ同期した。Legacy root 2xxを取得・作成ともfail-closedに固定し、provider/consumer 2 files / 116 tests、protected GET 384、protected POST 151、shape/query/frontend/auth/privacy/lint/format/typecheckを通した。Request status/payload/reason、formulary audit、stock mutation isolation、query、DB/schema、rendered UIを変えずdebtを19→17。
- `API-CONTRACT-001FZST`: Pharmacy drug-stocks detail/list GETとPOSTのroot `site/metadata`をstandard `meta`へ移し、DrugMaster detail/list/save readersをstrict exact-root schemasへ同期した。Legacy root 2xxを3 readerでfail-closedに固定し、provider/consumer 2 files / 118 tests、protected GET 384、protected POST 151、shape/query/frontend/auth/privacy/lint/format/typecheckを通した。Stock fields、formulary audit transaction、preferred-generic validation、count/filter、org/site scope、DB/schema、rendered UIを変えずdebtを17→14。
- `API-CONTRACT-001FZEA`: External-access管理GET一覧のroot `hasMore/nextCursor`をstandard `meta.has_more/next_cursor`へ移し、ExternalViewer readerをshared strict cursor schemaへ同期した。Legacy root pagination 2xxをfail-closedに固定し、provider/consumer 2 files / 49 tests、protected route matrices 615、shape/query/frontend/auth/privacy/lint/format/typecheckを通した。Token/OTP発行、scope、期限、grant visibility、監査、通知、PHI表示、DB/schemaを変えずdebtを14→13。
- `API-CONTRACT-001FZINT`: Prescription-intakes search/normal GETのroot cursor/total/facetsをstandard `meta`へ移し、shared strict list schemaでPrescriptionsWorkspaceとSearchを同期した。Workspaceはlegacy root paginationを拒否し、Searchはmalformed/legacy処方2xxをfalse-emptyではなくfailed categoryとして扱う。Provider/consumers 3 files / 118 tests、protected GET 384、shape/query/frontend/auth/privacy/lint/format/typecheckを通した。Intake/PHI DTO、assignment、filter/facet、bounded query、query-count header、POST、DB/schema、rendered UIを変えずdebtを13→11。
- `API-CONTRACT-001FZRX`: E-prescription POSTのnew intake、request-id replay、upstream canonical-id replay、unique-conflict replay 4 success分岐でroot `e_prescription_id/idempotent`をstandard `meta`へ移した。Repo内production HTTP consumerはなく、canonical-id replay testを追加してroute 1 file / 25 tests、protected POST 151、shape/query/frontend/auth/privacy/lint/format/typecheckを通した。Patient/case match、writable/assignment、adapter/status、cycle/intake transaction、error mapping、audit/hooks、DB/schemaを変えずdebtを11→7。
- `API-CONTRACT-001FZREP`: Care-reports GETのempty/non-empty palette/list 4分岐でroot cursor/delivery/search metadataをstandard `meta`へ移し、Print Hub、Visit detail、Searchをshared strict list schemaへ同期した。Print/Visitはlegacy root 2xxを拒否し、Searchはmalformed/legacy report 2xxをfalse-emptyではなくfailed categoryとして扱う。Provider/consumers 4 files / 145 tests、protected GET 384、shape/query/frontend/auth/privacy/lint/format/typecheckを通した。Report/PHI DTO、access、delivery summary semantics、query bounds、POST、DB/schema、rendered UIを変えずdebtを7→3。
- `API-CONTRACT-001FZVS`: Visit-schedule-proposals palette GETのroot `hasMore`とgeneration POST 2分岐のroot `alerts/diagnostics/replayed`をstandard `meta`へ移し、global palette、weekly optimizer、proposal dashboard、day plannerをshared strict schemaへ同期した。Legacy root 2xxをfail-closedに固定し、provider/consumers 7 files / 198 tests、protected GET+POST 535、schedule integration 541、checker 11、shape/query/frontend/auth/privacy/lint/format/typecheckを通した。Proposal/PHI DTO、planner、billing、idempotency、transaction、audit、notification、DB/schema、rendered UIを変えずexact-root debtを3→0。Schedule integration初回で見つかったvisit-schedules旧pagination fixtureも現行`meta`契約へ同期し再実行を通した。
- `API-CONTRACT-001FZTYPE`: `success` / measured helperをstrict `ApiSuccess<T>{ data, meta? }` inputへ固定し、root metadataとdata欠落を拒否するnegative type probeを復旧した。Flush-metrics shared jobをexact `{ data }`へ移し、最終再スキャンで検出したdispense-workbench患者listのvariable envelopeもbody不変のstatic `{ data, meta }`へ展開した。Focused helper/flush 3 files / 14 tests、dispense provider/consumer 3 files / 53 tests、protected routes 615 + GET再確認384、checker 11、shape 0/0、full typecheck、exact lint/format/diffを通し、flush side effect、auth/API-key、CloudWatch、error/log、schedule/IaC、患者list DTO/UIを変えていない。
- `API-CONTRACT-001FZERRCOMPAT`: Admin webhooks POSTのvalidation、credential URL、destination policy、encryption failure 5分岐をshared standard `{ code, message, details? }`へ移し、legacy root `error` / `fieldErrors` aliasesを廃止した。Repo-wide production callsiteがゼロになった`compatibilityError` / `validationCompatibilityError` helperも削除。Strict success型で再検出したGET aggregate metadataはstandard `meta`へ移し、sensitive no-storeを固定した。Focused helper/route 2 files / 27 tests、protected GET+POST 535、checker 11、shape 0/0、auth/privacy/full typecheck、exact lint/format/diffを通し、URL/secret検証、暗号化、DB/audit、auth/tenant、GET registration/count semanticsを変えていない。
- `API-CONTRACT-001FZCURSOR`: Production callsite 0のlegacy root cursor schema/parserと旧all-pages helperを削除し、tasks、communications、patient visits、conferences、schedules、my-dayのcurrent `data/meta` paginationを単一`fetchAllCursorPages` + shared schemaへ統合した。初回96 tests中2 failureでvisit-schedulesの正当な`meta.limit`をgeneric helperが拒否すると判明し、provider固有metaはgeneric用途だけ明示許可、個別strict schemaとroot envelopeは維持した。最終8 files / 97 tests、frontend/query/auth/privacy/shape/full typecheck、exact lint/format/diffを通し、provider wire/query/UI/auth/tenant/PHIを変えていない。
- `API-CONTRACT-001FZDATASTRICT`: Shared `apiDataSchema`をstrict `{ data, meta? }` rootへ固定し、unknown root keyをZod既定stripで黙認するfalse-success境界を除去した。初回6-file runは25 failuresで正規optional `meta`の必要性を示し、次は14 failuresでadmin setupのtemplates fixtureが旧root `hasMore`のままと判明した。Standard optional object `meta`を許可しfixtureをlive providerへ同期した最終runは5 consumer群 + schema 6 files / 105 tests。Frontend/privacy/shape/full typecheck、exact lint/format/diffも通し、provider wire/request/UI/auth/tenant/PHIを変えていない。
- `API-CONTRACT-001FZMUTACK`: Pharmacy cooperation workflowに残ったactivate share case、patient link、consent revoke、partner visit submit/reviewのschemaなし`readApiJson<unknown>` 5件をshared strict acknowledgement schemaへ接続した。全providerはexact `{ data }`を返すことをroute code/testsで照合し、legacy activation root 2xxがsuccess toastを出さない回帰を追加。Workflow 1 file / 33 tests、provider 5 files / 39 tests、frontend/privacy/shape/full typecheck、exact lint/format/diffを通し、patient sharing/consent/partner visit write、auth/tenant/audit/notification/medical meaningを変えていない。
- `API-CONTRACT-001FZACK`: Shared strict `readApiAcknowledgement`を追加し、handoff workspace/panelに残ったcreate/message/read/resolve/consult/receipt/confirm/supervision-requestのschemaなしmutation reader 8件をstandard `{ data, meta? }`へ接続した。Provider 6 filesは全success branchがexact `{ data }`であることを照合し、workspaceとconfirm panelのlegacy root 2xxがsuccess toastを出さない回帰を追加。Shared/consumer 3 files / 60 tests、provider 6 files / 84 tests、frontend/privacy/shape/auth/full typecheck、exact lint/format/diffを通し、handoff DTO/write/version/auth/tenant/audit/medical meaning/UIを変えていない。
- `API-CONTRACT-001FZFACACK`: Admin facilities画面のfacility create/update/deleteとunit create/update/deleteを担う4 mutation readersをshared strict acknowledgementへ接続した。Consumer testの旧 `{ok:true}` delete fixturesをlive providerの `{ data: { id } }`へ同期し、legacy root 2xxがdelete成功toastを出さない回帰を追加。Consumer 1 file / 17 tests、provider 4 files / 33 tests、frontend/privacy/shape/auth/full typecheck、exact lint/format/diffを通し、facility/unit DTO、write、optimistic version、auth/tenant/audit/UIを変えていない。
- `API-CONTRACT-001FZUSERACK`: Admin users画面のinvite、profile update、suspend/retire/reactivate/resend-invite actionを担う3 mutation readersをshared strict acknowledgementへ接続した。Legacy invite root 2xxがsuccess toastを出さない回帰を追加し、consumer 1 file / 16 tests、pharmacists provider 2 files / 35 tests、frontend/privacy/shape/auth/full typecheck、exact lint/format/diffを通した。Request payload、auth/authorization、tenant boundary、audit、account lifecycle、UIは変更していない。
- `API-CONTRACT-001FZSCHEDACK`: Business holidaysとservice areasのcreate/update/deleteを担うsave/delete 4 mutation readersをshared strict acknowledgementへ接続した。Consumer testsに残っていたroot `{ message }`とempty `{}`の2xx success fixturesをlive providerの`{ data }`へ同期し、両画面にfalse-success回帰を追加。Consumer 2 files / 35 tests、provider 4 files / 35 tests、frontend/privacy/shape/auth/full typecheck、exact lint/format/diffを通し、master write/validation/auth/tenant/UIを変えていない。
- `API-CONTRACT-001FZREPORTACK`: Report edit/detailのcontent save、pharmacist confirm、single send、bulk sendを担う4 mutation readersをshared strict acknowledgementへ接続した。Legacy root 2xxがdraft clear、confirmed表示、送付済み表示へ進まない回帰を追加し、consumer 2 files / 50 tests、care-report provider 2 files / 92 tests、frontend/privacy/shape/auth/full typecheck、exact lint/format/diffを通した。Report content、billing/medical meaning、recipient/idempotency、auth/audit/UIは変更していない。
- `API-CONTRACT-001FZBRIEFACK`: Visit brief cardとreview画面のfeedback mutations 2件をshared strict acknowledgementへ接続した。Live providerの201 exact `{ data: { ok: true } }`を維持し、legacy root 2xxがrating/confirmation successへ進まない回帰を両consumerに追加。Shared helper/consumers/provider 4 files / 45 tests、frontend/privacy/shape/auth/full typecheck、exact lint/format/diffを通し、provider、request payload、auth/assignment、audit/PHI、UIを変えていない。
- `API-CONTRACT-001FZLABACK`: Patient Labs cardのcreate/update 2 mutation readersをshared strict acknowledgementへ接続し、legacy root 2xxがform clear、success toast、cache invalidationへ進まない回帰を追加。Consumer/provider 3 files / 44 tests、frontend/privacy/shape/auth/full typecheck、exact lint/format/diffを通し、検査値payload/medical meaning、patient scope、auth/tenant/audit/DB/UIを変えていない。
- `API-CONTRACT-001FZNOTIFYACK`: Notifications inboxのmark-read mutationをshared strict acknowledgementへ接続した。Live PATCH providerのcurrent-user/org scoped writeとexact `{ data: { message } }`を維持し、legacy root、empty、non-JSON 2xxを同じconsumer回帰で全て拒否。Shared helper/consumer/provider 3 files / 36 tests、frontend/privacy/shape/auth/full typecheck、exact lint/format/diffを通し、notification DTO/read-write、auth/tenant、realtime、UIを変えていない。
- `API-CONTRACT-001FZVIEWACK`: Saved Viewsのcreate/rename/share/delete 4 mutation readersをshared strict acknowledgementへ接続し、legacy root 2xxがdraft clear、cache invalidation、success toastへ進まない回帰を追加。Shared helper/consumer/provider 3 files / 39 tests、frontend/privacy/shape/auth/full typecheck、exact lint/format/diffを通し、filter/scope/share/write、auth/tenant/audit、UIを変えていない。
- `API-CONTRACT-001FZEXTACK`: External viewerのself-report status updateとfollow-up task create 2 mutation readersをshared strict acknowledgementへ接続し、legacy root 2xxがstatus transitionやtask化後続処理へ進まない回帰を追加。Consumerとself-report/tasks providers 3 files / 77 tests、frontend/privacy/shape/auth/full typecheck、exact lint/format/diffを通し、patient scope/version、dedupe/task payload、auth/tenant/audit/UIを変えていない。
- `API-CONTRACT-001FZINSTACK`: Admin institutionsのcreate/update共通save readerとdelete readerをshared strict acknowledgementへ接続し、旧root message fixturesをlive providerの`{ data }`へ同期した。Legacy save/delete 2xxがdraft clearやsuccessへ進まない回帰を追加し、shared/consumer/providers 4 files / 60 tests、frontend/privacy/shape/auth/full typecheck、exact lint/format/diffを通した。Institution/処方集計DTO、write validation、admin auth/tenant/audit、UIは変更していない。
- `API-CONTRACT-001FZSHIFTACK`: Admin Shifts内の戻り値未使用9 mutation pathsをshared strict acknowledgementへ接続した。Shift save、holiday delete、pharmacist action、template deleteは従来non-2xxだけをparseしていたため成功時も検証する形へ統一し、create/update系も同じhelperへ収束。Legacy root 2xx regressionsを追加し、consumer 1 file / 19 tests、provider 7 files / 79 tests、frontend/privacy/shape/auth/full typecheck、exact lint/format/diffを通した。Shift/holiday/member/template write、auth/tenant/audit/UIと、戻り値を使う前月copy/template applyは変えていない。
- `API-CONTRACT-001FZSITEACK`: Admin Pharmacy Sitesのsite updateとinsurance config create/update/delete 3 mutation readersをshared strict acknowledgementへ接続し、空root mutation fixturesをlive providerのexact `{ data }`へ同期した。Site save、config save、config deleteそれぞれでlegacy root 2xxがform clear、cache invalidation、success toastへ進まない回帰を追加。Shared/consumer/providers 5 files / 68 tests、frontend/privacy/shape/auth/full typecheck、exact lint/format/diffを通し、site/config DTO、改定期間ロジック、write validation、admin auth/tenant/audit、UIを変えていない。
- `API-CONTRACT-001FZVEHACK`: Admin Vehiclesのcreate/update共通saveとavailability status 2 mutation readersをshared strict acknowledgementへ接続し、legacy root 2xxがdraft clear、cache invalidation、success toastへ進まない回帰を両pathに追加。Shared/consumer/providers 4 files / 50 tests、frontend/privacy/shape/auth/full typecheck、exact lint/format/diffを通し、vehicle DTO、状態遷移/割当競合、write validation、admin auth/tenant/audit、UIを変えていない。
- `API-CONTRACT-001FZCREDACK`: Admin pharmacist credentialsのcreate/update共通saveとdelete 2 mutation readersをshared strict acknowledgementへ接続し、legacy root 2xxがform close、cache invalidation、success toastへ進まない回帰を両経路へ追加。Consumer/provider 3 files / 42 tests、frontend/privacy/shape/auth/full typecheck、exact lint/format/diffを通し、credential/workload payload、validation、admin auth/tenant/audit/DB/UIを変えていない。
- `API-CONTRACT-001FZPACKACK`: Admin Packaging Methodsのcreate/update共通save readerをshared strict acknowledgementへ接続し、legacy root 201を成功扱いしない回帰を追加。Shared/consumer/providers 4 files / 35 tests、frontend/privacy/shape/auth/full typecheck、exact lint/format/diffを通し、packaging method DTO、set工程、write validation、admin auth/tenant/audit、UIを変えていない。
- `API-CONTRACT-001FZDRULEACK`: Admin document delivery ruleのcreate/update共通saveとdelete 2 mutation readersをshared strict acknowledgementへ接続し、legacy root 2xxがform clear、cache invalidation、success toastへ進まない回帰を両経路へ追加。Shared/consumer/providers 4 files / 51 tests、frontend/privacy/shape/auth/full typecheck、exact lint/format/diffを通し、rule/recipient/channel DTO、document generation/delivery、write validation、admin auth/tenant/audit、UIを変えていない。
...
- `API-CONTRACT-001FZTPLACK`: Admin Document Templatesのcreate/update共通saveとdelete 2 mutation readersをshared strict acknowledgementへ接続し、未使用のsave response payloadを除去した。Legacy root 2xxがdraft clear、cache invalidation、success toastへ進まない回帰を両経路へ追加。Shared/consumer/providers 4 files / 57 tests、frontend/privacy/shape/auth/full typecheck、exact lint/format/diffを通し、template content/version/effective/default DTO、document generation/delivery、write validation、admin auth/tenant/audit、UIを変えていない。
...
- `API-CONTRACT-001FZSITESELECTACK`: Select Siteのsite switch mutation readerをshared strict acknowledgementへ接続し、空root success fixtureをlive providerの`{ data: { site_id } }`へ同期した。Legacy root 200がsuccess toast、cache invalidation、dashboard navigationへ進まない回帰を追加。Shared/consumer/providers 4 files / 28 tests、frontend/privacy/shape/auth/full typecheck、exact lint/format/diffを通し、membership/site selection、session/current-site state、audit、auth/tenant、UIを変えていない。
...
- `API-CONTRACT-001FZCLERKSTRICT`: 事務サポートBFFのsix KPI、PHI-bearing task rows、consult boundaryをexact runtime schemaへ接続し、mixed-root 2xx rejectionと固定PHI-safe error copyを追加。Consumer/provider/shared 4 files / 25 tests、live ratchet 109 schema-backed / 268 allowlisted debt / 110 files、frontend/privacy/shape/plans/typecheck/lint/format/diffを通した。Task/KPI/consult provider DTO、disclosure、auth/tenant/no-store/DB/UIは不変（commit `bb7bb9ffb`）。
- `API-CONTRACT-001FZQ-VERIFY`: Qualification-checkのpatient-form consumerがmalformed/legacy 2xxを空payloadに変換し、取得失敗を「資格情報なし」と誤分類していた。Provider準拠のstrict `{ data, meta.capabilities }` runtime schemaとlegacy-root rejectionを追加。Patient form 27 tests、exact lint/format、frontend-contract、client-PHI-log、full typecheckがpass。
- `API-CONTRACT-001FZH-VERIFY`: Production `handoff-workspace` consumerがroot `visit_record_version` / `confirmation_policy`を参照し続けていたため、provider移行後に確認不可・version undefinedになる回帰を修正。Strict exact-root runtime schema、nested meta reader、`data:null` guard、fixtures、legacy-root rejectionを追加。Route/workspace 2 files / 71 tests、exact lint/format、frontend-contract、client-PHI-log、full typecheckがpass。
- `API-CONTRACT-001FT-VERIFY`: FT/FJ/FKの完了監査で、visit-request decision用schemaがshare-case activationへ誤接続され、実decision mutationはlegacy rootを含む任意2xxを成功扱いするconsumer driftを確認。Schemaを正しいmutationへ移し、legacy decision rejection testを追加した。Pharmacy workflow/billingのroute-mocked fixturesを現行data/meta契約へ同期し、欠落していたsafe `updated_at`を補完。Unit 70+29 tests、route-mocked Chromium 1/1（workflow、billing filter/invoice、axe、overflow、PHI非表示）、full typecheck、exact lint/format、frontend/privacy gatesを通した。Rollbackはworkflow component/test、billing fixture、browser fixture、ledger差分の逆適用でDB/schema復元不要。ただしschema誤配線の復元は任意2xx成功扱いを再導入する。
- `API-SCHEDULE-LIMIT-001`: admin performance の schedules query が route上限100に対して`limit=200`を送信し、live endpointではvalidation 400になるFE/BE不整合を修正した。readerを`limit=100`へ揃え、date range/data consumption/realtime invalidationは維持し、画面testで100送信と200不在を固定した。
- `COOP-ACTOR-ATTR-001`: pharmacy visit request decision の caller-supplied `pharmacist_id` を validation error で明示拒否し、accept/decline の persisted actor と audit metadata actor を認証済み `ctx.userId` に固定した。後続 partner visit review は legacy `accepted_by` を active same-org Membership + active User で再検証し、valid member だけを explicit notification recipient とする。invalid/orphan/inactive/null recipient は clinical review を止めず除外し、membership lookup failure は workflow write 前に sanitized no-store 500 で fail-closed にする。service-wide notification recipient filtering と外部薬剤師 identity proof は `NOTIFY-RECIPIENT-MEMBER-001` / `COOP-EXTERNAL-PHARMACIST-VERIFY-001` へ分離した。
- `SHARE-CONSENT-WRITE-001A`: patient-share active read/mutation predicate を共通化し、Asia/Tokyo business dateのcase starts/ends、case revoked/ended、partnership effective window/status、partner status、current unrevoked consentを一つのSSOTへ固定した。visit-request createはshare-case consent advisory lock後のactive predicate内だけを404で解決し、Serializable transactionでcontract lookup/create/auditを行う。acceptはinitial active relation filter、同lock後の再読、final `updateMany` relation predicateでexpiry/revoke raceをfail-closedにし、consent revoke routeも同一tenant/case lockへ参加する。declineその他のpost-expiry closeoutは変更せず `SHARE-CONSENT-CLOSEOUT-POLICY-001`、scope false-key/action mappingは `SHARE-CONSENT-SCOPE-NORMALIZE-001` へ分離した。
- `SHARE-CONSENT-SCOPE-NORMALIZE-001A`: patient-share consent POST の `scope` を既存canonical key + literal `true` のpartial recordへ限定し、false/unknown/non-booleanをRLS transaction前に400拒否する。safe DTOと登録auditは `enabledPatientShareScopeKeys` だけを公開し、legacy false/unknown/non-boolean JSONはread時のみ安全投影してbackfillしない。薬局間連携workflowはchecked keyだけを送信し、DB-backed E2E fixtureから未批准 `visit_records` scopeを除去した。action mapping、複数consent union、schema/migration、production backfillは変更しない。
- `SHARE-CONSENT-WRITE-001-SELF-REPORT`: OTP検証済みexternal grantを、id/org/patient/token hash/OTP hash/exact scope/expiry/revocation完全一致の`updateMany` CASとSerializable transactionへ接続した。CASが取得するgrant row lockをreport/event commitまで保持し、revoke/expiry/scope downgrade競合はgeneric 404・副作用0へ閉じる。P2034は同一idempotency contractで2回まで実行し、枯渇時は固定409、同一key/bodyの再送は既存exact-once replayを維持する。
- `SHARE-CONSENT-WRITE-001-PARTNER-SAVE`: partner visit record saveを共通active share-case mutation predicate、share-case consent advisory lock、lock後のactive target再読、Serializable transactionへ接続した。draft updateとvisit-request status CASにも同じactive relation guardを残し、同意競合は409・record/request/audit副作用0へ閉じる。既存のvisit-request status CAS 0件無視もtransaction rollback付き409へ修復し、recordだけ保存される部分成功を防止した。
- `SHARE-CONSENT-WRITE-001-PARTNER-SUBMIT`: partner visit record submitを共通active share-case mutation predicate、canonical consent lock、lock後再読、Serializable transactionへ接続した。record/request両CASに同じactive relation guardを残し、同意競合は409・record/request/notification/audit副作用0へ閉じる。既存のpatient/record version、owner/partnership、idempotent notification、transaction rollback契約を維持する。
- `SHARE-CONSENT-WRITE-001-PARTNER-REVIEW`: partner visit record reviewを共通active share-case mutation predicate、canonical consent lock、lock後再読、Serializable transactionへ接続した。record/request両CASに同じactive relation guardを残し、同意競合は409・record/request/claim support/notification/audit副作用0へ閉じる。既存のpatient/record version、owner/partnership、request transition rollback、安全なresponse projection、PHI-minimal audit契約を維持する。
- `SHARE-CONSENT-WRITE-001-PHYSICIAN-REPORT`: confirmed partner visit recordからの医師向け報告書draft作成を、共通active share-case mutation predicate、canonical consent lock、lock後再読、Serializable transactionへ接続した。visit-request final CASにもactive relation guardを残し、CAS 0件を成功扱いせずtransaction rollback付き409へ修復した。DB unique競合後にabort済みtransactionを再利用する不正な再読を廃止し、固定409後の安全な再試行で既存draftを返す契約へ変更した。
- `SHARE-CONSENT-WRITE-001-CORRECTION-CREATE`: correction request POSTを共通active share-case mutation predicate、canonical consent lock、lock後再読、Serializable transactionへ接続した。初回inactive/unavailableはneutral 404、lock後の同意競合は固定409とし、target lookup/create/audit副作用0へ閉じた。closeout policy未確定のcorrection GET閲覧契約は変更していない。
- `SHARE-CONSENT-WRITE-001-MESSAGE-CREATE`: pharmacy cooperation message POSTを共通active share-case mutation predicate、canonical consent lock、lock後のcontext再解決、Serializable transactionへ接続した。同意競合は409・thread lookup/create/message/thread update/audit/notification副作用0へ閉じた。thread unique競合後にabort済みtransactionを再利用する不正な再読も廃止し、固定409後の安全な再試行で既存threadを利用する契約へ変更した。active predicate済みGETは読取契約のまま維持した。
- `JOB-TENANT-EXECUTION-001A`: `/api/jobs/[jobType]`のhandler registrationを`global_only` / `tenant_or_global`の明示scope付きregistryへ置換した。org伝播未対応jobはAPI-key専用とし、tenant adminおよびlegacy tenant ownerからの全org scan起動をhandler前403へ閉じた。org対応済みhandlerはauthenticated orgを明示引数で維持し、API key requestの`x-org-id`はscopeとして採用しない。
- `JOB-TENANT-EXECUTION-001B`: 薬歴PDF bulk export enqueueのstale recovery、queue limit、patient existence/access、IntegrationJob作成、export auditをactor/org/request trace付き`withOrgContext`のSerializable transactionへ統合した。P2034再試行は毎回同じRLS contextを適用し、非RLS transactionへfallbackしない。worker claim/render/terminal write/notificationとNOBYPASSRLS実DB証跡は残スコープ。
- `JOB-TENANT-EXECUTION-001C`: 薬歴PDF bulk export workerを`jobId + orgId`必須契約に変更し、claim、requester membership/access再検証、heartbeat、完了/失敗CAS、export audit、ready/failure notificationをorg RLS transactionへ移した。cross-org job IDは404でclaimせず、失敗CAS 0件では旧workerがfailure通知を送らない。非RLS Serializable helperは削除した。PDF本文readの短時間RLS loader、global drain discovery、NOBYPASSRLS実DB証跡は残スコープ。
- `JOB-TENANT-EXECUTION-001D`: 薬歴PDFのbranding、patient、medication profile readerへtransaction client injectionを追加し、bulk workerはactor/org/trace付き短時間`withOrgContext`で全dataをloadした後にtransaction外でReact-PDF renderする。既存single PDF routeはdefault Prisma契約を維持し、長時間interactive transactionは導入しない。global drain discoveryとNOBYPASSRLS実DB証跡は残スコープ。
- `JOB-TENANT-EXECUTION-001E`: 薬歴PDF serviceからorg未指定のcross-tenant IntegrationJob recovery/selectを削除し、drain APIを`{ orgId: string }`必須にした。global schedulerはOrganizationをID cursor・100件pageで列挙し、各orgをtenant-bound serviceへ明示的に渡す。tenant failureは固定codeだけを集約し、raw errorを返さない。NOBYPASSRLS実DB証跡は残スコープ。
- `JOB-TENANT-EXECUTION-001F`: 共通`runJob`のorg指定時にduplicate lookup、IntegrationJob create、retry/completion/failure updateをすべて`withOrgContext`経由へ移し、失敗通知のadmin Membership lookupも同じorgへ固定した。org未指定system jobは既存base clientを維持する。RLS gap SSOTはrunner未対応から、NULL system ledger分離が残る現状へ更新した。
- `JOB-TENANT-EXECUTION-001G`: `SystemIntegrationJob`を新設し、migrationで既存`org_id IS NULL` rowを移送後、tenant `IntegrationJob.org_id`をNOT NULL化して`app_enforced_org_id()` FORCE RLSを適用した。共通runnerはsystem/tenant delegateを分離し、drug-master dedupeはsystem ledger、admin dashboardはorg RLS tenant ledger + bounded system ledgerを統合する。Data Explorerは両ledgerをbackend-onlyとし、IntegrationJobのmissing/nullable RLS gapを削除した。
- `SEC-SELF-REPORT-REPORTER-BOUNDS-001`: public/internal self-reportの`reported_by_name`を共有schemaへ統合し、NFC normalization、trim、100文字上限、Cc/Cf制御・format文字拒否を永続化前に固定した。public UIも同じschemaと`maxLength=100`を使い、invalid inputをPOSTしない。既存rowのread projection、DB TEXT、dashboard/queue表示、通知、PHI-minimal auditは変更せず、raw値をlog/audit diffへ追加しない。
- `NOTIFY-RECIPIENT-MEMBER-001`: `dispatchNotificationEvent` の explicit user、enabled rule `user_ids`、role候補をdispatch単位で集約し、same-org active Membership + active `User.is_active/account_status` を1回のnarrow queryで解決する。invalid recipientだけを除外し、有効recipientとdomain transactionは継続する。explicit/rule/role/multi-site重複はuser単位でdedupeし、resolver failureはnotification/realtime/push/external副作用前にfail-closed。external contact lookupはglobal Userのlegacy `org_id`をtenant proofにせず、eligible IDs + current-org active Membership relationでmulti-org userを扱う。caller signature、route-local prefilter、realtime/Web Push redaction、SMS/LINE best-effort、FAX/MCS/email no-op、adapter/outboxは変更しない。
