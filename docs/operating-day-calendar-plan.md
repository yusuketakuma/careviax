...
- **Q7:** 祝日算出を npm ライブラリ（`@holiday-jp/holiday_jp` 等、ローカル同梱・ネットワーク非依存＝egress なし）に委ねるか、内閣府CSV同梱の自前パーサにするか（§14.1）。
...
- **順序:** R1 は **S6（generate 整合）の前提**。S1（util 実装）→ R1（planner を util へ載せ替え・挙動不変を特性テストで固定）→ S6（generate に同じ util を適用）の順。
- **ガード:** planner の既存挙動（rejection code `'business_holiday'` を含む）を変えないことをテストで固定してからリファクタ。
...
- スライドは**自動確定しない**。調剤ワークベンチで終了日が薬局休業日/連休に当たると検知したら、**専用 Drawer**（Z軸 3）で「元終了日 → スライド先（直近稼働日・前倒し原則）＋該当休業日＋不足日数の上乗せ提案」を提示し、**薬剤師がワンタップ承認**。
...
- **候補（いずれも npm ローカルパッケージ＝実行時ネットワーク非依存）:**
  - `@holiday-jp/holiday_jp` — 内閣府データ由来、`between(start, end)` で期間内祝日を取得。データ同梱型。
  - `japanese-holidays`(js) — 振替休日/国民の休日を**算出**。`furikae:false` で振替を除外する等のオプション。
...
- **注意（規制薬）:** 麻薬・向精神薬は早期補充/つなぎ調剤に法的制約があり、緊急時の例外供給対象から除外されるのが通例。PH-OS は麻薬区分を持つため、**スライド/上乗せ提案は規制薬を別扱い**（自動提案せず薬剤師判断を必須化）とする方針を §5.2 に追記すること。
