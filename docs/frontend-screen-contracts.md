...
- A user-facing capability is complete only when its backend contract and
  frontend interaction ship together: authorization, validation, response
  schema, write semantics, loading/error/partial/conflict/offline UI, and tests.
  Backend-only foundations remain `Partial`; frontend controls must not open
  until the same server-side release gate allows the write route.
...
| `FE-VISIT-001`          | 訪問中                            | `src/app/(dashboard)/visits/[id]/record/page.tsx` mounts `visit-record-form.tsx`; voice/capture routes are adjacent                                                     | Read: `/api/visit-schedules/:id`, `/api/visit-preparations/:id`, `/api/cds/check`, `GET /api/patients/:id/medication-stock`. Write: `/api/visit-records`, `/api/visit-records/:id`, file presign/complete, optional patient PATCH, then idempotent `POST /api/visit-records/:id/medication-stock-observations`. The visit page and stock POST share one server-side release gate. | `VisitMedicationManagementSection`, `VisitMedicationStockObservationPanel`, stock request/route helpers, offline store, SOAP draft, conflict helpers, voice/capture draft helpers | Visit page/gate/route/helper/panel/form tests cover default-disabled UI/API, validation before visit save, Idempotency-Key reuse, partial-success lock, 409 retry, 503 unavailable, offline blocking, and false-success prevention. | Keep the shared gate disabled until migration and real DB RLS/idempotency evidence are approved. When enabled, preserve visit-first then stock-write partial-success recovery; never include stock observations in the visit-record payload or report both writes complete before the stock response succeeds. |
...
| 訪問中       | Covered schedule/stock skeleton          | Covered where preparation/detail/stock are truly empty            | Covered long form, gated stock draft, and record submit      | Covered carry-item acknowledgement and visit-saved/stock-pending partial success | Covered schedule/preparation/CDS/save/stock unavailable errors  | Covered offline store/autosave plus visit and stock idempotency conflicts  | Add gate-off browser/mobile fixtures now; add gate-on fixtures after deployment approval. |
...
| 訪問中       | Visit content, residual medication, gated stock observations, SOAP, voice transcript after local action        | Raw sync errors in toast/log, unencrypted offline stock drafts, stock success before the shared server gate and POST response, idempotency material in UI/logs                             |
...
- Visit: `pnpm vitest run 'src/app/(dashboard)/visits/[id]/record/page.test.tsx' 'src/app/(dashboard)/visits/[id]/record/visit-record-form.test.tsx' 'src/app/(dashboard)/visits/[id]/record/visit-record-form.shared.test.ts' 'src/components/features/visits/visit-medication-stock-observation-panel.test.tsx' 'src/lib/visits/medication-stock-observation-gate.server.test.ts' 'src/lib/visits/medication-stock-observation.test.ts' 'src/lib/visits/api-paths.test.ts' 'src/app/api/visit-records/[id]/medication-stock-observations/route.test.ts' --reporter=dot --testTimeout=30000`
...
- A frontend write control and its backend route do not use the same release
  gate, or either side reports completion before the paired mutation succeeds.
