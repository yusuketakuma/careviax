...
- 将来、在宅訪問看護・訪問診療などへの水平展開で `MemberRole` に `nurse`/`physician` 相当のロールを追加する場合の受け皿は以下の通り:
  - `CorePermission`（`canVisit`/`canManageOperationalTasks`/`canReport`/`canAuthorReport`/`canSendCareReport`/`canManageBilling`/`canManagePatientSharing`/`canViewDashboard`/`canAdmin`）は職種を問わず共通に付与しうるケイパビリティであり、そのまま流用できる。
