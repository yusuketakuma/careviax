...
### スコープ規模と段階リリース方針 (重要)
本仕様は 9 phase(P0-P8)・15+ スキーマ変更・2494行(visit-record-form)＋1682行(reports/[id])の巨大コンポーネント分解を含む **multi-quarter 級プログラム**であり、一括リリースを前提としない。出荷優先順位は次の通り段階化する:
- **最小出荷(算定の正本)**: P0 Catalog SSOT → P1 構造化capture(warning先行) → P3 coverage-checker + claim-record + cycle非依存生成 → P4 finalize/確定ロック → P5 算定ゲート連結。ここまでで「算定要件を機械カバーする報告書」の中核が成立。
- **後置(運用拡張)**: P6 inbound双方向/FAX取込/通知、P7 破壊的migration(FK昇格・残薬一本化・レガシーSOAP削除)、P8 retention/電子署名/TSA/STT/POCデバイス は中核成立後に段階投入。
- **non-goal(本リファクタに含めない/別改定対応)**: STT音声入力(egress方針確定まで非出荷)、電子署名/認定TSA(事業者選定後)、POCデバイス連携、2026診療報酬改定の数値確定(告示原文確認は別タスク)。各 phase は1 reviewed slice 単位、objective gate 通過を最終判定とする。
...
## Cross-spec 連携 (docs/schedule-route-build-plan.md = スケジュール/ルート計画 A との協調)

本仕様(B)は schedule-route-build-plan.md(A)より後着手(A の correctness phase P0-P5 完了後に開始)。両者が同一モジュール/モデルを触るため、以下を不変条件とする:

- **C-1 [課金検証ロジックの所有権 — 最重要]**: A/P3 は `billing-requirement-validator.ts` の**キャップ計算を修正**(caps-4/5/7: pending proposal計数・評価対象行の除外・Sun-Sat週)。B/P0 は `BillingRequirementCatalog` を新SSOT化し B/P3-P5 で coverage-checker/claimable gate へ**置換**する。**A/P3 のキャップ修正を B の Catalog 移行で破棄してはならない** — B/P0 着手時に A/P3 の cap-counting ロジック(pending dedupe・excludeScheduleId・cumulative tx cap)を Catalog/coverage-checker へ**継承**し、回帰テストで担保する。
- **C-2 [週境界 Sun-Sat の継承]**: A/確定判断#2 で算定週 = 日曜〜土曜(weekStartsOn:0)に確定済。B/P5「月/週キャップにオンライン46単位・緊急訪問通算を統合」も週キャップを扱うため、**A の Sun-Sat 週境界を必ず踏襲**する(独自の週定義を作らない)。
- **C-3 [datetime TZ-safe builder の再利用]**: B が追加する date 系フィールド(ManagementPlan `next_review_date`、CareReport retention 日付等の @db.Date)は A/P0 と同じ JST off-by-one リスクを持つ。**B は A/P0 の UTC-safe builders(date-boundary.ts / time-of-day.ts)を再利用**し、新規に local Date 構築を持ち込まない。B の visit_started_at/ended_at は full DateTime のため対象外。
- **整合済(衝突なし)**: VisitRecord は A=read-only consume / B=非破壊カラム追加で衝突せず。audit は両者とも既存 AuditLog 流用。lane は BE=Codex / FE=Claude で一致。`next_visit_suggestion_date` は B(訪問capture が書込)→ A(planner が deadline へ読込)の**統合シーム**として活用する。
- **migration 承認の協調**: A の唯一の migration(VisitScheduleOverride partial-unique, A/P9)と B の複数 migration(CareReport FK昇格・残薬一本化・レガシーSOAP削除・InboundResolutionLink 等)は、人間承認をまとめて取得し実行タイミングを調整する。

## フェーズ計画

- **P0 [BE]** BillingRequirementCatalog SSOT 新設（型付き/codegen + property test）
    - **(A/P3 cap fix 継承を含む — Cross-spec C-1 参照)**
...
- cycle_id 任意化(P3, §3.1): 緊急/ad-hoc/delivery_only/temporary 訪問から報告書生成を可能にするため `VISIT_SCHEDULE_CYCLE_REQUIRED_FOR_REPORT` ゲートを解除。**現行 cycle_id が NOT NULL FK なら nullable 化 = migration（要承認）、app-layer ガード解除のみで足りるなら non-migration。どちらかを実コードで確定してからフェーズ着手**（schema-changes に明記すべき項目として追記）
