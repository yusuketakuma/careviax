...
- **`dispense/dispense-workbench.shared.ts`** ← `components/features/dispense/medication-format-grid.tsx` が依存（削除対象外）
- **`dispense/dispense-work-queue.shared.tsx`** ← `prescriptions/prescriptions-workspace.tsx` が依存
- **`medication-sets/set-workspace.shared.ts`** ← `api/medication-sets/workspace/{route.ts,set-derivations.ts}` が依存 → `/set` へ移設し import パスのみ追従
...
- **維持**: `dispense-workbench.shared.test.ts`(12) / `dispense-work-queue.shared.test.tsx`(5)（対応 shared を keep するため）。
...
| **3つの `.shared` を誤削除**すると tsc/build が広範に破損 | 3 shared は全て keep（set-workspace.shared は移設＋import 追従のみ）。削除は本体 .tsx と本体専用テストに限定 |
...
6. `/medication-sets → /set` 改名 + `set-workspace.shared` 移設・API import 追従
...
- **Q4 component 配置 = `src/components/features/dispense-workbench/`（確定）**: 既存規約（`src/components/features/dispense/` 等）に一致。§3 の配置パスと §5-3 の `set-workspace.shared` 移設先はこれを基準に一意化（新 component 群＝`components/features/`、既存コロケート shared は移設のみ）。
...
- **P0（無いと結線不能）**: `PackagingGroup`（グループ実体）/ `SetBatch` セル状態列（set_state/audit_state/ng_code/held_*/set_by/audited_by/at）/ 共通 `RejectCode` enum(14種) / `CycleHold`（構造化保留）
