...
- E1 は Phase 1 で先行実施済み（2026-07-02）。結果は **`.agent-loop/API_REACHABILITY_LEDGER.md`**（専用台帳、`56c181ca`）に記録。FEATURE_QUEUE には台帳へのポインタのみ置く。
...
週次イメージ（スライスは常に小さく、1 commit = 1 review）:
  ...
  Phase 2:         A4-A6 既存部品への集約/昇格 + A5/A8 済 + B5/B6/B9/B10 + D1/D2
                    （A3/A7 は部品既存のため C wave の消費者移行へ移動）
