...
- React Compiler 前提のため、手動 `useMemo` / `useCallback` の**追加を要求する**メモ化規範は本書に書かない（手動メモ化の新設**禁止**の正本は「9.7」）。バックエンド不可侵（API 契約・DB スキーマ・サーバーロジックを UI 都合で変えない。API/UI の対の完成責任は「2.11」）。
