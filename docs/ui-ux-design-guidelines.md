...
- 改修レーンの着手順は高頻度優先: 患者詳細 → 患者一覧 → reports → billing → admin（大量画面は共通テンプレで波及）。各画面はスクショ → 改善（足し算引き算）→ 必要ならバックエンド修正（DB 除く）→ 再スクショで反復する（2026-06-26 ratified）。
...
- React Compiler 前提のため、手動 `useMemo` / `useCallback` によるメモ化規範は本書に書かない。バックエンド不可侵（API 契約・DB スキーマ・サーバーロジックを UI 都合で変えない）。
...
1. **Workbench first** — 一般画面も読み物ではなく業務を進める作業面として設計する。ヘッダー直下に「今やる操作」「判断材料」「一覧/詳細」を近接配置する。調剤・監査・セット系のメイン画面は保護対象とし、ワークベンチ本体の視覚を変更しない。
...
6. **System as product** — 共通部品を先に改善し、画面個別の似た実装を増やさない。新しい UI パターンは token / common component / screen pattern のどこに属するかを明示する。既存 API・権限・DB・監査・患者安全フローは UI 都合で変更しない。
...
| 名前                    | 実状                                                                                   |
| ----------------------- | -------------------------------------------------------------------------------------- |
| `SafetyTagBadge`        | 共通部品なし。`patients-board.tsx` にローカル関数として存在するのみ → 共通化が新設予定 |
...
- **安全タグは `+N` の裏に隠さない**: 麻薬・ハイリスク・アレルギー・抗凝固・冷所・インスリン等の重大タグは常時表示し、軽微タグのみ省略する。この挙動は `SafetyTagBadge`（**新設予定**、現状 `patients-board.tsx` ローカル実装）へ共通化する。
...
- 生 Tailwind（`text-blue-500` / `text-purple-500` 等）の直書きは禁止。既知の残存違反は `visit-record-detail.tsx` の `SoapSection` colorClass（FEUX-4 で `text-soap-*` へ置換）。新規 SOAP UI は最初からトークンを使うこと。
