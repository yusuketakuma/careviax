...
| **5. Longitudinal view**          | 処方変更、検査値、症状、訪問、連絡、文書、報告、task、請求の発生と解決                                                             | 共通時間軸へ揃え、最新を先にしつつ期間filterを持つ。snapshotを置換せず、因果・前後関係を確認する相補レイヤとする                    |
...
| 3      | **経過・結果**（`患者の動き`）              | 訪問、SOAP、処方変更、検査/画像、症状、連絡、文書、報告、taskの時系列                                       | event種別を同じ時間軸へ揃え、問題/期間/sourceでfilter。検査は最新を先頭、複合panelを分断せず、単位・基準範囲・異常flagを横スクロールなしで示す。[2025 SAFER Test Results](https://healthit.gov/resources/2025-safer-guide-test-results-reporting-and-follow-up/) |
...
| **timeline**                       | 処方・症状・検査・訪問・連絡の前後関係                                   | exact date/time、event種別、値/用量、source、filter、一覧fallback                             | snapshotの代替や全画面の既定にしない。初見で遅くなるtaskではsummaryを先に置く                |
