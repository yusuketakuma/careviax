...
- **Dashboard / operational cockpit disclosure は権限内で完全表示**する。ダッシュボード、患者詳細、患者の動き、受信インボックス、調剤/訪問/報告などの認証済み業務画面では、role / assignment / case scope / consent / support session / purpose により閲覧権限が確認できる情報は、患者名・薬剤名・残数・MCS/電話本文・連絡先・添付・訪問内容・処方内容・報告/請求の具体情報を含めて表示してよい。判断するのは現場の人間であり、UI は権限内情報を過度に隠さず、優先度・密度・展開行・右プレビュー・Drawer・詳細画面で整理する。OS 通知、SSE payload、監査差分、server log、外部共有、CSV/PDF export、public URL は別の出力境界であり、業務画面の表示内容をそのまま流用しない。
