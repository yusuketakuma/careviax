...
- 患者コンテキストを持つ画面は、最上部左上から始まる固定領域に、正式名/呼称・カナ・生年月日+年齢・safe patient/display ID から少なくとも2つの識別子を常時表示する。適用可能で業務上必要な場合は legal sex / gender identity、施設・居室、担当者を加え、アレルギー・禁忌・ハイリスクは直下の Safety strip に分離する（Pinned zone、「4.1」「4.1.1」参照。実装は `PatientPinnedHeader` / PatientHeader 系）。患者写真は誤患者オーダーを減らす非割り込み対策として**新設予定**の推奨拡張とするが、写真が無くても2つの文字識別子を維持し、患者の安全・文化・宗教・保護上の例外を許容する。出典: [ONC/ASTP 2025 SAFER Patient Identification](https://healthit.gov/resources/2025-safer-guide-patient-identification/) / [PMC7658731](https://pmc.ncbi.nlm.nih.gov/articles/PMC7658731/) / [JAMIA Open 2024](https://academic.oup.com/jamiaopen/article/7/3/ooae042/7703226)
- 患者識別に用いる氏名・カナ・生年月日は、狭幅でも `truncate` / ellipsis のみで末尾を隠さない。患者ヘッダーでは折返しを優先し、折返し不能な識別子にはタップ・キーボードで到達できる全文表示を併設する。施設名などの補助情報より患者識別を先に置き、モバイルでは識別クラスタを1行全幅に確保する。
...
#### 2.4.2 UI design generation policy（2026-07-06 ユーザー指示反映）

UI/UX の新規・再配置・大幅改善では、実装前に対象画面の既存コード、本書、必要な BFF/API contract を確認したうえで、`imagegen` を使って画面デザイン参照案を作る。`imagegen` 実行時の標準モデル指定は **`gpt-image-2`** とする。

- `gpt-image-2` の生成画像は、UI 参照案・情報設計確認・モバイル/失敗状態の検討に限定する。生成結果をそのまま模写せず、PH-OS の情報密度、状態5分離、44px target、権限/PHI 表示制約、監査導線へ翻訳して実装する。
- `gpt-image-2` の prompt には、実在患者名、住所、電話、処方本文、報告書本文、保険情報、外部共有 URL、secret を入れない。必要な業務文脈は架空データ、抽象ラベル、safe display id だけで表現する。
- 軽微な文言修正、既存部品の状態追加、データ供給元変更など、新規の視覚設計を伴わない slice では `gpt-image-2` 生成を省略できる。その場合も、`ops/refactor/STATE.md` に「省略理由」と本書への適合確認を残す。
- 患者詳細、患者一覧、訪問中モード、Command Center、通知/監査 dashboard など視覚的判断が重要な UI は、原則として `gpt-image-2` 生成案または同等の非 PHI デザイン参照を作り、acceptance に画面状態、失敗状態、モバイル状態を含める。

#### 2.4.3 Clinical Signal Workspace — 全画面統一テーマ（2026-07-14 ユーザー指示・shadcn公式確認）

PH-OSの全画面テーマを **Clinical Signal Workspace** と呼ぶ。革新性は新奇な装飾、カードの形、アニメーションではなく、患者・業務対象について「誰が見ているか → 誰/何を扱っているか → 何が危険か → 今何をするか → 何が変わったか → 何を根拠に判断するか → 操作結果と監査」を、画面遷移後も切れない一本のsignalとして扱うことに置く。本節はテーマの視覚・interaction正本であり、患者データの情報正本は「4.1.1」、状態意味は「6.7」、共通部品は「7.1」を参照する。

shadcn/ui公式は、shadcnを固定パッケージではなく、利用側が実コードを所有して構成・変更する **Open Code / Composition / Distribution / Beautiful Defaults** の仕組みとして定義している。テーマにはsemantic CSS variablesを推奨し、`components.json`の`style`、base color、CSS variables等をcomponent生成時の契約として扱う。PH-OSはこの所有モデルを採用し、shadcnの見た目を上位規範にはしない。2026-07-14確認: [shadcn/ui Introduction](https://ui.shadcn.com/docs) / [Theming](https://ui.shadcn.com/docs/theming) / [components.json](https://ui.shadcn.com/docs/components-json) / [Tailwind v4](https://ui.shadcn.com/docs/tailwind-v4)

**テーマ不変条件:** 「全画面統一」は全画面を同じgeometry・密度・3カラムにすることではない。同じtoken、type role、surface grammar、状態言語、signal順序、component ownership、responsive変換を使い、業務文脈に応じて縮尺することである。認証、legal、外部共有、admin、一覧、単一患者、調剤、訪問、報告、請求のいずれもsilent opt-outできない。患者文脈がない画面に患者bandを捏造せず、該当しないslotは描画しない。

**Clinical Signal Spine（読む順序）:** 次のslotをDOM/source orderと視覚順で一致させる。slot 3〜6は患者文脈がある時に適用し、0〜2、7〜9は対象業務に合わせる。

0. **Actor / organization / session** — 現在の利用者、所属、support/shared session等。患者Identityと混ぜない。
1. **Place / purpose** — 現在地、画面目的、対象期間・filter、戻り先。
2. **Identity / target** — 患者、薬剤、報告、請求、監査等の操作対象。患者では「4.1.1」の最低2識別子。
3. **Safety Horizon** — アレルギー、禁忌、ハイリスク、blocking、閲覧専用等を固定し、隠さない。
4. **Now / next** — 今やること、止まっている理由、担当、期限、次の一手。
5. **Current / delta** — 正確な現在値と、前回・変更前・競合sourceとの差分。
6. **Evidence / provenance** — source、観測/適用/記録時刻、freshness、原記録、確認者。
7. **Action beside evidence** — 操作を対象・根拠・影響・監査previewの近くに置く。
8. **Feedback / recovery** — 保存、送達、失敗、offline、競合、復旧を操作前と同じ位置で持続表示する。
9. **History / audit** — 現在値と混同しない長期経過・監査導線。

**PH-OS固有のsignature pattern:** これらはpattern名であり、同名componentが実在することを意味しない。実装前は`new-planned`として扱い、共通部品に落とすまでは画面ローカルに別実装を増やさない。

| pattern                   | 利用者が得るもの                                               | 実装上の不変条件                                                                                                                                |
| ------------------------- | -------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| **Safety Horizon**        | 画面を移動・スクロールしても患者同定と危険情報を見失わない     | `PatientHeader` + Safety strip、0操作、`+N`/tooltip-only/ellipsis禁止、固定高さとfocus-not-obscured                                             |
| **Clinical Signal Spine** | 情報を「今→変化→根拠→行動」の同じ順序で走査できる              | source orderを維持し、見える装飾線を使う場合も中立borderのみ。状態色の縦線を全sectionへ連結して偽のseverityを作らない                           |
| **Delta Lens**            | 現在値、変更前、適用時点、競合sourceを暗算せず比較できる       | exact text/table/diffが正本。色だけで差分を示さず、source・単位・時刻・未解決状態へ1操作で到達。患者変更履歴は実装済み、他domainは`new-planned` |
| **Evidence Dock**         | 主作業を離れず、選択した値の原文・添付・provenanceを確認できる | desktop/tabletは必要時だけ副列/Sheet、mobileは下Sheet/別page。選択対象とのbindingを保ち、主作業を恒常的に狭めない。`new-planned`                |
| **Recovery Anchor**       | エラーや競合後も「どの患者の何をしていたか」を失わず再開できる | Identity/入力を保持し、同じ領域に原因+次の行動+retry/returnを永続表示。toastだけ、別患者のstale値、成功表示の残留を禁止                         |

**surface grammar:** 境界は意味を表す。カード、影、角丸を画面の装飾単位にしない。

| surface              | 視覚言語                                                         | 用途・制約                                                                                                                                                             |
| -------------------- | ---------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Canvas**           | warm-neutral寄りの`background`、graphiteの`foreground`、影なし   | 全画面の基底。target tokenへの変更はlight/dark、contrast、印刷、screenshotを同一sliceで検証するまで「実装済み」としない                                                |
| **Navigation frame** | 深いnavy、現在地は形+label+控えめなinfo accent                   | system chrome。患者safety/status色をnavigation選択色へ流用しない                                                                                                       |
| **Pinned bands**     | 白/中立面、hairline separator、密な8pt rhythm                    | Actor、Identity、Safety、件数/freshness。大角丸カードやshadowで浮かせず、sticky時の高さとz-indexを共通化                                                               |
| **Work plane**       | 連続面+section heading+separator。外枠1つ、内側は罫線/list/table | Primary/current task。1項目1カード、card-in-card、dashboard tile mosaicを作らない                                                                                      |
| **Evidence surface** | `muted`/`card`の弱い差、source/freshnessを常時label              | Evidence Dock、履歴、原記録。主情報より弱くするが、文字contrastと正確な値は弱めない                                                                                    |
| **Overlay**          | `Sheet` / `Dialog`、背景文脈を残すかinertにするかをZ軸で選ぶ     | 参照継続はSheet、独立作業はfull page、通常確認はDialog。患者/対象Identityをheaderに再掲                                                                                |
| **Interrupt**        | `AlertDialog` / `AlertTier`、赤は患者固有の重大blockingだけ      | 公式Alert Dialogは「重要内容で中断し応答を求める」component。通常reminder、一般情報、成功には使わない。[公式](https://ui.shadcn.com/docs/components/base/alert-dialog) |

**visual palette / type / density:** 新しい色familyを増やさず、`background` / `foreground` / `card` / `muted` / `border` / `primary` / `tag-info`と「3.1」の状態tokenで構成する。基調はnear-white、graphite、deep navy、cool blue。赤はblocking、橙はconfirm/hazard、緑はdoneに限定する。境界はhairlineとtonal surface、shadowはModal等の重なりに必要な時だけ、radiusは`0.375rem`基準とする。氏名・薬剤名・本文は日本語sans、用量・検査・点数・金額・時刻は`tabular-nums`。密度は文字縮小でなく、row構造、罫線、task別fetch、progressive disclosureで調整する。

**shadcn ownership contract:** 現行`components.json`の実装正本は`style: "base-nova"`、RSC/TSX有効、Tailwind v4のconfig空欄、`src/app/globals.css`、`baseColor: "neutral"`、CSS variables有効、`lucide`、repo aliasである。再`init`、style/baseColor切替、生成componentの一括上書き、別primitive systemとの混在をテーマ刷新の手段にしない。

- shadcn primitiveは**構造とinteractionの下地**、PH-OS componentは**臨床意味と安全契約**を所有する。`Table`は`DataTable`の下地、`Badge`は`StateBadge`/`SafetyTagBadge`の下地、`Alert`は`AlertTier`の下地であり、raw primitiveをsemantic componentの代替にしない。[Table公式](https://ui.shadcn.com/docs/components/base/table)
- sourceを直接所有するOpen Codeの利点を使い、wrapper-on-wrapperで見た目を上書きしない。PH-OS共通要件はprimitiveまたはsemantic componentのvariantへ実装し、consumerの`className`例外を増やさない。
- theme tokenは`:root`と`.dark`へsemantic pairを定義し、Tailwind v4の`@theme inline`へ公開する。状態/カテゴリtokenは「3.1」「3.3」のregistryを先に更新する。radiusは単一`--radius`からのscaleを維持する。[Theming公式](https://ui.shadcn.com/docs/theming)
- 公式`Field` compositionはlabel / description / validation / orientationの参考にするが、repoには共有`Field`が未実在である。現行RHF+Controller+label/errorを壊して画面ごとに模倣せず、導入する場合は共有component、既存form移行、error summary、44px、screen-reader testを一つのreviewed sliceにする（`new-planned`）。[Field公式](https://ui.shadcn.com/docs/components/base/field)
- `components.json`はCLIの生成契約であって医療安全SSOTではない。CLI追加後はPH-OSの44px、5状態、focus、reduced motion、active confirmation、no-PHI-output、Base UI挙動を必ず再検証する。

**surface別の同一テーマ変換:** 同じsignal順とsurface grammarを保ち、患者文脈・リスク・作業密度だけを変える。

| surface class                      | 適用形                                                                                                                         |
| ---------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| auth / legal / public entry        | Actor/Place → purpose → form/content → feedback。内部患者chromeは出さず、同じcanvas/type/button/form/state言語を使う           |
| dashboard / list / search          | Actor/Place → count/freshness → compact Identity/Safety → current task → selection preview。row比較を優先                      |
| single-patient / visit / dispense  | full Safety Horizon → Now/next → Current/Delta → Evidence Dock → action。患者切替時はselection/mutation stateを破棄            |
| report / billing / admin workbench | target Identity → blocker/period → canonical values → evidence/audit → action。患者を含む時だけ患者Identity/Safetyを追加       |
| internal overlay / preview         | 選択targetをheaderへ固定し、背景targetと異なる操作を不可にする。previewとdelivery/output認可を分離                             |
| external share                     | 同じbrand/type/state grammarを使うが、内部navigation/actor権限を模倣せずgrant/consent/expiryに限定したIdentityとdataだけを表示 |
| print / PDF / export               | 画面themeの単純コピーではなく「4.9」の文書様式と別output contract。正確な値、見出し、状態label、provenanceは維持               |

**responsive transformation:** desktop画像を縮小しない。desktop 1440+は主作業を優先し、Evidence Dockは必要なtaskだけ副列化する。tablet 768は主+根拠の2列またはSheet、mobile 390はActor/Place → Identity → Safety → Now/next → Current/Delta → Evidence → Actionの1列source orderにする。mobileでEvidenceを閉じても患者同定・Safety・用量・単位・blocking・次の操作は隠さない。固定header/footerがfocus、error summary、最後のfieldを覆わない。

**illustration / visual media:** 生成済みのPHI-free `gpt-image-2`参照は情報重力・signal順の検討資料であり、normative screenshotではない。生成画像の架空個人情報、raw-looking ID、赤の広面積、微小文字、常設3カラム、body mapをそのまま実装しない。body map、写真、relationship diagram、timeline、chartは「4.1.1」の採用条件を満たす時だけEvidenceとして使い、正確なtext/table/sourceを置換しない。装飾illustrationを業務画面のabove-the-foldへ置かない。

**全画面conformance gate:** rolloutは`UI-THEME-CLINICAL-SIGNAL-001`で追跡し、big-bang置換を禁止する。共通token → 共通shell/semantic component → 高頻度surface →全routeの順に、before/afterとrollback可能なsliceで進める。

- **information architecture:** Spineのslot順、Identity/Safety 0操作、Now/next、sourceへの1操作を満たすか。
- **states:** loading/empty/error/partial/stale/offline/conflict/action-successが同じgeometry内で判別でき、false-empty/false-successがないか。
- **journey / trust:** ページ遷移、patient切替、overlay、保存、失敗、復旧後もactor/target/actionの連続性があるか。
- **specificity:** 一般的なSaaS dashboardのcard mosaicやmarketing motifではなく、薬剤・訪問・連絡・報告・請求の実taskに最適化されているか。
- **system alignment:** semantic token、shadcn-owned primitive、PH-OS semantic component、状態registryの所有境界を破っていないか。
- **responsive / accessibility:** 390/768/1440/1920、200% zoom、keyboard、screen reader、forced-colors、CVD、dark、reduced-motion、44pxを通すか。
- **evidence / performance:** PHI-free長名・多剤・長文・競合fixture、主要role/stateのscreenshot、task time/click/見逃し、LCP/INP/CLS、route/component testsを残したか。

生成参照だけ、token変更だけ、1画面だけ、happy pathだけでは全画面theme完了としない。未解決のsafe display ID、canonical medication projection、font/runtime token、dark/forced-colors、output境界、性能budgetは推測で埋めず、dependencyとして`Plans.md`と`ops/refactor/STATE.md`へ残す。
...
- **PHI の表示判断は画面種別ではなく disclosure boundary で行う**。一覧、ダッシュボード、読み取り専用、生成ビュー、プレビューという UI 形態だけを理由に PHI を一律マスクしない。認証済み業務画面で、適用される role / assignment / case scope / consent / support session / purpose 条件をすべて評価し、現在の業務に必要な閲覧が認可された情報は、正確な実値を表示することを既定とする。読み取り専用は低権限と同義ではなく、生成ビューは外部共有と同義ではない。
- **認可済みの業務主体には、業務判断と実行に必要な情報を完全かつ正確に表示する**。PH-OS の組織roleは `owner` / `admin` / `pharmacist` / `pharmacist_trainee` / `clerk` を基本とし、`driver` はroute固有capabilityとassignmentがある場合、外部専門職・`external_viewer` はcase / consent / grantで明示認可された場合だけ対象とする。患者氏名・カナ・生年月日、連絡先・住所・保険情報、薬剤名・用量・残数、アレルギー・禁忌・臨床記載、MCS/電話本文、添付、訪問・処方・報告・請求の具体情報を、担当業務に必要な範囲で伏字・要約・件数だけの表示へ劣化させない。職種名だけによる blanket masking は禁止し、フィールド単位の制限が必要なら capability / assignment / purpose と結び付ける。特に、連絡、本人確認、調剤、監査、訪問、請求に必要な値は、その操作と同じ画面で確認できること。
- **対象薬局に所属し認証されたstaff同士の視認を理由にprivacy filterを掛けない**。薬局内の認証済み業務画面では、他の所属staffから見える可能性だけを理由に、mask、blur、伏字、hover-to-reveal、二段階開示、画面角度制約を要求しない。離席lockと再認証はstaff間の秘匿ではなく、なりすまし操作、監査真正性、患者・来訪者・退職者・外部者・無効sessionによる無権限利用を防ぐcontrolとして維持する。所属薬局/tenant、capability、assignment/case/purpose/consent/grant、画面外出力の境界は緩和しない。
- **この disclosure / placement 契約は、患者情報を表示・選択・比較・編集・previewする全認証済み業務画面に適用する**。dashboard、一覧/worklist、検索/command palette、患者詳細とsubroute、右preview、Drawer/Modal、task、訪問、調剤/鑑査、報告、請求、内部の印刷/export previewはopt-outできない。単一患者と複数患者の縮尺差は「4.1.1」に従い、認可済み実値を画面種別だけで再マスクしない。実ファイルのdelivery、外部共有、public URLは別境界である。
- **情報密度は隠蔽ではなく配置で制御する**。患者識別、アレルギー・禁忌・ハイリスク、薬剤・用量、直近の重要変更、現在の操作に必要な連絡先は常時表示する。その他の認可済み詳細は、展開行、右プレビュー、Drawer、詳細画面で最大 2 階層以内に整理する。臨床判断や本人確認に必要な実値を tooltip、hover、別端末、CSV/PDF export の中だけに置かない。
- **認可されない情報は fail-closed で区別する**。対象患者・案件・目的のいずれかが範囲外なら、API 側で deny / omit / mask する。omit / mask するレスポンスには machine-readable な `visibility` / `withheld_reason` 相当を持たせ、UI はフィールド欠落から認可状態を推測せず、「記録なし」や空文字と「表示権限なし」を非 PHI ラベルで区別する。許可された実値を UI 側で独自に再マスクしたり、部分値を完全な値のように見せたりしない。
- **画面外への出力は別の最小化境界**とする。OS 通知、SSE payload、machine audit event / audit diff payload、server log、RUM、外部共有、public URL、生 JSON、CSV/PDF/印刷は、業務画面の表示内容をそのまま流用しない。ここでいう audit diff payload は、認可済み業務画面に表示する変更履歴・差分投影とは別契約である。各境界で recipient / purpose / consent / expiry / masking profile / audit を明示し、必要最小限にする。内部の認可済み帳票・exportで正確な値が必要な場合も、画面の認可を暗黙継承せず専用契約で許可する。
...
- **Backend-owned ordering.** 変更履歴、患者ヘッダー、監査ログ、期限、未処理キューなどで「最新」「優先」を示す場合、API は `selection_basis` / `sort_basis` と tie-breaker（例: `updated_at`, `created_at`, `id`）を明示する。past→current timelineのように表示順が選択順と異なる場合は `presentation_order` も返し、UIは選択済みbounded windowの不変コピーだけを反転する。membership・cursor・優先順位をfrontendで再実装しない。
...
- テーマ（フォント＋配色トークン）は「2.4.3」のClinical Signal Workspaceとしてアプリ全体に適用する。高密度なレセコン風レイアウト（F1-F12 ファンクション帯 / 1540px 固定幅 / グラデーション帯）は調剤ワークベンチ等の業務画面に限定し、一般画面には持ち込まない。
...
- `Card` の既存 `rounded-xl` は互換例外として残るが、`PageScaffold`、`PageSection`、共通ページヘッダは 2026-07-14 以降 `rounded-md`、hairline border、shadow/gradient なしを正とする。画面ローカルに大角丸コンテナを増殖させる根拠にせず、大角丸を使う場合は共通部品の明示 variant として用途・許容画面・検証スクショを定義する。
...
- **アイコンのホバー/フォーカスで詳細情報を表示する。** 略記・状態・安全タグ等のアイコンには共通 `Tooltip` 部品（**新設予定**——`src/components/ui/tooltip.tsx` を base-ui ベースで用意し、画面ごとの独自ツールチップを作らない＝「7.9」共通化）でマウスオーバー時に補足（正式名称・意味・数値の内訳・根拠）を出し、一覧の情報密度を上げつつ詳細へ辿れるようにする。制約: ① **hover はタッチ端末で発火しない**ため、キーボード `focus` と tap/長押しでも同一補足へ到達できること（「8.3」「8.2」）。② **患者安全に直結する情報（アレルギー・ハイリスク・禁忌・重症度）と、その判断に必要な正確な値・単位・日付・sourceを Tooltip の中だけに隠さない**——常時表示が正本（「2.3」「4.1.1」「4.3」）で、Tooltip は補足に限る。③ Tooltip は `aria-describedby` 等で支援技術へ露出し、装飾扱いにしない。
...
| 画面             | Pinned に置くもの                                                           |
| ---------------- | --------------------------------------------------------------------------- |
| 患者詳細         | 氏名/呼称・カナ・生年月日+年齢・safe display ID・アレルギー/禁忌/ハイリスク |
| 訪問記録入力     | 氏名/呼称・生年月日+年齢・safe display ID・現在の訪問日時・安全情報         |
| 調剤ワークベンチ | 氏名/呼称・生年月日+年齢・safe display ID・ハイリスク・現ステップ           |
| ワークリスト     | 日付・未対応/対応中/完了の件数ストリップ                                    |

#### 4.1.1 患者情報配置の全画面正本構造（2026-07-14 外部エビデンス統合）

患者情報は、保存元システムやDBテーブルの並びではなく、**患者を取り違えない／現在状態を把握する／次の行動を決める／根拠を比較する／経過を追う**という利用者の認知課題に沿って配置する。全データを1画面へ平坦に詰め込むことも、データ種別ごとに別画面へ分断することも禁止する。本節は患者workspaceだけでなく、患者文脈を扱う全認証済み業務画面の配置契約である。外部資料の版・リンクは2026-07-14に確認した。

根拠:

- 2025 SAFER Patient Identification Guide と NISTIR 7804-1 は、患者識別情報を全EHR画面の一貫した予約領域、特に左上へ固定し、スクロール後も同じ場所に残すことを推奨している。氏名だけでなく、正式名/呼称、生年月日+年齢、性別、医療記録番号、所在地、担当者、利用可能なら患者写真を組み合わせる。[ONC/ASTP 2025 SAFER Patient Identification](https://healthit.gov/resources/2025-safer-guide-patient-identification/) / [NISTIR 7804-1](https://nvlpubs.nist.gov/nistpubs/ir/2015/NIST.IR.7804-1.pdf)
- AHRQ/NIST のEHR usability研究は、1画面のoverviewと即時到達できるdetail、関連情報の近接、クリック/スクロール/暗算の削減、時間変化を示すグラフを推奨する。[AHRQ EHR Usability: Evaluation and Use Case Framework](https://digital.ahrq.gov/sites/default/files/docs/citation/09-10-0091-1-EF.pdf) / [NIST GCR 15-996](https://www.nist.gov/publications/technical-basis-user-interface-design-health-it)
- 2025年のEHR visualization systematic reviewは、臨床推論を支える表現を、現在状態を素早く把握する **snapshot** と、変化・関連を追う **timeline** の2系統として整理している。[Supporting clinical reasoning through visual summarization and presentation of patient data](https://pmc.ncbi.nlm.nih.gov/articles/PMC12361860/)
- 問題志向summaryと関連する薬剤・検査・処置・記録の同時表示は、情報検索の効率と認知負荷を改善する。複数画面を往復する従来EHRに比べ、同じ画面へ必要要素を集められるcomposable EHRでは同一データの再閲覧が72.6%から14.6%へ減った小規模研究がある。ただし自由配置をそのまま安全規範にせず、PH-OSでは後述の固定安全領域+task presetとして採用する。[Problem-oriented view study](https://pmc.ncbi.nlm.nih.gov/articles/PMC8068438/) / [User-composable EHR study](https://pmc.ncbi.nlm.nih.gov/articles/PMC4862763/)
- HL7 International Patient Summary (IPS) は、問題、アレルギー、薬剤を必須セクションとし、予防接種、検査、処置、医療機器、事前指示、alert、機能状態、患者の意向を共通の患者summary領域として定義する。データが無い場合も「既知なし」と「情報なし」を区別する。[HL7 IPS 2.0.1](https://hl7.org/fhir/uv/ips/Structure-of-the-International-Patient-Summary.html)
- NHS Englandのstaff-service designは、patient listやclinical dataを扱うstaff toolではsorting/filtering/paginationを含む高密度表示が必要になり得ること、shared deviceではcurrent actorを明確にすることを示す。PH-OSでは公開consumer UIの見た目を模倣せず、accessible hit targetを維持した業務密度とactor/org表示へ限定して適用する。[NHS England: One design system is better than two](https://digital.nhs.uk/blog/design-matters/2026/one-design-system-is-better-than-2)
- 厚生労働省「医療情報システムの安全管理に関するガイドライン 第7.0版」（2026年6月）は、必要時に電子記録を直ちに明瞭かつ整然と表示できる見読性、利用者の識別・認証、職種/所属等に基づく権限管理、離席時の端末保護を求める。個人情報保護委員会・厚生労働省の医療・介護関係事業者向けガイダンス（2026年4月改正）も、従業者の業務上必要な範囲へのアクセス制御を求める。[厚生労働省 ガイドライン第7.0版](https://www.mhlw.go.jp/stf/shingi/0000516275_00006.html) / [個人情報保護委員会 医療・介護関係事業者向けガイダンス](https://www.ppc.go.jp/personalinfo/legal/iryoukaigo_guidance/)
- 上記国内規範からPH-OSが採る実装上の結論は、**認可・session・端末・画面外出力の境界を強くし、その境界内の正当な業務画面では判断に必要な正確な値を見読可能にする**ことである。これは国内規範の逐語的なUI指定ではなく、見読性と必要範囲アクセスを同時に満たすPH-OSの設計判断であり、blanket maskingを法令要件として扱わない。
- JAHIS監査証跡メッセージ標準Ver.2.2は、read/create/update/delete/searchと印刷・ファイル出力・他system送信を監査上区別し、actor、patient、time、operation等を記録する相互運用上の基礎を示す。PH-OSはaccess/search/outputを別eventにし、監査log自身へraw本文や全面diffを複製しない。JAHIS電子保存ガイドVer.5は厚労省6.0対応版であり、policy判断は最新の厚労省7.0/PPCを優先する。[JAHIS監査証跡メッセージ標準Ver.2.2](https://www.jahis.jp/standard/detail/id%3D1189) / [JAHIS電子保存ガイドVer.5](https://www.jahis.jp/standard/detail/id%3D1120)
- 2025 SAFER Clinician Communication Guideは、重要な臨床communicationを適切な相手・時刻・方法で伝え、受領、解釈、対応、acknowledgement、記録、未応答時のescalationまで閉ループ化するよう求める。PH-OSの連絡表示は本文だけでなく、患者、緊急度、sender、recipient、送信/受領/確認時刻、次の行動、期限、escalationを同じまとまりに置く。[2025 SAFER Clinician Communication](https://healthit.gov/wp-content/uploads/2025/06/SAFER-Guide-1.-Clinical-Communication-Final.pdf)

**実装状態とidentifier契約:** 本節はtarget information architectureであり、現行model/API/routeに存在しないfield・panel・集約を実装済みとは扱わない。source contractが無い値を推測、hardcode、別fieldから合成して表示せず、`new-planned` としてmodel/APIから整備する。`safe patient/display ID` はAPIが表示用途に提供するpurpose-built identifierであり、raw internal DB ID、token、storage keyをそのまま表示してはならない。display ID自体も患者情報として認可境界に従う。`current medication SSOT` は、runtime sourceとconsumer projectionが証明されるまではtarget canonical projectionであり、既存の複数リストをcanonicalと宣言してはならない。

**固定優先順位:** `患者識別・安全 > blocking/期限/次アクション > 前回以降の重要変更 > 現在有効な臨床/薬剤/ケア状態 > 判断根拠 > 長期経過 > 終了済み履歴・管理情報`。単純な更新日時、DB作成順、カード追加順でPrimary zoneを決めない。

**正本レイヤ:** すべての患者関連情報は次のいずれかをprimary homeとし、別レイヤへ必要な時は複製せずcanonical IDで参照・投影する。同じ値の独立コピーを複数カードへ持たない。

| レイヤ                            | 置く情報                                                                                                                           | 配置・挙動                                                                                                                                                                                       |
| --------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **0. Identity strip**             | 正式名/呼称、カナ、生年月日+年齢、legal sex/必要時gender identity、safe patient/display ID、施設・居室、担当、利用可能なら患者写真 | 全患者文脈画面の左上から始まる固定領域。全文表示し、スクロール・タブ移動・Drawer表示でも消さない。写真は「2.3」の新設予定方針に従う                                                              |
| **1. Safety strip**               | アレルギー/副作用、禁忌、腎機能、嚥下、感染隔離、麻薬/抗凝固/インスリン等の高リスク、アーカイブ/閲覧専用状態                       | Identity直下に常時表示。折りたたみ、`+N`、tooltip-only、別タブ退避を禁止。未確認/情報なし/既知なしを分離する                                                                                     |
| **2. Command snapshot**           | 今やること、止まっている理由、期限、担当、今日のタスク、前回訪問/処方以降の重要変更、未確認通信、次回訪問/処方                     | 初期タブのPrimary zone。件数だけでなく最優先項目の対象・理由・次アクションを表示し、操作と根拠を近接させる                                                                                       |
| **3. Clinical snapshot**          | active problem、current medication SSOT、最新/重要検査、機能状態、処置・医療機器、care plan、同意/事前指示、患者の意向             | 現在状態を1画面で把握するsummary。各summaryから根拠レコードへ1操作で到達する。自動要約は「導出値」と明示し、元記録を隠さない                                                                     |
| **4. Task evidence workbench**    | 現在の課題に関係する薬剤、検査、訪問/SOAP、画像/文書、MCS/電話、疑義照会、報告、残薬、請求根拠                                     | 同時に比較する2〜4要素を同一画面へ並べる。保存元別画面を往復させず、選択したproblem/taskに関連する要素を上位へ寄せる                                                                             |
| **5. Longitudinal view**          | 処方変更、検査値、症状、訪問、連絡、文書、報告、task、請求の発生と解決                                                             | 現在snapshotを上に残し、臨床event timelineは取得範囲内を過去→現在で表示する。期間filterを持ち、snapshotを置換せず因果・前後関係を確認する相補レイヤとする。監査・運用tableの新しい順とは分離する |
| **6. Source record / provenance** | 正本フィールド、原文、添付、作成/観測/記録/更新時刻、確認元、更新者、version、監査導線                                             | summary・timelineから1操作で開く。編集は正本へ集約し、読み取り投影から独立コピーを作らない                                                                                                       |

**全画面への縮尺適用:** 同じdisclosure / identity / safety契約を、画面の患者数と作業深度に応じて次のように縮尺する。どのsurfaceも認可・source・状態表示を省略しない。

| context              | 対象surface                                                           | 必須配置                                                                                                                                                                |
| -------------------- | --------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **単一患者**         | 患者詳細/subroute、訪問、調剤/鑑査、task、報告、請求、患者固定preview | full Identity strip + Safety stripを固定し、Command/current taskとsource/freshnessを近接表示する。tab、scroll、route内遷移で患者文脈を消さない                          |
| **複数患者**         | dashboard、患者一覧、worklist、検索、command palette、比較            | 各row/card/resultに最低2識別子、業務上のcompact safety signal、source/freshnessを表示する。安全signalを色・点・件数だけにせず、選択後はfull Identity/Safetyへ連続させる |
| **overlay / action** | 右preview、Drawer、Modal、confirmation、command action                | 選択患者の最低2識別子と重要safetyをheaderへ固定する。背景surfaceと選択患者が異なるまま操作できず、overlayを開閉してもpatient IDを暗黙に差し替えない                     |
| **内部preview**      | 認証済み印刷preview、内部帳票preview、export preview                  | preview閲覧は現在の認可を再評価し、業務上必要な正確な実値を表示できる。実ファイル生成・delivery・外部共有はpreview認可を暗黙継承せず、別output contractで最小化する     |

- 共有端末ではcurrent actorとorganizationを常時確認できる位置に表示し、患者Identityと混同させない。操作確定時のactor/orgは監査契約へ接続する。
- 新規surfaceや低密度mobileでも、患者文脈をtooltip、hover、別screenだけへ退避しない。compact表示は省略ではなく、full contextへ連続する縮尺変換である。
- 画面固有の事情でfull ruleを適用できない場合は、非適用理由、代替control、owner、期限をinventoryに記録する。記録なしのsilent opt-outは禁止する。

**PH-OS患者ドメイン配置表:** 現行の患者カード作業台は、次の順でtask-firstに見せる。括弧内は現行タブ/領域との対応であり、実装済みを超える機能を意味しない。

| 表示順 | primary domain                              | 含める情報                                                                                                  | 配置規範                                                                                                                                                                                                                                                                                                                               |
| ------ | ------------------------------------------- | ----------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1      | **今・次**（`Command`）                     | next action、blocker、risk task、今日のtask、未確認/期限超過、前回以降の変更                                | 初期表示。状態の羅列でなく「対象・理由・担当・期限・次の操作」を1組で示す                                                                                                                                                                                                                                                              |
| 2      | **薬剤・安全・訪問準備**（`薬剤・訪問`）    | current medication SSOT、処方差分、用量/日数/残数、セット/一包化、疑義照会、CDS、検査、訪問前確認           | 薬剤名だけでなく用量・単位・頻度・期間・source/確認時点を同じ行に置く。AHRQの medication “One Source of Truth” に従い、職種ごとの別薬剤リストを作らない。[AHRQ MATCH](https://www.ahrq.gov/patient-safety/settings/hospital/match/chapter-3.html)                                                                                      |
| 3      | **経過・結果**（`患者の動き`）              | 訪問、SOAP、処方変更、検査/画像、症状、連絡、文書、報告、taskの時系列                                       | event種別を同じ時間軸へ揃え、取得範囲内を過去→現在で表示し、問題/期間/sourceでfilter。現在値・最新検査summaryはtimelineの上で先に確認可能にし、複合panelを分断せず、単位・基準範囲・異常flagを横スクロールなしで示す。[2025 SAFER Test Results](https://healthit.gov/resources/2025-safer-guide-test-results-reporting-and-follow-up/) |
| 4      | **共有・連絡・文書**（`共有・文書`）        | care team、家族/施設/医療者連絡先、MCS/電話、communication request、添付、報告、同意、外部共有              | 相手、channel、本文、送受信/既読/返信状態、次follow-upを同じまとまりで示す。現在の操作に必要な連絡先はこのタブだけに閉じ込めずCommand/Task evidenceへ正確な値を投影する                                                                                                                                                                |
| 5      | **正本・生活/ケア文脈**（`正本・在宅運用`） | 基本属性、住所/施設/居室、保険、ADL/認知/嚥下、生活・訪問条件、緊急連絡先、care team、case、management plan | 編集元の正本。現在値、確認元、確認日、更新者、staleを併記し、認可済み実値は「2.7」に従い表示する                                                                                                                                                                                                                                       |
| 6      | **請求・会議**（`請求・会議`）              | 算定候補、根拠、blocker、集金、report/conference、適用revision                                              | 臨床情報と財務情報を混在させず、算定根拠から原記録へ1操作で戻れること。金額/点数/期間はtabular-nums                                                                                                                                                                                                                                    |
| 7      | **履歴・出典**（`履歴・構造化`）            | field revision、過去problem、処置、麻薬、文書version、source record、監査への導線                           | 終了済み情報を現在値と混同しない。変更前後・適用時点・記録時点・確認元を明示し、「4.8」と一体運用する                                                                                                                                                                                                                                  |

**同時表示とpreset:**

- 既定layoutは全職種共通のIdentity/Safety/Commandを固定し、その下だけをtask preset（調剤、鑑査、訪問、連絡、請求等）で切り替える。presetは認可済み情報の表示順と取得対象をtask向けに最適化できるが、preset選択をauthorization判定に使わず、認可範囲を拡張しない。
- 利用者が非安全panelを並べ替え・ピン留めできる場合も、Identity/Safety、blocking alert、患者文脈、正本リンクは移動/非表示不可とする。共有presetはrole単位でreviewし、個人設定にはpatient ID・PHI本文を保存しない。必ず「標準に戻す」を用意する。
- taskに必要な情報が2つ以上のdomainへ跨る場合は、Task evidence workbenchでside-by-side表示する。別タブを順番に開いて記憶させる設計をacceptanceにしない。**2〜4要素、最大4panelは外部規格の数値ではなくPH-OSのbounded adaptation**とし、5つ以上はproblem/filterを絞る。
- 同じ情報の「別カードコピー」を作らず、canonical current value + context projection + source linkとする。projectionがstale/partialなら元値と同じ見た目で出さず、生成時刻・欠落source・再取得を示す。

**snapshot / timeline / provenance契約:**

- Snapshotは「いま」、timelineは「どう変わったか」、source recordは「何を根拠にしたか」を答える。3つのどれか1つで代替しない。
- 患者向け臨床timelineは、最新bounded windowの**選択**を `occurred_at DESC, stable ID DESC` 等の決定的な新しい順で行い、選択済みwindowの**表示**だけを `occurred_at ASC, stable ID ASC` の過去→現在へ投影する。cursorは古い方向の意味を変えない。Command抜粋、監査log、運用queue、通常tableは用途に応じた新しい順を維持し、timelineの表示順を横展開しない。
- 過去→現在の終端は、無filter・cursorなし・全source取得成功の時だけ「現在 / 最新」と `aria-current="time"` で示す。期間/種別/自由文filter、partial、older pageでは「絞込内最新」「取得できた範囲の最新」等へ明示的にscopeを下げ、患者の現在と呼ばない。
- event rowは exact effective/occurred time、異なる場合のrecorded time、event種別、source、actor/role、1つの証跡選択操作を持ち、詳細・正本導線は一覧外のEvidence Dockへ集約する。native `ol` / `li`、日付見出し、44px、DOM順=視覚順を維持し、timeline自体に自動focus・scroll・animationを加えない。[NHS App timeline](https://design-system.nhsapp.service.nhs.uk/components/timeline/) / [DWP timeline](https://design-system.dwp.gov.uk/components/timeline/how-it-works) / [MOJ timeline](https://design-patterns.service.justice.gov.uk/components/timeline/)
- HL7 IPSの考え方に合わせ、`known none`（既知なし）、`not recorded`（未記録）、`not asked`（未確認）、`not applicable`、`withheld/restricted`（認可上非表示）、`unavailable/error`（取得不能）、`stale/partial`を区別する。API は machine-readable な `empty_reason` / `visibility` 相当を返し、UI はフィールド欠落から状態を推測せず、空欄や`—`へ畳み込まない。
- 集約値・自動要約・AI/NLP抽出値には、対象期間、source count、生成時刻、confidence/確認状態、欠落source、根拠への導線を持たせる。未確認の導出値を正本と同じ視覚強度で表示しない。
- 各臨床値には、可能な範囲で `effective/observed_at`（患者に起きた時刻）、`recorded_at`（記録時刻）、`updated_at`、source、author/verifierを併記する。FHIR Provenanceが示す「誰が・何を・いつ・どの活動で生成/更新したか」をUI上の信頼判断へ翻訳し、外部由来データでは最後に臨床的な意味を持つ更新をした組織と直前の送信元を区別する。[HL7 FHIR Provenance](https://hl7.org/fhir/provenance.html) / [US Core 9.0.0 Basic Provenance](https://hl7.org/fhir/us/core/basic-provenance.html)
- 複数sourceの値が競合する場合、黙って1値へ統合しない。canonical判定、候補値、source、時刻、未解決状態を示し、解消操作と監査へ接続する。

**timeline / search統合契約:**

- client自由文検索は、表示済みbounded windowだけを対象にしてよいが、「読込済みN件を検索」「読込済み範囲に一致なし」と可視表示し、全履歴検索と呼ばない。件数はpre-mounted `aria-live="polite"`で通知し、query変更でfocusを移動しない。
- category/date/queryをbounded selectionの**後**でfilterするAPIは、`count_basis=bounded_latest_window`、window limit、適用filterをmetaで返し、window外の一致を検索済みと扱わない。全履歴検索を提供する場合は、認可済みsourceを先に検索してからlatest matching windowを選ぶsource-aware queryまたは専用の最小検索indexを使い、既存windowのfetch-allで代用しない。
- Commandの「直近履歴」は無filter・newest-firstの専用query snapshotを使い、患者の動きtabで選んだ期間/種別/local queryを継承しない。患者の動きtabはquery-key変更時も直前の認可済みwindowをplaceholderとして保持し、再取得中表示と`aria-busy`を出す。古いsnapshotを「現在」と呼ばず、完了後に同じ位置で置換する。
- global search / command paletteへtimelineを統合する時は、org / role / assignment / case scopeを再評価した最小DTO（patient safe display、event種別、発生時刻、正本anchor程度）に限定する。処方・訪問・文書本文、添付名、raw previous/current、自由文queryをURL/hash/logへ渡さない。全履歴event resolverが無い間は患者levelの `#patient-movement` 導線までとし、任意eventの直接選択を約束しない。
- 期間/種別のserver filter、読込済み範囲のlocal query、全体検索は別scopeとしてlabelとempty stateを分ける。0件、一部取得、権限で非表示、取得失敗を同じ「該当なし」に畳み込まない。検索結果からEvidence Dockへ選択してもtriggerのfocusを保持し、一覧内に重複linkを増やさない。
- 現行movementの複数sourceには、親行limit後のnested delivery、複数のeffective timestamp fallback、計算値優先order等があるため、単純にcategory/date/cursorを各既存queryへ押し下げても全履歴の正しさを証明できない。full-history movement自由文検索は、認可/RLS付きの正規化された最小event occurrence projection（本文・添付名・raw差分・queryを保存しない）または全adapterのfirst-class query再設計、atomic writer/backfill/index/再認可resolverが揃うまで実装しない。patient field revisionの全履歴cursorも、org context内transactionとcategory有無の複合indexを先に設計し、migration承認なしに現行latest 50契約を拡大しない。

**デスクトップ/モバイル配置:**

- desktop 1440以上はIdentity/Safetyを全幅固定し、Command/現在taskを主列、Task evidenceを必要時の副列にする。副列を常設chromeにせず、比較が必要なtaskだけ `minmax(0, 7fr) / minmax(20rem, 5fr)` 相当で同時表示する。
- tablet 768は主情報+根拠の2列を許可するが、横幅不足時は根拠をDrawerへ移し、主情報の用量・単位・患者識別を縮めない。
- mobile 390はsource orderを Identity → Safety → Command → current task → evidence → history とし、1カラムで正確な実値を表示する。臨床上必要な列を横スクロールへ逃がさず、label/valueの縦行または1項目1行へ変換する。
- 数値trendは基準範囲、単位、測定時点、最新値を併記できる時だけグラフ化する。自由記述、単発値、少数カテゴリを装飾chartへ変換しない。

**患者情報配置の検証ゲート:**

- `owner` / `admin` / `pharmacist` / `pharmacist_trainee` / `clerk`、route固有capabilityとassignmentを持つ`driver`、case / consent / grantで認可された外部専門職・`external_viewer`の実taskで、①患者同定、②アレルギー/禁忌、③現在薬と直近変更、④最新重要検査、⑤次回訪問と訪問条件、⑥連絡先と直近通信、⑦未完了task/blocker、⑧請求根拠、⑨各値のsource/時点へ到達できるかを、各roleの認可範囲内で確認する。
- before/afterでtask成功率、誤選択/見逃し、完了時間、クリック、screen transition、同一データ再閲覧、横スクロール、NASA-TLX等の認知負荷を測る。見た目の好みだけで合否を決めない。
- common taskは「2.5」の3クリック以内を維持し、summaryから根拠詳細は1操作、Identity/Safetyは0操作で確認可能とする。取得失敗・partial・staleを真正の空状態と誤認しないことをroute/component/browser testで固定する。
- 実データの分布（長い氏名、同姓同名、多剤、検査多数、長文MCS、連絡先複数、source競合、未記録/既知なし、mobile 390）を含むPHI-free fixtureで検証する。平均的な短いサンプルだけのスクリーンショットを合格根拠にしない。
- patient dataを扱うroute/component inventoryを持ち、surfaceごとに単一/複数/overlay/previewのcontext、identity/safety component、authorization source、source/freshness、empty/withheld/error/stale状態、output boundaryを記録する。新規・変更routeはinventory更新と該当contract testを必須にし、未登録surfaceをconformance passにしない。inventory/CI gate自体が未実装なら`new-planned`として台帳化し、実装済みを装わない。

**表示手法の選択規範:** 表現方法は「見栄え」ではなく、利用者が答える問いに合わせる。すべての視覚表現は正確な文字値、単位、時刻、sourceへ戻れなければならず、色・形・アイコン・画像だけを正本にしない。複雑な視覚表現にはtooltip-only / screen-reader-onlyではない可視の構造化text/table同等物を置き、visualが失敗・非対応・狭幅の場合はfallbackだけでもtaskを完遂できるようにする。装飾visualへ臨床上の意味を持たせず、above-the-foldのIdentity/Safety領域を消費させない。[W3C Accessibility Principles](https://www.w3.org/WAI/fundamentals/accessibility-principles/) / [W3C Tables Tutorial](https://www.w3.org/WAI/tutorials/tables/) / [W3C G111](https://www.w3.org/WAI/WCAG22/Techniques/general/G111.html)

| 表示手法                           | 採用する問い/情報                                                        | 必須契約                                                                                                                     | 禁止/条件付き                                                                                |
| ---------------------------------- | ------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------- |
| **正確な文字+構造化row**           | 患者識別、連絡先、住所、保険、薬剤名、用量、単位、頻度、期間、金額、点数 | 省略しない文字値、label、source/freshness、copy時の範囲明示                                                                  | アイコン、図、AI要約で置換しない                                                             |
| **比較table / diff**               | 薬剤照合、処方変更、source競合、請求根拠                                 | 列見出し、変更前/後、単位、適用時点、source、未解決状態。mobileは意味単位の縦rowへ変換                                       | 横スクロールしないと薬剤名と用量が対応しない配置、色だけの差分                               |
| **timeline**                       | 処方・症状・検査・訪問・連絡の前後関係                                   | current snapshotを先に置き、選択済み範囲は過去→現在、exact date/time、event種別、値/用量、source、filter scope、一覧fallback | snapshotの代替や全画面の既定にしない。初見で遅くなるtaskではsummaryを先に置く                |
| **trend chart / sparkline**        | 3点以上の連続数値、基準範囲との推移                                      | hoverなしでも最新の正確な値、単位、基準範囲、測定点、欠測、対象期間、table/detail導線                                        | 単発値、自由記述、名義尺度、異なる単位の無断混在、切断軸での誇張                             |
| **body map / 臨床illustration**    | 疼痛、創傷、貼付部位、注射部位等の空間位置                               | 左右、部位、表面/方向、必要時landmark/距離、観測日時、構造化テキスト、凡例、キーボード操作                                   | 診断、重症度、用量、正本記録を絵だけで表さない。自由描画だけをmachine-readableな所見にしない |
| **臨床写真/患者写真**              | 外観の経過記録、本人確認補助                                             | 撮影日/写真のage、source、左右/部位、必要なscale、consent/権限、明示zoom、2つ以上の文字識別子                                | 装飾利用、写真だけの本人確認、外部共有権限の暗黙継承                                         |
| **care-team/relationship diagram** | 担当、連絡経路、責任、escalation                                         | 氏名/役割/所属/連絡状態の文字fallback、現在責任者、未応答、次の連絡先、source/review時刻                                     | 通信本文や正式な担当割当の代替、ノード数無制限の常設                                         |
| **icon / badge / status mark**     | scan補助、状態・期限・警告の識別                                         | 共通registry、可視テキスト、`aria-label`/`aria-hidden`、色以外の手掛かり                                                     | アイコン/色のみ、severityの装飾化、独自記号の乱立                                            |
| **dashboard mini-visual**          | 件数、比率、期間比較、workload overview                                  | 分母、期間、集計時刻、source、正確なdrilldown、未取得/0件の分離                                                              | 実測0と未接続0の混同、patient-level actionを集計値だけで決めさせる                           |
| **map / route**                    | 訪問順、移動、施設位置                                                   | 住所表示権限、現在地の扱い、文字の訪問順/時刻fallback                                                                        | 臨床状態の表現、不要な患者住所の一括露出                                                     |
| **生成AI illustration**            | PHI-freeな患者教育、実装前の非PHIデザイン参照                            | clinical review、version、対象読者、限界、代替テキスト、元資料、PHI/secret-free prompt                                       | 診断根拠、患者固有のsource record、薬剤/用量の正本、未レビューの患者向け指示                 |

薬剤timelineは複数の一般的taskで表形式より成績を改善した小規模研究がある一方、timelineは最初の洞察までを遅くする場合もある。したがってPH-OSでは「current medicationの正確なtable + 変更timeline」を相補的に使い、片方だけにしない。[Medication timeline evaluation](https://pmc.ncbi.nlm.nih.gov/articles/PMC7647176/) / [Health Timeline evaluation](https://pmc.ncbi.nlm.nih.gov/articles/PMC6704521/) 臨床diagramも解釈支援の可能性はあるが理解率は一様でないため、標準化した図+文字fallbackを必須とする。[Medicons evaluation](https://pmc.ncbi.nlm.nih.gov/articles/PMC5446968/)

**配置を固定する参照wireframe:** これは見た目の完成案ではなく、情報順序、常時表示、正本への導線を固定する検証可能なschematicである。装飾画像へ変換して契約を曖昧にしない。

```text
単一患者 desktop/tablet
┌ Current actor / organization / session ───────────────────────────┐
├ Identity: 氏名・カナ・生年月日+年齢・safe ID・施設/担当 [固定] ──┤
├ Safety: アレルギー・禁忌・高リスク・未確認/既知なし [固定] ─────┤
├ Command: 次の行動・blocker・担当・期限・前回以降の変更 ─────────┤
├ Current task / canonical values ───┬ Evidence / source / freshness ┤
│ 正確な値・単位・状態・操作          │ 原文・差分・連絡・添付         │
├ Clinical snapshot ─────────────────┴──────────────────────────────┤
├ Timeline / history / provenance（taskに応じて展開）──────────────┤
└ Output boundary: preview / delivery / external shareを分離 ──────┘

複数患者 list/worklist/preview
┌ Actor/org ─ filters/sort ─ count/freshness ───────────────────────┐
│ 2 identifiers │ compact safety text │ current task │ due │ source │
│ 2 identifiers │ compact safety text │ current task │ due │ source │
├ 選択patient preview: full Identity + Safety [固定] ───────────────┤
│ task detail + exact values + source │ action / escalation          │
└ 選択patientとaction対象が一致しない場合は実行不可 ──────────────┘
```

#### 4.1.2 Fable5風の多面検討と段階導入計画

本節の「Fable5風」は、見た目の案を1つ選ぶのではなく、同じ要件を複数の専門レンズと異常系から反証し、API/data契約、UI、検証、rollbackまで一体で計画する方法を指す。各surfaceは正常表示だけでなく、押下後、取得失敗、部分取得、offline/未同期、stale、他職種の同時更新、競合解消、復旧後、監査確認まで設計して初めて完了とする。

**多面レンズ:**

| レンズ                        | 守る成果                                                                | 主な失敗仮説                                                         | 設計上の対策                                                                                                       |
| ----------------------------- | ----------------------------------------------------------------------- | -------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------ |
| **薬剤師・臨床安全**          | 患者同定、禁忌回避、現在薬/変更/根拠の把握、次の行動                    | マスク/折りたたみで見逃す、古い薬剤を現在薬と誤認                    | Identity/Safety 0操作、用量まで正確表示、current/ended分離、effective time/source、能動的高リスク確認              |
| **事務・訪問・連携運用**      | 連絡、本人確認、訪問条件、請求根拠、closed-loop communication           | 電話/住所/本文が別画面、未応答を完了扱い、担当不明                   | task preset、必要値の同画面投影、sender/recipient/ack/deadline/escalation、正本への1操作導線                       |
| **privacy/security**          | 必要範囲だけを正当な主体へ開示し、端末/出力/ログから漏らさない          | 職種名だけの過剰mask、assignment外開示、離席/通知/exportからの漏えい | capability+assignment+purpose、current actor、session protection、output別contract、deny/withheld reason           |
| **data/API/interoperability** | current value、empty、withheld、stale、競合、sourceを一意に解釈する     | 欠落を「なし」と誤認、複数sourceを黙って統合、UIだけで再mask         | typed visibility/empty/freshness/provenance、canonical ID、version/ETag、conflict response、consumer contract      |
| **frontend/a11y/performance** | 高密度でも読め、キーボード/狭幅で操作でき、必要情報が速く安定表示される | DOM/視覚順不一致、chart/icon依存、全件一括取得、layout shift         | source order、text fallback、virtualization/pagination、task別fetch、予約領域、性能budget、no patient ID URL       |
| **QA/運用/監査**              | 全surfaceで同じ契約を証明し、段階導入と切戻しを可能にする               | happy pathだけ合格、route漏れ、policyとruntimeの乖離                 | inventory/CI、role×surface×state matrix、PHI-free境界fixture、audit evidence、feature flagと旧投影の期限付き切戻し |

**全画面共通の状態契約:**

| 状態                                | 表示と操作                                                                                                               |
| ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------ |
| `loading`                           | Identity/Safetyの既知contextを保持し、対象領域の形状skeleton。別患者の古い値を残さない                                   |
| `current`                           | exact value + source + effective/freshnessを表示し、操作対象のpatient/versionを固定                                      |
| `known_none` / `not_recorded`       | 「既知なし」「未記録」「未確認」を別表示し、`—`や空欄に統合しない                                                        |
| `withheld`                          | 非PHIの理由と必要な権限/手続を表示。値はclientへ返さず、空値と区別                                                       |
| `partial`                           | 取得済みsourceと欠落source、集約時刻、再取得を表示。欠落を含むsummaryを完全値のように扱わない                            |
| `stale` / `offline`                 | 最終同期時刻とoffline範囲を固定表示。staleな臨床値を暗黙利用する高リスク確定を禁止                                       |
| `editing` / `saving` / `unsynced`   | 入力を保持し、保存中/端末内未同期/サーバ保存済みを分離。二重送信を防ぎ、閉じる前に残存状態を知らせる                     |
| `conflict` / `concurrent_update`    | 自分の基準version、他職種の更新値・時刻・更新者を比較し、再読込/再適用/中止を選べる。last-write-winsで黙って上書きしない |
| `error`                             | PHI-freeな失敗理由、retry/return/recover導線を永続表示。入力・患者contextを消さず、自動消滅toastだけにしない             |
| `success_after_action` / `recovery` | 対象患者、実行内容、反映時刻、監査/送達状態、次の行動を同位置で示す。復旧後は再取得し、古い成功表示を残さない            |

**段階導入:** big-bangで全画面を書き換えない。各phaseは前phaseのcontractと測定をgateにし、surface単位のfeature flag/presetで段階開放する。旧投影をrollback用に残す場合も、終了条件と削除期限を持たせ、二重正本にしない。

| phase                           | 実装slice                                                                                                        | 完了gate                                                                                                                          | rollback / dependency                                                                                    |
| ------------------------------- | ---------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------- |
| **0. Inventory & baseline**     | patient dataを扱うroute/component/outputを全件分類し、現行のmasking、PatientHeader、source、状態、task指標を記録 | 未登録surface 0、role×surface×state表、PHI-free長文/多剤/競合fixture、task時間/クリック/見逃しbaseline                            | read-only監査。漏れがあれば次phaseへ進まない                                                             |
| **1. Disclosure contract**      | role-wide maskをcapability/assignment/case/consent/purposeへ置換し、`visibility`/`withheld_reason`を型固定       | API authz/tenant/assignment/negative test、clientに非認可値なし、authorized taskでは必要実値あり、notification/log/export境界不変 | endpoint/field単位flag。認可証明が弱いfieldはfail-closed維持                                             |
| **2. Identity & Safety shell**  | shared PatientHeader/row/overlay variant、current actor、safe display ID、sticky高さを全surfaceへ展開            | 単一/複数/overlay/mobileで2識別子+Safety、患者切替/背景不一致の実行不可、CLS/keyboard/screen reader gate                          | surface単位で旧shellへ切戻し。safe ID/APIとpermission contractがdependency                               |
| **3. Canonical snapshot**       | current medication、problem、検査、訪問条件、communication、source/provenanceのtyped projection                  | current/ended/known-none/withheld/partial/stale/競合のcontract test、sourceへ1操作、複数source照合、薬剤師レビュー                | domain単位flag。canonical判定不能なら候補値+未解決を表示し自動統合しない                                 |
| **4. Task workbench & visuals** | 調剤/鑑査/訪問/連絡/請求preset、比較table、timeline、trend、必要な図/写真を導入                                  | representative task成功率、見逃し、時間、transition、NASA-TLXがbaseline比で悪化しない。文字fallback、単位/時刻/sourceを確認       | visual単位でtable fallbackへ即時切替。図/AI/graphは正本を持たない                                        |
| **5. Resilience & outputs**     | offline/unsynced、optimistic concurrency、復旧、印刷/export/share/notificationの別contract                       | version conflict、二重送信、partial outage、離席、再認証、delivery expiry、audit-before-contentのE2E/contract test                | mutationをread-onlyへ縮退。外部出力はfail-closed、内部参照を全面停止しない                               |
| **6. Controlled rollout**       | role/task/site別にcanaryし、training、support、telemetry、conformance CIを有効化                                 | 誤患者操作、見逃し、task時間、error率、INP/LCP/CLS、問い合わせ、privacy incidentに停止閾値とowner。2サイクル安定後に旧投影削除    | kill switch、前versionのread projection、監査付き切戻し。DB migration/deploy/production mutationは別承認 |

**採用しない案:** (1) PHIを全画面で一律mask、(2) 全情報を平坦に常時展開、(3) 保存元システム別タブの往復、(4) 全surfaceを同じ密度にするだけの画一化、(5) Identity/Safetyまで利用者が非表示にできる自由配置、(6) chart/illustration/写真/AI要約による正確な値の置換、(7) 自動要約をcanonical truth化、(8) 認可・API契約を後回しにしたUI先行、(9) 全routeのbig-bang rewrite。これらは、過剰秘匿、認知負荷、正本競合、誤認、accessibility欠落、切戻し不能のいずれかを増やす。
...
- **詳細画面**: ヘッダーグループ → サマリーグループ（固定表示の重要項目・在宅ステータス・重要タグ・アラート）→ 現在taskと関連根拠 → 正本の編集セクション群（各項目に項目メタを併記）→ 変更履歴 → 監査/補助情報。最上部に **at-a-glance 固定サマリー**を置き、スクロールせず判断できるようにする。患者詳細は「4.1.1」の正本レイヤ/ドメイン順を優先し、「在宅状態・年齢/要介護度・アレルギー・ハイリスク薬タグ（麻薬/インスリン/冷所/抗凝固）・最新処方・次回訪問・前回以降の重要変更」をfold内に収める。**ハイリスク/アレルギーは常時表示・折りたたみ禁止**。保存元別カードを上から並べるだけの詳細画面へ戻さない。
...
- 高さは `100dvh` ベース。固定 `px` / `100vh` / `min-h-screen` を使わない。height ユーティリティの cn 二重指定をしない。例外は「7.9」で正本化した共有 `AppHeader` の tokenized fixed height のみで、画面ローカルの固定高を許可する条項ではない。
...
- **変更履歴タイムライン**: patient field revision timelineは、最新bounded windowを `created_at DESC, id DESC` で選択した後、そのwindowだけを `created_at ASC, id ASC` の過去→現在で「日付・カテゴリ・項目名・変更前→変更後・確認元・更新者」を 1 件ずつ表示し、終端に「現在」を置く。カテゴリ（基本情報/住所/連絡先/病名/臨床/医療処置/麻薬/保険/連携 など）で絞り込めるフィルタを置く。訪問前確認ビューには全履歴ではなく「前回訪問以降の変更」だけを抜き出し、現在終端を付けない。監査log・運用table・一般のnewest-first履歴をこの例外で反転しない。
...
- **PHI の取り扱い**: 認証済み業務画面の差分・履歴・生成ビューでも、「2.7」の disclosure boundary を適用する。適用される role / assignment / case scope / consent / support session / purpose 条件を満たし、臨床判断、本人確認、連絡、監査、請求に必要な電話番号・保険者番号・住所等は、変更前後を含む正確な実値を表示してよい。正本編集画面への遷移を実値確認の必須条件にせず、読み取り専用・凍結スナップショットにも閲覧時点の認可を適用する。machine audit event / audit diff payload、外部共有、public URL、export等の画面外境界は別契約でマスク/最小化し、認可済み業務画面の変更履歴・差分投影と混同せず、認可外の値を復元可能な形で返さない。
...
- 二択の不可逆操作（「最新を使う=自分の入力破棄」/「自分で上書き」）は差分提示つきの共通確認部品で扱い、両側を同等の警告強度にする。実装正本は `ConflictDiffDialog`（`src/components/ui/conflict-diff-dialog.tsx`、`ConfirmDialog` の thin wrapper として focus 既定を継承し、非同期確定は成功まで閉じず pending 中の再確定・取消を防止する。2026-07-10 追加、初回消費者=offline-sync 競合解消）。画面ローカルの bespoke 確認ボックス再実装は禁止。
...
| `ConflictDiffDialog`                                                                                                                                                                     | `src/components/ui/conflict-diff-dialog.tsx`                        | 二択不可逆の差分提示つき確認（5.7）                                                       |
| `SyncStateBadge`                                                                                                                                                                         | `src/components/ui/sync-state-badge.tsx`                            | オフライン同期 4 状態バッジ（6.6/10.）                                                    |
| `PageScaffold` / `PageSection`                                                                                                                                                           | `src/components/layout/`                                            | ページ外枠と、見出し・本文slotを持つ大グループ（7.6/7.9）                                 |
| `PageHeaderFrame` / `PageContextBar`                                                                                                                                                     | `src/components/layout/`                                            | ページ見出しの共通geometry・現在地/戻る導線（7.9）                                        |
...
| `PatientFieldRevisionEntry` / `PatientFieldRevisionList`                                                                                                                                 | `src/components/features/patients/patient-field-revision-entry.tsx` | 患者詳細・訪問記録で共有する変更履歴行とexact差分（4.8、7.9）                             |
...
shadcn primitiveとPH-OS semantic componentの所有境界、現行`components.json`、theme tokenの更新規律は「2.4.3」を正本とする。raw shadcn componentを、`DataTable` / `StateBadge` / `SafetyTagBadge` / `AlertTier` / `PatientHeader`等の臨床意味を持つ共通部品の代替にしない。
...
- **ExpiryBadge 2 段閾値（2026-07-10 是正完了）**: `src/components/ui/expiry-badge.tsx` は `thresholds`（`criticalWithinDays` 既定 30 → `blocked`、`warnWithinDays` 既定 90 → `confirm`、以遠=中立、未設定=中立、不正値=`confirm`）の 2 段閾値実装へ是正済み。残日数は `Asia/Tokyo` の date key 基準で計算する（「2.8」Japan domestic date basis、ランタイム TZ 非依存）。`pharmacist-credentials-content.tsx` / `facility-standards-content.tsx` のローカル実装は共通部品（`showDate` モード）へ統合済みで、期限バッジの画面ローカル再実装は以後禁止。旧・単一閾値時代の暫定措置（既定 90 日繰上げ / 差分理由明記）は失効。
...
- `Card` の `rounded-xl` は既存互換例外（「3.6」）。`PageScaffold` の既定 `card` 面は `rounded-md` + hairline border + semantic `card` backgroundに固定し、decorative gradient、状態意味のないshadow、内容clipを持たせない。画面ローカルで同じ角丸・影・背景を複製せず、差分が必要なら共通部品へ明示 prop / variant を足す。
- `PageSection` は全大グループの共通work planeとして、`rounded-md`、hairline border、semantic surface、見出し/本文を分けるhairline、見出し`px-4 py-3`、本文16px padding、`overflow-visible`を所有し、shadow/gradient/内容clipを持たせない。状態差は`tone`、選択強調は`emphasis`、訪問記録等のimmersive mobile例外は`mobileSurface`、本文内部の配置差だけは`contentClassName`を使う。consumerの`className`で`rounded-*` / `border-*` / `bg-*` / `ring-*` / `p-*` / `overflow-hidden`を上書きしない。
...
- **同じ患者変更履歴は共有Delta Lensへ収束する。** 患者詳細と訪問記録は `PatientFieldRevisionEntry` / `PatientFieldRevisionList` を使い、カテゴリ、項目、変更種別、重要度、現在適用、正確な前後値、変更理由、更新者、確認者、適用期間、確認元の順序と密度制御を一致させる。構造化値は1操作のnative `details`で展開し、legacy presence-only値は復元せず「詳細値を表示できない」と明示する。画面ローカルの履歴カード、独自mask、独自日時短縮を再導入しない。
- **ページ面は `PageScaffold` の2 variantへ収束する。** 通常ページは既定`card`（canvas paddingは16px、largeは24px、各意味グループは`rounded-md` + hairline border + `bg-card`、shadow/gradientなし）、高密度workbench・印刷等は意図を持つ`bare`を使う。下端まで続くworkbenchだけはtyped `canvasInset="flush-bottom"`を使い、`className="pb-0"`等のgeometry上書きで実装しない。装飾を復活させるための`bare`選択やdirect-child selectorの画面ローカル上書きを禁止する。
- **大グループは `PageSection` の安定slotへ収束する。** root surface、見出しrow、本文rowを`data-slot`で共有し、既定のradius/border/background/overflow、見出しseparator、16px content paddingをroute間で変えない。状態・選択・immersive mobileの差は`tone` / `emphasis` / `mobileSurface`のtyped propへ集約し、consumerのraw geometry classで外枠を作り替えない。
- **ページ見出しは最上位1つだけを `PageHeaderFrame` で宣言する。** `WorkflowPageHeader` / `WorkflowPageIntro` / `AdminPageHeader`は同じ16px vertical rhythmを使い、内包するヘッダは`embedded`として二重の`data-page-header`を作らない。戻る/現在地は`PageContextBar`へ統一し、最小block size 44px、`rounded-md`、中立面、hairline borderを維持する。ページ見出し自体は内容可変なので固定高にせず、固定高は`AppHeader`だけが所有する。
- **同じ役割の要素は全画面で同じ位置・順序・余白に置く。** sticky/Pinned の高さ・`z-index`・背景（`backdrop-blur` 等）も共通化し、画面をまたいでヘッダやツールバーが跳ねないようにする（「4.1」／「4.4」）。
- **通常の認証済みシェルの上部バーは `AppHeader` が唯一の所有者で、`--app-header-height: 3.5rem`（既定 56 CSS px）に固定する。** 画面名、role、同期状態、通知件数、補助パネル有無、route 遷移によって block size を変えない。route 固有の操作はページヘッダまたは Drawer に置き、上部バーへ追加して折返しや多段化を起こさない。長い現在ページ名は単一行で省略し、完全名は既存の補助テキスト手段で到達可能にする。印刷、証跡撮影、モバイル訪問記録など、`AppShell` が意図的に chrome を外す最小シェルは例外とする。
- **上部バー直下の sticky/Pinned とフォーカス退避量は `--app-header-height` を参照する。** `top-14`、`56px`、`72px` のような同義のマジック値を画面ローカルに複製しない。ヘッダ寸法を変更する場合は token、`AppHeader`、scroll padding、代表 route の寸法テストを同一スライスで更新する。
...
- 認可外の主体・tenant・患者・case・purposeへのPHI生値露出、またはOS通知/SSE/log/RUM/machine audit payload/外部共有/public URL/export delivery等の画面外境界への無制御な生値再利用。逆に、認可済み業務画面でtaskに必要な正確値を不当に伏字化すること。破壊的・広域・準不可逆操作の確認なし即実行。破壊的操作の Thumb zone・密な行内配置（2.7/4.7/7.4）。
...
- 画面ローカルな `rounded-xl` / `rounded-2xl` / `rounded-3xl` コンテナ、decorative gradient、広域 `backdrop-blur`、意味のない shadow を増やすこと。`PageSection`の外枠をconsumerのraw `rounded` / `border` / `background` / `ring` / `padding` / `overflow-hidden` classで作り替えること（3.5/3.6/7.6/7.9）。
- shadcnを再`init`して現行`base-nova`/Base UI/component sourceを一括上書きすること。`components.json`やraw shadcn variantを医療安全・状態・PHI表示の意味SSOTとして扱うこと。別primitive system、wrapper-on-wrapper、画面ローカルoverrideでClinical Signal Workspaceを分岐させること（2.4.3/7.1）。

## 12. 変更履歴・経緯

- **2026-07-14（臨床timelineの過去→現在表示と検索scope統合）**: ユーザー指示により、現在snapshotを先に保ったまま、患者移動・patient field revision等の臨床event timelineは、最新bounded windowを決定的なDESC+IDで選択し、選択済みwindowだけをASC+IDの過去→現在へ表示する契約を「4.1.1」「4.8」へ確定した。監査log・運用queue・Command抜粋・通常tableの新しい順は維持する。current markerは無filter・cursorなし・全source成功時だけ許可し、filter/partial/older pageはscope付きlatestへ降格する。NHS App/DWP/MOJ timeline、FHIR Provenance、W3C/WCAGを根拠に exact effective/recorded time、source、actor、native ol/li、44px、Evidence Dock、no motionを固定。検索はlocal loaded-window、bounded server filter、global searchを分離し、全履歴を検索していない0件をfalse-emptyにしない。global導線は認可済み最小DTOとpatient-level anchorに限定し、query/event本文/添付/raw差分をURL・log・indexへ持ち込まない。PHI-free `gpt-image-2`参照を実装前に使用し、current snapshot、past→current axis、source/actor列、Evidence Dockの構造だけを採用した。
- **2026-07-14（認可済み患者変更履歴の共有exact Delta Lens）**: 患者詳細と訪問記録に重複していた変更履歴カードを `PatientFieldRevisionEntry` / `PatientFieldRevisionList` へ統合し、`FilterChipBar`の44px操作、罫線行リスト、正確なscalar値、構造化before/after、変更理由、重要度、現在適用、更新/確認/適用/provenanceを同じ情報順へ固定した。内部6 roleは既存のorg/assignment認可後にAPI投影された実値を表示し、`external_viewer`・未知role・service既定値はfail-closedのpresence-onlyを維持する。legacy行は実値を推測せず非復元ラベルを出す。machine audit diff、外部共有、通知、log、exportへの値拡張は行わない。既存のClinical Signal Workspace / Delta Lens表現を共通化するsliceのため新しい画像生成は行わず、既存PHI-free `gpt-image-2`参照と本SSOTのsurface grammarを再利用した。
- **2026-07-14（共有大グループwork plane）**: `PageSection`の78 production call sites / 25 client filesを監査し、routeごとに散在していた`rounded-xl/2xl`、ring、surface background、content clip、mobile border/padding解除を共通契約へ収束した。rootを`rounded-md` + hairline + semantic surface + `overflow-visible`、見出しrowを共通separatorと12/16px rhythm、本文を16px paddingへ固定し、`tone` / `emphasis` / `mobileSurface`をtyped variantとして所有する。consumerの業務内レイアウトは維持し、認可、PHI表示、情報量、操作順、APIは変更しない。代表routeのdesktop/mobile実測で`AppHeader` 56pxと同時にsurface、slot separator、padding、overflowを検証する。
- **2026-07-14（共有ページ面・ページ見出しframe）**: ユーザー指示「画面のUIは可能な限り共有化」「画面遷移時に負担が少ない」を、154 production files / 164 call sitesを持つ`PageScaffold`と3種の共通ページヘッダへ反映した。既定ページ面を16/24px canvas padding、`rounded-md`、hairline border、semantic `card` backgroundへ固定し、decorative gradient、状態意味のないshadow、内容clipを削除した。`PageHeaderFrame`でtop-level page headerを1つにし、`PageContextBar`で戻る/現在地行を44px以上へ統一、nested headerは`embedded`として二重markerを避ける。ページ見出しは内容可変のため固定高にせず、上部固定高は`AppHeader`のみが所有する。情報、認可、PHI表示、操作順は変更しない。
- **2026-07-14（共有シェル上部バーの固定寸法）**: ユーザー指示「画面遷移時もバー部分の高さが変わらない」を「7.9」の全画面契約へ反映した。通常の認証済み画面は `AppHeader` を唯一の上部バーとし、`--app-header-height: 3.5rem` から固定 block size、直下 sticky offset、focus scroll offsetを導出する。route固有操作の流入、多段化、画面ローカルな `top-14` / `56px` / `72px` 複製を禁止し、意図的にchromeを外す最小シェルだけを例外とした。代表routeのdesktop/mobile実測契約を必須にし、認可、PHI表示、業務状態の意味は変更しない。
- **2026-07-14（Clinical Signal Workspace全画面統一テーマ）**: ユーザー指示「今までにない革新的デザイン」「システム全体で統一されたテーマ」を、装飾刷新でなく患者/業務文脈と根拠が切れないClinical Signal Workspaceとして「2.4.3」へ正本化した。shadcn/ui公式のOpen Code / Composition / semantic CSS variables / `components.json` / Tailwind v4 / Alert Dialog / Field / Tableを確認し、現行`base-nova`・Base UI・RSC/TSX・CSS variables・Lucideを保つ所有境界を確定。Safety Horizon、Clinical Signal Spine、Delta Lens、Evidence Dock、Recovery Anchor、surface grammar、全surfaceへの縮尺、responsive変換、illustration制約、7観点conformance gateを追加した。PHI-free `gpt-image-2`参照を使用したが、架空個人情報、raw-looking ID、赤の広面積、微小文字、固定3カラム、body mapの正本化は明示的に棄却した。数値規範・認可・PHI output境界の緩和なし。
- **2026-07-14（Fable5風の多面計画・国内公式根拠・視覚表現の統合）**: 厚生労働省7.0、PPC/MHLW医療介護ガイダンス2026年4月改正、JAHIS監査証跡2.2、SAFER Clinical Communication、W3C、薬剤timeline/clinical diagramの原著研究を追加確認した。認可/session/端末/output境界を強めつつ、認可済み業務では目的に必要な正確値を見読可能にする国内適合上の設計判断、対象薬局の認証済みstaff相互の視認を理由にprivacy filterを掛けない方針、closed-loop communication表示、表示手法の採用/条件/禁止マトリクス、単一/複数患者wireframeを追加。正常、押下後、取得失敗、部分取得、offline/未同期、stale、同時更新競合、復旧/監査を、臨床安全・運用・privacy/security・data/API・frontend/a11y/performance・QA/監査の各面から反証し、inventoryからcontrolled rolloutまでのphase、gate、dependency、rollback、棄却案を「4.1.2」へ確定した。生成AI illustrationはPHI-free患者教育/設計参照に限定し、正本値・診断根拠・患者固有記録の代替を禁止した。
- **2026-07-14（Patient information architecture research統合）**: ONC/ASTP 2025 SAFER、NISTIR 7804-1 / GCR 15-996、AHRQ EHR usability/MATCH、HL7 IPS/FHIR Provenance、2025 EHR visualization systematic review、problem-oriented/composable EHR原著研究を照合し、患者情報配置の全画面正本を「4.1.1」へ追加した。患者識別・安全・Command snapshot・Clinical snapshot・Task evidence・Longitudinal・Source/provenanceの7レイヤ、PH-OS 7ドメイン配置、task preset、同時比較、snapshot/timeline/provenance、desktop/tablet/mobile、実task検証ゲートを確定。単一患者、複数患者、overlay/action、内部previewへ縮尺適用し、全route/component inventoryを必須化した。現行model/APIにないfield、safe display ID、current medication canonical projection、inventory/CI gateはtarget/new-plannedとして区別する。全データの平坦詰込みや保存元別screen fragmentationを避けつつ、認可済み実値を隠さない「2.7」と一体運用する。外部資料はPH-OSの在宅薬局workflowへ翻訳し、自由配置研究をそのまま採用せずIdentity/Safety固定とrole/task presetで安全側へ制約した。
- **2026-07-14（Operational PHI disclosure policy 明確化）**: ユーザー指示により、「PHI はマスクを既定」「一覧・生成ビュー・履歴は生値非表示」という旧記述と、認可済み業務画面の完全表示方針の矛盾を解消した。UI 形態や職種名だけの一律マスクを禁止し、適用される role / assignment / case scope / consent / support session / purpose 条件を満たす `owner` / `admin` / `pharmacist` / `pharmacist_trainee` / `clerk`、route固有認可を持つ`driver`、case / consent / grantで認可された外部専門職・`external_viewer`には、現在の業務に必要な患者識別、連絡先、保険、薬剤、臨床記載、通信、添付、訪問、報告、請求の正確な実値を表示する方針を「2.7」「4.8」へ確定した。認可による omit / mask は machine-readable な理由を必須とし、最小化は認可外データと画面外出力境界へ適用する。情報密度は常時表示・展開行・プレビュー・Drawer・詳細の最大2階層で整理し、外部共有、通知、SSE、machine audit event / audit diff payload、log、RUM、public URL、export deliveryの制限は緩和しない。
...
- **2026-07-07（Dashboard disclosure policy 明文化）**: ユーザー指示により、ダッシュボード等の認証済み業務画面では、権限内で閲覧可能な個人情報・医療情報・薬剤情報・残数・MCS/電話本文・添付・訪問/処方/報告/請求の具体情報を過度に隠さず表示する方針を「2.7」へ追加。表示制限は role / assignment / case scope / consent / support session / purpose による権限制御で行い、情報密度は展開・プレビュー・Drawer・詳細導線で制御する。OS 通知、SSE payload、machine audit event / audit diff payload、server log、外部共有、CSV/PDF export、public URL は別境界として従来どおりマスク/最小化を維持する。
