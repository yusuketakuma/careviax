...
- `Card` の既存 `rounded-xl` は互換例外として残るが、`PageScaffold` と共通ページヘッダは 2026-07-14 以降 `rounded-md`、hairline border、shadow/gradient なしを正とする。画面ローカルに大角丸コンテナを増殖させる根拠にせず、大角丸を使う場合は共通部品の明示 variant として用途・許容画面・検証スクショを定義する。
