...
- `PrescriptionLine`: 処方登録以降の業務同一性。`drug_master_id` があれば最優先、次に canonical `drug_code`、未解決時のみ namespaced `name:<drug_name>` fallback を使う。
...
- `PrescriptionLine.drug_code` はコメント上 YJ コードで、現時点では `DrugMaster` への外部キーはない。
...
1. `PrescriptionLine` に `drug_master_id`、`source_drug_code`、`source_drug_code_type`、`drug_resolution_status` を追加する expand/contract migration を設計する。
2. QR/JAHIS 取込で `review_required` の行を薬剤師が確定コードへ解決する UI/API を実装する。
