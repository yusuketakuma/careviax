...
- **Resolution (2026-07-11, `TBD` commit)**: `/views` の初回preferences/saved-views取得失敗を `SegmentError` + fixed cause/next action + `refetch()` に置換し、初期条件・空一覧への偽フォールバックを止めた。キャッシュが残るbackground refetch失敗は `SegmentStaleBanner` で鮮度低下と再読み込みを明示し、初期条件を取得できない間は「名前を付けて保存」を無効化する。保存API、mutation、URL、権限、PHI/監査、既存toastの契約は不変。focused 1 file / 17 tests、ESLint、Prettier、client PHI-log、frontend-contract、module-boundary、typecheck、diff-checkが通過。E2E、実機a11y/レスポンシブ、full buildは `NOT_EXECUTED`。
