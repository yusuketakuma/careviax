...
Current key count: 142. The earlier refactor note recorded 118 keys; the live repository now references 142.
...
| `BACKUP_MONITOR_STRICT`                      | infra optional       | Treat skipped backup monitor checks as degraded in production / recovery evidence mode. |
...
| `S3_OBJECT_LOCK_BUCKET_NAME`                 | infra optional       | S3 bucket checked by backup monitor for Object Lock; omitted from health output.        |
| `S3_OBJECT_LOCK_BUCKET_REGION`               | infra optional       | Region override for S3 Object Lock monitor.                                             |
