...
5. S3 Object Lock は admin health の `s3ObjectLock` で `enabled`、bucket default retention の mode / days / years のみを確認する。共有ファイルバケットでは bucket-level default retention を必須にしない。処方箋原本などはアプリの upload path が per-object retention header を付与する。
6. 監査ログアーカイブバケットのライフサイクルと Object Lock を確認する。
7. Cognito User Pool の Advanced Security が `ENFORCED` であることを確認する。
8. 結果、所要時間、失敗点、改善策を記録する。
...
- production / recovery evidence mode では `BACKUP_MONITOR_STRICT=true` を設定し、backup monitor の critical check が環境変数不足で skipped になった場合に `/api/health` を degraded として扱う。local / 開発環境では未設定のままにし、AWS入力不足を graceful skip として扱う。
- admin ヘルスチェック: `/api/health` に管理者としてアクセスし、`checks.backups.awsBackupVault`、`checks.backups.awsBackupRecoveryPoint`、`checks.backups.rdsInstanceBackupConfiguration`、`checks.backups.rdsSnapshot`、`checks.backups.s3Versioning`、`checks.backups.s3ObjectLock` が stale/error でないことを確認する。
...
health response、log、audit には AWS account id、raw ARN、RDS endpoint、KMS key ARN、security group、subnet、bucket名、vault名、snapshot id、user pool id、provider raw error を出さない。Object Lock について返してよいのは `enabled`、`defaultRetentionMode`、`defaultRetentionDays`、`defaultRetentionYears` のような設定状態のみ。
...
- Amazon S3 `GetObjectLockConfiguration`
