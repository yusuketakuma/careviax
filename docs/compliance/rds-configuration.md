...
CareViaX の主データベースである Amazon RDS PostgreSQL の構成を明文化する。
...
| 最終スナップショット | 削除時に自動作成（名称: `careviax-prod-final-snapshot-YYYYMMDD`） |
...
パラメータグループ名: `careviax-pg16-production`
...
サブネットグループ名: `careviax-rds-subnet-group`
