...
| R-02       | Permission policy commonality       | Implemented for v0.2 boundaries                    | `canManagePatientSharing`, `canManageBilling`, active-share read predicates, patient-share correction/edit policy, shared data output policy for attachment/print/PDF/download scope checks | Broader role-matrix browser proof after DB apply                        | In progress but sufficient for v0.2 surfaces                                                                 | P0       |
...
- Shared patient-share output policy maps attachment view/download, print, PDF output/download, and data download actions to required `share_scope` keys and fails closed for inactive share cases.
