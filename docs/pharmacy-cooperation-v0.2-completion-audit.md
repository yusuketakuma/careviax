...
This document is the current-state audit for the attached v0.2 in-home cooperating pharmacy specification. It is not a DB application report. Local e2e DB migration application and direct patient-card browser proof still require explicit approval because applying migrations is a DB mutation.
...
| Direct DB-backed browser proof | Not complete                        | Local e2e DB is behind 18 migrations; direct patient-card route can hit missing v0.2 columns                                                        |
| Migration application          | Not complete                        | `prisma migrate status` reports pending migrations; none were applied in this audit turn                                                            |
...
| ---------- | ----------------------------------- | -------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------- | ----------------------------------------------------------------------- | -------- |
| A-01       | Pharmacy partnerships               | Implemented                                        | `PharmacyPartnership` model; `/api/pharmacy-partnerships`; setup UI                                                            | DB apply proof                                                          | Uses shared org/auth route conventions                                  | P0       |
| A-02       | Patient share cases                 | Implemented                                        | `PatientShareCase`; `/api/patient-share-cases`; patient card create UI; workflow table                                         | Direct patient-card browser proof after DB apply                        | `patient-share-scope.ts`, `patient-share-access.ts`                     | P0       |
...
| B-01       | Management plan/version evidence    | Partially implemented                              | workflow references management plans and contract/version snapshots                                                            | Stronger end-to-end management-plan sharing proof                       | Existing management-plan services reused                                | P1       |
| B-02       | Visit request lifecycle             | Implemented                                        | `PharmacyVisitRequest`, decision API, estimate snapshot, lifecycle status alignment                                            | DB apply proof                                                          | Contract estimate resolver in route/service layer                       | P0       |
...
| R-01       | Patient reference commonality       | Partially implemented                              | share/access helpers and patient card/workflow consumers                                                                       | Broader cross-app patient summary resolver remains future work          | Started                                                                 | P1       |
| R-02       | Permission policy commonality       | Implemented for v0.2 boundaries                    | `canManagePatientSharing`, `canManageBilling`, active-share read predicates                                                    | Broader role-matrix browser proof after DB apply                        | In progress but sufficient for v0.2 surfaces                            | P0       |
...
| R-07       | State transition commonality        | Implemented for v0.2 mutation routes               | `pharmacy-partnerships.ts` transition helpers; patient-share consent/link/revoke/activate and visit/record/report/claim routes | Broader legacy-wide state-machine modeling remains future hardening     | Patient-share case, visit request, and partner record rules centralized | P1       |
...
| 2   | Phase 1 equivalent functions implemented                 | Mostly complete                                         | Models/APIs/UI listed above; `Plans.md` v0.2 track                                                              | Direct DB proof pending                                           |
| 3   | Major flows work                                         | Route-mocked complete                                   | `tools/tests/ui-route-mocked-smoke.spec.ts`                                                                     | Direct DB-backed patient-card creation proof pending              |
...
| 12  | Tests added and passing                                  | Complete for current slices                             | focused Vitest, route tests, route-mocked Playwright, typecheck/lint/format results in `CODEX_GOAL_PROGRESS.md` | Full DB-backed E2E gate pending                                   |
| 13  | DB migration rollback policy                             | Policy documented                                       | Existing `docs/phase5-rollback-playbook.md`; v0.2 policy below                                                  | Not exercised because migrations not applied                      |
| 14  | Final report                                             | Current-state report complete                           | This document and `CODEX_GOAL_PROGRESS.md`                                                                      | Final completion report cannot mark DB/browser items complete yet |

## Pending Migration Set

Read-only `prisma migrate status` against `ph_os_e2e` reported these pending migrations:
...
No migration should be applied without explicit approval. When approval is granted, run this sequence against the target environment:
...
- Shared pharmacy-cooperation lifecycle helpers for patient-share-case, visit request, and partner visit record status transitions.
