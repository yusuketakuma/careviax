...
| Direct DB-backed browser proof | Partially complete                  | Patient-card share-case creation, consent registration, patient-link approval/acceptance, activation, and active workflow display now pass against the local e2e DB; downstream visit/billing gate remains pending |
...
| A-02       | Patient share cases                 | Implemented with local DB-backed proof             | `PatientShareCase`; `/api/patient-share-cases`; patient card create UI; workflow table; local e2e patient-card creation/activation proof                                                                                                   | Downstream visit/billing browser flow                                   | `patient-share-scope.ts`, `patient-share-access.ts`                                                          | P0       |
...
| 3   | Major flows work                                         | Route-mocked complete; patient-share DB proof complete  | `tools/tests/ui-route-mocked-smoke.spec.ts`; `tools/tests/ui-major-screens.spec.ts` patient-card creation/consent/link/activation proof            | Visit request/partner record/report/billing DB-backed flow pending |
