...
| Direct DB-backed browser proof | Partially complete                  | Patient-card share-case creation with approved management-plan evidence now passes against the local e2e DB; full workflow gate remains pending     |
...
| A-02       | Patient share cases                 | Implemented with local DB-backed proof             | `PatientShareCase`; `/api/patient-share-cases`; patient card create UI; workflow table; local e2e patient-card creation proof                                                                                                              | Full consent/link/activation browser flow                               | `patient-share-scope.ts`, `patient-share-access.ts`                                                          | P0       |
...
| 3   | Major flows work                                         | Route-mocked complete; patient-card DB proof complete   | `tools/tests/ui-route-mocked-smoke.spec.ts`; `tools/tests/ui-major-screens.spec.ts` patient-card share-case creation proof                         | Full consent/link/activation-to-billing DB-backed flow pending     |
