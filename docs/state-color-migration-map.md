...
- 状態意味を持たない区分値(GENDER / CHANNEL / VisitType / PackagingMethod / ReportType / ContactRelation / 後発・先発(is_generic) 等)。これらは色を付けない(後発バッジは neutral か枠線のみ。状態でなく分類値)。
- **KPI / サマリーの件数表示は色を付けない**(neutral 強調のみ)。件数(カウント)は単一エンティティの状態ではなく、6軸トークンも識別用カテゴリ色も適用対象外。`text-foreground`(ゼロ件は `text-muted-foreground` で明度コントラストにより弱調)で表す。件数チップの区別はラベル＋空間分離で行い、数値そのものに状態色を塗ると「赤=危険」等の偽シグナルになる。確定例: `clerk-support-content` KPI(2026-06-21, commit eed6cc63)。

### 境界事例の判定（state 軸=移行する／category=維持する）

カテゴリにも状態にも読めるバッジは、(1)ラベルが識別を担い色は識別に不要、かつ(2)値が単一エンティティの進行状態に対応する場合は **state 軸＝移行**する。

- `card-workspace.tsx` `ACTIVITY_BADGE_CLASSES`（直近の動き: 工程 / 照会 / 取込）= **state 軸（移行対象, S5）**。ラベル併記で識別済みのため emerald/blue の固有 hue は不要。`transition→info` / `inquiry→confirm`（cycle の `inquiry_pending=confirm` と整合）/ `intake→neutral`(既に neutral)。麻薬/冷所の行ハイライトは引き続き臨床ハザードとして維持（下記）。
...
- **臨床ハザードパレット**: `safety-board.tsx`（麻薬=赤 / 冷所=teal / 一包化=青 / 注意=橙）、`card-workspace.tsx` の麻薬/冷所 行ハイライト、`prescription-history-content.tsx` の薬剤安全標識バッジ（麻薬 / 向精神 / ハイリスク / LASA / Tall Man name）。teal が7軸に無く、安全標識の臨床区別を最優先するため固有色維持。将来 `--hazard-*` 体系を定義する際にこれらを集約する（本移行スコープ外）。
- **カテゴリ型パレット**（distinct category = distinct hue。chart 系列と同種の扱い）:
  ...
  - 患者ステータスアイコン `status-icon.ts STATUS_ICON_CONFIG`（安定/新規/初回予定/要確認/要対応/訪問遅延/報告未提出/処方変更/入院/退院/連絡不通/休止の12値）— 各固有 hue + 専用 Lucide アイコン。6軸へ畳むと別オペレーション状態が同色化し、密な患者一覧での一覧性が退行するため固有色維持。
  - ガント区分 `schedule-team-board BLOCK_KIND_CLASSES`（visit/desk/prep/travel/break/idle）・取込カテゴリ `capture-content CaptureCategoryTone`（violet/emerald、枠線+aria-pressed 併用）— 区分カテゴリ。
  - 協働者プレゼンス色 `presence-contract`（blue/emerald/violet/amber/rose のローテ）— ユーザー識別色（chart 系列同等）。
  - カレンダー祝日/平日 `business-holidays-content` / `shifts-content`（祝日=赤/rose 行ハイライト）— 暦区分。
- **臨床グラデーションスケール**: `soap-options.ts`（服薬遵守 1-5 段階 red→green、服薬状況 full/partial/missed/refusal、副作用 mild/moderate/severe）— 程度を表す連続スケールで、6軸 state へ畳むと勾配の意味が失われる（chart/heatmap 同種）。
- **装飾/外部閲覧**: `realtime/page`（admin ダーク telemetry）・`shared-viewer-content`（外部閲覧の情報カード）— state 意味のない装飾。
- **現在地（now-marker）= info(青)** に統一済（`dashboard-cockpit` / `schedule-team-board` 両方）。

## 消費者移行フェーズ 完了ログ（2026-06-20, Claude）

基盤フェーズ後に残っていた「6軸と競合する状態色ドリフト」を 5 スライスで中央トークン（`STATUS_TOKENS` / `*_ROLE`）へ統一。各スライスは LOCK→検証→commit を独立実施（typecheck/lint/対象 unit green）。

- `tasks-content`（重複系 `badge-semantics.ts` 削除 + `TASK_STATUS_ROLE`/`PRIORITY_ROLE`）— `d7c1b7d5`
- `handoff-workspace.helpers` `buildStatusBadge`（lifecycle → STATUS_TOKENS、completed 灰→done緑）— `7e183d81`
- `prescription.shared` `CYCLE_STATUS_CONFIG`（`MEDICATION_CYCLE_STATUS_ROLE` 駆動、16値フル対応）— `f5793fe4`
- `set-workspace-shared` `SET_ROW_STATUS_PRESENTATIONS`（行ティント=`bg-state-*/低透過`）— `0dfede25`
- `schedule day-view`（status/priority/lock/offline、`PRIORITY_ROLE`/`VISIT_PROPOSAL_STATUS_ROLE` 準拠、patient_contact_pending=waiting と ready=info を分離）— `0df5dd1e`

以降、生パレット残存ファイルは全て上記「意図的に raw のまま残す」カテゴリ/臨床/印刷/装飾に該当。状態色ドリフトは残ゼロ。
