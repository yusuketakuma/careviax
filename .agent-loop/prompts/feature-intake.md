...
2. **gbrain search** for prior art and past decisions on this feature area.
   - **STATUS: gbrain MCP is NOT connected (Phase 3 pending).** Record "no recall available — gbrain not connected; run gstack `setup-gbrain`." Substitute by reading the repo (`src/features/**`, `src/components/**`) and `docs/` for existing prior art.
...
8. **Gates** green (Q5).
9. **Writeback** (Q6) — stage outcome; gbrain writeback is **pending** until Phase 3.
