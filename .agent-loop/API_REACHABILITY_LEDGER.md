...
| E3-3 | /api/admin/flush-metrics ↔ /api/jobs/flush-metrics                  | **解決（2026-07-02 Codex 実装）**: 認証境界は admin=`withAuthContext(canAdmin)`、jobs=`requireApiKeyOrAuthContext(JOB_API_KEY or canAdmin)` のまま維持し、フラッシュ実行・redacted failure logging・500 envelope を `src/server/services/flush-metrics-job.ts` に正準化。成功 body と失敗文言/log event は route 境界で維持。                                                                                                                                                                                                                  |
...
## 2026-07-11 patient workspace revalidation

This section is a non-normative FE/BE inventory update. `Plans.md` remains the execution queue and
`ops/refactor/STATE.md` remains the only progress ledger.

| Feature / task                                   | Frontend                                                                                                                                                                                         | Backend / contract                                                                                                                                                                        | Classification        | Missing side / remaining action                                                                                                                                                                                                               | Evidence                                                                                                                                                                                                                                                |
| ------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Patient board / `FE-PATIENT-LIST-001`            | `/patients` uses `fetchPatientBoard`, signed continuation cursors, merged pages, desktop selected preview, and a mobile bottom sheet.                                                            | `GET /api/patients/board` returns the shared `PatientBoardPageResponse`, validates filter-bound cursors, enforces auth/no-store, and supplies the existing card DTO used by the preview.  | `ALIGNED` / `PARTIAL` | No missing FE or BE side for the selected preview. The main route still derives exact safety-priority ordering from a broad deep-relation read; keep `PERF-DB-PATIENT-BOARD-CURSOR` separate and do not pre-limit before priority derivation. | `src/app/(dashboard)/patients/patients-board.tsx`, `src/app/api/patients/board/route.ts`, `src/types/patient-board.ts`, `tools/tests/ui-patient-board-preview-route-mocked.spec.ts`                                                                     |
| Patient detail Command / `FE-PATIENT-DETAIL-001` | `CardWorkspace` renders patient identity, next action, blockers, evidence, timeline, case-risk task sync, and reason-required waiver controls with loading/error/empty states.                   | Case-risk cockpit, task sync, and task resolution use existing case-scoped endpoints; selected waiver reason and free-text reason flow through the existing audited mutation contract.    | `ALIGNED` / `PARTIAL` | No missing API, persistence, audit, or UI side in the inspected Command island. Broader responsive, keyboard/zoom, state-matrix, and other patient-detail islands remain under the parent task.                                               | `src/app/(dashboard)/patients/[id]/card-workspace.tsx`, `src/app/api/cases/[id]/risk-cockpit/route.ts`, `src/app/api/cases/[id]/risk-cockpit/tasks/route.ts`, `src/app/api/cases/[id]/risk-cockpit/tasks/[taskId]/resolution/route.ts`                  |
| Schedule day board / `FE-SCHEDULE-001`           | `/schedules` reads the typed day board, renders staff routes and pending proposals, and sends bounded inline visit status changes from the shared Select with persistent exact-payload recovery. | `GET /api/visit-schedules/day-board` supplies `ScheduleDayBoardResponse`; `PATCH /api/visit-schedules/[id]` validates the expected status and retains org/assignment/auth/audit controls. | `ALIGNED` / `PARTIAL` | No missing FE or BE side for the inspected day-board status path. Proposal-state coverage, populated responsive runtime evidence, keyboard/zoom/forced-colors, and the broader proposal rail remain under the parent task.                    | `src/app/(dashboard)/schedules/schedule-team-board.tsx`, `src/app/(dashboard)/schedules/schedule-team-board.test.tsx`, `src/app/api/visit-schedules/day-board/route.ts`, `src/app/api/visit-schedules/[id]/route.ts`, `src/types/schedule-day-board.ts` |

Runtime revalidation separated development compilation from application latency: the first authenticated
patient-board request completed in 6.1s with 5.4s attributed to Next compilation and 681ms to application code;
subsequent warm requests completed in 375ms and 47ms. This does not close the query-shape task, but it supersedes the
earlier assumption that every request remains above 20 seconds.
