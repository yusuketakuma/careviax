...
| 3    | Shared typography/state primitives and shell convergence                    | Active. Shared shell minimum typography (`3a3c61cb2`), prescription list/detail states (`08023627a`) and patient safety header readability (`4bb4f5342`) landed with focused tests. Continue family-level burn-down.                                                                             |
...
| **T2** | 12px未満タイポ(`text-[9px]`/`[10px]`/`[11px]`) | shared shell + prescription list + patient header 後も live 43 files / 122 occurrences（2026-07-10）                                        | **In progress.** shared shell/chrome と patient safety header は `text-xs` 以上へ収束し静的 ratchet を追加。prescription list/workspace/inline detail も 12px minimum と shape-matched loading/retryable error へ更新済み。残りは patient history / visit / schedule / prescription intake / print 等を family 単位で、バッジ寸法・密度・折返しを同時検証して burn-down する。全件解消後は repo-wide typography gate に拡張する。                                                                                      | Med  |
