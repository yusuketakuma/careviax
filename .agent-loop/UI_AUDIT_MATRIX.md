...
| 3    | Shared typography/state primitives and shell convergence                    | Active. Shared shell minimum typography (`3a3c61cb2`), prescription list/detail states (`08023627a`), patient safety header readability (`4bb4f5342`), patient prescription/history independent states (`2e6e95fca`), schedule team-board independent states (`77d8b3533`), schedule calendar main/preview state separation, prescription-intake patient/QR/case/institution state parity, visit-brief stale/error/feedback/access parity, shared offline conflict keep/discard confirmation with exact-version API proof, and visit-medication preparation loading/error/stale/malformed-success parity landed with focused frontend + paired API tests. Continue family-level burn-down. |
...
| **T2** | 12px未満タイポ(`text-[9px]`/`[10px]`/`[11px]`) | visit medication management 後は live 35 files / 61 occurrences（2026-07-10）                                                               | **In progress.** shared shell/chrome、patient safety header、prescription list/workspace/inline detail、patient prescription/history、schedule team-board/calendar、prescription intake、visit brief、visit medication management を `text-xs` 以上へ収束し、family-level static ratchet を追加。visit medication では source summary/count 5 occurrences を解消し、source action 44px、checkbox label target、h3/h4、preparation loading/error/stale/malformed-success と guarded schedule path を frontend/backend tests で固定した。残りは visit reflected/voice/report/switcher、schedule offline/day sections、print 等を family 単位で burn-down する。全件解消後は repo-wide typography gate に拡張する。 | Med  |
