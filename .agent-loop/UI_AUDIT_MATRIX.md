...
| **T2** | 12px未満タイポ(`text-[9px]`/`[10px]`/`[11px]`) | patient history family 後は live 40 files / 100 occurrences（2026-07-10）                                                                   | **In progress.** shared shell/chrome、patient safety header、prescription list/workspace/inline detail、patient prescription/history を `text-xs` 以上へ収束し、family-level static ratchet を追加。patient history では 22 occurrences を解消し、バッジ寸法・44px filter/retry・折返し・partial/stale state も同時に検証した。残りは visit / schedule / prescription intake / print 等を family 単位で burn-down する。全件解消後は repo-wide typography gate に拡張する。                                            | Med  |
