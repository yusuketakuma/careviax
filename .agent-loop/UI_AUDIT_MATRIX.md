...
| 3    | Shared typography/state primitives and shell convergence                    | Active. Shared shell minimum typography (`3a3c61cb2`), prescription list/detail states (`08023627a`), patient safety header readability (`4bb4f5342`), patient prescription/history independent states (`2e6e95fca`), schedule team-board independent states (`77d8b3533`), and schedule calendar main/preview state separation landed with focused frontend + paired API tests. Continue family-level burn-down. |
...
| **T2** | 12px未満タイポ(`text-[9px]`/`[10px]`/`[11px]`) | schedule calendar family 後は live 38 files / 82 occurrences（2026-07-10）                                                                  | **In progress.** shared shell/chrome、patient safety header、prescription list/workspace/inline detail、patient prescription/history、schedule team-board/calendar を `text-xs` 以上へ収束し、family-level static ratchet を追加。calendar では status/safety/billing/overflow 5 occurrences を解消し、7列密度、44px month/detail controls、stale cache、安全な path helper も同時に検証した。残りは visit / schedule offline/day sections / prescription intake / print 等を family 単位で burn-down する。全件解消後は repo-wide typography gate に拡張する。 | Med  |
