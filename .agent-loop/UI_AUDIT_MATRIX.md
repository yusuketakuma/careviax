...
| 3    | Shared typography/state primitives and shell convergence                    | Active. Shared shell minimum typography (`3a3c61cb2`), prescription list/detail states (`08023627a`), patient safety header readability (`4bb4f5342`), patient prescription/history independent states (`2e6e95fca`), and schedule team-board independent initial/stale states landed with focused frontend + paired API tests. Continue family-level burn-down. |
...
| **T2** | 12px未満タイポ(`text-[9px]`/`[10px]`/`[11px]`) | schedule team-board family 後は live 39 files / 87 occurrences（2026-07-10）                                                                | **In progress.** shared shell/chrome、patient safety header、prescription list/workspace/inline detail、patient prescription/history、schedule team-board を `text-xs` 以上へ収束し、family-level static ratchet を追加。team-board では 13 occurrences を解消し、ガント・安全タグ・車両・受信調整の密度、stale cache、44px retry も同時に検証した。残りは visit / schedule calendar/offline / prescription intake / print 等を family 単位で burn-down する。全件解消後は repo-wide typography gate に拡張する。      | Med  |
