...
| **T2** | 12px未満タイポ(`text-[9px]`/`[10px]`/`[11px]`) | shared shell 是正後も live 47 files / 159 occurrences（2026-07-10）                                                                         | **In progress.** app header / sidebar / mobile nav / notification / offline / search / collaboration / workflow / action rail / help は `text-xs` 以上へ収束し静的回帰testを追加。残りは prescription / patient / visit / schedule / print 等を family 単位で、バッジ寸法・密度・折返しを同時検証して burn-down する。全件解消後は repo-wide typography gate に拡張する。                                                                                                                                              | Med  |
