...
| 3    | Shared typography/state primitives and shell convergence                    | Active. Shared shell minimum typography (`3a3c61cb2`), prescription list/detail states (`08023627a`), patient safety header readability (`4bb4f5342`), patient prescription/history independent states (`2e6e95fca`), schedule team-board independent states (`77d8b3533`), schedule calendar main/preview state separation, and prescription-intake patient/QR/case/institution state parity landed with focused frontend + paired API tests. Continue family-level burn-down. |
...
| **T2** | 12px未満タイポ(`text-[9px]`/`[10px]`/`[11px]`) | prescription intake family 後は live 37 files / 73 occurrences（2026-07-10）                                                                | **In progress.** shared shell/chrome、patient safety header、prescription list/workspace/inline detail、patient prescription/history、schedule team-board/calendar、prescription intake を `text-xs` 以上へ収束し、family-level static ratchet を追加。intake では price/YJ/safety/diff 9 occurrences を解消し、全対話要素の desktop 44px 縮小指定、mobile 2列強制、QR/患者/ケース/医療機関 state、動的 path helper も同時に検証した。残りは visit / schedule offline/day sections / print 等を family 単位で burn-down する。全件解消後は repo-wide typography gate に拡張する。 | Med  |
