...
- **icon-button-aria-labels**（owner: claude-lead, est S）: アイコンのみボタンに accessible name 欠落。`medication-calendar-content.tsx:223,232`（月送り）/ `patients/[id]/share/external-share-content.tsx:593,613`（コピー）/ `admin/shifts/shifts-content.tsx:943,952`（月送り）。正例: `business-holidays-content.tsx:312,316`, `billing-rules/page.tsx:204,214`。why: スクリーンリーダーで名前なし＝操作不能、bounded。
- **dispense-workbench-select-migration**（owner: claude-lead, est S）: 調剤ワークベンチ core の native select 2件 → 共有 Select。`components/features/dispense-workbench/right-pane.tsx:874`, `prescription-grid.tsx:510`。why: 最頻用の臨床調剤 UI を shared Select 標準へ整合。
