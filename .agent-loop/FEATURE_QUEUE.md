...
| FEUX-6 | T3   | 生 Tailwind 状態色 ~102箇所/56ファイル。**lint ガード追加 + 漸進 sweep**（単発 PR でなく）。正当 identity と drift を切り分け                                                                                        | claude | subagent | **in-progress（唯一の実質未完）**。sweep は ~102→~35 削減済み、うち identity 3ファイル(soap-options/presence-contract/status-icon 計25件)は SSOT §2 で意図的除外確定。**残: (1) lint ガード未追加＝中核, (2) drift ~13件/4ファイル(settings-catalog/result-builders/patient-field-revision/document-delivery-rule-manager)の token 化（一部は状態 vs 識別の切り分け判断要）**                               |
