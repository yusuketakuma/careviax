...
| R1    | codex        | claude   | planner/generate の休業日判定を S1 util へ単一化（§12.1、`buildOperatingCalendarLegacy` で behavior-preserving、特性テスト先行） | なし（S1 後）    | **queued（unblocked）** S1 approve 済、codex レーンで着手可。着手時 LOCK visit-schedule-planner.ts/generate route                                                           |
| S2    | codex        | claude   | `PharmacyOperatingHours` モデル + `BusinessHoliday` partial unique（§2.4）+ 既存 duplicate 解消 + migration + seed               | migration 承認済 | **queued（approved 2026-06-27）** ユーザー承認済（BLOCKED.md OPDAY-S2 RESOLVED）。条件: 追加のみ非破壊 / 本番 RDS 適用は別 prod_deploy ゲート / Codex maker・Claude checker |
