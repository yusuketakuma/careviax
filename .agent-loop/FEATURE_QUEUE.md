...
- [ ] **inventory-forecast-patient-runout-urgency**（owner: 未割当, est M, backend+analytics+FE）: `/admin/inventory-forecast` の影響患者カードに『切れ予定日/緊急度バッジ』を出す P2 #2 は、現 `AffectedPatientCard`（{ key, label, firstVisitDateKey, isFacilityBatch } のみ）に患者×薬の不足関係・run-out 日・緊急度が無いため FE 単独では実装不可（31b407b8 では訪問日チップを『訪問予定 M/D』明示に留め捏造回避）。`src/lib/analytics/inventory-forecast.ts` の `AffectedPatientCard` に患者別不足明細（関連 drugKey(s) / 推定 run-out or coverage 基準 / サーバ導出 urgency role）を追加し `/api/admin/inventory-forecast` で供給→FE が真の run-out/緊急度(StateBadge/StatusDot)を描画。状態色は 3省2 規約（要発注=blocked/発注候補=confirm/中立）と整合。evidence: claude consult＋Codex CONSULT_REPLY(2026-06-27)、`inventory-forecast-content.tsx` 影響患者セクション。
...
| ----- | ------------ | -------- | -------------------------------------------------------------------------------------------------------------------------------- | ------------------ | ------------------------------------------------------------------------ |
| S1    | claude       | codex    | 営業日計算 pure util `src/lib/calendar/operating-day.ts`（resolver+§1.1 precedence / nearest/add / legacy adapter）+ tests       | なし               | **patch_ready** c66daca3（vitest 28 + 全 gate green、codex review 待ち） |
| R1    | codex        | claude   | planner/generate の休業日判定を S1 util へ単一化（§12.1、`buildOperatingCalendarLegacy` で behavior-preserving、特性テスト先行） | なし（S1 後）      | queued（S1 approve 待ち、S6 の前提）                                     |
| S2    | codex        | claude   | `PharmacyOperatingHours` モデル + `BusinessHoliday` partial unique（§2.4）+ 既存 duplicate 解消 + migration + seed               | **migration 承認** | blocked（human gate、§6・BLOCKED.md）                                    |
| S3    | codex        | claude   | `/api/pharmacy-operating-hours` GET/PUT + resolved read model（§4.4 org-wide OR）+ @db.Time↔HH:mm adapter + no-store + tests     | なし（S2 後）      | queued                                                                   |
| S4    | claude       | codex    | 稼働日カレンダー UI（§13.1 月グリッド）+ 設定ハブ（§13.2、business-holidays 統合）                                               | なし（S3 後）      | queued                                                                   |
