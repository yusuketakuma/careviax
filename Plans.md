...
- 未実装として残すもの: `PERF-DB-006D` care-report index gate、`PAYLOAD-BUDGET-003`、`API-CONTRACT-*`、`DB-EVENT-001`、`FILE-LIFE-001`、`TENANT-SUPPORT-001`、7画面UIの実装slice、live AWS human gate。
...
- `care-reports` は `view=palette&q=` と通常list `q` 患者検索が bounded。`keyword` は cursor を拒否し、`CARE_REPORT_KEYWORD_SCAN_LIMIT=500` + plus-one read で overflow を検知し `search.count_basis='bounded_keyword_scan'` を返す。delivery summary は通常listでは既読page rows由来の `basis='page'`、keywordでは `basis='bounded_keyword_scan_result'` として全検索where aggregateを避ける。残るDB速度課題は EXPLAIN-backed index gate と payload budget である。
...
1. `PERF-DB-006D` index候補を SELECT-only EXPLAIN-backed backlog へ分離。
...
- `PERF-DB-006C-DELIVERY`（実装済み）: regular list rows は delivery summary を `basis='page'` として既取得page rowsから生成し、全検索whereに対する `DeliveryRecord` count/groupBy/findMany を走らせない。summaryを消してUI偽装せず、basisで意味を明示する。
