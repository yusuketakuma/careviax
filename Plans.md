...
| `PERF-DB-001`            | 一部実装済 | inbound-only queue source gating、report workspace inbound evidence direct count、dashboard stock signal count consolidation、patient detail root scoped read + timeline fan-out removal。 | patient master bounded select、movement caller-limit、care-report bounded search / index backlog。                                                        |
...
| PERF-DB-005B        | P0/P1  | DB速度/PHI    | `/api/patients/[id]`, `patient-state-snapshot`, patient overview base                                | 済: root GET は scoped transaction へ寄せ、timeline-only fan-out を削除済み。残: root/overview base の broad patient master `include` を bounded `select` + per-relation `take` へ置換する。`residences` / `contacts` / `conditions` / `cases.care_team_links` / `consents` は表示に必要な列だけを選び、上限と order を明示する。重い履歴・監査・lab・timeline は slice API / lazy tab の責務に固定する。 | route/service tests が broad include を拒否すること、RLS scoped transaction proof、payload budget、patient detail focused tests。 |
| PERF-DB-006         | P1     | DB速度/検索   | `/api/care-reports`, Prisma schema indexes                                                           | care-report list/search は patient name `contains` と keyword scan が重い。まず non-palette patient search を bounded にし、検索キー/pg_trgm/tenant-first composite index は `EXPLAIN` 後のmigration候補にする。候補: `CareReport(org_id, created_at, id)`, `CareReport(org_id, status, updated_at, id)`, `DeliveryRecord(org_id, status, updated_at, id)` 等。                                           | route tests for bounded search、SELECT-only EXPLAIN、migration rollback plan before index addition。                              |
| PERF-DB-007         | P1     | DB速度/履歴   | `patient-detail-timeline-registry`, `PatientMovementTimeline`                                        | movement timeline は多数 source を全て短く読んで最後にsliceする。初期PRでは registry rewriteを避け、callerの `timelineLimit` を `TimelineFetchCtx` に渡し、小さいlimit要求時だけ per-source take を下げる。formal phase/window query は別PRにする。                                                                                                                                                       | timeline ordering tests、source partial failure tests、movement route tests、false recency防止。                                  |
...
- `PERF-DB-005B`: root/overview base の broad patient master include を bounded select / relation take へ置換する。
- `PERF-DB-006`: care-report list/search の bounded search と EXPLAIN-backed index backlog を分離する。
- `PERF-DB-007`: movement timeline の sourceごとの短いreadを caller limit に合わせて抑える。
...
1. `PERF-DB-005B` patient master bounded select / per-relation take follow-up。
2. `PERF-DB-007` movement timeline caller-limit-aware source reads。
3. `PERF-DB-006` care-report bounded search / EXPLAIN-backed index backlog。
...
| ------------------------ | ---------- | ------ | ----------------- | ----------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| `PERF-DB-005B-TEST`      | 未実装     | P0     | DB/API tests      | `src/app/api/patients/[id]/route.test.ts`, `patient-state-snapshot.test.ts`, `patient-detail.test.ts` | 実装前に red test を追加する。root GET と `findPatientOverviewBase()` が `include` を持たず、`residences/contacts/conditions/consents/cases/care_team_links` に `select + orderBy + take` を持つことを固定する。`consents.document_url/document_file_id` と timeline/audit/lab/document body が root/overview base に戻らないことを検査する。 | focused Vitest が変更前に失敗し、実装後に通ること。既存 root API body shrink が許容できない未確認 client 依存が見つかったら停止。              |
| `PERF-DB-005B-IMPL`      | 未実装     | P0     | DB/API            | `/api/patients/[id]`, `patient-state-snapshot`, `patient-detail`                                      | patient master root/overview base を bounded select へ置換する。patient scalar は `PatientOverview` と編集defaultで使う列に限定する。relations は primary/active/emergency を落とさない deterministic order を持つ。root DTO は `...patient` による無制限列混入をやめ、互換維持対象を明示する。                                               | route/service focused tests、typecheck、payload budget smoke。RLS scoped transaction を外す必要が出たら停止。                                  |
| `PERF-DB-007-LIMIT`      | 未実装     | P1     | DB/API perf       | `patient-detail-timeline-registry`, `PatientMovementTimeline`, movement timeline routes               | movement timeline の caller `limit` を registry context へ渡し、小さい初期表示では各sourceの `take` を抑える。formal phase/window query rewrite は別PRにする。source partial failure と ordering は維持する。                                                                                                                                 | timeline ordering tests、partial failure tests、false recency防止。sourceごとの正しさを落とす場合は停止。                                      |
| `PERF-DB-006-SEARCH`     | 未実装     | P1     | DB/API search     | care-report list/search、patient search、既存 indexes                                                 | care-report list/search の bounded search を先に入れる。pg_trgm / composite index は SELECT-only `EXPLAIN` と rollback plan 後の候補に留める。                                                                                                                                                                                                | route tests、payload budget、SELECT-only EXPLAIN。blind index 追加や migration適用が必要になったら human gate。                                |
