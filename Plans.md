...
### 2026-07-08 Plan Status Registry — 実装済み / 未実装整理 `cc:ACTIVE`
...
| Medication Stock    | `STOCK-001-PRESCRIPTION` + `STOCK-001-VISIT`                                         | 処方供給adapterと訪問観測API/UIを別PRで実装。YJ/HOT/GS1(GTIN/JAN)/一般名/規格/剤形/メーカー候補、低confidence薬剤師review、append-only event、snapshot再計算を守る。                                                                                            | adapter unit/API tests、idempotency、unit conversion tests、mobile observation tests、stockout forecast tests。       | 単位変換不能な供給を自動加算する設計、または他職種signalをreviewなしでledgerへ直書きする設計。                               |
| Patient Movement    | `MOV-001-API`                                                                        | standalone movement API、map-less date card UX、formal inbound/stock/safety source、処方/訪問/文書 occurrence marker + deep link を整える。                                                                                                                     | movement API tests、relative href tests、raw omission snapshot、mobile/a11y tests。                                   | 一覧にSOAP本文、OCR全文、raw chat text、薬剤明細全文を出す必要が出たら停止。                                                 |
...
| Recovery / AWS      | `OPS-RECOVERY-EVIDENCE-001` → `OPS-RECOVERY-INTEGRITY-001` → `OPS-RECOVERY-LIVE-001` | Codexで実装可能な前段として、復旧証跡appendのredaction/構造化、復元済みDBの SELECT-only 業務整合 audit CLI、S3 Object Lock / strict skipped-check monitorを追加する。その後、human gate として live AWS strict validation / restore drill evidence を収集する。 | evidence redaction tests、integrity audit tests、PHI-free output snapshot、`backup:drill:check`、template validator。 | production DBへ誤接続する設計、runtime restore API、AWS destructive permission、PHI/ARN/endpoint/storage keyを出力する設計。 |
...
- `QUERY-SHAPE-WATCHLIST-003B`: watchlist追加前に修正が必要なBFFを分ける。`patients/board` は main patient reads を DB側 `take: limit + 1` / cursor / stable `orderBy` に寄せ、rail reads に `id` tie-breaker を追加する。`visit-schedules/day-board` は hidden proposal `skip` scan、task aggregate fan-out、staff/shift/vehicle master reads、schedule/proposal order を整理してから追加する。
...
- `OPS-RECOVERY-EVIDENCE-001`: `backup-recovery-check.ts` の `--append` が free text をそのまま永続化しないよう、ARN、account id、signed URL、token、db password、security group/subnet id、raw S3 key、患者名/電話らしき値を reject/redact する。live drill evidence は `environment`、ticket、approver、started/completed、RTO/RPO、health status、redaction check、sample counts などの構造化fieldへ寄せる。
- `OPS-RECOVERY-INTEGRITY-001`: live restore drill の前段として、復元済みDBまたはstaging/local DBへ SELECT-only で業務整合を検査する CLI を追加する。患者・訪問・報告・請求/タスク・添付・監査ログの count / orphan / latest timestamp / link integrity を PHI-free に出力し、`DATABASE_URL` が production-like の場合は `--allow-production` なしで拒否する。
- `OPS-RECOVERY-MONITOR-003`: S3 Object Lock の read-only monitor と production strict mode を追加する。Object Lock は `enabled`、mode、default retention だけを返し、bucket名/ARN/keyは出さない。production/admin evidence では backup-specific env 不足や critical skipped check を `ok` 扱いにしない。
- `OPS-RECOVERY-DOC-001`: `docs/compliance/backup-recovery-drill.md` を復旧SSOTにし、root `docs/backup-recovery-drill.md` は historical/reference と明記するか、AdministratorAccess、production object key、migration/deletion examples を least-privilege / non-destructive / synthetic-drill-only 手順へ整合させる。
...
| Medication Stock    | `STOCK-001-PRESCRIPTION` と `STOCK-001-VISIT` は別PR。処方供給adapterを先に、訪問観測UI/APIを次にする。                  | schema、append-only event、snapshot、accepted inbound signal apply、summary API。                  | YJ/HOT/GS1 matching、unit conversion、idempotency、low-confidence review、stockout forecast、mobile input tests。      |
...
| Recovery evidence   | `OPS-RECOVERY-EVIDENCE-001` は Codex実装可能な証跡hardening。live復元自体はしない。                                      | `backup-recovery-check.ts`、`external-readiness.ts`、drill docs。                                  | structured append fields、unsafe evidence reject/redact、live/tabletop誤認防止、PHI-free summary。                     |
| Recovery integrity  | `OPS-RECOVERY-INTEGRITY-001` は Codex実装可能な復元後検証補助。live復元自体はしない。                                    | backup drill docs、Prisma schema、read-only monitoring方針。                                       | SELECT-only CLI、PHI-free JSON/Markdown、production-like URL guard、integrity fixtures。                               |
| Recovery monitor    | `OPS-RECOVERY-MONITOR-003` は Codex実装可能なread-only監視拡張。restore/deleteはしない。                                 | backup monitor、health sanitizer、S3/Object Lock方針。                                             | Object Lock read-only check、strict skipped-check behavior、infra identifier redaction tests。                         |
...
- `Medication Stock Ledger` の長文仕様は現行 schema/domain 実装済み部分を再タスク化せず、処方供給、訪問観測、equivalence review、downstream connection だけを active に残す。
...
| `OPS-RECOVERY-002`       | 一部実装済 | AWS Backup vault / recovery point / RDS backup settings の read-only health monitor、sanitizer、IAM forbidden-action validator。                                                                                                                                                                                                                                                                                                                              | 本番相当roleの `--live-aws --strict`、admin health実AWS確認、live restore drill evidence。runtime restore APIは作らない。                                 |
...
| `PERF-DB-001`            | 一部実装済 | inbound-only queue source gating、report workspace inbound evidence direct count、dashboard stock signal count consolidation、patient detail root scoped read + timeline fan-out removal、patient master bounded select、movement timeline caller-limit-aware source reads、dashboard / patient timeline / inbound / stock measured payload budgets、payload budget matrix smoke、care-report SELECT-only EXPLAIN capture tool、query-shape watchlist guard。 | care-report EXPLAIN artifact取得 / index migration human gate、query-shape watchlist拡張、環境別 perf-smoke 実行証跡。                                    |
...
| `RELEASE/OPS`            | 一部実装済 | pilot readiness、performance utilities、backup monitor、route/auth guardrails の基礎。                                                                                                                                                                                                                                                                                                                                                                        | live AWS drift gate、CloudWatch alarm baseline、restore drill evidence、production metrics persistence。                                                  |
...
| Stock      | Medication Stock Ledger schema、append-only event、患者別summary API、inbound signal apply初期版、stockout/equivalence helper。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | `9a62e12c2`, `52b26755c`, `c5c2ce7cb`, `46f2df1a8`, `src/modules/pharmacy/medication-stock/**`。                                                                                                                                                                                                                                                                                                                                                                                                                                           | 残は処方供給、訪問観測、usage/refill、UI、VisitBrief/Schedule。                    |
| Movement   | `PatientMovementTimeline`、共通DTO、safe detail resolver API、処方/訪問/文書の occurrence-only 表示土台、`/api/patients/:id/timeline` payload budget / measured response。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | `1ae268009`, `src/app/(dashboard)/patients/[id]/patient-movement-timeline.tsx`, `src/types/patient-movement-timeline.ts`, `src/app/api/patients/[id]/timeline/[eventId]`, `src/app/api/patients/[id]/timeline/route.ts`, `src/lib/utils/route-payload-budgets.ts`。                                                                                                                                                                                                                                                                        | 残は standalone movement-timeline API、日付UI仕上げ、全source parity。             |
| AWS Backup | RDS/AWS Backup baseline、recovery point monitor、Restore Testing metadata、IaC validation、read-only assurance health hardening。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | `9a62e12c2`, `0d2f025a4`, `3ccc8b8e2`, `src/server/services/backup-monitor.ts`, `src/app/api/health/route.ts`。                                                                                                                                                                                                                                                                                                                                                                                                                            | 残は live AWS strict validation と live drill evidence。                           |
...
| MOV-STATUS-001             | P0/P1  | Movement       | movement tab、timeline DTO、safe resolver                                                            | `PatientMovementTimeline` 土台は再実装しない。残は standalone API、map-less date card UX、formal inbound/stock/safety sources、relative href builder、raw detail再認可。処方・訪問・文書は occurrence marker + deep link のまま維持する。                                                                                                                                                                                                                                                                                                                                     | movement API tests、raw omission snapshot、relative href tests、mobile/a11y tests。                                                                                     |
...
| OPS-RECOVERY-EVIDENCE-001  | P0/P1  | AWS復旧証跡    | `backup-recovery-check.ts`, `external-readiness.ts`, recovery docs                                   | 復旧drill evidence は operator free text ではなく構造化fieldへ寄せる。`--append` は ARN、account id、signed URL、token/password、DB endpoint、security group/subnet id、raw S3 key、患者名/電話らしき値を reject/redact し、Markdown table へ安全な summary だけを残す。`external-readiness` は tabletop-only / placeholder を live evidence と誤認せず、RTO/RPO、health status、redaction check、evidence completeness を safe derived fields として返す。                                                                                                                   | script unit tests、external-readiness tests、PHI/secret/infra identifier omission snapshot。raw notes をそのまま永続化する必要が出たら停止。                            |
| OPS-RECOVERY-INTEGRITY-001 | P0/P1  | AWS復旧/DB整合 | backup drill docs、Prisma schema、read-only recovery policy                                          | live restore drill の前に、復元済みDBへ対して SELECT-only で業務整合を検査するCLIを追加する。対象は患者、訪問予定/記録、報告書/送付、請求候補/タスク、FileAsset、AuditLog、Inbound/MedicationStock の主要link/count/latest timestamp。出力はcount/status/timestamp/hash程度に限定し、患者名、薬剤名、住所、電話、free text、storage key、ARN、endpoint、raw provider errorを出さない。`DATABASE_URL` が production-like の場合は `--allow-production` なしで停止する。                                                                                                        | `backup:drill:integrity` script、fixture tests、PHI-free output snapshot、production URL guard。DB mutation / restore API / AWS destructive call が必要になったら停止。 |
| OPS-RECOVERY-MONITOR-003   | P1     | AWS復旧監視    | `backup-monitor.ts`, health route, S3 compliance docs                                                | S3 Object Lock configuration を read-only で確認し、admin health には enabled/mode/default retention の safe fields だけを返す。production/strict evidence mode では backup vault/RDS/S3/audit archive/Cognito など critical check が設定不足で skip された場合に `ok` と誤認しない。snapshot id、bucket名、vault名、user pool id は platform infra capability ができるまでhash/omitする。                                                                                                                                                                                    | backup-monitor tests、health redaction tests、strict-mode tests。restore/delete/put-secret/pass-role が必要になったら停止。                                             |
| OPS-RECOVERY-DOC-001       | P1     | AWS復旧文書    | `docs/compliance/backup-recovery-drill.md`, `docs/backup-recovery-drill.md`                          | compliance doc を復旧SSOTにし、root runbook は historical/reference と明記するか、least-privilege role、dedicated recovery VPC/subnet/security group、synthetic/drill-only S3 prefix、SELECT-only integrity before migration、explicit ticket/approver へ整合させる。AdministratorAccess、production object key、delete/copy-object、unapproved migration を標準手順として残さない。                                                                                                                                                                                          | docs lint or rg check、review diff。緊急時break-glass権限やPHI sample方針が未決なら human policy gate。                                                                 |
...
| `DASH-P1-010-RAIL`           | 未実装     | P1     | Dashboard UI            | `DashboardCockpit`, `useDashboardCockpitViewModel`, existing segments                    | 左サマリーレールを追加する。初期は既存 summary/details/team/inbound/stock/report-billing から合成し、専用 rail BFF は必要になった時だけ追加する。狭幅では上部横スクロール card へ変形する。                                                                                                                                                                                                                                                                                                                       | dashboard focused component tests、link tests、mobile snapshot。summary/detailsの重複fetchを戻す設計なら停止。                                            |
| `DASH-P1-005-LINKS`          | 未実装     | P1     | Dashboard routing       | `ProcessNowSection`, route builders                                                      | 工程 tile と hidden queue / carryover を clickable にする。href は相対URLのみ、source別 filter を持つ。件数0も空一覧へ遷移できる。                                                                                                                                                                                                                                                                                                                                                                                | href allowlist unit tests、relative URL tests、dashboard UI tests。external/signed/storage URL を action href に入れる必要が出たら停止。                  |
...
| `STOCK-001-PRESCRIPTION`     | 未実装     | P1     | Pharmacy stock          | `src/modules/pharmacy/medication-stock`, DrugMaster/DrugPackage, prescription intake     | 処方供給 adapter を追加する。処方明細から stock item を探し、YJ/HOT/GS1(GTIN/JAN)/一般名/規格/剤形/メーカーで候補化する。低confidenceは薬剤師review taskへ回し、自動統合しない。                                                                                                                                                                                                                                                                                                                                  | adapter unit/API tests、idempotency、equivalence review tests。単位変換不能を自動加算する必要が出たら停止。                                               |
...
| `MOV-001-API`                | 未実装     | P1     | Patient detail          | movement tab, timeline routes, safe resolver                                             | standalone movement API を整える。初期表示は map-less date card UX、formal inbound/stock/safety sources、処方・訪問・文書 occurrence marker + deep link を返す。本文、薬剤明細、SOAP、OCR、raw text は一覧に載せない。                                                                                                                                                                                                                                                                                            | movement API tests、relative href tests、raw omission snapshot、mobile/a11y tests。event detail shellを全イベントのprimaryにする設計なら停止。            |
...
| `REPORT-020`                 | 未実装     | P1     | Report workspace        | report workspace, inbound normalized summary, masking profile                            | 他職種受信情報を報告書候補化する。raw_text は自動挿入しない。薬剤師が「報告書に含める / 申し送りのみ / 内部記録のみ」を選ぶ。                                                                                                                                                                                                                                                                                                                                                                                     | report workspace tests、masking profile tests、delivery output tests。外部送付本文へraw textを自動投入する必要が出たら停止。                              |
...
| `OPS-RECOVERY-EVIDENCE-001`  | 未実装     | P0/P1  | AWS recovery evidence   | `backup-recovery-check.ts`, `external-readiness.ts`, recovery docs                       | `tools/scripts/backup-recovery-check.ts` を構造化・redaction付き証跡appendへ硬化する。`--append` は incomplete input でdocumentをmutateせず、unsafe notes は reject/redact する。`external-readiness` は placeholder/tabletop と live evidence を区別し、raw notesではなく RTO/RPO/health/redaction/evidence completeness の derived fields だけを返す。                                                                                                                                                          | `backup-recovery-check.test.ts`、`external-readiness.test.ts`、unsafe identifier omission snapshot、`pnpm backup:drill:check`。                           |
| `OPS-RECOVERY-INTEGRITY-001` | 未実装     | P0/P1  | AWS recovery / DB audit | backup drill docs、Prisma schema、read-only DB access                                    | `tools/scripts/backup-recovery-integrity-audit.ts` を追加し、復元済みDB/staging/local DBに対して SELECT-only の整合auditを行う。患者・訪問・報告・請求/タスク・添付・監査ログ・Inbound/MedicationStock の orphan/count/latest timestamp を集計し、RPO補助情報を出す。JSON/Markdown出力はPHI-free、production-like `DATABASE_URL` は `--allow-production` なしで拒否する。runtime restore API、AWS restore call、Secrets書換、migration、UPDATE/DELETE/INSERT は実装しない。                                       | unit tests with mocked Prisma reader、PHI-free snapshot、production guard test、`pnpm backup:drill:check`。DB mutation or raw PHI output が必要なら停止。 |
| `OPS-RECOVERY-MONITOR-003`   | 未実装     | P1     | AWS recovery monitor    | `backup-monitor.ts`, `/api/health`, S3/Object Lock docs                                  | S3 Object Lock read-only check と strict skipped-check behavior を追加する。admin health は infra identifiers を最小化し、snapshotId/bucket/vault/userPoolId を直接出さない。production evidence mode では backup-specific env 未設定で critical checks が skip された場合に degraded/fail にする。                                                                                                                                                                                                               | backup-monitor tests、health route redaction tests、strict missing env tests。AWS write permission が必要なら停止。                                       |
| `OPS-RECOVERY-DOC-001`       | 未実装     | P1     | AWS recovery docs       | compliance/root recovery docs                                                            | `docs/compliance/backup-recovery-drill.md` をSSOT化し、root runbook の historical化またはleast-privilege整合を行う。AdministratorAccess / AmazonRDSFullAccess / production object key / destructive S3 examples / unapproved migration command を標準手順から外す。                                                                                                                                                                                                                                               | docs diff、unsafe phrase rg check、human policy note for emergency break-glass。                                                                          |
...
| `API-CONTRACT-001`           | 未実装     | P0     | API contract            | `src/lib/api/response.ts`, response shape guardrail                                      | success/error envelope を `ApiSuccess<T>{ data, meta? }` / `ApiError{ error:{ code,message,field_errors?,request_id? } }` へ統一する。`compatibilityError` は internal-only へ退避し、public route allowlist を減らす。                                                                                                                                                                                                                                                                                           | route snapshot tests、frontend reader tests、`api-response-shape:check` expectedCount減。既存routeを無根拠に一括変更する場合は停止。                      |
...
| RX-002   | 残薬/頓服/外用 | Medication Stock Ledger / Stock Risk           | `modules/pharmacy/medication-stock`, `visit-records`, patient board                                | 済: DB schema / RLS / index、患者別 stock summary API、accepted inbound medication-stock signal から append-only `MedicationStockEvent` への初期反映、snapshot再計算、review task close。残: 処方供給、訪問時観測、usage-delta/frequency/refill request、UI、VisitBrief/Schedule接続、stock risk provider の完全統合。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
>
> 残: 正式 review detail shell、raw_text 再認可 UI、FAX/メール/手入力 source、VisitBrief/Schedule/Report 連動、usage-delta/frequency/refill request の MedicationStock 反映。raw text は通知・監査・共有・timeline一覧・queue item・report workspace・case risk に直接出さない。
...
- `PatientDetailTab` に `movement` があり、`#patient-movement` / `#patient-timeline` / `#inbound-communications` / `#inbound-signals` / `#medication-stock-events` は movement へ解決される。
...
>
> 2026-07-08 整理: schema / RLS / index、domain helper、患者別 summary API、accepted inbound signal -> append-only `MedicationStockEvent` -> snapshot 再計算 -> review task close は実装済み。再実装しない。
> 残: 処方供給連動、訪問時観測API/UI、usage-delta / frequency / refill request、equivalence review UI、既存 `ResidualMedication` からの段階移行、VisitBrief/Schedule/Report/External Share 接続、正式 `RiskFindingProvider` の完全統合。
...
| `MedicationStockEvent`               | 実装済み | 残数変動・観測の append-only 台帳。削除ではなく補正 event で訂正する。                                 | `prescription_supply`、`visit_observation`、`usage_delta`、`usage_frequency_update`、`refill_request`、`disposal/correction` の service/API を追加する。                                                                                |
...
| Phase 4 | VisitRecord adapter。既存 residual input から `visit_observation` event を作り、snapshot 再計算。                                                       | 未実装。visit-record API tests、idempotency、legacy response compatibility。                 |
| Phase 5 | Prescription supply adapter。YJ/HOT/GS1/DrugPackage 連動、単位換算、unlinked supply task。                                                              | 未実装。prescription intake integration tests、DrugMaster/DrugPackage matching tests。       |
