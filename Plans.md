...
| Bucket                   | Count | 入口                                                                                                    |
| ------------------------ | ----: | ------------------------------------------------------------------------------------------------------- |
| Done / frozen            |    16 | 下の Done 表。active backlog ではなく、回帰防止・docs参照・watchlist対象として扱う。                    |
| Partial / residual track |     8 | 下の Partial 表。既存土台は再作成せず、残スコープだけ implementation queue へ切る。                     |
| Implementation queue     |    31 | `Implementation-ready queue`。次PRに切れる backend/platform/ops/API タスク。Human gate を含む。         |
...
3. `STOCK-001-PRESCRIPTION-HORIZON`: 次回処方/補充予定を不足予測horizonに使う正本sourceを設計する。`refill_request` だけから予定日を推定しない。
...
| Medication Stock base    | schema、RLS/index、append-only event、snapshot、summary API、stockout/equivalence helper、accepted inbound signal apply、処方供給adapter v1。                                                                                                                        | 残は処方供給follow-up、訪問観測migration適用/DB integration/UI、処方/補充horizon、usage/refill、equivalence review UI。       |
...
| Stock visit downstream   | 訪問観測後の `MedicationStockSnapshot` `urgent` / `shortage_expected` を OperationalTask fan-out、Case Risk Cockpit provider、VisitBrief、Schedule候補理由、Patient Movement occurrence marker へ接続した。                                                          | 再実装しない。残は migration/DB integration gate、write-enabled UI、処方/補充horizon、equivalence review UI。                 |
| Patient Movement base    | `movement` tab、共通DTO、safe detail resolver、処方/訪問/文書 occurrence marker、MedicationStockSnapshot occurrence marker、timeline list payload budget、standalone `/api/patients/:id/movement-timeline` 初期alias。                                               | 残は cursor/date/category filter、map-less date card UX、formal inbound/safety sources、remaining deep link coverage。        |
...
| `RX-002/STOCK`  | MedicationStock ledger base、accepted inbound signal apply、prescription supply adapter v1、visit observation API、訪問観測後の次回訪問/JST stockout forecast、downstream task/risk/brief/schedule/movement 連動。 | prescription supply follow-up、visit context migration apply、visit observation UI、処方/補充horizon、usage_delta/frequency/refill、equivalence review UI。完了済みforecast/downstreamを再作成せず、補充horizonだけ別IDで進める。 |
| `MOV-001`       | Movement tab/DTO/resolver/occurrence marker、MedicationStockSnapshot occurrence marker、standalone movement-timeline 初期alias。                                                                                   | cursor/date/category filter、日付カードUX、formal inbound/safety sources、remaining source parity、relative href builder、raw再認可detail。                                                                                       |
...
| `MOV-001-API`                      | Partial     | P1       | Patient detail    | standalone `/api/patients/:id/movement-timeline` 初期aliasと MedicationStockSnapshot occurrence source は実装済み。残は cursor/date/category filter、map-less date card UX、formal inbound/safety sources。処方・訪問・文書は occurrence marker + deep linkのみ。  | movement API tests、payload budget、raw omission、mobile/a11y。SOAP/OCR/raw chat/薬剤明細全文を一覧に出すなら停止。                                    |
...
| `FRONTEND-ENTRYPOINT-MAP-001`    | 7画面UI改善の最初の成果物として、患者一覧、患者詳細、調剤、スケジュール、訪問、報告書、他職種受信の route、primary component、BFF/API、shared component、existing tests、missing state を1表にする。                                                                                                                                                            | AppShell、Sidebar、WorkspaceActionRail、DataTable、既存 route/BFF。                                 | `docs/ui-ux-design-guidelines.md` と access-control policy 参照、画面ごとの state matrix、exact-path validation list。未実装API前提のproduction-like UIを作るなら停止。 |
...
3. `STOCK-001-PRESCRIPTION-HORIZON`: 次回処方/補充予定を stockout horizon に使う正本sourceを決める。
