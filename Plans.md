...
1. `FILE-COMPLETE-001`: 完了済み。次の file/export PHI slice は `FILE-001` / `DEV-PHI-001` の snapshot breadth と retention/download guard へ進める。
...
- `src/app/(dashboard)/visits/[id]/record/visit-record-form.tsx`: `useWatch({ control: form.control })` の全体 watch、30 秒 autosave、5 秒固定 sync polling、音声/添付/CDS/report readiness/location/offline を同一巨大 form に含む。`VISIT-SYNC-001` と同時に render split が必要。
...
| ID              | 優先度 | 既存レーン                                           | タスク                                              | 実装単位                                                                                                                                                                                                                                                           | 受入条件 / validation                                                                                                                                                                   |
| --------------- | ------ | ---------------------------------------------------- | --------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| AWS-ARCH-001    | P0/P1  | `12-4`, `0-5a`, `OPS-RECOVERY-001`                   | Pilot-to-production architecture decision record    | `docs/architecture/aws-phos-deployment-stages.md` を作成し、Lightsail pilot、本番最小ECS/RDS、拡張構成のサービス、env、データ境界、移行条件、後回しサービスをADR化する。`aws-lightsail-pilot-plan.ts` / `aws-ecs-express-plan.ts` の既存見積もりと差分を照合する。 | 実証/本番/拡張のサービス差分、PHI投入前必須条件、ECS/RDS移行条件、使わないAWSサービスが1文書で判断できる。営業・運用・開発の前提が矛盾しない。                                          |
| AWS-LS-001      | P0/P1  | `12-4`, `OPS-RATE-001`, `S3-PHI-001`                 | Lightsail pilot readiness gate                      | Lightsail App VM、Lightsail PostgreSQL、S3 public access block/versioning/Object Lock/SSE、Cognito MFA、SES、DynamoDB rate limit、CloudWatch alarms、ECR、Route53/ACM のpilot checklistを作る。                                                                    | PHI投入前にDB非公開、S3 Object Lock、RLS、no-store、audit、rate limit DDB、backup/runbook がreadyであることを `pilot-readiness` で確認できる。LightsailがHA本番ではない警告を残す。     |
...
| S3-PHI-001      | P0/P1  | `FILE-LIFE-001`, `DATA-RET-001A`, `OPS-RECOVERY-001` | S3 Object Lock / versioning / KMS policy gate       | 処方箋、報告書PDF、訪問写真、契約書、監査証跡、exports のS3 bucket/prefix policyを整理する。Block Public Access、TLS必須、versioning、Object Lock、SSE-S3初期、SSE-KMS本番強化、presigned URL短時間、CloudTrail data events検討をplan化する。                      | PHI file はpublic bucket不可。Object Lock/retention/legal hold と FileAsset lifecycle が矛盾しない。presigned URL と public API は original_name/storage_key を返さない。               |
