...
| Bucket                   | Count | 入口                                                                                                    |
| ------------------------ | ----: | ------------------------------------------------------------------------------------------------------- |
| Done / frozen            |    24 | 下の Done 表。active backlog ではなく、回帰防止・docs参照・watchlist対象として扱う。                    |
| Partial / residual track |     7 | 下の Partial 表。既存土台は再作成せず、残スコープだけ implementation queue へ切る。                     |
| Implementation queue     |    25 | `Implementation-ready queue`。次PRに切れる backend/platform/ops/API タスク。Human gate を含む。         |
...
3. `MOV-001-API`: raw detail reauth と browser/mobile/a11y validation。map-less date card shell と deep-link guard は完了。
4. `FRONTEND-CONTRACT-001`: 7画面UI改善を entrypoint/BFF/state/mobile/validation の契約に落とす。
5. `QUERY-SHAPE-WATCHLIST-003-FOLLOW`: DB read-speed follow-up を zero-debt watchlist で維持する。
6. `OPS-RECOVERY-LIVE-001`: AWS live restore evidence は credential/承認が揃うまで human gate。
...
| `MOV-001`       | Movement tab/DTO/resolver/occurrence marker、MedicationStockSnapshot occurrence marker、standalone movement-timeline API、cursor/date/category filter、source parity matrix、loaded-window date card UI、unsafe href guard、legacy/API deep-link guard。 | raw再認可detail、browser/mobile/a11y validation。                                                                                                                                                         |
...
| `MOV-001-API`                      | Partial     | P1       | Patient detail    | standalone `/api/patients/:id/movement-timeline`、cursor/date/category filter、MedicationStockSnapshot occurrence source、formal source parity、map-less date card shell、legacy/API deep-link guard は実装済み。旧 `/api/patients/:id/timeline` list alias と旧 detail shell は残さない。残は raw detail reauth、browser/mobile/a11y validation。処方・訪問・文書は occurrence marker + deep linkのみ。 | movement API tests、payload budget、raw omission、mobile/a11y。SOAP/OCR/raw chat/薬剤明細全文を一覧に出すなら停止。                                    |
...
| `MOV-001-API`                  | `GET /api/patients/:id/movement-timeline` を既存 timeline registry から切り出す。occurrence-only source、cursor/date/category filter、source parity matrix、map-less date card shell、legacy/API deep-link guard は実装済み。次は raw再認可detail、browser/mobile/a11y validation。                                                                             | movement tab、共通DTO、safe detail resolver、処方/訪問/文書 occurrence marker。 | raw omission、relative href、partial failure、payload budget、mobile date-card rendering。                                                                              |
...
3. `MOV-001-API`: raw detail reauth と browser/mobile/a11y validation。map-less date card shell と deep-link guard は完了。
4. `FRONTEND-CONTRACT-001`: 7画面UI改善のslice contract / state matrix。
5. `QUERY-SHAPE-WATCHLIST-003-FOLLOW`: query-shape対象拡張とzero-debt維持。
6. `PERF-DB-006D-INDEX`: SELECT-only EXPLAIN artifact と rollback plan の実環境証跡が揃ってから human gate へ進める。
