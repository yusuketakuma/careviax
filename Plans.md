...
| Patient Movement base      | `movement` tab、共通DTO、safe detail resolver、処方/訪問/文書 occurrence marker、MedicationStockSnapshot occurrence marker、timeline list payload budget、standalone `/api/patients/:id/movement-timeline`。                                                                                                                                                                                                                                                                                                                                                                                                    | 残は map-less date card UX、formal inbound/safety sources、remaining deep link coverage。                                                                                           |
...
| `MOV-001`       | Movement tab/DTO/resolver/occurrence marker、MedicationStockSnapshot occurrence marker、standalone movement-timeline API、cursor/date/category filter。                                                                                           | 日付カードUX、formal inbound/safety sources、remaining source parity、relative href builder、raw再認可detail。                                                                                            |
...
| `MOV-001-API`                      | Partial     | P1       | Patient detail    | standalone `/api/patients/:id/movement-timeline`、cursor/date/category filter、MedicationStockSnapshot occurrence source は実装済み。旧 `/api/patients/:id/timeline` list alias は残さない。残は map-less date card UX、formal inbound/safety sources。処方・訪問・文書は occurrence marker + deep linkのみ。                     | movement API tests、payload budget、raw omission、mobile/a11y。SOAP/OCR/raw chat/薬剤明細全文を一覧に出すなら停止。                                                                            |
...
| `MOV-001-API`                  | `GET /api/patients/:id/movement-timeline` を既存 timeline registry から切り出す。occurrence-only source と cursor/date/category filter は実装済み。次は formal inbound/stock/safety sources。                                                                                                                                                                   | movement tab、共通DTO、safe detail resolver、処方/訪問/文書 occurrence marker。                                                                                | raw omission、relative href、partial failure、payload budget、mobile date-card rendering。                                                                              |
