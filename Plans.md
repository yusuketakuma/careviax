...
| `STOCK-001-VISIT-CONTEXT`          | Partial     | P0/P1    | Pharmacy stock    | `MedicationStockEvent` を append-only 正本のまま維持し、訪問観測の `observed_date_key_jst`、`last_used_at`、`unobserved_reason_code`、source confidence/context、confirmation level は 1:1 sidecar `MedicationStockObservationContext` へ逃がす migration candidate を追加した。migration適用は human gate。free-text reason は v1 で保存しない。 | `prisma validate` と DB contract test は必須。Oracle/DB review と migration適用承認は継続 gate。`event_at` を最終使用日や未確認理由の代用にするなら停止。 |
