...
| Bucket                   | Count | 入口                                                                                                    |
| ------------------------ | ----: | ------------------------------------------------------------------------------------------------------- |
| Done / frozen            |    21 | 下の Done 表。active backlog ではなく、回帰防止・docs参照・watchlist対象として扱う。                    |
| Partial / residual track |     8 | 下の Partial 表。既存土台は再作成せず、残スコープだけ implementation queue へ切る。                     |
| Implementation queue     |    27 | `Implementation-ready queue`。次PRに切れる backend/platform/ops/API タスク。Human gate を含む。         |
...
3. `INBOUND-002-REVIEW-DETAIL`: raw detail API、audited detail panel、review lifecycle、MedicationStock適用条件/不可表示、source detail layout、audited apply target selector、FAX/email/manual canonical intake、source mapping route/UI、VisitBrief downstream は完了。残は Schedule/Report/Share 導線。
4. `MOV-001-API`: remaining deep-link coverage と raw detail reauth。map-less date card shell は完了。
...
| `INB-001/002`   | Formal inbound schema/API/inbox/signals/task/risk/stock apply、raw detail再認可API/read audit、audited detail panel、dashboard raw omission、review lifecycle UI、source detail layout、MedicationStock適用条件/不可表示、audited apply target selector、FAX/email/manual canonical intake、source mapping route/UI、VisitBrief downstream。 | Schedule/Report/Share。                                                                                                                                                                                   |
...
| `MOV-001`       | Movement tab/DTO/resolver/occurrence marker、MedicationStockSnapshot occurrence marker、standalone movement-timeline API、cursor/date/category filter、source parity matrix、loaded-window date card UI、unsafe href guard。                                                                                                                 | remaining deep-link coverage、raw再認可detail、browser/mobile/a11y validation。                                                                                                                           |
...
| ---------------------------------- | ----------- | -------- | ----------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `INBOUND-002-REVIEW-DETAIL`        | Partial     | P0/P1    | Inbound UI/API    | raw detail API、既存 review panel の audited raw表示、候補ごとの review lifecycle、MedicationStock 適用条件/台帳直書き不可、source detail layout、actual apply target selector、FAX/email/manual canonical intake、source mapping route/UI、VisitBrief downstream は完了。残は Schedule/Report/Share 連動。raw_textは既存 detail API とread audit経由に限定。                            | raw omission snapshots、review lifecycle、MedicationStock apply allowed/forbidden tests。raw_textをlist/通知/監査changesへ流す、または `target_stock_item_id` なしに apply UI を出すなら停止。 |
...
| `MOV-001-API`                      | Partial     | P1       | Patient detail    | standalone `/api/patients/:id/movement-timeline`、cursor/date/category filter、MedicationStockSnapshot occurrence source、formal source parity、map-less date card shell は実装済み。旧 `/api/patients/:id/timeline` list alias は残さない。残は remaining deep-link coverage、raw detail reauth、browser/mobile/a11y validation。処方・訪問・文書は occurrence marker + deep linkのみ。 | movement API tests、payload budget、raw omission、mobile/a11y。SOAP/OCR/raw chat/薬剤明細全文を一覧に出すなら停止。                                                                            |
...
| ------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `INBOUND-002-REVIEW-DETAIL`    | raw detail API、既存 review panel の audited raw表示、review action lifecycle、MedicationStock apply forbidden/conditions、source detail layout、actual apply target selector、FAX/email/manual canonical intake、source mapping route/UI、VisitBrief downstream は完了。次は Schedule/Report/Share 連動だけを切る。                                            | Inbound schema、phone/MCS登録、FAX/email/manual intake、inbox、signal materialize、task/risk bridge、MedicationStock apply、raw detail API、audited detail panel、review lifecycle UI、source detail layout、apply target selector、source mapping route/UI、VisitBrief downstream。 | list DTO raw omission snapshot、MedicationStock apply allowed/forbidden、`target_stock_item_id` と観測値がない状態で apply しないこと。                                 |
...
| `MOV-001-API`                  | `GET /api/patients/:id/movement-timeline` を既存 timeline registry から切り出す。occurrence-only source、cursor/date/category filter、source parity matrix、map-less date card shell は実装済み。次は remaining deep-link coverage、raw再認可detail、browser/mobile/a11y validation。                                                                           | movement tab、共通DTO、safe detail resolver、処方/訪問/文書 occurrence marker。                                                                                                                                                                                                      | raw omission、relative href、partial failure、payload budget、mobile date-card rendering。                                                                              |
...
3. `INBOUND-002-REVIEW-DETAIL`: FAX/email/manual、source mapping、VisitBrief/Schedule/Report/Share 連動。
4. `MOV-001-API`: remaining deep-link coverage と raw detail reauth。map-less date card shell は完了。
