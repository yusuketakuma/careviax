...
- 未実装として残すもの: `PERF-DB-006C-D` care-report delivery summary / index gate、`PAYLOAD-BUDGET-003`、`API-CONTRACT-*`、`DB-EVENT-001`、`FILE-LIFE-001`、`TENANT-SUPPORT-001`、7画面UIの実装slice、live AWS human gate。
...
- `care-reports` は `view=palette&q=` と通常list `q` 患者検索が bounded。`keyword` は cursor を拒否し、`CARE_REPORT_KEYWORD_SCAN_LIMIT=500` + plus-one read で overflow を検知し `search.count_basis='bounded_keyword_scan'` を返す。残るDB速度課題は delivery summary が page rows ではなく検索 `where` 全体へかかる点と、EXPLAIN-backed index gate である。
...
1. `PERF-DB-006C/006D` delivery summary / index候補を EXPLAIN-backed backlog へ分離。
...
- `PERF-DB-006B-KEYWORD`（実装済み）: keyword scan は `limit + 1` / plus-one read で overflow を検出し、cursor拒否と矛盾する `nextCursor` を返さない。`search.keyword_scan_limit`、`search.keyword_scan_truncated`、`search.result_window_truncated`、`search.count_basis='bounded_keyword_scan'` を返し、本文検索を無制限DB scanへ広げない。
