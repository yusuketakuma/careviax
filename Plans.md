...
| RX-002   | 残薬/頓服/外用 | PRN / topical Stock Risk service                | `medication-stock-risk.ts`, `visit-records`, `visit-record.ts`, patient board                               | `cc:PARTIAL 2026-07-07` pure domain first slice として `medication-equivalence`、`stockout-forecast`、`external-observation` を追加。YJ/HOT と GS1/GTIN/JAN を区別し、外部由来残数情報は薬剤師レビュー staging に留める。`cc:PARTIAL 2026-07-07` `medication-stock-signal-adapter` で INB medication_stock signal を external observation staging へ接続し、使用量と実残量、患者/家族申告、薬剤identity不確実性を区別。`cc:PARTIAL 2026-07-07` `medication-stock-risk-adapter` で staging result を controlled RiskFinding へ変換し、不足報告、使用量報告、残数報告、名寄せ確認をPHI-freeに表現。`cc:PARTIAL 2026-07-07` medication stock task types を `TaskTypeRegistry` に登録し、残数不足、使用頻度未確認、名寄せ確認、処方供給未紐づけ、他職種残数報告を module-prefixed task として作成可能にした。残: DB/API/UI、ledger正本化、VisitBrief/Schedule接続、正式 provider 統合。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| Phase 5 | `cc:PARTIAL 2026-07-07` Task source 追加。既存 `Task` の患者/ケース紐づき行を `task_created` / `task_resolved` として summary-only 表示。                                                                                                              | task source tests、free text omission tests。残: safety finding source。                            |
| Phase 6 | `cc:PARTIAL 2026-07-07` MedicationStock source 追加。既存 `ResidualMedication` は visit record 単位の summary-only bridge として表示済み。残: 正式 `MedicationStockEvent`、equivalence review、shortage finding を表示。                               | stock integration tests、risk/task link tests、drug/quantity omission tests。                       |
...
- `src/app/(dashboard)/visits/[id]/record/visit-record-form.tsx`: `VISIT-SYNC-001` の一部として autosave は 5秒 debounce、訪問開始/終了は即時 draft 保存、sync count refresh は未同期件数>0かつ表示中のみ polling へ更新済み。legacy plaintext sync payload は tombstone 化し、OfflineSyncBridge の false-green `lastSyncedAt` 更新も抑止済み。step transition / 残薬 / 添付追加の即時保存、指数 backoff、保存状態 UI/mobile E2E は未完。
...
- `src/components/features/dispense-workbench/dispensing-workbench.store.ts`: 実データ時の `partialize` は `{}` を返して clinical state を保存しないが、過去 mock/旧実装の `chouzai-workbench` key を起動時に削除する処理はない。
- `src/app/sw.ts`: CacheStorage policy は API/認証済み navigation を network-only にしている一方、push event では `data.title` / `data.body` / `data.link` をそのまま OS 通知と click URL に使う。
...
| ID             | 優先度 | 既存レーン                                                     | タスク                                        | 実装単位                                                                                                                                                                                                                                                                                                        | 受入条件 / validation                                                                                                                                                                                                                              |
| -------------- | ------ | -------------------------------------------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| FE-RT-001      | P0/P1  | `PERF-BFF-001`, `PERF-X-001`, `FE-BUD-001`                     | Realtime invalidation policy                  | `invalidateOn=[]` を「明示指定なし」に変更し、全 event 再取得は `invalidateOn: 'all'` 等の明示 contract にする。event type + source/payload scope で対象 query を絞り、同一 tick/burst の invalidate を 100-300ms debounce/batch する。                                                                         | 主要画面で invalidate 対象 event が明示される。`workflow_refresh` 乱発で dashboard/patient/report/schedule/task が一斉 refetch しない。burst test で API invalidate が1回にまとまる。既存 callers は移行表と test で backfill する。               |
| FE-RT-002      | P0/P1  | `NTF-STREAM-001`, `SEC-001`, `DEV-PHI-001`                     | Realtime client payload schema hardening      | `normalizeRealtimeEventPayload()` を event type 別 allowlist に変更する。`workflow_refresh`、`cycle_transition`、`comment_refresh`、`presence_update`、`notification`、`qr_draft_created`、`visit_schedule_proposals_confirm` などの許可 field を registry 化する。                                             | SSE payload に `patient_name`、住所、電話、薬剤名、`raw_message`、`metadata`、`provider_error`、`storage_key`、`signed_url` が混じっても React state に入らない。unknown event は PHI-free `workflow_refresh` 相当へ丸める。snapshot test を追加。 |
| FE-PUSH-001    | P0/P1  | `NTF-PUSH-001`, `MOB-CACHE-001`, `DEV-PHI-001`                 | Service Worker push redaction backstop        | `src/app/sw.ts` の push handler で title/body/link を直接信用しない。OS通知用 pure normalizer を server/client 共用可能な module へ切り出し、title は固定、body は type 別汎用文言、url は `/notifications` に丸める。                                                                                          | Service Worker に患者名や本文、patientId/reportId/visitId/deep link が届いても OS 通知と click 先に出ない。deep link は通知センター内で再認可して開く。SW unit/browser test と server Web Push payload test を追加。                               |
| FE-SHELL-001   | P1     | `FE-X-001`, `FE-BUD-001`, `UX-MOB-001`                         | AppShell heavy globals dynamic import         | `CommandPalette` は open 時、`ShortcutHelpModal` は help open 時、`InstallPrompt` は PWA 条件成立後、`MobileOrientationGuard` は mobile/tablet viewport 確定後に lazy mount。`SessionTimeoutModal` は auth/session event 連動で遅延可能か検証。                                                                 | 通常画面初期表示の AppShell bundle が減る。初回 command palette は skeleton/loading を出す。print/capture/visit record immersive では不要 global UI を mount しない。AppShell tests は dynamic import 境界を mock して維持。                       |
| FE-STORAGE-001 | P1     | `MOB-CACHE-001`, `DEV-PHI-001`, `DSP-UX-002`                   | Clinical localStorage cleanup                 | 実データ環境起動時に `localStorage.removeItem('chouzai-workbench')` を実行する。mock mode でも患者名/薬剤名/住所を永続化しない seed-only policy にする。storage audit の browser harness と連動。                                                                                                               | 実データ環境では旧 `chouzai-workbench` key が残らない。過去 mock/旧 clinical state は初回起動で削除。localStorage/sessionStorage/IndexedDB snapshot test で患者名、薬剤名、住所、storage key が検出されない。                                      |
| FE-TBL-001     | P1     | `UX-TBL-001`, `DEV-PHI-001`, `EXP-002`                         | DataTable client export policy tightening     | PHI を含む画面では client CSV を禁止し、serverExport registry descriptor を必須にする。client CSV を許す場合は `nonPhiExport: true` 等の明示 flag と非 PHI filename validator を要求する。`hasMore=true` では client export を原則 disabled。                                                                   | 患者、処方、訪問、報告、請求、監査ログ画面で client CSV が使われない。全件出力は server-side masking/audit 経由。`enableExport` だけでは PHI CSV を出せないことを DataTable unit と代表画面 snapshot で固定。                                      |
| FE-TBL-002     | P1     | `FE-BUD-001`, `DEV-UI-001`                                     | DataTable filter performance                  | shared `DataTable` の global/column filter に debounce または deferred value を内蔵する。100行超で pagination 無効の場合は dev warning、200行超は server pagination / virtualized list / DataTable pagination へ誘導する。                                                                                      | 検索 keystroke の再描画が budget 内に収まる。filter 入力の a11y label と immediate text value は維持。DataTable tests に debounce timer と warning を追加。                                                                                        |
...
| FE-REPORT-001  | P1     | `REPORT-PERF-001`, `PERF-BFF-001`, `FE-RT-001`                 | Reports realtime query migration              | `/reports` workspace を `useQuery` 固定 polling から `useRealtimeQuery` へ移行する。connected 時は polling 停止、disconnected 時だけ `fallbackRefetchInterval=60_000`。invalidateOn は report/workflow/comment/delivery event に限定する。                                                                      | report workspace の二重 BFF fetch 解消済みを維持しつつ、realtime connected 時に 60秒 polling が止まる。delivery update / draft generation / comment refresh で必要 query だけ invalidate する。component test を更新。                             |
...
| FE-NAV-001     | P1     | `DEV-UI-001`, `UX-MOB-001`                                     | Navigation active-state matrix test           | `navigation-utils` / `navigation-config` に route matrix test を追加する。`/patients`、`/patients/new`、`/patients/[id]`、`/prescriptions`、`/prescriptions/new`、`/prescriptions/intake`、`/qr-scan`、`/visits/[id]/record`、`/reports/print`、`/admin/*` を固定。                                             | 患者一覧とカード、処方取込と処方一覧、訪問 record immersive、admin group の active が回帰しない。route 追加時は matrix にケースを足す。                                                                                                            |
...
| FE-ERR-001     | P1     | `UX-ERR-001`, `DEV-UI-001`, `PERF-BFF-001`                     | Segment boundary pattern                      | `SegmentLoading`、`SegmentError`、`SegmentStaleBanner`、`SegmentRetryButton`、`SegmentEmptyButNotError` を shared pattern 化し、dashboard summary/details/team/comments の fail-soft を患者詳細 tabs、報告、訪問準備、スケジュール、task/admin へ展開する。                                                     | 一部 BFF segment 失敗で画面全体が落ちない。空状態と取得失敗を分離。request_id/route/generated_at/retry_count を必要時表示。ErrorState と重複しない wrapper として設計。                                                                            |
...
- `FE-RT-001 / FE-RT-002 / FE-PUSH-001` partial（2026-07-06）:
  ...
  残: server-side `NTF-STREAM-001` の SSE user-channel/DB polling normalizer、Web Push 送信側 snapshot、
  FE-REPORT-001 の report workspace realtime migration、FE-OFFLINE-001 の browser storage audit。
