...
| `PERF-DB-001`            | 一部実装済 | inbound-only queue source gating、report workspace inbound evidence direct count。                                               | dashboard stock signal count consolidation、patient detail bounded read、movement caller-limit、care-report bounded search / index backlog。              |
...
| DB速度     | inbound inbox と report workspace の不要な queue fan-out 削減。                                                                   | `src/server/services/communication-queue.ts` の `sourceScope: 'requested'`、`src/app/api/communications/inbound/route.ts`、`src/app/api/care-reports/today-workspace/route.ts`。 | 残は `PERF-DB-002/005/006/007`。                                 |
...
1. `PERF-DB-002` Dashboard medication-stock signal reader count consolidation。
2. `PERF-DB-005` legacy patient detail full read bounded scoped read。
3. `PERF-DB-007` movement timeline caller-limit-aware source reads。
