...
3. `MOV-001-API`: browser/mobile/a11y validation。map-less date card shell、deep-link guard、raw detail reauth は完了。
...
| `MOV-001`       | Movement tab/DTO/resolver/occurrence marker、MedicationStockSnapshot occurrence marker、standalone movement-timeline API、cursor/date/category filter、source parity matrix、loaded-window date card UI、unsafe href guard、legacy/API deep-link guard、raw detail reauth gate。 | browser/mobile/a11y validation。                                                                                                                                                                          |
...
| `MOV-001-API`                      | Partial     | P1       | Patient detail    | standalone `/api/patients/:id/movement-timeline`、cursor/date/category filter、MedicationStockSnapshot occurrence source、formal source parity、map-less date card shell、legacy/API deep-link guard、raw detail reauth gate は実装済み。旧 `/api/patients/:id/timeline` list alias と旧 page detail shell は残さない。残は browser/mobile/a11y validation。処方・訪問・文書は occurrence marker + deep linkのみ。 | movement API tests、payload budget、raw omission、mobile/a11y。SOAP/OCR/raw chat/薬剤明細全文を一覧に出すなら停止。                                    |
...
| `MOV-001-API`                  | `GET /api/patients/:id/movement-timeline` を既存 timeline registry から切り出す。occurrence-only source、cursor/date/category filter、source parity matrix、map-less date card shell、legacy/API deep-link guard、raw detail reauth gate は実装済み。次は browser/mobile/a11y validation。                                                                      | movement tab、共通DTO、safe detail resolver、処方/訪問/文書 occurrence marker。 | raw omission、relative href、partial failure、payload budget、mobile date-card rendering。                                                                              |
