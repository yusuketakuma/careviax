...
| `RX-002/STOCK`  | MedicationStock ledger base、accepted inbound signal apply、prescription supply adapter v1、visit observation API、訪問観測後の次回訪問/JST stockout forecast。           | prescription supply follow-up、visit context migration apply、visit observation UI/downstream残（Schedule / Movement）、処方/補充horizon、usage_delta/frequency/refill、equivalence review UI。完了済みforecastを再作成せず、補充horizonだけ別IDで進める。 |
...
| `STOCK-001-VISIT-DOWNSTREAM`       | Partial     | P1       | Pharmacy stock    | 訪問観測後の snapshot/risk を OperationalTask、Case Risk Cockpit、VisitBrief、Schedule候補、Patient Movement へ接続する。OperationalTask fan-out、Case Risk Cockpit provider、VisitBrief 連動は実装済み。残は Schedule / Movement 連動。task metadata は ids/status/code のみで、患者名・薬剤名・raw reason は入れない。 | risk-task bridge tests、VisitBrief/Schedule tests、PHI metadata snapshot。ledger更新だけで業務follow-upが出ないなら停止。                              |
...
| `STOCK-001-VISIT-DOWNSTREAM`     | ledger write後の fan-out を `risk/task` → `VisitBrief` → `Schedule` → `Movement` の順に分割する。`risk/task`、Case Risk Cockpit provider、VisitBrief 連動は完了済み。次は Schedule候補、Movement occurrence の順に payload PHI boundary を固定する。                                                                                                            | append-only event、snapshot、次回訪問/JST forecast、OperationalTask shortage fan-out、Case Risk provider、VisitBrief snapshot surfacing。 | schedule candidate reason、movement occurrence event。                                                                                                                  |
...
3. `STOCK-001-VISIT-DOWNSTREAM`: OperationalTask fan-out、Case Risk Cockpit provider、VisitBrief 連動は完了。残る Schedule / Movement 連動を実装する。
