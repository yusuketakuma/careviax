...
1. `PAYLOAD-BUDGET-001C/D` inbound / stock list DTO と perf-smoke matrix の payload-size smoke / PHI omission snapshot。
2. `DASH-P1-010` Dashboard Summary Rail と process tile / drilldown。
3. `FRONTEND-CONTRACT-001` 7画面UI改善の slice contract / state matrix 整理。
4. `PLANS-HYGIENE-002` 長大な reference spec の段階移管。
...
| ------------------------ | ---------- | ------ | ----------------- | ---------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `PERF-DB-006D-EXPLAIN`   | 実装済み   | P1     | DB perf evidence  | `tools/scripts/explain-care-report-index-candidates.ts`, package script                  | `pnpm db:explain-care-report-index-candidates -- --org-id <org_id>` を追加済み。対象query shapeは default list、patient_id list、bounded `q` patient候補、delivery filter relation、report_id/org_id delivery relation select。`EXPLAIN (FORMAT JSON)` を既定にし、DDL/DML/ANALYZE/live AWS/本番DB mutation は禁止する。出力は redacted JSON/Markdown artifact のみ。                                                                                                                                                                                                                                                                                        | mocked pg client unit tests、SQL verb guard、artifact redaction、CLI help。`CREATE/ALTER/DROP/INSERT/UPDATE/DELETE/TRUNCATE/REINDEX/VACUUM` を含むSQLは拒否。                                                                                 |
