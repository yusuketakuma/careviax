...
| Phase 4 | `cc:PARTIAL 2026-07-07` `INB-001` source 追加。既存 `PatientMcsMessage` と `PartnerVisitRecord` は summary-only で表示済み。正式 `InboundCommunicationEvent` / `InboundCommunicationSignal` source は後続。 | MCS/協力薬局訪問 record tests、raw_text omission tests。残: phone/FAX/email/inbound_signal source。             |
...
| Phase 6 | MedicationStock source 追加: stock event、equivalence review、shortage finding を表示。                                                                                                                     | stock integration tests、risk/task link tests。                                                                 |
