...
| QUERY-SHAPE-WATCHLIST-003 | P1     | DB速度/品質   | `tools/scripts/check-query-shape.mjs`, `tools/query-shape-watchlist.json`                            | 既存の watchlist guard を patients board、reports remaining detail surfaces、schedule/visit heavy BFF へ段階拡張する。新規watch path はまず broad include / unbounded findMany / unstable top-N / aggregate fan-out を0 debtで通す。false positive が出る場合は expectedCount allowlist ではなく、query shape の局所改善を優先する。                                                                                                                                                                                                                                          | `pnpm db:query-shape:check`、fixture tests、focused route/service tests、CI/perf-smoke wiring evidence。 |
...
3. `QUERY-SHAPE-WATCHLIST-003` critical read path watchlist の patients board / reports / schedule / visit への段階拡張。
