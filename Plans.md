...
| Bucket                   | Count | 入口                                                                                                    |
| ------------------------ | ----: | ------------------------------------------------------------------------------------------------------- |
| Done / frozen            |    23 | 下の Done 表。active backlog ではなく、回帰防止・docs参照・watchlist対象として扱う。                    |
...
3. `INBOUND-002-REVIEW-DETAIL`: raw detail API、audited detail panel、review lifecycle、MedicationStock適用条件/不可表示、source detail layout、audited apply target selector、FAX/email/manual canonical intake、source mapping route/UI、VisitBrief/Schedule/Report downstream は完了。残は Share 導線。
...
3. `INBOUND-002-REVIEW-DETAIL`: FAX/email/manual、source mapping、VisitBrief/Schedule/Report downstream は完了。残は Share 連動。
4. `MOV-001-API`: remaining deep-link coverage と raw detail reauth。map-less date card shell は完了。
