...
module registry / collaboration / risk provider / task type registry / report template registry / share scope registry の基盤タスクはここに残さない。ここでは未接続・未実装の port と、既存 allowlist を減らすための依存反転だけを残す。
...
| --------------- | ------ | ------------------------------------------------------------------------- | ------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| MOD-BOUND-001   | P0/P1  | `boundaries:check`, `DEV-REF-001`, `PLAN-REV-001`                         | Registry-driven module boundary gate | 現 allowlist は 0 imports / 0 files。残: module public entrypoint 例外、provider/adapter contract、allowlist増加禁止の CI coverage を広げる。 | 新規 core -> pharmacy / sibling module import / API route から module internal import が fail。`src/modules/active-modules.ts` は composition root として明示許可。既存 allowlist count は0を維持し、増やす場合は architecture review 必須。 |
...
| MOD-CI-001      | P1     | `API-DTO-001`, `DB-TENANT-001`, `DEV-PHI-001`, `PLAN-REV-001`             | Module/debt ratchet CI gates         | `boundaries:check` は allowlist 0 を通常CIで強制済み。Risk provider は duplicate/order/failure fail-soft contract 済み。Collaboration provider は active entity order/schema/unknown/failure fail-closed contract 済み。残: `api-response-shape:check`、`dto-direct-prisma-return:check`、`task-type-registry:check`、`rls-policy-contract` との役割整理。               | 各MOD PRは「module境界化 + DEBT削減 + DTO/presenter明確化 + 境界テスト + 回帰テスト」を含む。薬局E2E/主要API回帰を維持。module-boundary allowlist は0を維持し、例外確認は `PHOS_ALLOW_MODULE_BOUNDARY_DEBT=1` のaudit runに限定。                               |
...
- `module-boundary:check`: `boundaries:check` の module graph / allowlist reason / expectedCount ratchet 版。通常CIでは allowlist 0 を強制済み。
...
- `risk-provider-contract:test`: provider order、domain coverage、unknown/failure policy、PHI-safe finding detail を検査。provider order / duplicate / failure fail-soft は core registry test 済み。
- `collaboration-provider-contract:test`: unknown entity fail-closed、provider exception fail-closed、room name互換、assignment/org predicate を検査。registry fail-closed、active entity order/schema、core/pharmacy provider predicate は focused tests 済み。

**推奨 PR / slice 分割**:

1. `MOD-BOUND-001`: allowlist count を減らす依存反転へ進む。
2. `MOD-PATIENT-001`: patient workspace を複数 panel DTO と common / pharmacy 表示境界へ拡張する。患者詳細 tab/island split と整合。
3. `MOD-VISIT-001`: visit brief を common brief + pharmacy contributor に分ける。
4. `MOD-REPORT-001` + `MOD-SHARE-001`: 出力/masking/audit境界を固定する。
5. `MOD-IO-001`: 外部I/O adapter contract を整え、AWS/通知/Webhook/経路計算の raw error / PHI 境界を揃える。
6. `MOD-DATA-001` + 既存 `TENANT-*` / `DB-EVENT-001`: service_line / discipline / coverage / support session / outbox を migration plan へ接続する。
7. `MOD-CI-001`: 以後のPRで allowlist expectedCount を減らす ratchet と provider/adapter contract tests をCIへ接続する。
...
- `core -> modules/pharmacy` import が増えない。既存 allowlist の該当負債をさらに削減する。
