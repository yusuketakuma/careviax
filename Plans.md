...
| Bucket                   | Count | 入口                                                                                                    |
| ------------------------ | ----: | ------------------------------------------------------------------------------------------------------- |
| Done / frozen            |    19 | 下の Done 表。active backlog ではなく、回帰防止・docs参照・watchlist対象として扱う。                    |
| Partial / residual track |     8 | 下の Partial 表。既存土台は再作成せず、残スコープだけ implementation queue へ切る。                     |
| Implementation queue     |    29 | `Implementation-ready queue`。次PRに切れる backend/platform/ops/API タスク。Human gate を含む。         |
...
4. `MOV-001-API`: map-less date card UX と remaining deep-link / raw detail reauth。
...
| Patient Movement base          | `movement` tab、共通DTO、safe detail resolver、処方/訪問/文書 occurrence marker、MedicationStockSnapshot occurrence marker、timeline list payload budget、standalone `/api/patients/:id/movement-timeline`。                                                                                                                                                                                                                                                                                                                                                                                                    | 残は map-less date card UX、remaining deep link coverage、raw detail reauth。                                                                                                       |
...
| `MOV-001`       | Movement tab/DTO/resolver/occurrence marker、MedicationStockSnapshot occurrence marker、standalone movement-timeline API、cursor/date/category filter、source parity matrix。                                                                     | 日付カードUX、remaining deep-link coverage、raw再認可detail。                                                                                                                                             |
...
| `MOV-001-API`                      | Partial     | P1       | Patient detail    | standalone `/api/patients/:id/movement-timeline`、cursor/date/category filter、MedicationStockSnapshot occurrence source、formal source parity は実装済み。旧 `/api/patients/:id/timeline` list alias は残さない。残は map-less date card UX、remaining deep-link coverage、raw detail reauth。処方・訪問・文書は occurrence marker + deep linkのみ。 | movement API tests、payload budget、raw omission、mobile/a11y。SOAP/OCR/raw chat/薬剤明細全文を一覧に出すなら停止。                                                                            |
...
| `MOV-001-API`                  | `GET /api/patients/:id/movement-timeline` を既存 timeline registry から切り出す。occurrence-only source、cursor/date/category filter、source parity matrix は実装済み。次は map-less date card UX、remaining deep-link coverage、raw再認可detail。                                                                                                              | movement tab、共通DTO、safe detail resolver、処方/訪問/文書 occurrence marker。                                                                                | raw omission、relative href、partial failure、payload budget、mobile date-card rendering。                                                                              |
