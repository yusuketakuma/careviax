...
| MOD-CI-001      | P1     | `API-DTO-001`, `DB-TENANT-001`, `DEV-PHI-001`, `PLAN-REV-001`             | Remaining debt-ratchet CI gates    | 残: `api-response-shape:check`、`rls-policy-contract` 拡張と役割整理。実装済みの `dto-direct-prisma-return:check` / `task-types:check` / module-boundary gate は維持し、以後は allowlist を増やさず削減する。                                                                               | 各MOD PRは「module境界化 + DEBT削減 + DTO/presenter明確化 + 境界テスト + 回帰テスト」を含む。薬局E2E/主要API回帰を維持し、module-boundary / DTO direct Prisma return / task type registry gate を維持する。 |
...
6. `MOD-CI-001`: API response / RLS contract の残 gate をCIへ接続する。
