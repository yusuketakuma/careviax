...
- 未実装として残すもの: `PERF-DB-006D` care-report index gate、`API-CONTRACT-*`、`DB-EVENT-001`、`FILE-LIFE-001`、`TENANT-SUPPORT-001`、7画面UIの実装slice、live AWS human gate。
...
- `PERF-DB-006D-EXPLAIN`: care-report index候補の SELECT-only EXPLAIN capture script / artifact を作る。DDL/DML、migration適用、live ANALYZE は含めない。
...
1. `PERF-DB-006D-EXPLAIN` index候補を SELECT-only EXPLAIN-backed artifact へ分離する。migration は human gate のまま。
2. `PAYLOAD-BUDGET-001C/D` inbound / stock list DTO と perf-smoke matrix の payload-size smoke / PHI omission snapshot。
3. `DASH-P1-010` Dashboard Summary Rail と process tile / drilldown。
4. `FRONTEND-CONTRACT-001` 7画面UI改善の slice contract / state matrix 整理。
5. `PLANS-HYGIENE-002` 長大な reference spec の段階移管。
...
| ------------------------ | ---------- | ------ | ----------------- | ---------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `PERF-DB-006D-EXPLAIN`   | 未実装     | P1     | DB perf evidence  | `src/app/api/care-reports/route.ts`, Prisma schema, Postgres EXPLAIN                     | SELECT-only の care-report index候補 capture script / artifact を追加する。対象query shapeは default list、patient_id list、bounded `q` patient候補、delivery filter relation、report_id/org_id delivery relation select。`EXPLAIN (FORMAT JSON)` を既定にし、DDL/DML/ANALYZE/live AWS/本番DB mutation は禁止する。                                                                                                                                                                                                                                                                                                                                          | mocked pg client unit tests、SQL verb guard、artifact redaction。`CREATE/ALTER/DROP/INSERT/UPDATE/DELETE/TRUNCATE/REINDEX/VACUUM` を含むなら停止。                                                                                            |
...
| `PAYLOAD-BUDGET-001A`    | 実装済み   | P1     | API perf          | dashboard cockpit segments, `withRoutePerformance`, `route-payload-budgets`              | `GET /api/dashboard/cockpit/{details,inbound,stock-risks,report-billing,comments,team}` を critical payload budget registry に追加し、dashboard segment response が measured JSON success helper で `Content-Length` を返すようにする。DTO内容は変えず、認証済み業務画面の権限内表示は維持する。                                                                                                                                                                                                                                                                                                                                                             | `f67a0db1b`。route budget unit tests、dashboard segment `Content-Length` tests。                                                                                                                                                              |
| `PAYLOAD-BUDGET-001B`    | 実装済み   | P1     | Patient timeline  | `/api/patients/:id/timeline`, movement DTO, safe resolver                                | Movement timeline list を payload-budgeted route にする。`withAuthContext` 内の `withRoutePerformance` に乗せ、route body は measured JSON helper で `Content-Length` を返す。`limit` / `partial_failures` など list metadata を維持する。詳細本文、SOAP、OCR、raw chat text は timeline list に出さず、detail resolver へ deep link する。                                                                                                                                                                                                                                                                                                                  | `src/app/api/patients/[id]/timeline/route.ts`、route `Content-Length` test、payload budget test。                                                                                                                                             |
| `PAYLOAD-BUDGET-001C`    | 未実装     | P1     | Inbound/stock API | `/api/communications/inbound`, `/api/communications/inbound/signals`, stock summary      | Inbound inbox/signals と MedicationStock summary の list/summary DTO に `visible_count` / `hidden_count` / `generated_at` / `partial_failures` 相当を揃える。対象3routeは `successWithMeasuredJsonPayload` を使い、`/api/communications/inbound`、`/api/communications/inbound/signals`、`/api/patients/:id/medication-stock` を payload budget registry へ追加する。既存の `total_visible_count`、`filtered_count`、`visible_item_count` などは互換維持し、正規化metadataをadditiveに足す。raw_text、storage key、signed URL、provider raw error、分類用 raw payload はlistに出さない。権限内 detail surface では本文・連絡先・添付を再認可後に表示できる。 | inbound/stock DTO snapshot、Content-Length tests、payload budget tests、forbidden tests、raw/detail separation tests。dashboard inbound のraw/detail業務表示をこのsliceで削る設計、またはraw_textをlistやnotificationへ出す必要が出たら停止。 |
...
- `PERF-DB-006D-EXPLAIN`: まず SELECT-only EXPLAIN capture script / artifact を作る。`EXPLAIN (FORMAT JSON)` を既定にし、DDL/DML/ANALYZE/live DB mutation は含めない。
...
- `PLAN-ACTIVE-LINT-001`: `Plans.md` の `cc:REFERENCE` / `cc:WIP` 節に残る未チェックboxを active backlog と誤認しないよう、active ID は registry / implementation queue / frontend queue に存在するものだけとする軽量 lint または `rg` 手順を追加する。
