...
- 実装済みに分類するもの: Dashboard の lightweight summary / segment invalidation / inbound panel / stock-risks / report-billing / urgent主要source / ViewModel / Clock Island、Inbound の正式 schema/API/task/risk、Medication Stock の schema/domain/summary/API/apply 初期版、Movement の tab/DTO/detail resolver、AWS Backup read-only monitor、権限SSOT基本表、care-report non-palette `q` 患者検索の bounded/id-only 化。
...
- 未実装として残すもの: `PERF-DB-006B-D` care-report keyword scan / delivery summary / index gate、`PAYLOAD-BUDGET-003`、`API-CONTRACT-*`、`DB-EVENT-001`、`FILE-LIFE-001`、`TENANT-SUPPORT-001`、7画面UIの実装slice、live AWS human gate。
...
| PERF-DB-006         | P1     | DB速度/検索   | `/api/care-reports`, Prisma schema indexes                                                           | `PERF-DB-006A` は完了。残は keyword scan、delivery summary全体集計、pg_trgm/composite index 評価へ分離する。候補index: `CareReport(org_id, created_at DESC, id DESC)`, `CareReport(org_id, patient_id, created_at DESC, id DESC)`, `CareReport(org_id, status, updated_at DESC, id)`, `DeliveryRecord(report_id, created_at DESC, id DESC)` または tenant-safe variant、`Patient.name/name_kana` の pg_trgm。 | payload budget、query-shape tests、SELECT-only EXPLAIN、migration rollback plan before index addition。 |
...
1. `PERF-DB-006B/006C/006D` keyword scan / delivery summary / index候補を EXPLAIN-backed backlog へ分離。
2. `PAYLOAD-BUDGET-003` `/api/care-reports` list/search payload budget。
