...
| Bucket                   | Count | 入口                                                                                                    |
| ------------------------ | ----: | ------------------------------------------------------------------------------------------------------- |
| Done / frozen            |    18 | 下の Done 表。active backlog ではなく、回帰防止・docs参照・watchlist対象として扱う。                    |
| Partial / residual track |     8 | 下の Partial 表。既存土台は再作成せず、残スコープだけ implementation queue へ切る。                     |
| Implementation queue     |    30 | `Implementation-ready queue`。次PRに切れる backend/platform/ops/API タスク。Human gate を含む。         |
...
3. `INBOUND-002-REVIEW-DETAIL`: raw detail API、audited detail panel、review lifecycle、MedicationStock適用条件/不可表示は完了。残は source detail layout、actual apply target selector、downstream導線。
4. `MOV-001-API` + `MOV-002-SOURCE-PARITY`: standalone movement API と formal source parity。
5. `FRONTEND-CONTRACT-001`: 7画面UI改善を entrypoint/BFF/state/mobile/validation の契約に落とす。
6. `QUERY-SHAPE-WATCHLIST-003-FOLLOW`: DB read-speed follow-up を zero-debt watchlist で維持する。
7. `OPS-RECOVERY-LIVE-001`: AWS live restore evidence は credential/承認が揃うまで human gate。
...
| Inbound core               | `InboundCommunicationEvent` / `InboundCommunicationSignal` schema、RLS、phone/MCS登録、inbox、signal materialize、task bridge、risk bridge、MedicationStock accepted-signal apply。                                                                                                                                                                                                                                                                                                                                                                                                                             | 正本DB/APIは固定。残は 3カラム review shell、review lifecycle UI、FAX/email/manual、source mapping UI、VisitBrief/Schedule/Report/Share。                                           |
| Inbound raw detail API/UI  | `GET /api/communications/inbound/:id/detail` で raw_text / 送信者詳細を `purpose`、coded `read_reason`、`request_id`、再認可、read audit、no-store 経由に限定。`/communications/inbound` の選択中 review panel に明示ボタン「原文を監査付きで表示」を追加し、押下後だけ detail API を取得する。dashboard inbound / urgent / stock-risk DTO は raw_text、sender_contact、signal extracted_text を返さない。候補カードには作業ライフサイクルと MedicationStock 適用条件/台帳直書き不可を表示し、`target_stock_item_id` と観測値の明示入力なしに `apply_to_medication_stock` を呼ばないことを UI test で固定した。 | 再実装しない。残は source detail layout、actual MedicationStock apply target selector、FAX/email/manual/source mapping UI。                                                         |
...
| `INB-001/002`   | Formal inbound schema/API/inbox/signals/task/risk/stock apply、raw detail再認可API/read audit、audited detail panel、dashboard raw omission、review lifecycle UI、MedicationStock適用条件/不可表示。                                              | source detail layout、actual MedicationStock apply target selector、FAX/email/manual、source mapping UI、VisitBrief/Schedule/Report/Share。                                                               |
...
| ---------------------------------- | ----------- | -------- | ----------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `INBOUND-002-REVIEW-DETAIL`        | Partial     | P0/P1    | Inbound UI/API    | raw detail API、既存 review panel の audited raw表示、候補ごとの review lifecycle、MedicationStock 適用条件/台帳直書き不可の UI は完了。残は source detail layout、actual apply target selector、FAX/email/manual 取込、source mapping、VisitBrief/Schedule/Report/Share 連動。raw_textは既存 detail API とread audit経由に限定。 | raw omission snapshots、review lifecycle、MedicationStock apply allowed/forbidden tests。raw_textをlist/通知/監査changesへ流す、または `target_stock_item_id` なしに apply UI を出すなら停止。 |
...
| ------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `INBOUND-002-REVIEW-DETAIL`    | raw detail API、既存 review panel の audited raw表示、review action lifecycle、MedicationStock apply forbidden/conditions は完了。次は actual apply target selector と source detail layout を、正本API/DTOが揃う範囲だけで閉じる。                                                                                                                             | Inbound schema、phone/MCS登録、inbox、signal materialize、task/risk bridge、MedicationStock apply、raw detail API、audited detail panel、review lifecycle UI。 | list DTO raw omission snapshot、MedicationStock apply allowed/forbidden、`target_stock_item_id` と観測値がない状態で apply しないこと。                                 |
...
3. `INBOUND-002-REVIEW-DETAIL`: source detail layout、actual apply target selector、FAX/email/manual/source mapping/downstream 連動。
