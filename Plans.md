...
- 今回の追加照合で未実装に分類するもの: dashboard segment の payload budget は `summary` のみ登録済みで、`details` / `inbound` / `stock-risks` / `report-billing` / `comments` / `team` は未登録。`withRoutePerformance` は `Content-Length` がないと payload を `unmeasured` とするため、既存 `success()` route は shared measured JSON helper へ寄せる必要がある。Movement timeline、Inbound inbox/signals、MedicationStock summary も payload-budgeted critical read path として未実装。
...
1. `PAYLOAD-BUDGET-001A` dashboard segment routes を payload-budgeted + measured response にする。低リスクで UI/DB 変更前の計測土台になる。
2. `PERF-DB-006D` index候補を SELECT-only EXPLAIN-backed backlog へ分離する。migration は human gate のまま。
3. `PAYLOAD-BUDGET-001B/C` movement / inbound / stock list DTO の payload-size smoke と PHI omission snapshot。
...
| `PAYLOAD-BUDGET-001A`    | 未実装     | P1     | API perf          | dashboard cockpit segments, `withRoutePerformance`, `route-payload-budgets`              | `GET /api/dashboard/cockpit/{details,inbound,stock-risks,report-billing,comments,team}` を critical payload budget registry に追加し、dashboard segment response が measured JSON success helper で `Content-Length` を返すようにする。DTO内容は変えず、認証済み業務画面の権限内表示は維持する。                                                                                                | route budget unit tests、dashboard segment `Content-Length` tests、既存 raw/provider-error omission tests。DTOを理由なく削る設計なら停止。         |
| `PAYLOAD-BUDGET-001B`    | 未実装     | P1     | Patient timeline  | `/api/patients/:id/timeline`, movement DTO, safe resolver                                | Movement timeline list を payload-budgeted route にする。`withRoutePerformance` と measured JSON を追加し、`limit` / `generated_at` / `partial_failures` / `next_cursor` など list metadata を維持する。詳細本文、SOAP、OCR、raw chat text は timeline list に出さず、detail resolver へ deep link する。                                                                                       | movement route tests、payload budget tests、raw omission snapshot、relative href tests。detail routeをlist payloadへ混ぜる設計なら停止。           |
| `PAYLOAD-BUDGET-001C`    | 未実装     | P1     | Inbound/stock API | `/api/communications/inbound`, `/api/communications/inbound/signals`, stock summary      | Inbound inbox/signals と MedicationStock summary の list/summary DTO に `visible_count` / `hidden_count` / `generated_at` / `partial_failures` 相当を揃える。raw_text、storage key、signed URL、provider raw error、分類用 raw payload は出さない。権限内 detail surface では本文・連絡先・添付を再認可後に表示できる。                                                                         | inbound/stock DTO snapshot、payload size smoke、forbidden tests、raw/detail separation tests。raw_textをlistやnotificationへ出す必要が出たら停止。 |
...
- `PERF-DB-006D-INDEX`: index追加は human gate。まず SELECT-only EXPLAIN と候補indexごとの rollback plan を evidence として残し、blind migration は作らない。
