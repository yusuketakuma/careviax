...
| Recovery / AWS base      | AWS Backup/RDS read-only monitor、S3 Object Lock read-only monitor、strict skipped-check degradation、template validator、structured redacted drill evidence、SELECT-only restored DB integrity audit。  | runtime restore APIは作らない。残は root runbook cleanup、live AWS drill evidence。                                           |
...
1. `OPS-RECOVERY-DOC-001`: root runbook を compliance SSOT と整合させ、least-privilege / non-destructive / synthetic-drill-only へ整理する。
2. `OPS-RECOVERY-LIVE-001`: human gate として本番相当roleで `--live-aws --strict`、admin health実AWS確認、restore drill evidenceを収集する。
3. `DASH-P1-010-RAIL` + `DASH-P1-005-LINKS`: Dashboard Summary Rail と drilldown。既存BFFを再作成しない。
4. `INBOUND-002-REVIEW`: 3カラムreview shell + raw再認可detail。
5. `STOCK-001-PRESCRIPTION`: 処方供給adapter。YJ/HOT/GS1候補と低confidence review。
6. `MOV-001-API`: standalone movement API と formal source。
7. `FRONTEND-CONTRACT-001`: 7画面UI改善のslice contract / state matrix。
8. `PLANS-ACTIVE-LINT-001` + `PLAN-ARCHIVE-001`: reference spec と active backlog の分離を継続。
...
| Recovery / AWS      | `OPS-RECOVERY-DOC-001` → `OPS-RECOVERY-LIVE-001`    | `OPS-RECOVERY-EVIDENCE-001`、`OPS-RECOVERY-INTEGRITY-001`、`OPS-RECOVERY-MONITOR-003` は実装済み。次は root runbook cleanup、その後 human gate として live AWS strict validation / restore drill evidence を収集する。 | backup-monitor regression、`backup:drill:check`、template validator、unsafe phrase rg。                         | runtime restore API、AWS destructive permission、PHI/ARN/endpoint/storage keyを出力する設計。                   |
