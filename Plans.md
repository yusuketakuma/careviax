...
5. `INBOUND-002-REVIEW-DETAIL`: 3カラム review shell と raw detail 再認可。
6. `MOV-001-API` + `MOV-002-SOURCE-PARITY`: standalone movement API と formal source parity。
7. `FRONTEND-CONTRACT-001`: 7画面UI改善を entrypoint/BFF/state/mobile/validation の契約に落とす。
8. `QUERY-SHAPE-WATCHLIST-003-FOLLOW`: DB read-speed follow-up を zero-debt watchlist で維持する。
9. `OPS-RECOVERY-LIVE-001`: AWS live restore evidence は credential/承認が揃うまで human gate。
...
| Stock visit API          | `POST /api/visit-records/:id/medication-stock-observations`、`applyVisitMedicationStockObservations`、idempotency/replay/409 conflict、route auth/rate-limit/catalog/display-id/boundary tests。                                                                     | 再実装しない。残は DB migration適用 gate、DB integration/e2e evidence、訪問UI、risk/task/brief/schedule/movement連動。        |
| Stock visit forecast     | 訪問観測後の snapshot 再計算で Asia/Tokyo civil date と active future `VisitSchedule` 由来の次回訪問日を使い、次回訪問前不足を `shortage_expected` に分類する。                                                                                                      | 再実装しない。次回処方/補充予定horizonは `STOCK-001-PRESCRIPTION-HORIZON` で正本sourceを決めてから接続する。                  |
| Patient Movement base    | `movement` tab、共通DTO、safe detail resolver、処方/訪問/文書 occurrence marker、timeline list payload budget、standalone `/api/patients/:id/movement-timeline` 初期alias。                                                                                          | 残は cursor/date/category filter、map-less date card UX、formal inbound/stock/safety sources、deep link coverage。            |
...
| `MOV-001`       | Movement tab/DTO/resolver/occurrence marker、standalone movement-timeline 初期alias。                                                                                     | cursor/date/category filter、日付カードUX、formal inbound/stock/safety sources、relative href builder、raw再認可detail。                                                                                                                                       |
...
| `STOCK-001-VISIT-UI`               | Not started | P1       | Visit UI          | 訪問記録フォームに外用/頓服/OTC/他院薬の残数入力を追加。前回値、今回値、差分、最終使用日、未確認理由、次回までの不足見込みを大きい入力UIで表示する。正本API完了後に production UI 化する。                                                                         | component/mobile tests、visit form regression。正本APIなしで見た目だけ確定するなら停止。                                                               |
| `STOCK-001-VISIT-DOWNSTREAM`       | Not started | P1       | Pharmacy stock    | 訪問観測後の snapshot/risk を OperationalTask、Case Risk Cockpit、VisitBrief、Schedule候補、Patient Movement へ接続する。task metadata は ids/status/code のみで、患者名・薬剤名・raw reason は入れない。                                                          | risk-task bridge tests、VisitBrief/Schedule tests、PHI metadata snapshot。ledger更新だけで業務follow-upが出ないなら停止。                              |
| `STOCK-001-PRESCRIPTION-HORIZON`   | Not started | P1       | Pharmacy stock    | 次回処方/補充予定を stockout forecast の horizon として使う正本sourceを決める。候補は `MedicationCycle`、処方明細の交付日/日数、確認済み補充予定task、または薬局側の確定 schedule。単位変換、名寄せ、他院薬/OTCを含めて、false negative を作らない条件で設計する。 | Oracle/DB review、unit/integration tests。`refill_request` や自由記述だけで補充予定日を作るなら停止。                                                  |
...
| `MOV-001-API`                      | Partial     | P1       | Patient detail    | standalone `/api/patients/:id/movement-timeline` 初期aliasは実装済み。残は cursor/date/category filter、map-less date card UX、formal inbound/stock/safety sources。処方・訪問・文書は occurrence marker + deep linkのみ。                                         | movement API tests、payload budget、raw omission、mobile/a11y。SOAP/OCR/raw chat/薬剤明細全文を一覧に出すなら停止。                                    |
...
| -------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `INBOUND-002-REVIEW-DETAIL`      | `review detail resolver` + `3カラム shell` を分割。先に detail API/drawer で raw_text 再認可、purpose、read audit、request_id を固定し、その後 review shell へ接続する。           | Inbound schema、phone/MCS登録、inbox、signal materialize、task/risk bridge、MedicationStock apply。 | list DTO から raw_text が出ない snapshot、detail route forbidden、read audit write、review action lifecycle、MedicationStock apply allowed/forbidden。 |
| `STOCK-001-VISIT-UI`             | `VisitMedicationStockObservationPanel` を visit record form の section として追加。API未接続状態は disabled/review待ちで表現し、migration gate通過後に write有効化する。           | visit observation API、idempotency、snapshot forecast、context sidecar。                            | mobile 44px tap target、offline/conflict/idempotency key、未確認理由、前回値/今回値/差分、次回訪問前不足表示、保存失敗時の false success 防止。        |
| `STOCK-001-VISIT-DOWNSTREAM`     | ledger write後の fan-out を `risk/task` → `VisitBrief` → `Schedule` → `Movement` の順に分割する。各PRで payload PHI boundary を固定する。                                          | append-only event、snapshot、次回訪問/JST forecast。                                                | risk finding/provider tests、task dedupe metadata snapshot、visit brief ordering、schedule candidate reason、movement occurrence event。               |
| `STOCK-001-PRESCRIPTION-HORIZON` | 処方/補充horizon の正本sourceを design doc + focused DB read proof で決める。`MedicationCycle`、処方交付日/日数、確認済み補充task、確定scheduleを比較する。                        | refill_request自由記述からの予定日推定。                                                            | Oracle/DB review、false negative scenarios、unit conversion fallback、OTC/他院薬 handling、horizon unknown時の risk level。                            |
| `MOV-001-API`                    | `GET /api/patients/:id/movement-timeline` を既存 timeline registry から切り出す。まず occurrence-only source と cursor/date/category filter、その後 formal inbound/stock sources。 | movement tab、共通DTO、safe detail resolver、処方/訪問/文書 occurrence marker。                     | raw omission、relative href、partial failure、category filter、payload budget、mobile date-card rendering。                                            |
...
| `FRONTEND-CONTRACT-001`          | 7画面の entrypoint/BFF/state/mobile/PHI boundary を1枚の contract にする。実装前に画面ごとの exact files、existing tests、missing states、paired backend task を固定する。         | AppShell、Sidebar、WorkspaceActionRail、Movement/Inbound/Stockの既存土台。                          | `docs/ui-ux-design-guidelines.md` 適合、権限内表示方針、right rail action、state matrix、imagegen省略/実施理由、exact-path validation list。           |
...
- `QUERY-SHAPE-WATCHLIST-003F`: patient timeline list/detail API と single visit preparation brief API を query-shape watchlist に追加済み。これらの route shell は直接DBを広く読まず、今後 direct `findMany` / broad `include` / aggregate fan-out が混入したら `pnpm db:query-shape:check` で止める。allowlist debt は 0 のまま維持。
...
- `MOV-001-API-PATH-001`: `GET /api/patients/:id/movement-timeline` を既存 scoped timeline handler の shared facade として追加済み。旧 `/api/patients/:id/timeline` は互換aliasとして残し、UI fetch path、PHI read audit view、payload budget、read-path SLO、query-shape watchlist、rate-limit shared bucket、route testsで固定した。残る cursor/date/category/include と formal source parity は親 `MOV-001-API` / `MOV-002-SOURCE-PARITY` で扱う。
...
- `STOCK-VISIT-DOWNSTREAM-001`: visit observation API 成功後の snapshot/risk/task/brief/schedule/movement fan-outを段階実装に分け、ledger writeだけで業務follow-upが出ない状態を検出する。
...
- `FRONTEND-ENTRYPOINT-MAP-001`: 7画面UI改善の前に、各画面の route、primary component、BFF/API、shared component、existing tests、missing state を1枚の表にする。`FRONTEND-CONTRACT-001` の最初の成果物として扱う。
...
5. `INBOUND-002-REVIEW-DETAIL`: 3カラムreview shell + raw再認可detail。
6. `MOV-001-API` + `MOV-002-SOURCE-PARITY`: movement API と formal source。
7. `FRONTEND-CONTRACT-001`: 7画面UI改善のslice contract / state matrix。
8. `QUERY-SHAPE-WATCHLIST-003-FOLLOW`: query-shape対象拡張とzero-debt維持。
9. `PERF-DB-006D-INDEX`: SELECT-only EXPLAIN artifact と rollback plan の実環境証跡が揃ってから human gate へ進める。
...
| Patient Movement    | `MOV-001-API`                                          | standalone API 初期aliasは固定済み。残は cursor/date/category filter、map-less date card UX、formal inbound/stock/safety source、処方/訪問/文書 occurrence marker + deep link を整える。                                                             | movement API tests、relative href tests、raw omission snapshot、mobile/a11y tests。                                                       | 一覧にSOAP本文、OCR全文、raw chat text、薬剤明細全文を出す必要が出たら停止。                                    |
...
| MOV-STATUS-001             | P0/P1  | Movement                   | movement tab、timeline DTO、safe resolver、standalone API 初期alias                                  | `PatientMovementTimeline` 土台は再実装しない。残は cursor/date/category filter、map-less date card UX、formal inbound/stock/safety sources、relative href builder、raw detail再認可。処方・訪問・文書は occurrence marker + deep link のまま維持する。                                                                                                                                                                                                                                                                                                                        | movement API tests、raw omission snapshot、relative href tests、mobile/a11y tests。                                                                               |
...
| `MOV-001-API`                | Partial    | P1     | Patient detail          | movement tab, timeline routes, safe resolver, standalone API 初期alias                   | `/api/patients/:id/movement-timeline` 初期aliasは実装済み。残は cursor/date/category filter、map-less date card UX、formal inbound/stock/safety sources、処方・訪問・文書 occurrence marker + deep link を返す。本文、薬剤明細、SOAP、OCR、raw text は一覧に載せない。                                                                                                                                                                                                                                            | movement API tests、relative href tests、raw omission snapshot、mobile/a11y tests。event detail shellを全イベントのprimaryにする設計なら停止。                                                                  |
...
> 残: 処方供給follow-up（manual retry API、DrugPackage/GS1数量換算、review taskからのstock item作成/適用導線）、訪問時観測API/UI、usage-delta / frequency / refill request、equivalence review UI、既存 `ResidualMedication` からの段階移行、VisitBrief/Schedule/Report/External Share 接続、正式 `RiskFindingProvider` の完全統合。
> 2026-07-08 追加レビュー: 現行 `MedicationStockEvent` は `event_at`、残数、使用頻度を持つが、PRN/外用薬で重要な `last_used_at` と「未確認理由」を永続化する専用fieldがない。訪問観測APIを公開する前に、controlled observation context を追加する。`event_at` を最終使用日や未確認理由の代替にしない。
...
| `MedicationStockEvent`               | 実装済み | 残数変動・観測の append-only 台帳。削除ではなく補正 event で訂正する。                                 | `prescription_supply` v1 は実装済み。残は manual retry/API化、DrugPackage/GS1数量換算、`visit_observation`、`usage_delta`、`usage_frequency_update`、`refill_request`、`disposal/correction` の service/API を追加する。                |
| `MedicationStockObservationContext`  | Partial  | 訪問時観測に必要な `last_used_at`、`unobserved_reason_code`、controlled source context を保持する。    | `STOCK-001-VISIT-CONTEXT` で `MedicationStockEvent` 1:1 sidecar と migration candidate を追加済み。free-text reason は v1 で保存せず、migration適用は human gate。                                                                      |
...
| `POST /api/visit-records/:id/medication-stock-observations`    | 訪問記録由来の残数観測、使用量、使用頻度、未確認、補充希望を append-only event として追加する。v1の主endpoint。 |
...
| Phase 4a | Visit observation context contract。`last_used_at`、`unobserved_reason_code`、controlled source context、audit metadata、migration rollback を固定。                           | 未実装。DB contract test、DTO omission test、Oracle/DB/medical safety review。migration適用はhuman gate。                                         |
| Phase 4b | VisitRecord observation API。`POST /api/visit-records/:id/medication-stock-observations` で `source_entity_type='visit_record'` の append-only event を作り、snapshot 再計算。 | 未実装。route/service/idempotency/no-store/RLS tests、same-key replay、different-fingerprint 409、unit mismatch reject。                          |
| Phase 4c | VisitRecord UI adapter。既存 residual input から安全に `visit_observation` / `usage_delta` / `usage_frequency` / `not_observed` を送る。                                       | 未実装。visit form/mobile tests、legacy residual compatibility。正本APIなしでUI確定しない。                                                       |
