...
| MOD-CI-001      | P1     | `API-DTO-001`, `DB-TENANT-001`, `DEV-PHI-001`, `PLAN-REV-001`             | Module/debt ratchet CI gates         | `boundaries:check` は allowlist 0 を通常CIで強制済み。残: `api-response-shape:check`、`dto-direct-prisma-return:check`、`task-type-registry:check`、`risk-adapter-cross-reference:test`、`collaboration-adapter-contract:test`、`rls-policy-contract` との役割整理。               | 各MOD PRは「module境界化 + DEBT削減 + DTO/presenter明確化 + 境界テスト + 回帰テスト」を含む。薬局E2E/主要API回帰を維持。module-boundary allowlist は0を維持し、例外確認は `PHOS_ALLOW_MODULE_BOUNDARY_DEBT=1` のaudit runに限定。                               |
