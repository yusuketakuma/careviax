...
- 追加照合: `src/lib/utils/route-payload-budgets.ts` には `/api/care-reports/today-workspace` の budget はあるが、通常 list/search の `/api/care-reports` budget は未登録。`PAYLOAD-BUDGET-003` は未実装として残す。
...
- `CARE-REPORT-SEARCH-TEST-001`: `PERF-DB-006A/B/C` の red tests は実装済み。残は `PERF-DB-006D` の SELECT-only EXPLAIN 証跡と、`PAYLOAD-BUDGET-003` の payload肥大検知に限定する。
...
| `PAYLOAD-BUDGET-003`     | 未実装     | P1     | API payload/PHI   | `/api/care-reports`, `route-payload-budgets.ts`, care-report route tests                 | `/api/care-reports` list/search 用 budget を追加する。summary/list rows は `delivery_records` 配列、delivery raw recipient detail、`pdf_url`、storage/signed URL、hidden report content、search-only helper fieldsを返さない。`include_content=1` は本文ではなく既存 summary contract に限定し、payload budget の対象・例外を明示する。                                                         | `resolveRoutePayloadBudget` unit、care-report route snapshot、payload byte smoke、PHI/URL omission snapshot。print/exportで詳細が必要な場合は detail/print-audit APIへ分離し、list payloadを戻す設計なら停止。 |
...
- `PAYLOAD-BUDGET-003`: `src/lib/utils/route-payload-budgets.ts` に `/api/care-reports` list/search budget を追加する。unit budget と production-like perf smoke は分ける。`delivery_records` 配列、delivery raw recipient detail、`pdf_url`、storage/signed URL、hidden report content、`_searchable_report_text` を list/search response に戻したら test が落ちるようにする。
