...
| AWS Backup | RDS/AWS Backup baseline、recovery point monitor、Restore Testing metadata、IaC validation、read-only assurance health hardening。 | `9a62e12c2`, `0d2f025a4`, `src/server/services/backup-monitor.ts`, `src/app/api/health/route.ts`。                                        | 残は live AWS strict validation と live drill evidence。         |
...
| OPS-RECOVERY-002   | P0/P1  | AWS復旧   | AWS Backup Assurance Monitor Hardening            | 済: `DescribeBackupVault`、RDS `DescribeDBInstances` / automated backup / deletion protection / backup retention の read-only health monitor、`/api/health` backup details sanitizer、leakage regression tests、IAM allowlist / runbook 更新、restore/delete/secret-write 禁止 action の template validator 補強。残: `--live-aws --strict` と本番相当 role での AccessDenied 確認、live drill evidence。runtime `StartRestoreJob` API は作らない。 |
...
3. `PERF-DB-001` read-path performance inventory。
4. `INBOUND-STATUS-001` raw detail / source mapping / review shell。
5. `STOCK-STATUS-001` prescription supply / visit observation / stockout provider。
