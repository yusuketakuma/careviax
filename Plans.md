...
> 根拠は 2026-07-08 時点の `git log --oneline -30`、現行コード検索、既存テスト、`docs/compliance/access-control-policy.md`。

**ステータス定義**:

| 区分       | 判定基準                                                                 | Plans.md での扱い                                                           |
| ---------- | ------------------------------------------------------------------------ | --------------------------------------------------------------------------- |
| 実装済み   | code / route / schema / test / commit evidence がある。                  | 再実装しない。残タスクがあれば「一部実装済み」へ分離する。                  |
| 一部実装済 | 正本または初期BFFはあるが、UI、downstream、権限test、運用runbookが残る。 | 完了部分と残部分を同じ行で明記し、残は具体的PRへ切る。                      |
| 未実装     | 永続コードパス、schema、API、UI、test のいずれも確認できない。           | Active backlog として受入条件、validation、依存タスクを追加する。           |
...
| Inbound    | 正式 `InboundCommunicationEvent` / `InboundCommunicationSignal` schema、RLS、inbox、task bridge。  | `e024b66c5`, `d994ad693`, `46f2df1a8`, `prisma/schema/communication.prisma`、`src/app/api/communications/inbound/**`。                    | 残は3カラムreview UI、raw detail再認可、FAX/メール/手入力。      |
| Stock      | Medication Stock Ledger schema、append-only event、患者別summary API、inbound signal apply初期版。 | `9a62e12c2`, `52b26755c`, `c5c2ce7cb`, `46f2df1a8`, `src/modules/pharmacy/medication-stock/**`。                                          | 残は処方供給、訪問観測、usage/refill、UI、VisitBrief/Schedule。  |
| Movement   | `PatientMovementTimeline`、共通DTO、detail resolver API の土台。                                   | `1ae268009`, `src/app/(dashboard)/patients/[id]/patient-movement-timeline.tsx`, `src/app/api/patients/[id]/timeline/[eventId]/route.ts`。 | 残は standalone API、日付UI仕上げ、全source parity。             |
| AWS Backup | RDS/AWS Backup baseline、recovery point monitor、Restore Testing metadata、IaC validation。        | `9a62e12c2`, `0d2f025a4`。                                                                                                                | 残は read-only assurance hardening と live drill。               |
...
| ID                 | 優先度 | 分類      | タスク                                            | 拡充Plan / 受入条件                                                                                                                                                                                                                                                                                                                  |
| ------------------ | ------ | --------- | ------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| PLANS-HYGIENE-001  | P0     | 計画運用  | Plan hygiene ratchet                              | 新規Plan行には status、owner lane、根拠ファイル、依存、acceptance、validation を持たせる。実装済み項目はこの registry へ移し、同じIDの重複TODOを作らない。月次または主要PR後に `git log` と `rg` で status を更新する。                                                                                                              |
| PERF-DB-001        | P0/P1  | DB速度    | Read-path performance inventory and index backlog | 患者一覧、患者詳細、dashboard segments、movement timeline、inbound queue、medication-stock summary、report workspace の read path を棚卸しする。`EXPLAIN` 可能なquery、payload size、N+1、unbounded `findMany`、重複fetch、RLS条件を記録し、既存 index で足りない箇所だけ migration 案にする。blind index 追加は禁止。               |
| OPS-RECOVERY-002   | P0/P1  | AWS復旧   | AWS Backup Assurance Monitor Hardening            | 実装済み recovery point monitor に加え、read-only で `DescribeBackupVault`、RDS `DescribeDBInstances` / automated backup / deletion protection / backup retention を確認する。health response は arn/account/raw AWS error を出さず、IAM allowlist と docs/compliance runbook を更新する。runtime `StartRestoreJob` API は作らない。 |
| PERM-DOC-001       | P0/P1  | 権限SSOT  | Account Role Capability Matrix sync               | `docs/compliance/access-control-policy.md` と `permission-matrix.ts`、route tests、RLS proof を同期する。新規 account 種別（フリーランス薬剤師、PH-OS運営者support mode、外部連携者、施設/家族）を導入するPRでは、capability表、scope、consent、support_session、audit requirement、forbidden test を同時更新する。                  |
| DASH-STATUS-001    | P0/P1  | Dashboard | Dashboard backlog burn-down                       | `DASH-P0-005` は実装済みに移し、残P0は Clock Island / render boundary、P1は Summary Rail / process clickable / drilldown / role layout、P2は density / visual regression に整理する。既存 `stock-risks` / `report-billing` API は再作成しない。                                                                                      |
| INBOUND-STATUS-001 | P0/P1  | Inbound   | Inbound remaining surface split                   | 正本DB/API/Task/Risk/MedicationStock apply は実装済みとして固定する。残は review detail shell、raw_text再認可、FAX/email/manual source、source mapping UI、VisitBrief/Schedule/Report/Share downstream に分割する。                                                                                                                  |
| STOCK-STATUS-001   | P0/P1  | Stock     | MedicationStock remaining write/read split        | DB/schema/summary/apply初期版は再実装しない。残は prescription supply adapter、visit observation UI/API、usage-delta/frequency/refill request、equivalence review UI、stockout provider、backfill、VisitBrief/Schedule/Report/Share連動に分割する。                                                                                  |
| UI-STATUS-001      | P1     | Frontend  | 7画面UI redesign readiness tracking               | `UI-REDESIGN-001` は見た目の全面作り直しではなく既存 current path の上書き改善。各画面は paired backend task、DTO readiness、権限内表示方針、raw/detail境界、mobile/a11y、validation を持つ slice contract にしてから着手する。                                                                                                      |

**次に着手する推奨順**:

1. `DASH-P0-004` Dashboard Clock Island / render boundary。
2. `DASH-P1-010` Dashboard Summary Rail と drilldown / process clickable。
3. `OPS-RECOVERY-002` AWS Backup Assurance Monitor Hardening。
4. `PERF-DB-001` read-path performance inventory。
5. `INBOUND-STATUS-001` raw detail / source mapping / review shell。
...
| OPS-RECOVERY-002 | P0/P1  | 復旧/BCP         | AWS Backup Assurance Monitor Hardening   | AWS Backup vault、RDS instance backup settings、admin health、docs/compliance、IAM allowlist                              | `DescribeBackupVault`、`ListRecoveryPointsByBackupVault`、RDS `DescribeDBInstances` / backup retention / deletion protection / automated backup を read-only に確認する。health response、log、audit には AWS account id、raw ARN、provider raw error を出さない。runtime restore API は作らず、復元は runbook/live drill で実施する。 |
