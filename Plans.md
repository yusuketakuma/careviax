...
個別タスクは `Plans.md` から削除し、残タスクは外部前提条件と Wave 3 以降へ集約する。
...
> `latest acceptance lock 2026-07-07`: 処方・訪問・文書登録 marker の完成判定は、タイムライン上で「処方があった」「訪問があった」「文書登録があった」と日付順に分かり、各 marker の CTA から正本画面で詳細確認できること。Patient Movement Timeline は索引であり、処方内容・訪問内容・文書内容を表示する場所ではない。
...
3. `RX-REG-FACET-001`: prescription intake facet の status 集計を raw aggregate / denormalized contract へ進める。
...
| FE-ERR-001     | P1     | `UX-ERR-001`, `DEV-UI-001`, `PERF-BFF-001`                     | Segment boundary pattern                      | `cc:PARTIAL 2026-07-07` `SegmentLoading`、`SegmentError`、`SegmentStaleBanner`、`SegmentRetryButton`、`SegmentEmptyButNotError` を shared wrapper として追加。既存 `ErrorState` / `EmptyState` / `SkeletonRows` を再利用し、route metadata は query/hash と ID-like segment を丸める。最初の実利用として `/reports` workspace の全体取得失敗表示を `SegmentError` へ移行した。追加 partial として dashboard summary/details/team/comments の初回失敗を `SegmentError`、背景 refetch 失敗を `SegmentStaleBanner` へ移行した。追加 partial として患者詳細の Patient Movement Timeline 取得失敗を `SegmentError`、読み込み中を `SegmentLoading` へ移行し、raw error message を表示しない retryable segment とした。追加 partial として `/visits` today-preparation、`/schedules` calendar、`/schedules` list board の取得失敗を `SegmentError` / `SegmentLoading` へ寄せ、raw `Error.message`、patient name、token、storage key、API route/query を表示しない retryable segment として固定した。追加 partial として shared `GuardedWorkspaceActionRail` の loading/error を `SegmentLoading` / `SegmentError` に統一し、report workspace の action rail と全体取得失敗から raw `workspaceQuery.error.message` detail を除去した。追加 partial として `/tasks` main list 取得失敗を `SegmentError` へ移行し、false-empty と偽0件に倒さず retry できる状態へ統一した。追加 partial として admin alert-rules の signal tuning 取得失敗/読み込みを `SegmentError` / `SegmentLoading` へ移行し、保存済み安全シグナル設定を all-standard に見せない fail-closed 表示へ統一した。追加 partial として admin contact-profiles の送付先一覧/サマリー取得失敗を `SegmentError` / `SegmentLoading` へ移行し、取得失敗時に表示中/未完了/方法未設定を `0件` として見せない fail-closed 表示へ統一した。追加 partial として admin settings の設定値/外部連携監視/店舗一覧取得失敗を `SegmentError`、設定値読み込みを `SegmentLoading` へ移行し、raw query error message を表示しない retryable state へ統一した。追加 partial として admin UAT の launch dossier / summary / org audit / saved feedback / collaborator 取得失敗を `SegmentError`、loading を `SegmentLoading` へ移行し、raw query error message を表示しない retryable state へ統一した。追加 partial として admin realtime の workflow KPI / live workbench / notifications 取得失敗を `SegmentError`、loading を `SegmentLoading` へ移行し、false-empty と raw query error message を表示しない retryable state へ統一した。追加 partial として admin packaging-methods の配薬方法マスター取得失敗を `SegmentError`、pending/loading を `SegmentLoading` へ移行し、未登録 false-empty と raw query error message を表示しない retryable state へ統一した。 | 一部 BFF segment 失敗で画面全体が落ちない。空状態と取得失敗を分離。request_id/route/generated_at/retry_count を必要時表示し、route metadata は ID/query を出さない。`segment-state.test.tsx` は loading/error/stale/empty/retry/route sanitization を固定済み。Dashboard tests は details/team/comments fail-soft と stale retry を固定済み。Patient detail test は movement timeline failure で raw error detail を表示せず再取得できることを固定済み。Visits/schedules tests は today-preparation、calendar、day-board failure が false-empty にならず raw error detail を出さないことを固定済み。Action rail/report workspace tests は unsafe backend detail、patient name、storage key、token、API route が loading/error segment に表示されないことを固定済み。Tasks tests は main list failure が false-empty にならず shared SegmentError + retry になることを固定済み。Alert-rules signal tuning tests は raw backend detail を出さず、panel を all-standard false-default にしないことを固定済み。Settings tests は settings/health/sites の raw backend detail、org id、token、storage key、provider error が表示されず retry できることを固定済み。UAT tests は launch dossier / summary / org audit / saved feedback / collaborator failure で raw backend detail、org id、patient name、storage key、token、provider error が表示されず retry できることを固定済み。Realtime tests は workflow / notifications failure で patient name、storage key、token、provider error、API route を表示せず retry できることを固定済み。Packaging-methods tests は配薬方法マスター取得 failure で patient name、storage key、token、API route を表示せず、未登録 false-empty に倒さないことを固定済み。残: admin screen 群への段階展開。 |
...
FE-ERR-001 追加証跡（2026-07-07）:

- admin `service-areas` の拠点一覧 / 訪問エリア一覧取得失敗を `SegmentError`、訪問エリア pending/loading を `SegmentLoading` へ移行した。tests で patient name、storage key、token、provider error、API route を表示せず、拠点 selector の silent-empty と訪問エリアの false-empty に倒さないことを固定済み。

**推奨 PR / slice 分割**:

1. `FE-RT-001` + `FE-RT-002`: Realtime/SSE の PHI 境界と無駄 refetch を主要画面の source rule へ段階展開する。
...
## Phase 12: インフラ・運用基盤整備 `cc:TODO` <!-- 2026-07-03 再監査: 12-5(Secrets Manager) は旧完了項目のため注記から除外。実残は 12-4(staging 環境) と 12-7 の RUM サブ項目のみ。12-8 は外部依存(AWS 実環境) -->
