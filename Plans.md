...
### 2026-07-08 Active Plan Board v5 — 実装済み / 未実装分類 `cc:ACTIVE`

> この v5 が `Plans.md` の唯一の実装入口。後段の v4/v3、詳細証跡、長大なプロンプト型仕様は `cc:REFERENCE` / `cc:WIP` として読む。
...
> 追加照合: `STOCK-001-VISIT-*` は DB/API/医療安全レビューで、現行 `MedicationStockEvent` だけでは `last_used_at` と未確認理由を永続化できないことを確認した。訪問観測APIは先に controlled observation context を設計する。
...
| `STOCK-001-VISIT-CONTEXT`          | Not started | P0/P1    | Pharmacy stock    | 訪問観測で必要な `event_at/observed_at`、`last_used_at`、`unobserved_reason_code`、source confidence を永続化する contract を先に決める。候補は controlled `MedicationStockEvent.observation_context` または `VisitMedicationStockObservation`。free-text reason は v1 で保存しない。              | DB contract/migration design、DTO omission tests、Oracle/DB review。`event_at` を最終使用日や未確認理由の代用にするなら停止。                            |
| `STOCK-001-VISIT-API`              | Not started | P1       | Pharmacy stock    | `STOCK-001-VISIT-CONTEXT` 後に `POST /api/visit-records/:id/medication-stock-observations` を追加する。訪問記録由来の `observed_absolute`、`usage_delta`、`usage_frequency`、`not_observed`、`refill_request` を append-only event と snapshot 再計算へ接続し、`残り4枚` と `2枚使用` を区別する。 | service/API/idempotency/RLS/no-store/PHI omission tests。direct `ResidualMedication` write、inbound apply の流用、reviewなし他職種signal直書きなら停止。 |
...
- `PLANS-ACTIVE-LINT-001`: `Plans.md` の active ID はこの `Active Plan Board v5` と implementation/frontend queue に存在するものだけとし、`cc:REFERENCE` / `cc:WIP` の未チェックboxを backlog 件数に数えない軽量lintまたは `rg` 手順を作る。
...
- `STOCK-VISIT-CONTEXT-SCHEMA-001`: `last_used_at`、`unobserved_reason_code`、controlled source context を `MedicationStockEvent` 周辺に永続化する最小設計を作る。free-text reason は保存対象外。migration適用は human gate。
...
1. `STOCK-001-VISIT-CONTEXT`: PHI/薬剤/DB write を含む訪問観測APIの前提。Oracle / GPT-5.5 Pro または同等の厳格レビューを継続し、migration design は human gate。
2. `STOCK-001-VISIT-API`: context 契約後に idempotent append-only route/service を実装する。
...
| Section                            | 扱い                        | 実装時の読み方                                                                                     |
| ---------------------------------- | --------------------------- | -------------------------------------------------------------------------------------------------- |
| `Active Plan Board v5`             | Active                      | 実装済み / Partial / Not started / Human gate の正。ここを最初に確認する。                         |
...
> この v3 は 2026-07-08 時点の旧分類証跡。現在の実装入口は上の `Active Plan Board v5` のみ。
...
| Archived `Implementation-ready queue` / `Frontend queue` | Archive                     | v3当時の抽出結果。現在は v5 の queue だけを実装入口にする。                |
| `Detailed Status Evidence Archive`                       | Archive                     | 実装済み証跡と旧分類の履歴。active backlog として数えない。                |
| `cc:PARTIAL`                                             | Reference + residual detail | 実装済み土台は再作成せず、残タスクをactive boardの対応IDへ戻してから着手。 |
| `cc:REFERENCE`                                           | Reference only              | 背景、受入条件、停止条件、仕様詳細。直接sprint backlog化しない。           |
| `cc:WIP`                                                 | Program context             | 大きなプログラム背景。実装する場合はactive boardへ小IDで切り出す。         |
...
> 「この registry を入口にする」等の旧文言は当時の履歴として残すが、現在の active 入口は上の `Active Plan Board v5` のみ。
...
> 旧運用ではこの registry を `Plans.md` の入口にしていた。現在の実装判断は `Active Plan Board v5` を優先する。
...
- 旧運用では、この `Plan Status Registry` と下の `Implementation-ready queue` を入口にしていた。現在の実装判断は `Active Plan Board v5` を入口にする。
...
- 旧運用では `Plans.md` 内の active 実装入口をこの registry、`Implementation-ready queue`、`Frontend implementation queue` に限定していた。現在は v5 の queue に限定する。`cc:REFERENCE` 節のチェックボックスは、そのまま sprint backlog として数えない。
...
| Section                         | 扱い                        | 実装時の読み方                                                                     |
| ------------------------------- | --------------------------- | ---------------------------------------------------------------------------------- |
| `Plan Status Registry`          | Archive                     | 旧分類証跡。現在の正は `Active Plan Board v5`。                                    |
| `Implementation-ready queue`    | Archive                     | v2当時の候補。現在は v5 の queue だけを実装入口にする。                            |
| `Frontend implementation queue` | Archive                     | v2当時の候補。現在は v5 の queue だけを実装入口にする。                            |
...
| `VisitMedicationStockObservationContext` または `MedicationStockEvent.observation_context` | 未実装/要設計 | 訪問時観測に必要な `last_used_at`、`unobserved_reason_code`、controlled source context を保持する。    | `STOCK-001-VISIT-CONTEXT` で設計する。free-text reason は v1 で保存せず、controlled enum と audit metadata のみ。migration適用は human gate。                                                                                           |
