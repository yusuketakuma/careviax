...
- 実装済みに分類するもの: Dashboard の lightweight summary / segment invalidation / inbound panel / stock-risks / report-billing / urgent主要source / ViewModel / Clock Island、Inbound の正式 schema/API/task/risk、Medication Stock の schema/domain/summary/API/apply 初期版、Movement の tab/DTO/detail resolver、AWS Backup read-only monitor、権限SSOT基本表。
...
- 未実装として残すもの: `PERF-DB-006` care-report bounded search、`API-CONTRACT-*`、`DB-EVENT-001`、`FILE-LIFE-001`、`TENANT-SUPPORT-001`、7画面UIの実装slice、live AWS human gate。
...
- `care-reports` の現行確認では、`view=palette&q=` は bounded だが、通常list検索の `matchingPatients` は `Patient.name/name_kana contains` に `take` がなく、`keyword` は cursor を拒否して `CARE_REPORT_KEYWORD_SCAN_LIMIT=500` 件を読み application side で本文検索する。delivery summary も page rows ではなく検索 `where` 全体へかかるため、これらを `PERF-DB-006-*` に分解する。
...
| `MOV-001`                | 一部実装済 | `movement` tab、`PatientMovementTimeline`、共通DTO、safe detail resolver、処方/訪問/文書のoccurrence-only土台。                                                                                                                                                              | standalone movement API、map-less date card UX、正式 inbound / stock / safety source、deep link coverage、raw再認可detail。                               |
...
| Movement   | `PatientMovementTimeline`、共通DTO、safe detail resolver API、処方/訪問/文書の occurrence-only 表示土台。                                                                                                                                                               | `1ae268009`, `src/app/(dashboard)/patients/[id]/patient-movement-timeline.tsx`, `src/types/patient-movement-timeline.ts`, `src/app/api/patients/[id]/timeline/[eventId]`。                                                                                                                                   | 残は standalone API、日付UI仕上げ、全source parity。                           |
...
| PERF-DB-006         | P1     | DB速度/検索   | `/api/care-reports`, Prisma schema indexes                                                           | care-report list/search は通常検索の patient name `contains`、keyword scan、delivery summary の全体集計が重い。まず non-palette `matchingPatients` を bounded にし、keyword scan、delivery summary集計、pg_trgm/composite index は分離する。候補index: `CareReport(org_id, created_at DESC, id DESC)`, `CareReport(org_id, patient_id, created_at DESC, id DESC)`, `CareReport(org_id, status, updated_at DESC, id)`, `DeliveryRecord(report_id, created_at DESC, id DESC)` または tenant-safe variant、`Patient.name/name_kana` の pg_trgm。 | route tests for bounded search、payload budget、query-shape tests、SELECT-only EXPLAIN、migration rollback plan before index addition。 |
...
| PAYLOAD-BUDGET-001  | P1     | 派生/性能/PHI | dashboard segments、movement API、patients board、inbound/stock                                      | list/summary DTO は `visible_count` / `hidden_count` / `generated_at` / `partial_failures` を持ち、raw text、storage key、signed URL、provider raw errorを含めない。権限内 detail surface では患者名・薬剤名・本文・連絡先・添付等の判断情報を表示してよいが、summary/list payload は肥大化させず、詳細/preview/drawerへ分ける。                                                                                                                                                                                                              | DTO snapshot、payload size smoke、PHI omission snapshot、forbidden tests。                                                              |

**`PERF-DB-005B` 実装契約 — patient master bounded select follow-up**:

```text
対象:
  - src/app/api/patients/[id]/route.ts::authenticatedGET
  - src/server/services/patient-state-snapshot.ts::findPatientOverviewBase
  - src/server/services/patient-detail.ts::getPatientOverview の戻り contract

やること:
  - root GET と overview base の broad relation include を禁止する。
  - Patient scalar は `PatientOverview` が実際に使う列だけを `select` する。
  - one-to-many relation は必ず `orderBy` + `take` + 最小 `select` を持たせる。
  - relation上限の初期値は、residences 4、contacts 12、conditions 12、consents 8、cases 8、care_team_links/case 12 を上限候補にする。
  - consents は active/revoked 条件と最小 metadata に絞り、document_url / file id を root/overview base へ載せない。
  - 重い timeline / audit / lab / document body は root GET に戻さず、slice API / lazy tab の責務に固定する。

残す互換フィールド:
  - patient: id, name, name_kana, birth_date, gender, phone, medical/care insurance numbers, billing_support_flag, primary/backup pharmacist/staff ids, allergy_info, notes, archived fields, updated_at.
  - residences: id, address, building_id, facility_id, facility_unit_id, unit_name, is_primary.
  - contacts: id, relation, name, phone, email, fax, organization_name, department, address, is_primary, is_emergency_contact, notes.
  - conditions: id, condition_type, name, is_primary, is_active, noted_at, notes.
  - cases: id, display_id, status, referral/start/end fields, primary/backup pharmacist ids, notes, created_at, updated_at, required_visit_support.
  - care_team_links: id, external_professional_id, role, name, organization_name, department, phone, email, fax, address, is_primary, notes.

禁止:
  - `residences: true`, `contacts: true`, `consents: true`, `cases: { include: ... }`, `care_team_links: true`。
  - root GET で timeline source / audit history / actor name resolution を復活させる。
  - API互換のために `...patient` へDB列を無制限に混ぜる。

tests:
  - route test: `patient.findFirst` が broad `include` を持たず、relationごとの `take/select/orderBy` を持つこと。
  - service test: `findPatientOverviewBase` が broad relation include を持たないこと。
  - UI/service regression: contacts、foundation、share-case selector、intake summary、conditions、care team の既存表示が欠けないこと。
  - payload budget smoke: root GETの relation row数と余剰fieldが増えないこと。
```
...
- `PERF-DB-006A`: care-report list/search の non-palette patient search を bounded にする。palette search は既に bounded なので再実装しない。
- `PERF-DB-006B`: care-report keyword scan は現行 `CARE_REPORT_KEYWORD_SCAN_LIMIT=500` の application-side scan なので、payload/query-shape smoke と将来 search index 設計へ分離する。
- `PERF-DB-006C`: care-report delivery summary は list where を `DeliveryRecord` aggregate/groupBy へ再利用するため、検索whereが重い時の影響を別途 EXPLAIN で確認する。
- `PERF-DB-006D`: care-report index backlog は blind migration ではなく SELECT-only EXPLAIN と rollback plan 後に human gate へ送る。
- `CARE-REPORT-SEARCH-TEST-001`: `PERF-DB-006A-D` の前に red test を置く。unbounded `patient.findMany`、truthful truncation metadata、keyword plus-one scan、delivery list payload肥大、payload budget を検知する。
- `PAYLOAD-BUDGET-003`: `/api/care-reports` list/search payload budget を追加し、summary/list rows に `delivery_records` 配列や delivery raw detail を戻さない。
...
1. `PERF-DB-006A` care-report non-palette patient search bounded 化。
2. `PERF-DB-006B/006C/006D` keyword scan / delivery summary / index候補を EXPLAIN-backed backlog へ分離。
3. `PAYLOAD-BUDGET-001` movement / dashboard / inbound list DTO の payload-size smoke と PHI omission snapshot。
4. `DASH-P1-010` Dashboard Summary Rail と process tile / drilldown。
5. `FRONTEND-CONTRACT-001` 7画面UI改善の slice contract / state matrix 整理。
6. `PLANS-HYGIENE-002` 長大な reference spec の段階移管。

**Implementation-ready queue — 未実装 / Partial の拡充Plan（入口）**:

> 下表は、後段の詳細仕様から「次PRに切れる粒度」だけを抽出した実装キュー。<br>
> 実装済みの再作成、巨大UI一括変更、blind index追加、migration適用、AWS live操作はここから除外する。

| ID                       | Status     | 優先度 | Owner lane        | 依存 / 現行足場                                                                          | 実装Plan / DoD                                                                                                                                                                                                                                                                                                                                                                                  | validation / stopping condition                                                                                                                |
| ------------------------ | ---------- | ------ | ----------------- | ---------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| `PERF-DB-006A-SEARCH`    | 未実装     | P1     | DB/API search     | `src/app/api/care-reports/route.ts`, `route.test.ts`, patient search                     | 通常list検索の `matchingPatients` に route-local cap を入れる。`view=palette` は既に `take: resolvedPaletteLimit + 1` なので触らない。`patient_id` 明示時の intersection と `buildCareReportAccessWhere()` による assignment/org scope を維持する。                                                                                                                                             | red/green route testsで `patient.findMany` の `take/select/orderBy` を検査。response contract を変えず、migration不要で完了する。              |
| `PERF-DB-006B-KEYWORD`   | 未実装     | P1/P2  | DB/API search     | `CARE_REPORT_KEYWORD_SCAN_LIMIT`, content JSON, `canUseDbPagination=false`               | keyword本文検索は現行500件scanを即拡大しない。まず payload/query-shape smoke と user-facing truncation方針を固定し、将来 `SearchIndex` / tsvector / trigram へ寄せる設計に分離する。                                                                                                                                                                                                            | keyword + cursor rejection test維持、content selectの余剰field snapshot、payload smoke。本文検索を無制限DB scanへ広げる必要が出たら停止。      |
| `PERF-DB-006C-DELIVERY`  | 未実装     | P1/P2  | DB/API aggregate  | `buildDeliverySummaryForWhere`, `DeliveryRecord`, careReport where                       | delivery summary の `count/groupBy/findMany` が search where を引き継ぐため、bounded search後に query count / EXPLAIN を取り、`DeliveryRecord` 側の tenant-first/report/status index候補を評価する。                                                                                                                                                                                            | SELECT-only EXPLAIN、route tests。summaryを消してUI偽装する案、blind index追加、migration適用が必要なら human gate。                           |
| `PERF-DB-006D-INDEX`     | Human gate | P1/P2  | DB migration plan | Prisma schema indexes, Postgres EXPLAIN                                                  | `CareReport(org_id, created_at DESC, id DESC)`、`CareReport(org_id, patient_id, created_at DESC, id DESC)`、`CareReport(org_id, status, updated_at DESC, id)`、`DeliveryRecord(org_id, status, updated_at DESC, id)`、`DeliveryRecord(report_id, created_at DESC, id DESC)` または tenant-safe variant、`Patient.name/name_kana` pg_trgm は EXPLAIN と rollback plan 後に migration候補化する。 | SELECT-only EXPLAIN artifact、migration rollback plan、Oracle/DB review。blind index migration は禁止。                                        |
...
**PERF-DB-006 test contract（実装前 red test）**:

- `PERF-DB-006A-SEARCH`: `src/app/api/care-reports/route.test.ts` に non-palette `q` patient search が bounded であることを固定する test を追加する。期待値は `patient.findMany` が `select: { id: true }`、`take: candidateLimit + 1`、stable `orderBy` を持ち、`careReport.findMany.where.patient_id.in` が cap 内のIDだけを受け取ること。現状は `take` がなく、赤になる。
- `PERF-DB-006A-META`: cap + 1 検出時は `search.patient_matches_truncated`、`count_basis: 'bounded_patient_search'`、または同等の truthful metadata を返す方針を決める。既存 response contract を変えない slice では follow-up task として明示し、UIが完全検索を装わないことをDoDに含める。
- `PERF-DB-006B-KEYWORD`: keyword scan は `limit + 1` / plus-one read で overflow を検出し、cursor拒否と矛盾する `next_cursor` を返さないことを test する。`keyword_scan_limit`、`keyword_scan_truncated`、`count_basis: 'bounded_keyword_scan'` を検討し、本文検索を無制限DB scanへ広げない。
- `PERF-DB-006C-DELIVERY`: regular list rows は delivery summary 中心とし、`delivery_records` 配列、recipient raw detail、failure raw detail を list payload に戻さない。既存consumerを `rg` で確認してから互換削除/縮小を決める。
- `PAYLOAD-BUDGET-003`: `src/lib/utils/route-payload-budgets.ts` に `/api/care-reports` list/search budget を追加する。unit budget と production-like perf smoke は分け、payload肥大検知を次PRの受入条件にする。
...
- `QUERY-SHAPE-TEST-001`: DB速度改善の各PRで、実装前 red test として broad include / unbounded findMany / missing take / missing orderBy を検出する query-shape test を先に置く。
- `PAYLOAD-BUDGET-002`: dashboard / movement / patients board / inbound / stock / reports について list/summary/detail の payload budget を表にし、summary/list では raw text・storage key・signed URL・provider raw error を出さないことを snapshot で固定する。
- `RIGHT-RAIL-ACTION-001`: dashboard、患者一覧、患者詳細、調剤、スケジュール、訪問、報告、他職種の右レール/下部CTAに「次に何をするか」が必ずあることをUI checklist化する。
