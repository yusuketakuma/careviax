...
### 2026-07-08 Active Plan Board v6 — 実装済み / 未実装分類 `cc:ACTIVE`

> この v6 が `Plans.md` の唯一の実装入口。後段の v4/v3、詳細証跡、長大なプロンプト型仕様は `cc:REFERENCE` / `cc:WIP` として読む。
...
> 追加照合: `STOCK-001-VISIT-*` は DB/API/医療安全レビューと Oracle/GPT-5.5 Pro レビューで、現行 `MedicationStockEvent` だけでは `last_used_at` と未確認理由を永続化できないことを確認した。訪問観測APIは `MedicationStockObservationContext` sidecar contract を前提にする。

**現在の分類サマリー**:

| Bucket                   | Count | 入口                                                                                                    |
| ------------------------ | ----: | ------------------------------------------------------------------------------------------------------- |
| Done / frozen            |     8 | 下の Done 表。active backlog ではなく、回帰防止・docs参照・watchlist対象として扱う。                    |
...
1. `STOCK-001-VISIT-API`: sidecar contract を使う idempotent append-only route/service。
2. `STOCK-001-VISIT-FORECAST`: 次回訪問/処方予定と Asia/Tokyo date-key を使う不足予測。
3. `DASH-P1-010-RAIL` + `DASH-P1-005-LINKS`: 既存Dashboard BFFを再作成せず、Summary Rail と drilldown を足す。
4. `INBOUND-002-REVIEW-DETAIL`: 3カラム review shell と raw detail 再認可。
5. `MOV-001-API` + `MOV-002-SOURCE-PARITY`: standalone movement API と formal source parity。
6. `FRONTEND-CONTRACT-001`: 7画面UI改善を entrypoint/BFF/state/mobile/validation の契約に落とす。
7. `QUERY-SHAPE-WATCHLIST-003-FOLLOW`: DB read-speed follow-up を zero-debt watchlist で維持する。
8. `OPS-RECOVERY-LIVE-001`: AWS live restore evidence は credential/承認が揃うまで human gate。
...
| Medication Stock base    | schema、RLS/index、append-only event、snapshot、summary API、stockout/equivalence helper、accepted inbound signal apply、処方供給adapter v1。                                                | 残は処方供給follow-up、訪問観測context/API/UI、usage/refill、equivalence review UI、downstream接続。                          |
...
| `RX-002/STOCK`  | MedicationStock ledger base、accepted inbound signal apply、prescription supply adapter v1。 | prescription supply follow-up、visit observation context/API/UI、usage_delta/frequency/refill、equivalence review UI、stock risk provider完全統合。 |
...
| `STOCK-001-VISIT-API`              | Not started | P1       | Pharmacy stock    | `MedicationStockObservationContext` sidecar contract を前提に `POST /api/visit-records/:id/medication-stock-observations` を追加する。訪問記録由来の `observed_absolute`、`usage_delta`、`usage_frequency`、`not_observed`、`refill_request` を append-only event と snapshot 再計算へ接続し、`残り4枚` と `2枚使用` を区別する。 | service/API/idempotency/RLS/no-store/PHI omission tests。direct `ResidualMedication` write、inbound apply の流用、reviewなし他職種signal直書きなら停止。 |
| `STOCK-001-VISIT-FORECAST`         | Not started | P1       | Pharmacy stock    | 訪問観測後の risk 判定に次回訪問日・次回処方/補充予定・Asia/Tokyo civil date を渡す。snapshotで足りない場合は risk provider 側で同じ stockout domain helper を使い、schedule 側へロジックを複製しない。                                                                                                                           | stockout forecast/snapshot tests、JST境界tests。bufferだけで次回訪問前不足を見逃すなら停止。                                                             |
...
| `PERF-DB-READ-SLO-001`             | Not started | P1       | DB read speed     | 主要read pathの `p95`、payload bytes、max rows、allowed include depth、expected indexesを表にし、dashboard/patients/report/visit の BFF 単位で SLO を固定する。                                                                                                                                                                   | route-level metrics smoke、payload snapshots、query-shape check。SLO未定義のまま新しい広域list APIを足すなら停止。                                       |
...
- `PLANS-ACTIVE-LINT-001`: `Plans.md` の active ID はこの `Active Plan Board v6` と implementation/frontend queue に存在するものだけとし、`cc:REFERENCE` / `cc:WIP` の未チェックboxを backlog 件数に数えない軽量lintまたは `rg` 手順を作る。
...
- `STOCK-VISIT-IDEMPOTENCY-001`: visit record id、stock item id、client observation id、payload fingerprint で idempotent replay / 409 conflict を固定する。raw `Idempotency-Key` は保存・返却しない。
...
- `STOCK-VISIT-FORECAST-CONTEXT-001`: `stockout before next visit/refill` 判定に必要な次回訪問/処方予定の取得境界を定義し、Asia/Tokyo date-key の境界テストを追加する。
...
1. `STOCK-001-VISIT-API`: context 契約候補を前提に、idempotent append-only route/service を実装する。Oracle / GPT-5.5 Pro または同等の厳格レビューを継続し、migration適用は human gate。
2. `STOCK-001-VISIT-FORECAST`: 訪問観測後の stockout 判定へ次回訪問/処方予定と Asia/Tokyo date-key を渡す。
3. `DASH-P1-010-RAIL` + `DASH-P1-005-LINKS`: Dashboard Summary Rail と drilldown。既存BFFを再作成しない。
4. `INBOUND-002-REVIEW-DETAIL`: 3カラムreview shell + raw再認可detail。
5. `MOV-001-API` + `MOV-002-SOURCE-PARITY`: movement API と formal source。
6. `FRONTEND-CONTRACT-001`: 7画面UI改善のslice contract / state matrix。
7. `QUERY-SHAPE-WATCHLIST-003-FOLLOW`: query-shape対象拡張とzero-debt維持。
8. `PERF-DB-006D-INDEX`: SELECT-only EXPLAIN artifact と rollback plan の実環境証跡が揃ってから human gate へ進める。

**Plans.md セクション分類**:

| Section                            | 扱い                        | 実装時の読み方                                                                                     |
| ---------------------------------- | --------------------------- | -------------------------------------------------------------------------------------------------- |
| `Active Plan Board v6`             | Active                      | 実装済み / Partial / Not started / Human gate の正。ここを最初に確認する。                         |
...
> この v3 は 2026-07-08 時点の旧分類証跡。現在の実装入口は上の `Active Plan Board v6` のみ。
...
| Archived `Implementation-ready queue` / `Frontend queue` | Archive                     | v3当時の抽出結果。現在は v6 の queue だけを実装入口にする。                |
| `Detailed Status Evidence Archive`                       | Archive                     | 実装済み証跡と旧分類の履歴。active backlog として数えない。                |
| `cc:PARTIAL`                                             | Reference + residual detail | 実装済み土台は再作成せず、残タスクをactive boardの対応IDへ戻してから着手。 |
| `cc:REFERENCE`                                           | Reference only              | 背景、受入条件、停止条件、仕様詳細。直接sprint backlog化しない。           |
| `cc:WIP`                                                 | Program context             | 大きなプログラム背景。実装する場合はactive boardへ小IDで切り出す。         |
...
> 「この registry を入口にする」等の旧文言は当時の履歴として残すが、現在の active 入口は上の `Active Plan Board v6` のみ。
...
> 旧運用ではこの registry を `Plans.md` の入口にしていた。現在の実装判断は `Active Plan Board v6` を優先する。
...
- 旧運用では、この `Plan Status Registry` と下の `Implementation-ready queue` を入口にしていた。現在の実装判断は `Active Plan Board v6` を入口にする。
...
- 旧運用では `Plans.md` 内の active 実装入口をこの registry、`Implementation-ready queue`、`Frontend implementation queue` に限定していた。現在は v6 の queue に限定する。`cc:REFERENCE` 節のチェックボックスは、そのまま sprint backlog として数えない。
...
| Section                         | 扱い                        | 実装時の読み方                                                                     |
| ------------------------------- | --------------------------- | ---------------------------------------------------------------------------------- |
| `Plan Status Registry`          | Archive                     | 旧分類証跡。現在の正は `Active Plan Board v6`。                                    |
| `Implementation-ready queue`    | Archive                     | v2当時の候補。現在は v6 の queue だけを実装入口にする。                            |
| `Frontend implementation queue` | Archive                     | v2当時の候補。現在は v6 の queue だけを実装入口にする。                            |
