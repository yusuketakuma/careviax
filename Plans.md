...
- 今回の追加照合で未実装に分類するもの: dashboard segment payload budget と Movement timeline list payload budget は実装済み。`withRoutePerformance` は `Content-Length` がないと payload を `unmeasured` とするため、今後の critical read path も shared measured JSON helper を使う。残る payload-budget 未実装は Inbound inbox/signals、MedicationStock summary、横断 perf-smoke matrix。
...
| Bucket             | 今回の分類                                                                                                                                                                                                                                                                                                                                                                                                                                    | 実装判断                                                                                                                                    |
| ------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- |
| Done / frozen      | dashboard segment payload budgets、patient movement timeline payload budget、care-report list payload budget、care-report bounded patient/keyword search、care-report delivery summary page-basis、care-report SELECT-only EXPLAIN tooling、formal inbound schema/API/task/risk、MedicationStock schema/summary/accepted-signal apply、movement tab/common DTO/safe detail resolver、AWS backup read-only monitor、access-control policy base | 再実装しない。関連する後段TODOが残っていても、回帰防止testか残scopeだけを扱う。                                                             |
...
| ----------------------- | --------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------- |
| Payload / PHI / DB read | `PAYLOAD-BUDGET-001C-A/B/C/D`                       | `GET /api/communications/inbound`、`GET /api/communications/inbound/signals`、`GET /api/patients/:id/medication-stock` を route 別に measured JSON + payload budget へ入れる。既存互換 field は残し、`visible_count` / `hidden_count` / `partial_failures` / `generated_at` を additive に揃える。 | route testsで `Content-Length` と byte length 一致、budget registry tests、raw_text/storage key/signed URL/provider raw error/classifier raw payload omission snapshot。 | raw textをlist/notification/audit diffへ出す必要が出たら停止。auth/side-effect semantics を変えるならOracle review。 |
| Payload / perf tooling  | `PAYLOAD-BUDGET-001D`                               | dashboard / movement / patients board / inbound / stock / reports の payload smoke matrix を作る。measured/unmeasured/over-budget をroute family単位で出す。                                                                                                                                       | perf-smoke unit tests、budget registry snapshot、unmeasured route warning。                                                                                              | live DB/AWSを必須にするならHuman gateへ移す。                                                                        |
...
- `PAYLOAD-BUDGET-001C-A/B/C/D`: `001C` を inbound inbox、inbound signals、patient medication-stock summary、budget registry/tests の4小sliceへ分ける。3routeを一括編集する場合でもテストはroute別に失敗点を切り分ける。
- `QUERY-SHAPE-TEST-002`: DB速度改善PRごとに broad include、unbounded `findMany`、missing `take`、missing stable `orderBy`、count/groupBy fan-out を検出する targeted test を追加する。
...
1. `PAYLOAD-BUDGET-001C-A/B/C/D` inbound inbox / inbound signals / stock summary / budget registry の payload-size smoke と PHI omission snapshot。
2. `PAYLOAD-BUDGET-001D` dashboard / movement / patients board / inbound / stock / reports の perf-smoke matrix。
3. `DASH-P1-010-RAIL` + `DASH-P1-005-LINKS` Dashboard Summary Rail と process tile / drilldown。
4. `FRONTEND-CONTRACT-001` 7画面UI改善の slice contract / state matrix 整理。
5. `PLANS-HYGIENE-002` / `PLANS-ACTIVE-LINT-001` 長大な reference spec の段階移管と active backlog lint。
6. `PERF-DB-006D-INDEX` は SELECT-only EXPLAIN artifact と rollback plan の実環境証跡が揃ってから human gate へ進める。
...
> 2026-07-08 整理で `PERF-DB-006D-EXPLAIN`、`PAYLOAD-BUDGET-001A`、`PAYLOAD-BUDGET-001B` はこの active queue から削除し、上位の Done / frozen evidence にだけ残した。
...
| `PAYLOAD-BUDGET-001C-A`  | 未実装     | P1     | Inbound API       | `/api/communications/inbound`, inbound inbox presenter                                   | Inbound inbox list DTO に `visible_count` / `hidden_count` / `generated_at` / `partial_failures` を additive に足し、既存の `total_visible_count` / `filtered_count` 等は互換維持する。`successWithMeasuredJsonPayload` を使い、budget registry に `communications-inbound-list` を追加する。raw_text、添付storage key、signed URL、provider raw errorはlistに出さない。権限内の本文・連絡先・添付は detail surface で再認可する。                                                                                | inbound route DTO snapshot、Content-Length test、payload budget test、raw omission snapshot、forbidden test。raw_textをlist/notification/audit changeへ出す設計なら停止。              |
| `PAYLOAD-BUDGET-001C-B`  | 未実装     | P1     | Inbound signals   | `/api/communications/inbound/signals`, signal presenter                                  | Inbound signal list DTO に count/meta を追加し、review/action status、domain/type、薬剤名候補、数量候補、source confidence だけを一覧に出す。分類用raw payload、raw_text、provider raw errorは出さない。route を measured JSON 化し、budget registry に `communications-inbound-signals-list` を追加する。                                                                                                                                                                                                        | signal route snapshot、Content-Length test、payload budget test、classifier raw payload omission test、review permission test。low confidence signal を自動反映する設計なら停止。      |
| `PAYLOAD-BUDGET-001C-C`  | 未実装     | P1     | Stock API         | `/api/patients/:id/medication-stock`, MedicationStock summary presenter                  | 患者 medication-stock summary/list を measured JSON 化し、`visible_count` / `hidden_count` / `generated_at` / `partial_failures` を追加する。権限内では薬剤名・残数・リスク・次アクションを表示してよいが、storage key、signed URL、raw inbound text、provider raw error、不要なイベント全文はsummary/listに出さない。budget registry に `patient-medication-stock-summary` を追加する。                                                                                                                          | stock summary route snapshot、Content-Length test、payload budget test、PHI/detail boundary snapshot、tenant forbidden test。summaryで全イベント本文や添付詳細を返す必要が出たら停止。 |
| `PAYLOAD-BUDGET-001C-D`  | 未実装     | P1     | Budget tests      | route payload budget registry, measured response helper                                  | `001C-A/B/C` の3routeを registry / tests / docs でまとめて固定する。dynamic id/query normalization、`Content-Length` byte length一致、over-budget failure、unmeasured route warningをroute family単位で検査する。route-local helperを増やさず、既存 shared measured JSON helperへ寄せる。                                                                                                                                                                                                                         | route-payload-budgets tests、performance normalization tests、route family snapshot。既存 route DTO を破壊的にrenameする必要が出たら停止。                                             |
| `PAYLOAD-BUDGET-001D`    | 未実装     | P1/P2  | Perf tooling      | `tools/scripts/perf-smoke.ts`, `performance.ts`, budget registry                         | dashboard / movement / patients board / inbound / stock / reports の payload budget smoke matrix を作る。route family、budget bytes、measured/unmeasured、over-budget をCIで読める形にし、`Content-Length` 未測定routeを発見できるようにする。                                                                                                                                                                                                                                                                    | perf-smoke unit tests、budget registry snapshot、unmeasured route warning。live DBや本番AWSを前提にするなら human gate。                                                               |
