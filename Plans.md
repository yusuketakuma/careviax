...
- 残:
  - domain 別 resolve predicate、Task Health Board の UI 接続 / browser smoke。
...
| CORE-002 | 横断基盤       | Risk Finding -> Operational Task Bridge         | `risk-task-bridge.ts`, `operational-tasks.ts`, `task-registry.ts`                                           | `cc:PARTIAL` active blocking/urgent finding を PHI-minimized dedupe task input へ変換し、case-scoped waiver route / audit / resolution note / Task Health の孤児 audit まで追加。残: domain 別 resolve predicate、Task Health UI 接続。 |
...
| TASK-001 | task/SLA       | Operational Task Health Board                   | `operational-tasks.ts`, `task-registry.ts`, `app/api/tasks/health-board/route.ts`                           | `cc:PARTIAL` API/service は期限超過、SLA超過、担当未割当、患者安全、請求締め、報告遅延、孤児 risk task を no-store / PHI-minimized に集計。残: UI 接続、担当/工程 filter UI、browser smoke。                                            |
