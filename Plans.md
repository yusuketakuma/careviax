...
| Recovery / AWS      | `OPS-RECOVERY-INTEGRITY-001` → `OPS-RECOVERY-MONITOR-003` → `OPS-RECOVERY-LIVE-001` | `OPS-RECOVERY-EVIDENCE-001` は実装済み。次は復元済みDBの SELECT-only 業務整合 audit CLI、S3 Object Lock / strict skipped-check monitorを追加する。その後、human gate として live AWS strict validation / restore drill evidence を収集する。 | integrity audit tests、PHI-free output snapshot、`backup:drill:check`、template validator。                     | production DBへ誤接続する設計、runtime restore API、AWS destructive permission、PHI/ARN/endpoint/storage keyを出力する設計。 |
