...
### 2026-07-08 Plan Status Registry — 実装済み / 未実装整理 `cc:WIP`

> 目的: `Plans.md` 内の古い TODO と最新 main の実装状態を混ぜない。実装済み項目は再タスク化せず、未実装項目だけを次PRに落とせる粒度へ拡充する。
> 根拠は 2026-07-08 時点の `git log --oneline -80`、現行コード検索、既存テスト、`docs/compliance/access-control-policy.md`。
...
| ------------------------ | ---------- | -------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `DASH-OPS`               | 一部実装済 | summary/details軽量化、segment invalidation、inbound / stock-risks / report-billing segment、urgent主要source、ViewModel hook。  | Clock Island、Summary Rail、process tile clickable、drilldown最終UX、quick action安全境界、visual regression。                                            |
...
| `PERF-DB-001`            | 未実装     | 計画のみ。                                                                                                                       | dashboard / patients / movement / inbound / stock / reports の read path inventory、既存query軽量化、必要な index backlog。                               |
...
| Dashboard  | ViewModel hook。                                                                                                                  | `src/app/(dashboard)/dashboard/use-dashboard-cockpit-view-model.ts`。                                                                                                      | `DASH-P0-005` は完了扱い。残は Clock Island と render boundary。 |
...
| PERF-DB-002         | P0/P1  | DB速度        | `readDashboardMedicationStockRisks()`、`readDashboardMedicationStockLedgerRisks()`                   | dashboard medication-stock signal reader は `findMany + count x6` になっている。既存 ledger reader の window aggregate pattern に寄せ、rows と total/urgent/shortage/usage/equivalence/linked counts を1 readerに集約する。                                                                                                                                     | focused unit for SQL/presenter、dashboard stock-risks route tests、typecheck、payload/query note。   |
| PERF-DB-003         | P0/P1  | DB速度        | `listCommunicationQueue()`, `/api/communications/inbound`                                            | inbound-only API は `queueTypes: ['inbound_communication']` を指定しているが、shared queue reader は self report、callback、delivery、external share、care/tracing report 等を先に読む。`summaryScope: 'requested'` などの明示optionを追加し、requested-only では inbound 以外の delegate を呼ばない。既定動作は互換のため維持する。                            | communication-queue unitで不要delegate未呼び出しを検証、inbound route tests、query count note。      |
| PERF-DB-004         | P0/P1  | DB速度        | `/api/care-reports/today-workspace`, `listCommunicationQueue()`                                      | report workspace は action rail の `他職種受信` evidence 数だけのために full communication queue を読んでいる。直接 `InboundCommunicationEvent` の count または exists を読む専用 helper へ置換し、assignment/case scope を明示する。UIが必要とするのが件数か boolean かを固定し、`limit: 1` の visible-window count を総件数として扱わない。                   | today-workspace route tests、inbound evidence fixture更新、summary count/exists semantics test。     |
| PERF-DB-005         | P0/P1  | DB速度/PHI    | `/api/patients/[id]`, patient detail slice APIs                                                      | legacy patient detail full read は broad include と20+ query fan-out がある。既存の timeline / medication-stock slice API と同じ scoped runner / RLS posture へ寄せ、full route は bounded select と per-relation `take` を持つ。重い履歴・監査・lab・timeline は slice API / lazy tab へ逃がす。                                                               | patient detail route tests、bounded relation tests、RLS scoped transaction proof、payload budget。   |
...
| DASH-STATUS-001     | P0/P1  | Dashboard     | `DashboardCockpit`, `DashboardUrgentItem`, `useDashboardCockpitViewModel`                            | `DASH-P0-005` は実装済みに移し、残P0は Clock Island / render boundary、P1は Summary Rail / process clickable / drilldown / role layout、P2は density / visual regression に整理する。既存 `stock-risks` / `report-billing` API は再作成しない。                                                                                                                 | dashboard focused tests、render-boundary smoke、relative href tests、mobile snapshot。               |
...
| PAYLOAD-BUDGET-001  | P1     | 派生/性能/PHI | dashboard segments、movement API、patients board、inbound/stock                                      | list/summary DTO は `visible_count` / `hidden_count` / `generated_at` / `partial_failures` を持ち、raw text、storage key、signed URL、provider raw errorを含めない。権限内 detail surface では必要情報を表示できるが、summary/list payload は肥大化させない。                                                                                                   | DTO snapshot、payload size smoke、PHI omission snapshot、forbidden tests。                           |

**派生タスクとして今回の整理で追加したもの**:

- `PERF-DB-002`: `readDashboardMedicationStockRisks()` の inbound signal count fan-out を window aggregate reader へ寄せる。
- `PERF-DB-003`: inbound-only queue request で unrelated queue sources を読まない source gating を入れる。
- `PERF-DB-004`: report workspace の action rail 用 inbound evidence を full queue reader から直接 count/exists helper へ置換する。
- `PERF-DB-005`: legacy patient detail full read の broad include / query fan-out を bounded scoped read へ寄せる。
...
1. `PERF-DB-003` inbound-only communication queue source gating。
2. `PERF-DB-004` report workspace inbound evidence direct count/exists helper。
3. `PERF-DB-002` Dashboard medication-stock signal reader count consolidation。
4. `DASH-P0-004` Dashboard Clock Island / render boundary。
5. `DASH-P1-010` Dashboard Summary Rail と process tile / drilldown。
...
| DASH-OPS | ダッシュボード | Dashboard Operations Command Center            | `DashboardCockpit`, dashboard cockpit BFF, INB/RX/report/billing segments                          | 済: `DASH-P0-002` summary/details 重複取得削減、`DASH-P0-003` segment-specific invalidation、`DashboardUrgentItem` 初期導入、inbound segment、stock-risks/report-billing segment、task/visit/report/callback/billing/stock urgent source、role priority、source drilldown、`DASH-P0-005` ViewModel hook。残: `DASH-P0-004` Clock Island / render boundary、Summary Rail、Process tile clickable、dashboard quick action、visual regression。監査中心の「今すぐ対応」を、他職種受信、残数不足、報告送付失敗、折返し期限、請求blockerまで含む運用司令塔に拡張する。ダッシュボードは認証済み業務画面なので、権限内の患者名・薬剤名・残数・MCS/電話本文・連絡先・添付導線など判断に必要な情報は表示してよい。制限は dashboard 固有の blanket redaction ではなく、role / assignment / scope / consent / purpose による権限制御で行う。 |
...
| ----------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| `src/app/(dashboard)/dashboard/dashboard-cockpit.tsx` | `summary` / `details` / `team` / `comments` / `inbound` を segment query として取得。comments、summary、details、team、inbound の realtime invalidation は source 別に分離済み。                   | segment 分割と source 別 invalidation は維持する。ViewModel hook は実装済み。次は Clock Island、Summary Rail、render boundary を追加する。 |
...
6. 30秒更新、Clock Island、section memoization でレンダリング更新範囲を小さくする。ViewModel hook は実装済みのため再タスク化しない。
...
- DASH-P0-005 Dashboard ViewModel hook
  ...
  - 残: hook 自体ではなく、Clock Island / render boundary / visual regression で更新波及を証明する。
...
| PR   | 内容                                                                                                                                                                                      | validation                                                                             |
| ---- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------- |
| PR-1 | `DASH-P0-004` Clock Island。`DASH-P0-002/003/005` 済みの構造を崩さず、30秒更新と freshness/countdown 更新を小さい component boundary に閉じる。                                           | dashboard-cockpit unit tests、render count smoke、clock-only update tests、typecheck。 |
| PR-2 | `DASH-P0-001` Unified Urgent Queue の仕上げ。audit/inbound/stock/report/callback/billing/visit_preparation/task と drilldown は初期統合済み。次は drilldown UX と role focus を調整する。 | urgent item presenter tests、relative href tests、role/scope tests。                   |
...
2. DASH-P1-010 Dashboard Summary Rail
3. DASH-P0-001 Unified Urgent Queue の drilldown / role focus 仕上げ
```

理由: `DASH-P0-002`、`DASH-P0-003`、`DASH-P0-005`、主要 urgent source、stock-risks/report-billing API は完了済みなので、次はUI追加前に再描画範囲を整理する。
その後、左レールと drilldown で「今すぐ対応」の運用導線を強くする。
