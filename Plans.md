...
| `STOCK-001-VISIT-DOWNSTREAM`       | Partial     | P1       | Pharmacy stock    | 訪問観測後の snapshot/risk を OperationalTask、Case Risk Cockpit、VisitBrief、Schedule候補、Patient Movement へ接続する。OperationalTask fan-out と Case Risk Cockpit provider は実装済み。残は VisitBrief / Schedule / Movement 連動。task metadata は ids/status/code のみで、患者名・薬剤名・raw reason は入れない。 | risk-task bridge tests、VisitBrief/Schedule tests、PHI metadata snapshot。ledger更新だけで業務follow-upが出ないなら停止。                              |
...
| `STOCK-001-VISIT-DOWNSTREAM`     | ledger write後の fan-out を `risk/task` → `VisitBrief` → `Schedule` → `Movement` の順に分割する。`risk/task` と Case Risk Cockpit provider は完了済み。次は VisitBrief、Schedule候補、Movement occurrence の順に payload PHI boundary を固定する。                                                                                                              | append-only event、snapshot、次回訪問/JST forecast、OperationalTask shortage fan-out、Case Risk provider。 | VisitBrief ordering、schedule candidate reason、movement occurrence event。                                                                                             |
...
3. `STOCK-001-VISIT-DOWNSTREAM`: OperationalTask fan-out と Case Risk Cockpit provider は完了。残る VisitBrief / Schedule / Movement 連動を実装する。
