...
| `STOCK-001-VISIT-FORECAST`         | Partial     | P1       | Pharmacy stock    | 2026-07-08実装: 訪問観測後の snapshot 再計算に Asia/Tokyo civil date と active future `VisitSchedule` 由来の次回訪問日を渡し、次回訪問前不足を `shortage_expected` にする。残: 次回処方/補充予定は正本source未確定のため `STOCK-001-PRESCRIPTION-HORIZON` で設計してから接続する。 | stockout forecast/snapshot/service/route tests、JST境界tests。`refill_request` だけから補充予定日を推定するなら停止。                                  |
...
- `STOCK-VISIT-FORECAST-CONTEXT-001`: 2026-07-08に次回訪問分は実装済み。`stockout before next refill/prescription` 判定は正本source未確定のため `STOCK-001-PRESCRIPTION-HORIZON` へ分離する。
