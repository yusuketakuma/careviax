...
3. `INBOUND-002-REVIEW-DETAIL`: raw detail API は完了。残は 3カラム review shell と detail drawer 接続。
...
| Inbound core               | `InboundCommunicationEvent` / `InboundCommunicationSignal` schema、RLS、phone/MCS登録、inbox、signal materialize、task bridge、risk bridge、MedicationStock accepted-signal apply。                                                                                                                                                                                         | 正本DB/APIは固定。残は review detail UX、detail drawer接続、FAX/email/manual、source mapping UI、VisitBrief/Schedule/Report/Share。                                                 |
| Inbound raw detail API     | `GET /api/communications/inbound/:id/detail` で raw_text / 送信者詳細を `purpose`、`read_reason`、`request_id`、再認可、read audit、no-store 経由に限定。dashboard inbound / urgent / stock-risk DTO は raw_text、sender_contact、signal extracted_text を返さない。                                                                                                        | 再実装しない。残は 3カラム review shell、detail drawer接続、review lifecycle UI、FAX/email/manual/source mapping UI。                                                               |
...
| `INB-001/002`   | Formal inbound schema/API/inbox/signals/task/risk/stock apply、raw detail再認可API/read audit、dashboard raw omission。                                                                                                                           | 3カラム review shell、detail drawer接続、FAX/email/manual、source mapping UI、VisitBrief/Schedule/Report/Share。                                                                                          |
...
| ---------------------------------- | ----------- | -------- | ----------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `INBOUND-002-REVIEW-DETAIL`        | Partial     | P0/P1    | Inbound UI/API    | raw detail API は完了。残は 3カラム review shell と detail drawer接続。左=原文/添付/送信者/日時、中央=signal候補、右=反映先/Task/Record-only/Reject/MedicationStock。raw_textは既存 detail API とread audit経由に限定。                                           | raw omission snapshots、review lifecycle、drawer/detail route tests。raw_textをlist/通知/監査changesへ流すなら停止。                                   |
...
| ------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `INBOUND-002-REVIEW-DETAIL`    | detail API は完了。次は detail drawer と 3カラム shell を既存 `GET /api/communications/inbound/:id/detail` に接続し、review action lifecycle と MedicationStock apply 導線を UI で閉じる。                                                                                                                                                                      | Inbound schema、phone/MCS登録、inbox、signal materialize、task/risk bridge、MedicationStock apply、raw detail API。 | list DTO raw omission snapshot、detail drawer forbidden/404、review action lifecycle、MedicationStock apply allowed/forbidden。                                         |
