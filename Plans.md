...
- `tools/scripts/check-api-response-shape.mjs` / `tools/api-response-shape-allowlist.json`: non-envelope `success()` と route-local legacy error JSON の再発検出 gate は CI 接続済み。残作業は API envelope 本体の移行、request_id/error code 統一、allowlist burn-down に絞る。
- `tools/scripts/check-dto-direct-prisma-return.mjs` / `tools/dto-direct-prisma-return-allowlist.json`: route が Prisma result を直接 `success()` へ返す再発検出 gate は CI 接続済み。残作業は allowlist burn-down、presenter/serializer 移行、DTO snapshot 拡張に絞る。
...
| MOD-CI-001      | P1     | `API-DTO-001`, `DB-TENANT-001`, `DEV-PHI-001`, `PLAN-REV-001`             | Remaining debt-ratchet CI gates    | 残: `rls-policy-contract` 拡張と既存 gate の役割整理。実装済みの `api-response-shape:check` / `dto-direct-prisma-return:check` / `task-types:check` / module-boundary gate は維持し、以後は allowlist を増やさず削減する。                                                                  | 各MOD PRは「module境界化 + DEBT削減 + DTO/presenter明確化 + 境界テスト + 回帰テスト」を含む。薬局E2E/主要API回帰を維持し、module-boundary / API response shape / DTO direct Prisma return / task type registry gate を維持する。 |
...
6. `MOD-CI-001`: RLS contract 拡張をCIへ接続し、既存 response/DTO/task/module gate の allowlist burn-down を進める。
