...
- 対象: `src/app/(dashboard)/workflow/workflow-dashboard-content.tsx` とtest。
- 実施: emergency draft / inquiry create / inquiry resolve / refill proposal mutation responses を
  `readApiJson` へ移行し、failed workflow mutation の API JSON `message` 保持テストを追加。
- 挙動変更: workflow dashboard UI mutation response handling の helper 収束のみ。GET helper、
  endpoints、org JSON headers、request bodies、success toasts、invalidation は維持。
- 安全: workflow dashboard UI internals のみ。SSOT の必要時変更許可
  (product API/DB/auth/authorization/PHI/billing/deploy/package dependency) は維持しつつ、本sliceでは不要。
- 検証: focused workflow-dashboard Vitest `1 file / 9 tests` green。
  scoped eslint/prettier/diff-check green。`pnpm typecheck` green。
- レビュー: self-verified。commit 1493006d。
- 残課題: R40/R44 は partial。

## 2026-07-05 R40/R44-intervention-panel 29c99563

- 分類: query-helper / client fetch error handling → `readApiJson` 収束。
- 対象: `src/components/features/medications/intervention-panel.tsx` とtest。
- 実施: intervention list/create/outcome-save responses を `readApiJson` へ移行し、
  failed create/outcome-save の API JSON `message` 表面化テストを追加。
- 挙動変更: intervention panel UI response handling の helper 収束のみ。endpoints、methods、
  request bodies、initial fetch suppression、loading/error/empty states、local update、dialog reset は維持。
- 安全: intervention panel UI internals のみ。SSOT の必要時変更許可
  (product API/DB/auth/authorization/PHI/billing/deploy/package dependency) は維持しつつ、本sliceでは不要。
- 検証: focused intervention-panel Vitest `1 file / 5 tests` green。
  scoped eslint/prettier/diff-check green。`pnpm typecheck` green。
- レビュー: self-verified。commit 29c99563。
- 残課題: R40/R44 は partial。
...
- 分類: query-helper / client fetch error handling → `readApiJson` 収束。
- 対象: `src/app/(dashboard)/admin/facilities/facilities-content.tsx` とtest。
- 実施: admin facility unit save/delete mutation responses を `readApiJson` へ移行し、
  failed unit save/delete の API JSON `message` 表面化テストを追加。
- 挙動変更: facility unit UI response handling の helper 収束のみ。facility/unit path helpers、
  org headers、create/update bodies、success toasts、unit invalidation は維持。
- 安全: facilities UI internals のみ。SSOT の必要時変更許可
  (product API/DB/auth/authorization/PHI/billing/deploy/package dependency) は維持しつつ、本sliceでは不要。
- 検証: focused facilities Vitest `1 file / 15 tests` green。
  scoped eslint/prettier/diff-check green。`pnpm typecheck` green。
- レビュー: self-verified。commit 523f6946。
- 残課題: R40/R44 は partial。
...
- 分類: query-helper / client fetch error handling → `readApiJson` 収束。
- 対象: `src/components/visit-brief/visit-brief-card.tsx`,
  `src/app/(dashboard)/visits/[id]/brief/visit-brief-review-content.tsx` と各test。
- 実施: visit brief card / pharmacist review の feedback POST responses を
  `readApiJson` へ移行し、failed feedback の API JSON `message` 表面化テストを追加。
- 挙動変更: visit brief UI feedback response handling の helper 収束のみ。endpoint、method、
  org JSON headers、feedback request body、success toast/local state は維持。
- 安全: visit brief UI internals のみ。SSOT の必要時変更許可
  (product API/DB/auth/authorization/PHI/billing/deploy/package dependency) は維持しつつ、本sliceでは不要。
- 検証: focused visit-brief Vitest `2 files / 16 tests` green。
  scoped eslint/prettier/diff-check green。`pnpm typecheck` green。
- レビュー: self-verified。commit b466bbf0。
- 残課題: R40/R44 は partial。

## 2026-07-05 R40/R44-billing-candidate-mutations a20ecb91
...
- 実施: billing candidate export preview / generate / review / close responses を
  `readApiJson` へ移行し、failed generation/review の API JSON `message` 表面化テストを追加。
- 挙動変更: billing UI response handling の helper 収束のみ。query params、request bodies、
  org headers、disabled reasons、billing calculation/close behavior は維持。
- 安全: billing UI internals のみ。SSOT の必要時変更許可
  (product API/DB/auth/authorization/PHI/billing/deploy/package dependency) は維持しつつ、本sliceでは不要。
- 検証: focused billing-candidates-content Vitest `1 file / 9 tests` green。
  scoped eslint/prettier/diff-check green。`pnpm typecheck` green。
- レビュー: self-verified。commit a20ecb91。
- 残課題: R40/R44 は partial。

## 2026-07-05 R40/R44-report-detail-mutations ecb66652

- 分類: query-helper / client fetch error handling → `readApiJson` 収束。
- 対象: `src/app/(dashboard)/reports/[id]/page.tsx`,
  `src/app/(dashboard)/reports/[id]/page.test.tsx`
- 実施: report detail confirm / single-send / bulk-send mutation responses を
  `readApiJson` へ移行し、failed response の API JSON `message` 表面化テストを追加。
- 挙動変更: JSON payload を返す mutation response handling の helper 収束のみ。care-report
  confirm/send endpoints、request bodies、idempotency headers、org JSON headers は維持。
- 安全: report detail product UI mutation internals のみ。SSOT の必要時変更許可
  (product API/DB/auth/authorization/PHI/billing/deploy/package dependency) は維持しつつ、本sliceでは不要。
- 検証: focused report detail Vitest `1 file / 39 tests` green。
  scoped eslint/prettier/diff-check green。`pnpm typecheck` green。
- レビュー: self-verified。commit ecb66652。
- 残課題: R40/R44 は partial。

## 2026-07-05 R40/R44-patient-share-mutations 5d836984

- 分類: query-helper / client fetch error handling → `readApiJson` 収束。
- 対象: `src/app/(dashboard)/patients/[id]/share/external-share-content.tsx`
- 実施: patient share follow-up task / reply-request mutation responses を
  `readApiJson` へ移行。
- 挙動変更: JSON payload を返す mutation response handling の helper 収束のみ。tasks /
  communication-requests endpoints、POST method、request bodies、org JSON headers は維持。
- 安全: patient share product UI mutation internals のみ。SSOT の必要時変更許可
  (product API/DB/auth/authorization/PHI/billing/deploy/package dependency) は維持しつつ、本sliceでは不要。
- 検証: focused external-share-content Vitest `1 file / 12 tests` green。
  scoped eslint/prettier/diff-check green。`pnpm typecheck` green。
- レビュー: self-verified。commit 5d836984。
- 残課題: R40/R44 は partial。

## 2026-07-05 R40/R44-report-share-mutations 084b5736

- 分類: query-helper / client fetch error handling → `readApiJson` 収束。
- 対象: `src/app/(dashboard)/reports/[id]/share/interprofessional-share-content.tsx`
- 実施: report share follow-up task / reply-request mutation responses を
  `readApiJson` へ移行。
- 挙動変更: JSON payload を返す mutation response handling の helper 収束のみ。tasks /
  communication-requests endpoints、POST method、request bodies、org JSON headers は維持。
- 安全: report share product UI mutation internals のみ。SSOT の必要時変更許可
  (product API/DB/auth/authorization/PHI/billing/deploy/package dependency) は維持しつつ、本sliceでは不要。
- 検証: focused interprofessional-share-content Vitest `1 file / 28 tests` green。
  scoped eslint/prettier/diff-check green。`pnpm typecheck` green。
- レビュー: self-verified。commit 084b5736。
- 残課題: R40/R44 は partial。

## 2026-07-05 R40/R44-report-editor-save 37bd8bb6

- 分類: query-helper / client fetch error handling → `readApiJson` 収束。
- 対象: `src/components/features/reports/report-edit-form.tsx`
- 実施: report editor save mutation response を `readApiJson` へ移行。
- 挙動変更: JSON payload を返す mutation response handling の helper 収束のみ。encoded
  care-report endpoint、PATCH method、expected_updated_at/content body、org JSON header は維持。
- 安全: report editor product UI mutation internals のみ。SSOT の必要時変更許可
  (product API/DB/auth/authorization/PHI/billing/deploy/package dependency) は維持しつつ、本sliceでは不要。
- 検証: focused report-edit-form Vitest `1 file / 4 tests` green。
  scoped eslint/prettier/diff-check green。`pnpm typecheck` green。
- レビュー: self-verified。commit 37bd8bb6。
- 残課題: R40/R44 は partial。

## 2026-07-05 R40/R44-communication-follow-up c6bc1af8

- 分類: query-helper / client fetch error handling → `readApiJson` 収束。
- 対象: `src/app/(dashboard)/communications/requests/requests-content.tsx`,
  `src/app/(dashboard)/communications/requests/requests-content.test.tsx`
- 実施: communication resolve-followup mutation response を `readApiJson` へ移行し、
  failed response の API JSON `message` 表面化テストを追加。
- 挙動変更: JSON payload を返す mutation response handling の helper 収束のみ。encoded
  resolve-followup endpoint、expected_updated_at / response / followup body、org JSON header は維持。
- 安全: communication product UI mutation internals のみ。SSOT の必要時変更許可
  (product API/DB/auth/authorization/PHI/billing/deploy/package dependency) は維持しつつ、本sliceでは不要。
- 検証: focused communications requests Vitest `1 file / 15 tests` green。
  scoped eslint/prettier/diff-check green。`pnpm typecheck` green。
- レビュー: self-verified。commit c6bc1af8。
- 残課題: R40/R44 は partial。
...
- 分類: query-helper / client fetch error handling → `readApiJson` 収束。
- 対象: `src/app/(dashboard)/patients/[id]/consent/consent-records-content.tsx`,
  `src/app/(dashboard)/patients/[id]/consent/consent-records-content.test.tsx`
- 実施: consent templates / consent records list read GET を `readApiJson` へ移行し、
  failed response の API JSON `message` 表面化テストを追加。
- 挙動変更: read fetch 実装内部の helper 収束のみ。endpoints、org header、
  queryKeys、enabled gates、DataTable false-empty prevention、upload/create/update/revoke mutations は維持。
- 安全: consent product UI read internals のみ。SSOT の必要時変更許可
  (product API/DB/auth/authorization/PHI/billing/deploy/package dependency) は維持しつつ、本sliceでは不要。
- 検証: focused consent-records Vitest `1 file / 10 tests` green。
  scoped eslint/prettier/diff-check green。`pnpm typecheck` green。
- レビュー: self-verified。commit 3e04a3fd。
- 残課題: R40/R44 は partial。

## 2026-07-05 R40/R44-schedule-day-preparation 596b4942

- 分類: query-helper / client fetch error handling → `readApiJson` 収束。
- 対象: `src/app/(dashboard)/schedules/schedule-day-preparation.ts`,
  `src/app/(dashboard)/schedules/schedule-day-preparation.test.ts`
- 実施: schedule day preparation details read GET を `readApiJson` へ移行し、
  failed response の API JSON `message` 表面化テストを更新。
- 挙動変更: read fetch 実装内部の helper 収束のみ。endpoint、schedule-id encoding、
  org header、pack identity guard、readiness behavior、save/mark-ready mutation は維持。
- 安全: product UI read fetch internals のみ。SSOT の必要時変更許可
  (product API/DB/auth/authorization/PHI/billing/deploy/package dependency) は維持しつつ、本sliceでは不要。
- 検証: focused schedule-day-preparation Vitest `1 file / 19 tests` green。
  scoped eslint/prettier/diff-check green。`pnpm typecheck` green。
- レビュー: self-verified。commit 596b4942。
- 残課題: R40/R44 は partial。
...
- 分類: query-helper / client fetch error handling → `readApiJson` 収束。
- 対象: `src/app/(dashboard)/prescriptions/qr-drafts/page.tsx`,
  `src/app/(dashboard)/prescriptions/qr-drafts/page.test.tsx`
- 実施: QR draft list の all / unmatched read fetchers を `readApiJson` へ移行し、
  failed response の API JSON `message` 表面化テストを追加。
- 挙動変更: read fetch 実装内部の helper 収束のみ。endpoints/query params、org header、
  queryKeys、fallback refetch、realtime invalidation、enabled gates、DataTable states、
  row navigation、keyboard shortcuts は維持。
- 安全: product UI read fetch internals のみ。SSOT の必要時変更許可
  (product API/DB/auth/authorization/PHI/billing/deploy/package dependency) は維持しつつ、本sliceでは不要。
- 検証: focused QR draft list Vitest `1 file / 1 test` green。
  scoped eslint/prettier/diff-check green。`pnpm typecheck` green。
- レビュー: self-verified。commit 09120529。
- 残課題: R40/R44 は partial。

## 2026-07-05 R40/R44-schedule-proposals b4deef16
...
- 実施: schedule proposals dashboard list / case search / vehicle resources /
  billing preview batch / detail read query fetchers を `readApiJson` へ移行し、
  failed response の API JSON `message` 表面化テストを追加。
- 挙動変更: read fetch 実装内部の helper 収束のみ。endpoints/query params、org header、
  queryKeys、realtime invalidation、enabled gates、dashboard/detail states、mutations は維持。
- 安全: product UI read fetch internals のみ。billing preview body shape も維持。
  SSOT の必要時変更許可 (product API/DB/auth/authorization/PHI/billing/deploy/package dependency) は維持しつつ、本sliceでは不要。
- 検証: focused schedule proposals Vitest `1 file / 40 tests` green。
  scoped eslint/prettier/diff-check green。`pnpm typecheck` green。
- レビュー: self-verified。commit b4deef16。
- 残課題: R40/R44 は partial。
...
- 実施: weekly optimizer の cases / case search / proposals / shifts / vehicle resources /
  billing preview read fetchers を `readApiJson` へ移行し、failed response の API JSON `message` 表面化テストを追加。
- 挙動変更: read fetch 実装内部の helper 収束のみ。endpoints/query params、org header、queryKeys、
  enabled gates、board states、URL sync、route reorder/facility aggregation mutations は維持。
- 安全: product UI read fetch internals のみ。SSOT の必要時変更許可
  (product API/DB/auth/authorization/PHI/billing/deploy/package dependency) は維持しつつ、本sliceでは不要。
  live DB/external send/secret/push/destructive operation 不変。
- 検証: focused weekly-optimizer Vitest `1 file / 11 tests` green。
  scoped eslint/prettier/diff-check green。`pnpm typecheck` green。
- レビュー: self-verified。commit c13f5942。
- 残課題: R40/R44 は partial。

## 2026-07-05 R40/R44-medication-calendar 07e701d6

- 分類: query-helper / client fetch error handling → `readApiJson` 収束。
- 対象: `src/app/(dashboard)/patients/[id]/medication-calendar/medication-calendar-content.tsx`,
  `src/app/(dashboard)/patients/[id]/medication-calendar/medication-calendar-content.render.test.tsx`
- 実施: medication calendar の current medication profile read GET を `readApiJson` へ移行し、
  failed GET の API JSON `message` 表面化テストを追加。
- 挙動変更: read fetch 実装内部の helper 収束のみ。medication-profiles endpoint、encoded patient_id、
  is_current/limit params、org header、queryKey、enabled gate、PDF href、loading/error/empty states は維持。
- 安全: product UI read fetch internals のみ。SSOT の必要時変更許可
  (product API/DB/auth/authorization/PHI/billing/deploy/package dependency) は維持しつつ、本sliceでは不要。
  live DB/external send/secret/push/destructive operation 不変。
- 検証: focused medication-calendar Vitest `2 files / 8 tests` green。
  scoped eslint/prettier/diff-check green。`pnpm typecheck` green。
- レビュー: self-verified。commit 07e701d6。
- 残課題: R40/R44 は partial。

## 2026-07-05 R40/R44-prescription-history d65d08d5

- 分類: query-helper / client fetch error handling → `readApiJson` 収束。
- 対象: `src/app/(dashboard)/patients/[id]/prescriptions/prescription-history-content.tsx`,
  `src/app/(dashboard)/patients/[id]/prescriptions/prescription-history-content.test.tsx`
- 実施: patient prescription history GET / drug-master batch enrichment POST を `readApiJson` へ移行し、
  failed response の API JSON `message` 表面化テストを追加。
- 挙動変更: read/enrichment fetch 実装内部の helper 収束のみ。patient path helper、limit、
  org headers、queryKeys、enabled gates、batch body、non-blocking notice、mutations/cache invalidation は維持。
- 安全: product UI read/enrichment fetch internals のみ。SSOT の必要時変更許可
  (product API/DB/auth/authorization/PHI/billing/deploy/package dependency) は維持しつつ、本sliceでは不要。
  live DB/external send/secret/push/destructive operation 不変。
- 検証: focused prescription-history Vitest `1 file / 29 tests` green。
  scoped eslint/prettier/diff-check green。`pnpm typecheck` green。
- レビュー: self-verified。commit d65d08d5。
- 残課題: R40/R44 は partial。
...
- 実施: prescription intake の generic candidate lookup read GET を `readApiJson` へ移行し、
  failed GET の API JSON `message` 表面化テストを追加。
- 挙動変更: read fetch 実装内部の helper 収束のみ。drug-master path helper、q/generic/limit/includeTotal
  params、org header、queryKey、enabled gate、generic-name checkbox、candidate selection、submit payload は維持。
- 安全: product UI read fetch internals のみ。SSOT の必要時変更許可
  (product API/DB/auth/authorization/PHI/billing/deploy/package dependency) は維持しつつ、本sliceでは不要。
  live DB/external send/secret/push/destructive operation 不変。
- 検証: focused prescription-intake-form Vitest `1 file / 7 tests` green。
  scoped eslint/prettier/diff-check green。`pnpm typecheck` green。
- レビュー: self-verified。commit 68ac7d85。
- 残課題: R40/R44 は partial。

## 2026-07-05 R40/R44-conflict-resolution 67ba5eef

- 分類: query-helper / client fetch error handling → `readApiJson` 収束。
- 対象: `src/app/(dashboard)/schedules/conflicts/conflict-resolution-content.tsx`,
  `src/app/(dashboard)/schedules/conflicts/conflict-resolution-content.test.tsx`
- 実施: schedule conflict resolution の pharmacist lookup read GET を `readApiJson` へ移行し、
  failed GET の API JSON `message` 表面化テストを追加。
- 挙動変更: read fetch 実装内部の helper 収束のみ。`/api/pharmacists`、org header、queryKey、
  enabled gate、visit schedule window fetcher、false-empty prevention、plan adoption/reconfirmation
  mutations、toast error handling は維持。
- 安全: product UI read fetch internals のみ。SSOT の必要時変更許可
  (product API/DB/auth/authorization/PHI/billing/deploy/package dependency) は維持しつつ、本sliceでは不要。
  live DB/external send/secret/push/destructive operation 不変。
- 検証: focused conflict-resolution-content Vitest `1 file / 8 tests` green。
  scoped eslint/prettier/diff-check green。`pnpm typecheck` green。
- レビュー: self-verified。commit 67ba5eef。
- 残課題: R40/R44 は partial。

## 2026-07-05 R40/R44-prescriptions-workspace 7a079828

- 分類: query-helper / client fetch error handling → `readApiJson` 収束。
- 対象: `src/app/(dashboard)/prescriptions/prescriptions-workspace.tsx`,
  `src/app/(dashboard)/prescriptions/prescriptions-workspace.test.tsx`
- 実施: prescription intake list infinite-query read GET を `readApiJson` へ移行し、
  failed GET の API JSON `message` 表面化テストを追加。
- 挙動変更: read fetch 実装内部の helper 収束のみ。limit/include_total/cursor/status/source params、
  org header、queryKey、infinite pagination、realtime invalidation、load-more、error retry、detail panel は維持。
- 安全: product UI read fetch internals のみ。SSOT の必要時変更許可
  (product API/DB/auth/authorization/PHI/billing/deploy/package dependency) は維持しつつ、本sliceでは不要。
  live DB/external send/secret/push/destructive operation 不変。
- 検証: focused prescriptions-workspace Vitest `1 file / 10 tests` green。
  scoped eslint/prettier/diff-check green。`pnpm typecheck` green。
- レビュー: self-verified。commit 7a079828。
- 残課題: R40/R44 は partial。

## 2026-07-05 R40/R44-prescription-detail d22ec557
...
- 実施: prescription intake detail の read GET を `readApiJson` へ移行し、
  failed GET の API JSON `message` 表面化テストを追加。
- 挙動変更: read fetch 実装内部の helper 収束のみ。intake API path helper、hostile-id encoding、
  dot-segment fail-closed、org header、queryKey、enabled gate、retry/back error UI、display-id、
  patient link helper は維持。
- 安全: product UI read fetch internals のみ。SSOT の必要時変更許可
  (product API/DB/auth/authorization/PHI/billing/deploy/package dependency) は維持しつつ、本sliceでは不要。
  live DB/external send/secret/push/destructive operation 不変。
- 検証: focused prescription-detail-content Vitest `1 file / 8 tests` green。
  scoped eslint/prettier/diff-check green。`pnpm typecheck` green。
- レビュー: self-verified。commit d22ec557。
- 残課題: R40/R44 は partial。

## 2026-07-05 R40/R44-drug-masters fe9edc77
...
- 実施: pharmacy sites / drug master cursor page / import status / import logs の
  read GET 4本を `readApiJson` へ移行し、failed GET の API JSON `message` 表面化テストを追加。
- 挙動変更: read fetch 実装内部の helper 収束のみ。site scoping、cursor/page params、
  import log filters、org header、queryKey、enabled gates、DataTable load-more、detail/formulary queries、
  mutations は維持。
- 安全: product UI read fetch internals のみ。SSOT の必要時変更許可
  (product API/DB/auth/authorization/PHI/billing/deploy/package dependency) は維持しつつ、本sliceでは不要。
  live DB/external send/secret/push/destructive operation 不変。
- 検証: focused drug-master-content Vitest `1 file / 88 tests` green。
  scoped eslint green。prettier は touched files formatting 後 green。diff-check green。`pnpm typecheck` green。
- レビュー: self-verified。commit fe9edc77。
- 残課題: R40/R44 は partial。detail/formulary subquery/mutation fetchers は未変更。
...
## 2026-07-05 R40/R44 patient constraints save readApiJson slice

- 分類: query-helper / client mutation error handling → `readApiJson` 収束。
- 実施:
  - patient packaging の `buildPatientApiPath(patientId, '/packaging')` PUT save response を
    `readApiJson<unknown>` へ移行。
  - visit constraints の `buildPatientApiPath(patientId, '/visit-constraints')` PUT save response を
    `readApiJson<unknown>` へ移行。
  - failed save の API JSON `message` が toast へ表面化する契約テストを両カードへ追加。
  - focused fetch fixture を呼び出しごとに新しい `Response` を返す形へ補正し、
    `readApiJson` の one-body-read semantics と揃えた。
- 挙動変更: save response handling internals の helper 収束のみ。patient path helper、
  hostile-id encoding、dot-segment fail-closed、org JSON headers、PUT methods、request body shape、
  success toast、patient-care cache invalidation は維持。
- 安全性: patient detail UI save internals のみ変更。DB/schema、API route、auth/authorization、
  PHI projection、billing、deployment、package dependency、live DB operation、external send、
  secret handling、push、destructive operation は不変。SSOT では必要時の product
  API/DB/auth/authorization/PHI/billing/deploy/package dependency 変更許可を確認済みだが、
  この slice では不要。
- 検証: focused patient packaging / visit constraints Vitest `2 files / 19 tests` green、
  scoped ESLint green、targeted Prettier check green、targeted `git diff --check` green、
  `pnpm typecheck` green。
- commit: `40d5b1d0` (`refactor(ui): reuse readApiJson for patient constraints saves`)。
...
- 分類: query-helper / client mutation error handling → `readApiJson` 収束。
- 実施:
  - visit vehicle resource create/update の POST/PATCH responses を `readApiJson<unknown>` へ移行。
  - availability toggle の PATCH response を `readApiJson<unknown>` へ移行。
  - failed save / availability update の API JSON `message` が toast へ表面化する UI 契約テストを追加。
- 挙動変更: mutation response handling internals の helper 収束のみ。`/api/visit-vehicle-resources`
  read、`/api/pharmacy-sites` read、`VISIT_VEHICLE_RESOURCES_API_PATH`、
  `buildVisitVehicleResourceApiPath`、hostile-id encoding、dot-segment fail-closed、
  org headers、request bodies、success toasts、`admin-visit-vehicle-resources` invalidation は維持。
- 安全性: admin vehicles UI mutation internals のみ変更。DB/schema、API route、
  auth/authorization、PHI projection、billing、deployment、package dependency、live DB operation、
  external send、secret handling、push、destructive operation は不変。SSOT では必要時の product
  API/DB/auth/authorization/PHI/billing/deploy/package dependency 変更許可を確認済みだが、
  この slice では不要。
- 検証: focused vehicles Vitest `1 file / 11 tests` green、
  scoped ESLint green、targeted Prettier check green、targeted `git diff --check` green、
  `pnpm typecheck` green。
- commit: `e2cc0fbf` (`refactor(ui): reuse readApiJson for vehicle mutations`)。
- 残課題: R40/R44 は broad。追加の client fetcher/mutation は response body read が PHI-safe かを
  個別確認してから段階移行する。
  未所有 `refactor-instructions.md` と `.agents/skills/**` / `skills-lock.json` は保持。
...
- 分類: query-helper / client mutation error handling → `readApiJson` 収束。
- 実施:
  - business-holiday create/update の POST/PATCH responses を `readApiJson<unknown>` へ移行。
  - business-holiday delete の DELETE response を `readApiJson<unknown>` へ移行。
  - failed save / delete の API JSON `message` が toast へ表面化する UI 契約テストを追加。
- 挙動変更: mutation response handling internals の helper 収束のみ。`/api/pharmacy-sites` read、
  `/api/business-holidays` read、`BUSINESS_HOLIDAYS_API_PATH`、`buildBusinessHolidayApiPath`、
  hostile-id encoding、dot-segment fail-closed、org headers、request bodies、success toasts、
  `business-holidays` invalidation は維持。bulk holiday creation は複数 response を集約する
  partial-failure contract が別なので未変更。
- 安全性: admin business holidays UI mutation internals のみ変更。DB/schema、API route、
  auth/authorization、PHI projection、billing、deployment、package dependency、live DB operation、
  external send、secret handling、push、destructive operation は不変。SSOT では必要時の product
  API/DB/auth/authorization/PHI/billing/deploy/package dependency 変更許可を確認済みだが、
  この slice では不要。
- 検証: focused business-holidays Vitest `1 file / 16 tests` green、
  scoped ESLint green、targeted Prettier check green、targeted `git diff --check` green、
  `pnpm typecheck` green。
- commit: `fe3056b9` (`refactor(ui): reuse readApiJson for business holidays`)。
- 残課題: R40/R44 は broad。追加の client fetcher/mutation は response body read が PHI-safe かを
  個別確認してから段階移行する。
  未所有 `refactor-instructions.md` と `.agents/skills/**` / `skills-lock.json` は保持。
...
## 2026-07-05 R40/R44 notification read-state readApiJson slice

- 分類: query-helper / client mutation error handling → `readApiJson` 収束。
- 実施:
  - notification read-state の `NOTIFICATIONS_API_PATH` PATCH response を
    `readApiJson<unknown>` へ移行。
  - failed read-state PATCH の API JSON `message` が toast へ表面化する契約テストを追加。
- 挙動変更: mutation response handling internals の helper 収束のみ。notifications collection path、
  `buildOrgJsonHeaders`、PATCH method、`{ ids }` body、inbox invalidation、realtime inbox、
  offline pending-sync row、loading/error states、notification navigation は維持。
- 安全性: notifications UI mutation internals のみ変更。DB/schema、API route、auth/authorization、
  PHI projection、billing、deployment、package dependency、live DB operation、external send、
  secret handling、push、destructive operation は不変。SSOT では必要時の product
  API/DB/auth/authorization/PHI/billing/deploy/package dependency 変更許可を確認済みだが、
  この slice では不要。
- 検証: focused notifications content Vitest `1 file / 12 tests` green、
  scoped ESLint green、targeted Prettier check green、targeted `git diff --check` green、
  `pnpm typecheck` green。
- commit: `c45b384d` (`refactor(ui): reuse readApiJson for notification read state`)。
- 残課題: R40/R44 は broad。追加の client fetcher/mutation は response body read が PHI-safe かを
  個別確認してから段階移行する。
  未所有 `refactor-instructions.md` と `.agents/skills/**` / `skills-lock.json` は保持。
...
## 2026-07-05 R40/R44 operational policy save readApiJson slice

- 分類: query-helper / client mutation error handling → `readApiJson` 収束。
- 実施:
  - operational policy の `/api/settings/operational-policy` PATCH response を
    `readApiJson<{ data: OperationalPolicyResponse }>` へ移行。
  - failed operational policy update の API JSON `message` が toast へ表面化する契約テストを追加。
- 挙動変更: mutation response handling internals の helper 収束のみ。settings policy endpoint、
  `buildOrgJsonHeaders`、PATCH method、request body shape、success toast、cockpit / policy query
  loading states は維持。
- 安全性: settings UI mutation internals のみ変更。DB/schema、API route、auth/authorization、
  PHI projection、billing、deployment、package dependency、live DB operation、external send、
  secret handling、push、destructive operation は不変。SSOT では必要時の product
  API/DB/auth/authorization/PHI/billing/deploy/package dependency 変更許可を確認済みだが、
  この slice では不要。
- 検証: focused operational policy content Vitest `1 file / 8 tests` green、
  scoped ESLint green、targeted Prettier check green、targeted `git diff --check` green、
  `pnpm typecheck` green。
- commit: `74d93c1a` (`refactor(ui): reuse readApiJson for operational policy save`)。
- 残課題: R40/R44 は broad。追加の client fetcher/mutation は response body read が PHI-safe かを
  個別確認してから段階移行する。
  未所有 `refactor-instructions.md` と `.agents/skills/**` / `skills-lock.json` は保持。

## 2026-07-05 R40/R44 saved views readApiJson slice

- 分類: query-helper / client mutation error handling → `readApiJson` 収束。
- 実施:
  - saved-view preferences の `/api/me/preferences` PATCH response を
    `readApiJson<{ data?: PreferencesValue }>` へ移行。
  - named saved-view の create / rename / share-toggle / delete responses を
    `readApiJson<unknown>` へ移行し、ローカル `readErrorMessage` を削除。
  - failed preferences save / named saved-view create の API JSON `message` が toast へ表面化する
    UI 契約テストを追加。
- 挙動変更: mutation response handling internals の helper 収束のみ。preferences / saved-views
  endpoints、path helpers、hostile-id encoding、dot-segment fail-closed、org headers、request
  bodies、query keys、invalidation、success toasts、saved-view recall navigation は維持。
- 安全性: saved views UI mutation internals のみ変更。DB/schema、API route、auth/authorization、
  PHI projection、billing、deployment、package dependency、live DB operation、external send、
  secret handling、push、destructive operation は不変。SSOT では必要時の product
  API/DB/auth/authorization/PHI/billing/deploy/package dependency 変更許可を確認済みだが、
  この slice では不要。
- 検証: focused saved views content Vitest `1 file / 15 tests` green、
  scoped ESLint green、targeted Prettier check green、targeted `git diff --check` green、
  `pnpm typecheck` green。
- commit: `dc81e08b` (`refactor(ui): reuse readApiJson for saved views`)。
- 残課題: R40/R44 は broad。追加の client fetcher/mutation は response body read が PHI-safe かを
  個別確認してから段階移行する。
  未所有 `refactor-instructions.md` と `.agents/skills/**` / `skills-lock.json` は保持。

## 2026-07-05 R40/R44 site switching readApiJson slice

- 分類: query-helper / client mutation error handling → `readApiJson` 収束。
- 実施:
  - site switching の `/api/me/site` PUT response を `readApiJson<unknown>` へ移行。
  - failed site switch の API JSON `message` が toast へ表面化し、`/dashboard` へ遷移しない
    UI 契約テストを追加。
- 挙動変更: mutation response handling internals の helper 収束のみ。`/api/me/sites` read、
  `/api/me/site` PUT endpoint、`buildOrgHeaders`、`buildOrgJsonHeaders`、request body、
  success toast、`me-sites` invalidation、successful dashboard navigation は維持。
- 安全性: site selection UI mutation internals のみ変更。DB/schema、API route、auth/authorization、
  PHI projection、billing、deployment、package dependency、live DB operation、external send、
  secret handling、push、destructive operation は不変。SSOT では必要時の product
  API/DB/auth/authorization/PHI/billing/deploy/package dependency 変更許可を確認済みだが、
  この slice では不要。
- 検証: focused select-site content Vitest `1 file / 3 tests` green、
  scoped ESLint green、targeted Prettier check green、targeted `git diff --check` green、
  `pnpm typecheck` green。
- commit: `d88a6fa0` (`refactor(ui): reuse readApiJson for site switching`)。
- 残課題: R40/R44 は broad。追加の client fetcher/mutation は response body read が PHI-safe かを
  個別確認してから段階移行する。
  未所有 `refactor-instructions.md` と `.agents/skills/**` / `skills-lock.json` は保持。
...
- 分類: query-helper / client mutation error handling → `readApiJson` 収束。
- 実施:
  - service-area create/update の POST/PATCH responses を `readApiJson<unknown>` へ移行。
  - service-area delete の DELETE response を `readApiJson<unknown>` へ移行。
  - failed save / delete の API JSON `message` が toast へ表面化する UI 契約テストを追加。
- 挙動変更: mutation response handling internals の helper 収束のみ。`/api/pharmacy-sites` read、
  `/api/service-areas` read、`SERVICE_AREAS_API_PATH`、`buildServiceAreaApiPath`、
  hostile-id encoding、dot-segment fail-closed、org headers、request bodies、success toasts、
  `service-areas` invalidation は維持。
- 安全性: admin service areas UI mutation internals のみ変更。DB/schema、API route、
  auth/authorization、PHI projection、billing、deployment、package dependency、live DB operation、
  external send、secret handling、push、destructive operation は不変。SSOT では必要時の product
  API/DB/auth/authorization/PHI/billing/deploy/package dependency 変更許可を確認済みだが、
  この slice では不要。
- 検証: focused service-areas Vitest `1 file / 17 tests` green、
  scoped ESLint green、targeted Prettier check green、targeted `git diff --check` green、
  `pnpm typecheck` green。
- commit: `b6a7cf80` (`refactor(ui): reuse readApiJson for service areas`)。
- 残課題: R40/R44 は broad。追加の client fetcher/mutation は response body read が PHI-safe かを
  個別確認してから段階移行する。
  未所有 `refactor-instructions.md` と `.agents/skills/**` / `skills-lock.json` は保持。
