...
| R23 | 20    | dup-helper (M)            | `messageFromError(e, fallback)`抽出候補 — 同型ternaryが174箇所/66ファイルに重複（141箇所はtoast.error内）。既存 `src/lib/utils/error-message.ts` を使い、prescription intake submit/form の2箇所を収束し empty Error message fallback を test-locked(a510f2c2)。                                                                                              | partial(a510f2c2)                |
