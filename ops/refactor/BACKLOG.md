...
| R25 | 20    | pattern-inconsistency (M) | ErrorState retry定型句(`action={{label:'再試行'...}}`)が106箇所で手組み。`ErrorState.retryLabel` を追加し admin analytics/realtime の5箇所を `onRetry` shorthand へ移行済み(2e8589a4)。残: 他画面の段階移行。                                                                                                                            | partial(2e8589a4)                |
