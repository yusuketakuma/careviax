...
| R23 | 20    | dup-helper (M)            | `messageFromError(e, fallback)`抽出候補 — 同型ternaryが174箇所/66ファイルに重複（141箇所はtoast.error内）。既存 `src/lib/utils/error-message.ts` を使い、prescription intake submit/form の2箇所を収束し empty Error message fallback を test-locked(a510f2c2)。patient form、dispense workbench、schedule proposals、print hub、search、audit logs、drug master import preview、patient medications add-dialog、billing rules JSON validation、MFA setup/verify、QR scan draft-send、schedule optimizer facility aggregation の query/mutation/print-history/search/export/import-preview/add-dialog/validation/auth/send/aggregation error formatting も helper へ収束(a47b52fa,ad956006,6d098a9b,42d866e3,4f1c7321,872624db,fea1cd37,318f04d1,8c3b18aa,bae0f7c1,74b7e6ea,58c42de5)。残スキャンは query detail optional / sentinel / API error-code checks が中心。 | partial(a510f2c2+a47b52fa+ad956006+6d098a9b+42d866e3+4f1c7321+872624db+fea1cd37+318f04d1+8c3b18aa+bae0f7c1+74b7e6ea+58c42de5) |
| R24 | 20    | pattern-inconsistency (M) | 約27箇所の手組みカーソルページネーションを既存`buildCursorPage`へ収束。`/api/billing-candidates` の `data/hasMore/nextCursor + summary` envelope は helper へ移行し overflow contract を test-lock(b4185e59)。`/api/medication-cycles` の offset cursor envelope も helper へ移行し numeric cursor contract を test-lock(c8f03405)。`/api/consent-records` も document URL redaction/audit visible-row semantics を維持して helper 化(fd774353)。correction-requests の audit visible-row slice も response page と同じ helper に収束(5febff57)。 | partial(b4185e59+c8f03405+fd774353+5febff57) |
| R25 | 20    | pattern-inconsistency (M) | ErrorState retry定型句(`action={{label:'再試行'...}}`)が106箇所で手組み。`ErrorState.retryLabel`/`retryVariant`/`retrySize` を追加し、admin analytics/realtime、admin performance、patient medications、prescription intake、operating-hours、drug-masters、business-holidays、pharmacy-sites、data-explorer、dispense patient-list、schedule calendar、route/global boundary、statistics/admin metrics/analytics stale、tasks/handoff、admin panels、document-template/pharmacist credential、alert/audit、prescription/QR、patient context、schedule/visit、reports/partner billing/prescriptions table/notification/PCA、drug-master detail/formulary/shifts の retry action を `onRetry` shorthand へ移行済み。AST scan 残は non-retry href ErrorState action 5件のみ。 | done(2e8589a4+1ea41977+06430afb+b9a55665+a33d3c0b+06784c51+e42242d2+176035a0+b9095cfa+596afee7+7461d41f+823307ae+00344de5+90388a2e+e45a869e+fc496fa2+e56519ab+ad234b5a+47cbe882+901c4276+e6e73fa4+72b1f57c+0f3d0151+3ed6ad78) |
...
| R46 | 17    | pattern-inconsistency (S) | `buildCursorPage`未採用の手組みhasMore/slice/nextCursorが約20ルートに残存。billing-candidates の単純 cursor envelope は helper 収束済み(b4185e59)。medication-cycles の numeric offset cursor envelope も helper 収束済み(c8f03405)。consent-records の PHI-redacted cursor envelope も helper 収束済み(fd774353)。correction-requests の audit visible-row slice も helper 収束済み(5febff57)。`meta.has_more`/keyset/scan-window/hidden-count/summary variants は個別分析を継続。 | partial(b4185e59+c8f03405+fd774353+5febff57) |
