...
| `ID-2-W3` | infra/db           | display_id 第3波。`visit.prisma` の10 model + `communication.prisma` の14 direct org-scoped model に nullable `display_id` と tenant-local unique 宣言を追加し、W1/W2 と同じ partial unique migration + generic backfill script で既存 row を org別 `created_at,id` 順に E2 allocator range 採番する。`HandoffItem` は orgViaParent のため W7 残余。 | `prisma/schema/visit.prisma`, `prisma/schema/communication.prisma`, `tools/scripts/backfill-display-ids.ts`, `src/lib/db/display-id.test.ts` | 5    | 5    | 5          | いいえ   | in-progress(codex) |
