...
| `ID-2-W1` | infra/db           | display_id 第1波。`patient.prisma` の18 org-scoped model に nullable `display_id` と tenant-local unique 宣言を追加し、DB migration は partial unique index + generic backfill script で既存 row を org別 `created_at,id` 順に E2 allocator range 採番する。                                                                  | `prisma/schema/patient.prisma`, `tools/scripts/backfill-display-ids.ts`, `src/lib/db/display-id.test.ts`                 | 5    | 5    | 5          | いいえ   | in-progress(codex) |
