...
| R24 | 20    | pattern-inconsistency (M) | 約27箇所の手組みカーソルページネーションを既存`buildCursorPage`へ収束。`/api/billing-candidates` の `data/hasMore/nextCursor + summary` envelope は helper へ移行し overflow contract を test-lock(b4185e59)。`/api/medication-cycles` の offset cursor envelope も helper へ移行し numeric cursor contract を test-lock(c8f03405)。       | partial(b4185e59+c8f03405)       |
...
| R46 | 17    | pattern-inconsistency (S) | `buildCursorPage`未採用の手組みhasMore/slice/nextCursorが約20ルートに残存。billing-candidates の単純 cursor envelope は helper 収束済み(b4185e59)。medication-cycles の numeric offset cursor envelope も helper 収束済み(c8f03405)。`meta.has_more`/keyset/scan-window/hidden-count/summary variants は個別分析を継続。                 | partial(b4185e59+c8f03405)       |
