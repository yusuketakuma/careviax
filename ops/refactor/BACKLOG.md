...
| `ID-2-UR` | infra/db           | display_id 波 残余/追跡。opus W4 レビュー M-1: `User` は registry で `scope='org'` だが波計画では global(W6) 扱いで W4 除外 → registry と wave 計画の分類矛盾。orphan 化しないよう `CXR2-RLS02`（User の auth/global identity design 判定待ち）の結論で確定させ、W6 で registry 是正 or User org-wave 追加のいずれかを実施する。`HandoffItem`(orgViaParent) も W7 残余。L-1: 全 `scope='org'`/`orgViaParent` registry model がいずれかの wave.models に含まれることを保証する completeness assertion を `display-id.test.ts` に追加すると取りこぼしを自動検知できる。 | `src/lib/db/display-id-registry.ts`, `src/lib/db/display-id.test.ts`, `docs/design/display-id-design.md`                                     | 4    | 3    | 4          | いいえ   | open           |
