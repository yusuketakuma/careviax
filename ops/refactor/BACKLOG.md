...
| R25 | 20    | pattern-inconsistency (M) | ErrorState retry定型句(`action={{label:'再試行'...}}`)が106箇所で手組み。`ErrorState.retryLabel` を追加し admin analytics/realtime の5箇所(2e8589a4)、admin performance の5箇所(1ea41977)、patient medications の5箇所(06430afb)を `onRetry` shorthand へ移行済み。残: 他画面の段階移行。                                            | partial(2e8589a4+1ea41977+06430afb) |
