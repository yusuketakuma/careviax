...
| `ID-1b`   | infra/db           | ID-1a で採用した E2 の採番基盤。`IdSequence` 内部カウンタ表、138件 prefix registry、`allocateDisplayId(tx, model, orgId)` / range / global sentinel、RLS intentional exclusion、direct access grep gate、local e2e DB concurrent/rollback テストを追加する。                                                                  | `docs/design/display-id-design.md:238-407`, `src/lib/db/display-id.ts`                                                   | 5    | 5    | 5          | いいえ   | in-progress(codex) |
