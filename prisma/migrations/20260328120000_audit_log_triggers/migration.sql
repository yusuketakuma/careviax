-- =============================================================================
-- CareViaX: DB-side audit logging for critical workflow tables
...
CREATE OR REPLACE FUNCTION careviax_to_snake_case(input_text TEXT)
RETURNS TEXT
LANGUAGE sql
IMMUTABLE
AS $$
...
CREATE OR REPLACE FUNCTION careviax_generate_audit_log_id()
RETURNS TEXT
LANGUAGE sql
AS $$
...
CREATE OR REPLACE FUNCTION careviax_write_audit_log()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
DECLARE
  ...
  v_target_type TEXT := careviax_to_snake_case(TG_TABLE_NAME);
  v_target_id TEXT;
  v_org_id TEXT;
  v_changes JSONB;
BEGIN
...
FOR EACH ROW EXECUTE FUNCTION careviax_write_audit_log();
