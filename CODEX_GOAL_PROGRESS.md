...
- Latest backend/API slice: `GET /api/dashboard/overdue` now wraps all handled GET responses in sensitive no-store headers, is registered in the protected GET matrix and curated route catalog, and returns only summary counts for overdue visits, unsent reports, and overdue tasks. Removed existing detail arrays/fields (`unrecorded_visits`, `unsent_reports`, `overdue_tasks`, patient ids/names, report metadata/timestamps, task titles, task priorities, and due timestamps) plus the detail `findMany`/patient-enrichment reads because no direct UI caller consumed them and they exposed PHI/free-text operational detail without product need. Fixed a correctness bug where the route's local task-scope helper undercounted tasks directly assigned to a scoped user; it now reuses `buildDashboardTaskAssignmentWhere()` so direct assignments remain visible even when no assigned cases/patients exist. `GET /api/meta/route-catalog` now also returns sensitive no-store headers on 200/403 catalog reads. Existing `canViewDashboard`, org scoping, summary response envelope, route-catalog admin permission, schema, migrations, external sends, and destructive operations are unchanged. Focused overdue/protected GET/catalog/meta-catalog Vitest, focused ESLint, focused Prettier, focused diff whitespace check, and full `pnpm typecheck` PASS. Previous Codex-only slices `GET /api/dashboard/dispensing-stats`, `GET /api/dashboard/clerk-support`, `GET /api/dashboard/monthly-stats`, `GET /api/dashboard/medication-deadlines`, `GET /api/dashboard/workflow`, `GET /api/dashboard/cockpit`, `GET /api/set-plans`, `GET /api/visit-schedule-proposals`, `GET /api/patients/:id/prescriptions`, `GET /api/cases`, `GET /api/patients?view=match`, `GET /api/patients` full-list response minimization, `GET /api/patients` no-store/duplicate-query hardening, `GET /api/patients/check-duplicate`, `GET /api/tasks`, `GET /api/dispense-tasks`, `GET /api/billing-candidates/export`, `GET /api/first-visit-documents`, `GET /api/billing-candidates`, `GET /api/medication-cycles`, and `GET /api/prescription-intakes` are committed.
