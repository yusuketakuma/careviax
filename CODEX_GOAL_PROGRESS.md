...
- Latest backend/API slice: `GET /api/dashboard/cockpit` now wraps all handled GET responses in sensitive no-store headers, rejects explicit blank/padded/unsupported/duplicate `scope` before cache or DB reads, and is registered in the route catalog plus protected GET matrix no-store assertions. Existing default/no-query caller behavior, valid `scope=mine|team`, non-admin `team` downgrade to `mine`, assignment scoping, server-side cockpit cache TTL/keying, response envelope, schema, migrations, external sends, and destructive operations are unchanged. Focused dashboard cockpit/protected GET/catalog Vitest, focused ESLint, focused Prettier, focused diff whitespace check, and full `pnpm typecheck` PASS. Privacy and API contract reviewers returned PASS. Previous Codex-only slices `GET /api/set-plans`, `GET /api/visit-schedule-proposals`, `GET /api/patients/:id/prescriptions`, `GET /api/cases`, `GET /api/patients?view=match`, `GET /api/patients` full-list response minimization, `GET /api/patients` no-store/duplicate-query hardening, `GET /api/patients/check-duplicate`, `GET /api/tasks`, `GET /api/dispense-tasks`, `GET /api/billing-candidates/export`, `GET /api/first-visit-documents`, `GET /api/billing-candidates`, `GET /api/medication-cycles`, and `GET /api/prescription-intakes` are committed.
