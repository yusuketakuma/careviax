...
- Latest backend/API slice: `GET /api/patients/check-duplicate` hardening is implemented with strict required `name` / `date_of_birth` / `gender` query validation, sensitive no-store wrapping for success/validation/auth/forbidden responses, duplicate candidate DTO minimization without `name_kana`, route-catalog and protected GET matrix coverage, privacy review coverage, focused lint/format/tests including patient-form compatibility, and full typecheck PASS. Commit is being prepared as a Codex-owned backend slice. Previous Codex-only slices `GET /api/tasks`, `GET /api/dispense-tasks`, `GET /api/billing-candidates/export`, `GET /api/first-visit-documents`, `GET /api/billing-candidates`, `GET /api/medication-cycles`, and `GET /api/prescription-intakes` are committed.
