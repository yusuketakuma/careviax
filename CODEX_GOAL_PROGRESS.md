...
- Latest backend/API slice: `GET /api/dashboard/clerk-support` now wraps all handled GET responses in sensitive no-store headers, is registered in the protected GET matrix and curated route catalog, and locks the clerk task item response to the minimum intended fields (`id`, `kind_label`, `patient_name`, `next_action`, `due_label`, `href`). Existing six-KPI aggregation, encoded proposal detail hrefs, patient-name task contract, `canViewDashboard`, org scoping, query-less caller behavior, schema, migrations, external sends, and destructive operations are unchanged. Focused clerk-support/protected GET/catalog Vitest, focused ESLint, focused Prettier, focused diff whitespace check, and full `pnpm typecheck` PASS. API contract and privacy reviewers returned no blocking findings; remaining low-risk follow-ups are whether thrown framework 500 responses should be route-wrapped with no-store and whether `canViewDashboard` remains the intended permission boundary for named clerk task data. Previous Codex-only slices `GET /api/dashboard/monthly-stats`, `GET /api/dashboard/medication-deadlines`, `GET /api/dashboard/workflow`, `GET /api/dashboard/cockpit`, `GET /api/set-plans`, `GET /api/visit-schedule-proposals`, `GET /api/patients/:id/prescriptions`, `GET /api/cases`, `GET /api/patients?view=match`, `GET /api/patients` full-list response minimization, `GET /api/patients` no-store/duplicate-query hardening, `GET /api/patients/check-duplicate`, `GET /api/tasks`, `GET /api/dispense-tasks`, `GET /api/billing-candidates/export`, `GET /api/first-visit-documents`, `GET /api/billing-candidates`, `GET /api/medication-cycles`, and `GET /api/prescription-intakes` are committed.
