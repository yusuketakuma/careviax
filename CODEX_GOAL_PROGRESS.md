...
- Latest backend/API slice: `GET /api/tasks` hardening is implemented with strict `task_type` / `task_types` / `status` / `priority` / `assigned_to` / `related_entity_type` / `related_entity_id` query validation, sensitive no-store wrapping for success/validation/auth/forbidden responses, minimal task list DTO projection, route-catalog and protected GET matrix coverage, API-contract review coverage, focused lint/format/tests, and full typecheck PASS. Commit is being prepared as a Codex-owned backend slice. Previous Codex-only slices `GET /api/dispense-tasks`, `GET /api/billing-candidates/export`, `GET /api/first-visit-documents`, `GET /api/billing-candidates`, `GET /api/medication-cycles`, and `GET /api/prescription-intakes` are committed.
