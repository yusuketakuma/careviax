...
- Latest backend/API slice: `GET /api/dashboard/workflow` now wraps all handled GET responses in sensitive no-store headers and rejects explicit blank/padded/unsupported/duplicate `view` before assignment-scope resolution, workflow cache reads, or dashboard aggregate DB/service reads. Existing no-query full dashboard behavior, exact `view=full|phase|realtime|performance`, assignment scoping, cache key shape, response envelope, route catalog entry, schema, migrations, external sends, and destructive operations are unchanged. Focused dashboard workflow/protected GET/catalog Vitest, focused ESLint, focused Prettier, focused diff whitespace check, and full `pnpm typecheck` PASS. Privacy and API contract reviewers returned PASS. Previous Codex-only slices `GET /api/dashboard/cockpit`, `GET /api/set-plans`, `GET /api/visit-schedule-proposals`, `GET /api/patients/:id/prescriptions`, `GET /api/cases`, `GET /api/patients?view=match`, `GET /api/patients` full-list response minimization, `GET /api/patients` no-store/duplicate-query hardening, `GET /api/patients/check-duplicate`, `GET /api/tasks`, `GET /api/dispense-tasks`, `GET /api/billing-candidates/export`, `GET /api/first-visit-documents`, `GET /api/billing-candidates`, `GET /api/medication-cycles`, and `GET /api/prescription-intakes` are committed.
