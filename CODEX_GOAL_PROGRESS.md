...
- Latest backend/API slice: `GET /api/first-visit-documents` hardening is implemented with strict `patient_id` / `case_id` query validation, sensitive no-store wrapping, route-catalog/protected GET coverage, focused tests, patient document UI compatibility verification, API-contract review PASS, privacy review PASS, and full typecheck PASS. Commit is being prepared as a Codex-owned backend slice. Previous Codex-only slices `GET /api/billing-candidates`, `GET /api/medication-cycles`, and `GET /api/prescription-intakes` are committed.
