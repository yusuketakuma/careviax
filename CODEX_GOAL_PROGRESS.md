...
- Latest backend/API slice: `GET /api/visit-schedule-proposals` now wraps GET responses in sensitive no-store headers, rejects explicit blank/padded/duplicate `case_id`, `patient_id`, `status`, `pharmacist_id`, `date_from`, `date_to`, `q`, `limit`, and `view` filters before proposal/user DB reads, and is registered in the route catalog plus protected GET matrix no-store assertions. Existing `canVisit`, org scoping, assignment scoping, valid schedule/search callers, palette minimal projection, list projection, POST/PUT proposal mutation behavior, schema, migrations, external sends, and destructive operations are unchanged. Focused visit-schedule-proposals/protected GET/catalog Vitest, focused ESLint, focused Prettier, focused diff whitespace check, and full `pnpm typecheck` PASS. Privacy reviewer returned PASS; API contract reviewer initially caught padded `q`, which was fixed and re-reviewed PASS. Previous Codex-only slices `GET /api/patients/:id/prescriptions`, `GET /api/cases`, `GET /api/patients?view=match`, `GET /api/patients` full-list response minimization, `GET /api/patients` no-store/duplicate-query hardening, `GET /api/patients/check-duplicate`, `GET /api/tasks`, `GET /api/dispense-tasks`, `GET /api/billing-candidates/export`, `GET /api/first-visit-documents`, `GET /api/billing-candidates`, `GET /api/medication-cycles`, and `GET /api/prescription-intakes` are committed.
