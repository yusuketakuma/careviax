...
- Latest backend/API slice: `GET /api/patients/board` now wraps all handled GET responses in sensitive no-store headers, rejects duplicate single-value board query params before patient/aggregate reads, is registered in the protected GET matrix and curated route catalog, and has direct 200/400 no-store regression tests plus matrix 401/403 coverage. This endpoint is a high-frequency patient board BFF, but response body shape, `canVisit` permission, org scoping, valid `scope`/`foundation_issue` behavior, frontend caller contract, schema, migrations, external sends, and destructive operations are unchanged. Focused patients-board/protected GET/catalog/meta-catalog Vitest, focused ESLint, focused Prettier, focused diff whitespace check, and full `pnpm typecheck` PASS. API contract and privacy reviews returned PASS with no blockers; the API review's non-blocking field-error assertion note was addressed before final validation. Previous Codex-only slices `GET /api/patients/:id/overview`, `GET /api/dispense-tasks/:id/workbench`, `GET /api/dashboard/overdue`, `GET /api/dashboard/dispensing-stats`, `GET /api/dashboard/clerk-support`, `GET /api/dashboard/monthly-stats`, `GET /api/dashboard/medication-deadlines`, `GET /api/dashboard/workflow`, `GET /api/dashboard/cockpit`, `GET /api/set-plans`, `GET /api/visit-schedule-proposals`, `GET /api/patients/:id/prescriptions`, `GET /api/cases`, `GET /api/patients?view=match`, `GET /api/patients` full-list response minimization, `GET /api/patients` no-store/duplicate-query hardening, `GET /api/patients/check-duplicate`, `GET /api/tasks`, `GET /api/dispense-tasks`, `GET /api/billing-candidates/export`, `GET /api/first-visit-documents`, `GET /api/billing-candidates`, `GET /api/medication-cycles`, and `GET /api/prescription-intakes` are committed.
