...
- Latest backend/API slice: `GET /api/dashboard/medication-deadlines` now wraps all handled GET responses in sensitive no-store headers, rejects blank/padded/duplicate/malformed `within_days`, `limit`, and `q` before medication-deadline visit-schedule reads, and is registered in the route catalog plus protected GET matrix no-store assertions. Existing omitted `within_days` default of 7, exact integer `within_days=0..365`, valid global-search `q`/`limit` caller, response envelope, auth/org scoping, schema, migrations, external sends, and destructive operations are unchanged. Focused dashboard medication-deadlines/protected GET/catalog Vitest, focused ESLint, focused Prettier, focused diff whitespace check, and full `pnpm typecheck` PASS. Privacy and API contract reviewers returned PASS. Previous Codex-only slices `GET /api/dashboard/workflow`, `GET /api/dashboard/cockpit`, `GET /api/set-plans`, `GET /api/visit-schedule-proposals`, `GET /api/patients/:id/prescriptions`, `GET /api/cases`, `GET /api/patients?view=match`, `GET /api/patients` full-list response minimization, `GET /api/patients` no-store/duplicate-query hardening, `GET /api/patients/check-duplicate`, `GET /api/tasks`, `GET /api/dispense-tasks`, `GET /api/billing-candidates/export`, `GET /api/first-visit-documents`, `GET /api/billing-candidates`, `GET /api/medication-cycles`, and `GET /api/prescription-intakes` are committed.
