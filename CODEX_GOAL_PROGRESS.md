...
- Latest backend/API slice: `GET /api/set-plans` now wraps GET responses in sensitive no-store headers, rejects explicit blank/padded/duplicate/overlong `cycle_id` and `patient_id` filters before set-plan DB reads, and is registered in the route catalog plus protected GET matrix no-store assertions. Existing `canSet`, org scoping, assignment scoping, print-hub no-query caller, valid `cycle_id` and `patient_id` filters, response envelope, POST create behavior, schema, migrations, external sends, and destructive operations are unchanged. Focused set-plans/protected GET/catalog Vitest, focused ESLint, focused Prettier, focused diff whitespace check, and full `pnpm typecheck` PASS. Privacy and API contract reviewers returned PASS. Previous Codex-only slices `GET /api/visit-schedule-proposals`, `GET /api/patients/:id/prescriptions`, `GET /api/cases`, `GET /api/patients?view=match`, `GET /api/patients` full-list response minimization, `GET /api/patients` no-store/duplicate-query hardening, `GET /api/patients/check-duplicate`, `GET /api/tasks`, `GET /api/dispense-tasks`, `GET /api/billing-candidates/export`, `GET /api/first-visit-documents`, `GET /api/billing-candidates`, `GET /api/medication-cycles`, and `GET /api/prescription-intakes` are committed.
