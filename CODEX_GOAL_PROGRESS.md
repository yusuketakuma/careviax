...
- Latest backend/API slice: `GET /api/billing-candidates/export` hardening is implemented with strict `billing_month` / `patient_id` / `billing_domain` / `format` / `preview` query validation, sensitive no-store wrapping for auth failures and export responses, focused tests, protected GET no-store coverage, API-contract review PASS, privacy review PASS, and full typecheck PASS. Commit is being prepared as a Codex-owned backend slice. Previous Codex-only slices `GET /api/first-visit-documents`, `GET /api/billing-candidates`, `GET /api/medication-cycles`, and `GET /api/prescription-intakes` are committed.
