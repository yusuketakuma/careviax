...
- Latest backend/API slice: `GET /api/staff-workload` now wraps handled and unexpected GET responses in sensitive no-store headers, rejects padded/duplicate `date` query params before staff/task/visit/dispense reads, is registered in the protected GET matrix and curated route catalog, and minimizes staff preview DTOs to only the current tasks screen fields: visits `{id, patient_name}` and open tasks `{id, title}` capped at two each. The route now returns a fixed `INTERNAL_ERROR` no-store 500 envelope on unexpected DB/read failures without raw error text. Existing `canVisit`, tenant scoping, valid date semantics, current tasks-screen behavior, schema, migrations, external sends, and destructive operations are unchanged; the only frontend touch is a type contract shrink to match the minimized response. Focused staff-workload/protected GET/catalog/meta-catalog/tasks-content Vitest, focused ESLint, focused Prettier, focused diff whitespace check, and full `pnpm typecheck` PASS under long-gate token `C34991F1-A046-4A7A-A50D-539F278A214A`. API contract and privacy reviews returned PASS with no blockers. Previous Codex-only slices `GET /api/visit-schedules/day-board`, `GET /api/visits/today-preparation`, `GET /api/patients/board`, `GET /api/patients/:id/overview`, `GET /api/dispense-tasks/:id/workbench`, `GET /api/dashboard/overdue`, `GET /api/dashboard/dispensing-stats`, `GET /api/dashboard/clerk-support`, `GET /api/dashboard/monthly-stats`, `GET /api/dashboard/medication-deadlines`, `GET /api/dashboard/workflow`, `GET /api/dashboard/cockpit`, `GET /api/set-plans`, `GET /api/visit-schedule-proposals`, `GET /api/patients/:id/prescriptions`, `GET /api/cases`, `GET /api/patients?view=match`, `GET /api/patients` full-list response minimization, `GET /api/patients` no-store/duplicate-query hardening, `GET /api/patients/check-duplicate`, `GET /api/tasks`, `GET /api/dispense-tasks`, `GET /api/billing-candidates/export`, `GET /api/first-visit-documents`, `GET /api/billing-candidates`, `GET /api/medication-cycles`, and `GET /api/prescription-intakes` are committed.
