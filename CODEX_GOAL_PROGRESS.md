...
- Latest backend/API slice: `GET /api/patients?view=match` now provides a bounded minimal identity response for QR scan and prescription patient typeahead (`id`, `name`, `name_kana`, `birth_date`, `gender`, `hasMore`) using an exact Prisma `select` without full-list enrichment. The route rejects missing/blank `q`, malformed/oversized `limit`, duplicate `view`, and full-list-only filters before patient DB/enrichment reads, preserving sensitive no-store coverage. QR scan and prescription-intake callers now use `view=match&limit=10` instead of the full patient list. Privacy reviewer returned no blocking issues; low test-coverage findings were addressed by locking 401/403 no-store and match org-boundary assertions. Focused patients/protected GET/caller contract Vitest, focused ESLint, focused targeted Prettier, focused diff whitespace check, and full `pnpm typecheck` PASS; repo-wide `pnpm format:check` is currently blocked only by unrelated untracked `.agent-loop/plans/*` files outside this Codex slice. Previous Codex-only slices `GET /api/patients` full-list response minimization, `GET /api/patients` no-store/duplicate-query hardening, `GET /api/patients/check-duplicate`, `GET /api/tasks`, `GET /api/dispense-tasks`, `GET /api/billing-candidates/export`, `GET /api/first-visit-documents`, `GET /api/billing-candidates`, `GET /api/medication-cycles`, and `GET /api/prescription-intakes` are committed.
