...
- Latest backend/API slice: `GET /api/visit-schedules/day-board` now wraps all handled GET responses in sensitive no-store headers, rejects blank/padded/duplicate `date` query params before day-board DB reads, is registered in the protected GET matrix and curated route catalog, and has direct 200/400 no-store regression coverage plus matrix 401/403 coverage. This endpoint is a high-frequency schedule board BFF returning patient/staff names, visit/proposal/task scheduling data, and vehicle-resource summaries, but response body shape for valid callers, `canVisit` permission, tenant scoping, frontend caller contract, schema, migrations, external sends, and destructive operations are unchanged. Focused day-board/protected GET/catalog/meta-catalog Vitest, focused ESLint, focused Prettier, focused diff whitespace check, and full `pnpm typecheck` PASS. API contract and privacy reviews returned PASS with no blockers. Previous Codex-only slices `GET /api/visits/today-preparation`, `GET /api/patients/board`, `GET /api/patients/:id/overview`, `GET /api/dispense-tasks/:id/workbench`, `GET /api/dashboard/overdue`, `GET /api/dashboard/dispensing-stats`, `GET /api/dashboard/clerk-support`, `GET /api/dashboard/monthly-stats`, `GET /api/dashboard/medication-deadlines`, `GET /api/dashboard/workflow`, `GET /api/dashboard/cockpit`, `GET /api/set-plans`, `GET /api/visit-schedule-proposals`, `GET /api/patients/:id/prescriptions`, `GET /api/cases`, `GET /api/patients?view=match`, `GET /api/patients` full-list response minimization, `GET /api/patients` no-store/duplicate-query hardening, `GET /api/patients/check-duplicate`, `GET /api/tasks`, `GET /api/dispense-tasks`, `GET /api/billing-candidates/export`, `GET /api/first-visit-documents`, `GET /api/billing-candidates`, `GET /api/medication-cycles`, and `GET /api/prescription-intakes` are committed.
