...
- Latest backend/API slice: `GET /api/dashboard/monthly-stats` now wraps all handled GET responses in sensitive no-store headers and rejects invalid/blank/padded/duplicate explicit `month` before monthly visit/patient reads. Existing omitted `month` current-month default, exact `YYYY-MM` behavior, patient/insurance-basis response envelope, auth/org scoping, route catalog entry, schema, migrations, external sends, and destructive operations are unchanged. Focused dashboard monthly-stats/protected GET/catalog Vitest, focused ESLint, focused Prettier, focused diff whitespace check, and full `pnpm typecheck` PASS. Privacy and API contract reviewers returned PASS. Previous Codex-only slices `GET /api/dashboard/medication-deadlines`, `GET /api/dashboard/workflow`, `GET /api/dashboard/cockpit`, `GET /api/set-plans`, `GET /api/visit-schedule-proposals`, `GET /api/patients/:id/prescriptions`, `GET /api/cases`, `GET /api/patients?view=match`, `GET /api/patients` full-list response minimization, `GET /api/patients` no-store/duplicate-query hardening, `GET /api/patients/check-duplicate`, `GET /api/tasks`, `GET /api/dispense-tasks`, `GET /api/billing-candidates/export`, `GET /api/first-visit-documents`, `GET /api/billing-candidates`, `GET /api/medication-cycles`, and `GET /api/prescription-intakes` are committed.
