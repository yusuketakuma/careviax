...
- Latest backend/API slice: `GET /api/patients` full-list response minimization is implemented by stripping raw contact phone/email/fax, care-team phone/email/fax, and preferred contact name/phone from patient list responses while preserving readiness/foundation filtering with raw rows internally. Privacy reviewer findings were addressed. Focused patients/protected GET Vitest, focused ESLint, focused Prettier with snapshot ignored as unknown, diff-check, and full `pnpm typecheck` PASS. Commit is being prepared as a Codex-owned backend privacy slice. Previous Codex-only slices `GET /api/patients` no-store/duplicate-query hardening, `GET /api/patients/check-duplicate`, `GET /api/tasks`, `GET /api/dispense-tasks`, `GET /api/billing-candidates/export`, `GET /api/first-visit-documents`, `GET /api/billing-candidates`, `GET /api/medication-cycles`, and `GET /api/prescription-intakes` are committed.
