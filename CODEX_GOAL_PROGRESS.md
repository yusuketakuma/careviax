...
- Latest backend/API slice: `GET /api/patients/:id/prescriptions` now wraps all GET responses in sensitive no-store headers, rejects explicit blank/padded/duplicate `case_id` filters before patient/case/prescription DB reads, and is registered in route catalog plus protected GET matrix no-store assertions. Existing `canVisit`, patient assignment access, accessible-case scoping, keyset cursor behavior including legacy numeric cursor compatibility, valid callers using `limit=5/20/100`, schema, migrations, external sends, and destructive operations are unchanged. Focused patient-prescriptions/protected GET/catalog Vitest, focused ESLint, focused Prettier, focused diff whitespace check, and full `pnpm typecheck` PASS. Privacy reviewer did not return after repeated waits and was closed by shutdown with no blocking result received before commit. Previous Codex-only slices `GET /api/cases`, `GET /api/patients?view=match`, `GET /api/patients` full-list response minimization, `GET /api/patients` no-store/duplicate-query hardening, `GET /api/patients/check-duplicate`, `GET /api/tasks`, `GET /api/dispense-tasks`, `GET /api/billing-candidates/export`, `GET /api/first-visit-documents`, `GET /api/billing-candidates`, `GET /api/medication-cycles`, and `GET /api/prescription-intakes` are committed.
