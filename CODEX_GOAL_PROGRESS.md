...
- Latest backend/API slice: `GET /api/cases` now wraps all GET responses in sensitive no-store headers and rejects explicit blank/duplicate/malformed `patient_id`, `status`, `q`, `limit`, and `cursor` query values before case/pharmacist DB reads. Existing `canVisit`, org scoping, assignment scoping, valid callers using `limit=8/20/100`, valid `status=active`, valid non-empty `q`, POST create behavior, schema, migrations, external sends, and destructive operations are unchanged. Privacy reviewer returned no blocking issues. Focused cases/protected GET Vitest, focused ESLint, focused Prettier, focused diff whitespace check, and full `pnpm typecheck` PASS after one type-only fix; repo-wide `pnpm format:check` remains blocked only by unrelated untracked `.agent-loop/plans/*` files outside Codex-owned slices. Previous Codex-only slices `GET /api/patients?view=match`, `GET /api/patients` full-list response minimization, `GET /api/patients` no-store/duplicate-query hardening, `GET /api/patients/check-duplicate`, `GET /api/tasks`, `GET /api/dispense-tasks`, `GET /api/billing-candidates/export`, `GET /api/first-visit-documents`, `GET /api/billing-candidates`, `GET /api/medication-cycles`, and `GET /api/prescription-intakes` are committed.
