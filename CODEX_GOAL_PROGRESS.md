...
- Latest backend/API slice: `GET /api/dispense-tasks` hardening is implemented with strict `status` / `cycle_id` / `assigned_to` query validation, sensitive no-store wrapping for success/validation/auth/forbidden responses, route-catalog and protected GET matrix coverage, cursor-pagination regression tests, API-contract review coverage, focused lint/format/tests, and full typecheck PASS. Commit is being prepared as a Codex-owned backend slice. Previous Codex-only slices `GET /api/billing-candidates/export`, `GET /api/first-visit-documents`, `GET /api/billing-candidates`, `GET /api/medication-cycles`, and `GET /api/prescription-intakes` are committed.
