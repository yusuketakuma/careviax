...
- Latest backend/API slice: `GET /api/visit-records/:id` now wraps handled and unexpected visit-record detail responses in sensitive no-store headers, returns a fixed `INTERNAL_ERROR` no-store 500 envelope on unexpected failures without raw error text, and runs all GET DB reads through `withOrgContext(ctx.orgId, ..., { requestContext: ctx })`. The route also removes `patient_state_snapshot` and `visit_geo_log` from the returned raw visit-record body before the success spread, while preserving existing top-level success shape, `canVisit`, org and assignment checks, PATCH behavior, schema, migrations, external sends, and destructive-operation boundaries. Protected GET matrix coverage now asserts 401/403 no-store for the detail read, and route catalog/meta-catalog tests explicitly pin `/api/visit-records` and `/api/visit-records/:id` entries. Focused visit-record detail/protected GET/catalog/meta-catalog Vitest, focused ESLint, focused Prettier, focused diff whitespace check, and full `pnpm typecheck` PASS under long-gate token `14DECAD7-CB41-40E5-A55E-5063A8403F26`. API contract reviewer returned PASS; privacy reviewer initially found missing RLS context and `visit_geo_log` exposure, then returned PASS after both fixes. Previous Codex-only slices `GET /api/management-plans`, `GET /api/management-plans/:id`, `GET /api/care-reports/analytics`, `GET /api/staff-workload`, `GET /api/visit-schedules/day-board`, `GET /api/visits/today-preparation`, `GET /api/patients/board`, `GET /api/patients/:id/overview`, `GET /api/dispense-tasks/:id/workbench`, `GET /api/dashboard/overdue`, `GET /api/dashboard/dispensing-stats`, `GET /api/dashboard/clerk-support`, `GET /api/dashboard/monthly-stats`, `GET /api/dashboard/medication-deadlines`, `GET /api/dashboard/workflow`, `GET /api/dashboard/cockpit`, `GET /api/set-plans`, `GET /api/visit-schedule-proposals`, `GET /api/patients/:id/prescriptions`, `GET /api/cases`, `GET /api/patients?view=match`, `GET /api/patients` full-list response minimization, `GET /api/patients` no-store/duplicate-query hardening, `GET /api/patients/check-duplicate`, `GET /api/tasks`, `GET /api/dispense-tasks`, `GET /api/billing-candidates/export`, `GET /api/first-visit-documents`, `GET /api/billing-candidates`, `GET /api/medication-cycles`, and `GET /api/prescription-intakes` are committed.
