...
- Latest backend/API slice: `GET /api/patients` hardening is implemented with sensitive no-store wrapping for success/validation/auth/forbidden responses, duplicate known query-key rejection before patient list/filter/enrichment reads, protected GET matrix coverage, and explicit compatibility coverage for legacy unknown query keys plus padded `limit`. Privacy and verifier subagents reviewed the slice. Focused patients/protected GET Vitest, focused ESLint, focused Prettier, diff-check, and full `pnpm typecheck` PASS. Commit is being prepared as a Codex-owned backend slice. Previous Codex-only slices `GET /api/patients/check-duplicate`, `GET /api/tasks`, `GET /api/dispense-tasks`, `GET /api/billing-candidates/export`, `GET /api/first-visit-documents`, `GET /api/billing-candidates`, `GET /api/medication-cycles`, and `GET /api/prescription-intakes` are committed.
