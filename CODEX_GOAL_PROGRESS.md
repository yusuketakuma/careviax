...
- Latest backend/API slice: `GET /api/dispense-tasks/:id/workbench` now wraps all handled GET responses in sensitive no-store headers and is covered by the protected GET matrix. The route returns patient name, prescription/medicine names, allergy/renal/caution labels, count rows, resolved inquiries, and operator context for a high-frequency dispensing/audit workbench, so 200/400/401/403/404 cache regression tests were added without changing the response shape. The route catalog now documents `/api/dispense-tasks/:id/workbench` as two method-accurate entries: `GET purpose-based` for `canDispense || canAuditDispense || canReport`, and `POST canDispense` for interrupt registration. Existing GET permissions, POST interrupt behavior, org/assignment scoping, response payload, schema, migrations, external sends, and destructive operations are unchanged. Focused workbench/protected GET/catalog/meta-catalog Vitest, focused ESLint, focused Prettier, focused diff whitespace check, and full `pnpm typecheck` PASS. API contract re-review returned PASS after catalog split and matrix setup fix; privacy review returned PASS and its low 400/404 no-store test-gap was addressed. Previous Codex-only slices `GET /api/dashboard/overdue`, `GET /api/dashboard/dispensing-stats`, `GET /api/dashboard/clerk-support`, `GET /api/dashboard/monthly-stats`, `GET /api/dashboard/medication-deadlines`, `GET /api/dashboard/workflow`, `GET /api/dashboard/cockpit`, `GET /api/set-plans`, `GET /api/visit-schedule-proposals`, `GET /api/patients/:id/prescriptions`, `GET /api/cases`, `GET /api/patients?view=match`, `GET /api/patients` full-list response minimization, `GET /api/patients` no-store/duplicate-query hardening, `GET /api/patients/check-duplicate`, `GET /api/tasks`, `GET /api/dispense-tasks`, `GET /api/billing-candidates/export`, `GET /api/first-visit-documents`, `GET /api/billing-candidates`, `GET /api/medication-cycles`, and `GET /api/prescription-intakes` are committed.
