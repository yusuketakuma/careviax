...
- Latest backend/API slice: `GET /api/dashboard/dispensing-stats` now wraps all handled GET responses in sensitive no-store headers, is registered in the protected GET matrix and curated route catalog, and returns only the three raw KPI fields used by the statistics screen (`pendingTasks`, `auditPendingTasks`, `completedToday`). Removed existing but unused `completedLast7Days` response data and its 7-day `dispenseTask.findMany` read because the current UI never displayed it and it exposed low-volume daily dispensing activity patterns into API/client memory without product need. Existing `canViewDashboard`, org scoping on all three counts, raw non-enveloped response contract, 403 locked-state caller behavior, schema, migrations, external sends, and destructive operations are unchanged. Focused dispensing-stats/protected GET/catalog/statistics caller Vitest, focused ESLint, focused Prettier, focused diff whitespace check, and full `pnpm typecheck` PASS. API contract and privacy reviewers returned no blocking findings after minimization, and privacy's tenant-boundary test gap was addressed with count-query assertions. Previous Codex-only slices `GET /api/dashboard/clerk-support`, `GET /api/dashboard/monthly-stats`, `GET /api/dashboard/medication-deadlines`, `GET /api/dashboard/workflow`, `GET /api/dashboard/cockpit`, `GET /api/set-plans`, `GET /api/visit-schedule-proposals`, `GET /api/patients/:id/prescriptions`, `GET /api/cases`, `GET /api/patients?view=match`, `GET /api/patients` full-list response minimization, `GET /api/patients` no-store/duplicate-query hardening, `GET /api/patients/check-duplicate`, `GET /api/tasks`, `GET /api/dispense-tasks`, `GET /api/billing-candidates/export`, `GET /api/first-visit-documents`, `GET /api/billing-candidates`, `GET /api/medication-cycles`, and `GET /api/prescription-intakes` are committed.
