...
test.beforeEach(async ({ context }) => {
  await context.addInitScript(() => {
    let owner = Object.getPrototypeOf(navigator) as object | null;
    while (owner && !Object.prototype.hasOwnProperty.call(owner, 'serviceWorker')) {
...
function buildGanttSchedule(args: {
  ...
  return {
    ...
    scheduled_date: `${GANTT_DATE}T00:00:00.000Z`,
    time_window_start: `${GANTT_DATE}T${args.start}:00.000Z`,
    time_window_end: `${GANTT_DATE}T${args.end}:00.000Z`,
    pharmacist_id: args.pharmacistId,
    assignment_mode: 'primary',
    route_order: args.routeOrder,
    facility_batch_id: null,
    confirmed_at: `${GANTT_DATE}T07:30:00.000Z`,
    case_: {
      patient: {
        id: args.patientId,
    ...
    preparation: {
      ...
      prepared_at: args.prepared ? `${GANTT_DATE}T07:45:00.000Z` : null,
...
function buildProposalBulkRouteMockProposal(args: {
  ...
  return {
    ...
    proposed_date: `${PROPOSAL_BULK_DATE}T00:00:00.000Z`,
    time_window_start: `${PROPOSAL_BULK_DATE}T${args.start}:00.000Z`,
    time_window_end: `${PROPOSAL_BULK_DATE}T${args.end}:00.000Z`,
    ...
    contact_logs: [],
    updated_at: `${PROPOSAL_BULK_DATE}T00:00:00.000Z`,
  };
}
...
function buildProposalBillingPreviewMap(
  proposals: ReadonlyArray<(typeof PROPOSAL_BULK_ROUTE_MOCK_PROPOSALS)[number]>,
) {
    ...
    proposals.map((proposal) => [
      proposal.id,
      {
        ...
        cadence: {
          ...
          suggested_dates: [],
          reason: 'RouteMock 算定間隔確認',
        },
        recommended_visit_type: 'regular',
        recommended_priority: 'normal',
        suggested_schedule_slot_count: 0,
        effective_revision_code: 'route_mock_revision',
        effective_revision_label: 'RouteMock revision',
        site_config_status: 'resolved',
        site_config_revision_code: 'route_mock_revision',
        warnings: [],
...
function proposalBulkTargetName(
  ...
  return `${patientName} 2026/05/08 ${timeRange} / 薬剤師A / 社用車A (最大6件 / 180分以内) / ケース ${shortEntityIdentifier(ids.caseId)} / 候補 ${shortEntityIdentifier(ids.proposalId)}`;
}
    ...
    await fulfillJson(route, {
      data: {
        ...
        shared_summary: {
          ...
          next_visit_date: '2026-04-30T00:00:00.000Z',
        },
        scope: {
          allergy_info: true,
...
async function installPharmacyCooperationRouteMocks(
  ...
  const state = {
    ...
    partnerRecordCreated: options.partnerRecordCreated ?? false,
  ...
  await page.route(
    ...
    async (route) => {
      await fulfillJson(route, { data: buildPharmacyCoopDocumentsSnapshot() });
    },
  );
  ...
  await page.route(apiPathPattern('/api/pharmacy-cooperation-message-threads'), async (route) => {
    ...
    if (request.method === 'POST') {
      ...
      await fulfillJson(route, { data: { thread, notification_count: 1 } }, 201);
      return;
    }
  ...
  await page.route(apiPathPattern('/api/partner-visit-records'), async (route) => {
    ...
    const url = new URL(request.url);
    const statusFilter = url.searchParams.get('status');
    const rows = record && (!statusFilter || record.status === statusFilter) ? [record] : [];
    const statusCounts = {
      draft: 0,
      submitted: 0,
      confirmed: 0,
  ...
  await page.route(
    ...
    async (route) => {
      ...
      await fulfillJson(route, {
        data: { id: PHARMACY_COOP_PARTNER_RECORD_ID, status: 'submitted' },
      });
    },
  );
  ...
  await page.route(
    ...
    async (route) => {
      ...
      await fulfillJson(route, {
        data: { id: PHARMACY_COOP_PARTNER_RECORD_ID, status: 'confirmed' },
      });
    },
  );
...
async function installFormularyRouteMocks(page: Page) {
  ...
  await page.route(apiPathPattern('/api/drug-master-imports/status'), async (route) => {
    await fulfillJson(route, {
      data: {
        sources: [
          ['ssk', '支払基金 医薬品マスター', true],
          ['mhlw_price', '厚労省 薬価基準収載品目リスト', true],
          ['mhlw_generic', '厚労省 後発医薬品リスト', true],
          ['hot', 'HOTコードマスター', false],
          ['pmda', 'PMDA 添付文書', false],
          ['manual_clinical', '手動臨床マスター', true],
        ].map(([source, label, isFree]) => ({
          source,
          label,
          is_free: isFree,
          threshold_days: 30,
  ...
  await page.route(apiPathPattern('/api/pharmacy-drug-stocks/usage-mismatch'), async (route) => {
    await fulfillJson(route, {
      data: {
        ...
        list_counts: Object.fromEntries(
          [
            'frequent_unstocked',
            'unused_stocked',
...
async function installScheduleProposalBulkRouteMocks(page: Page) {
  ...
  await page.route(
    ...
    async (route) => {
      ...
      await fulfillJson(route, {
        data: buildProposalBillingPreviewMap(PROPOSAL_BULK_ROUTE_MOCK_PROPOSALS),
      });
    },
  );
...
async function installScheduleProposalSearchRouteMocks(page: Page) {
  ...
  await page.route(
    ...
    async (route) => {
      billingPreviewRequests.push(captureRouteRequest(route));
      await fulfillJson(route, {
        data: buildProposalBillingPreviewMap(PROPOSAL_SEARCH_ROUTE_MOCK_PROPOSALS),
      });
    },
  );
...
test.describe('schedule proposals route-mocked bulk safety smoke', () => {
  ...
  test('keeps proposal bulk reject keyboard flow PHI-minimized and target-specific', async ({
    ...
    expect(patchBodiesByProposalId.get(PROPOSAL_BULK_SUCCESS_ID)).toEqual({
      action: 'reject',
      expected_updated_at: `${PROPOSAL_BULK_DATE}T00:00:00.000Z`,
...
test.describe('pharmacy cooperation route-mocked browser workflow smoke', () => {
  ...
  test('keeps partner-record exact counts and server cursor filters accessible', async ({
    context,
  }, testInfo) => {
    test.skip(testInfo.project.name !== 'chromium');
    ...
    await openStableRoute(page, '/workflow/pharmacy-cooperation');
    const recordsTable = page.getByRole('table', { name: '協力訪問記録一覧' });
    const recordRow = recordsTable.getByRole('row').filter({
      hasText: PHARMACY_COOP_PARTNER_RECORD_ID,
    });
    await expect(recordRow).toBeVisible({ timeout: 30_000 });
    await expect(
      page.getByText('協力訪問記録 読込済み 1 / 全 1 件（全件読込済み）').first(),
    ).toBeVisible();

    await page.getByLabel('協力訪問記録状態').selectOption('draft');
    await expect
      .poll(
        () =>
          requests.partnerVisitRecords.some((request) => {
            if (request.method !== 'GET') return false;
            const searchParams = new URL(request.url).searchParams;
            return searchParams.get('status') === 'draft' && !searchParams.has('cursor');
          }),
        { message: 'partner-record status changes should start a fresh cursor chain at page one' },
      )
      .toBe(true);
  ...
  test('proves share consent, link activation, visit, billing, and report draft flow', async ({
    ...
    await expect
      ...
      .toBe(true);
    await page.getByLabel('協力訪問記録状態').selectOption('all');
    await expect(partnerRecordRow).toBeVisible();
