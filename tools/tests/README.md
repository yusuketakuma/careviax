...
that the local `careviax_e2e` database target, app port, DB port, required Playwright
...
`careviax_e2e` database so it does not accidentally use the default development DB.
...
`careviax_e2e` database using the same pinned connection string.
