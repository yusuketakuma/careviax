...
const APPROVED_S3_PREFIX_FRAGMENTS = [
  'arn:${AWS::Partition}:s3:::${EvidenceBucketName}/prescriptions/*',
  'arn:${AWS::Partition}:s3:::${EvidenceBucketName}/consent-documents/*',
  'arn:${AWS::Partition}:s3:::${EvidenceBucketName}/visit-photos/*',
  'arn:${AWS::Partition}:s3:::${EvidenceBucketName}/reports/*',
  'arn:${AWS::Partition}:s3:::${EvidenceBucketName}/set-audits/*',
  'arn:${AWS::Partition}:s3:::${EvidenceBucketName}/contract-documents/*',
...
export function validateEcsExpressRuntimePolicyTemplate(input: {
  ...
  add(
    checks,
    hasAll(templateText, [...APPROVED_S3_PREFIX_FRAGMENTS]) ? 'pass' : 'fail',
    's3-prefix-scope',
    'S3 object permissions are limited to approved PH-OS object prefixes.',
    'Scope S3 object access to every prefix generated by file-storage.ts and do not widen to a bucket-level wildcard.',
  );
