...
import {
  websocketAwsClientConnectionTimeoutMs,
  websocketAwsClientConfig,
  websocketAwsClientRequestTimeoutMs,
  withWebsocketAwsClientTimeout,
} from './aws-client';

describe('websocket AWS client helper', () => {
  afterEach(() => {
    vi.unstubAllEnvs();
    vi.restoreAllMocks();
  });
  ...
  it('clamps retry and timeout settings from environment variables', () => {
    ...
    expect(websocketAwsClientConfig()).toMatchObject({ maxAttempts: 5 });
    expect(websocketAwsClientRequestTimeoutMs()).toBe(30_000);
  });
  ...
  it('configures a Node HTTP handler with bounded connection and request timeouts', async () => {
    ...
    const config = websocketAwsClientConfig();
    expect(config.requestHandler).toBeInstanceOf(NodeHttpHandler);
    expect(websocketAwsClientConnectionTimeoutMs()).toBe(900);
  ...
  it('adds an abort signal to AWS SDK send calls without overriding caller signals', async () => {
    ...
    const client = withWebsocketAwsClientTimeout({ send });

    await expect(client.send({ command: 'PutItem' })).resolves.toEqual({ ok: true });
    ...
    await client.send({ command: 'PutItem' }, { abortSignal: controller.signal });
  ...
  it('unrefs and clears internally-created timeout timers after AWS send resolves', async () => {
    ...
    const send = vi.fn().mockResolvedValue({ ok: true });
    const client = withWebsocketAwsClientTimeout({ send }, 1234);
